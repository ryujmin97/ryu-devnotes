## 115차 — pre-112차(commit b67c291) 실측 로그 4건, 112차 threshold 실측검증 확장 — SMOOTH 완전PASS/ROUTE_A·B 저속게이트 부분개선/ROUTE_B 메인이벤트는 게이트 무관 진짜 급감속

**배경**: 사용자가 zip 2건(`10시28분28초_전후_감속분석_급감없이_부드러움.zip`,
`저속주행급감.Zip`) 업로드, "이번 패치 적용 이전 브랜치"라고 제보. 이후
사용자가 정확한 커밋을 확인해줌: `b67c2912a2d34b983f2c25fed9ec21547b9ea331`
("Merge c3-ms-curv into c3-ms-dev (81,82,84,85,87,91차 통합)",
2026-08-27 10:23 KST 커밋). `git log b67c291..8a7baa0`로 대조한 결과 이
커밋 이후 94/98/100/101/109/112차가 전부 아직 미반영된 상태에서 캡처된
로그로 확인.
...
**핵심 발견**:
1. **smooth(1028) — 완전 PASS**: 구threshold 기준 26프레임(자연수렴
   대비 1.247s 조기구간) 발동 예상되나 신threshold는 **0프레임**. 이
   라우트에서만큼은 112차 패치가 저속게이트 오탐을 실측으로 완전히
   제거함을 확인 — "급감없이 부드러움"이라는 제보 라벨과 정합.
...
감속" 케이스에서는 조기발동 구간을 줄이는 효과가 실측으로 재확인됐고
(smooth는 완전 제거, a/b부수는 부분개선), lowspeed_b 메인 이벤트처럼
