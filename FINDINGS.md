...
### → route3(`866476e5c3` seg18, "곡선_vturn_이상함") — **신규 발견: vturn이 정점 통과 직후, 아직 곡률이 남아있는데도 조기에 감속 해제(mid-curve premature release)**

- 파일명이 지목한 "이상함"의 정체로 보이는 패턴을 찾음. 정점(t=4785.98,
  steer=-10.9deg) 통과 후 steer는 서서히 줄되 **t=4795.8까지도 -4.5~-7.3deg
  대를 계속 유지**(즉 곡선 자체는 아직 안 끝남, 완만하게 계속 휘어지는
  중) — 그런데 **`vturn` 원시 후보값은 t=4786.9(steer 아직 -7.5,
  한창 곡선 중)부터 단 1초 만에 103km/h → 149km/h로 거의 무제한 근처까지
  튀어오름**(t=4787.88). 즉 vturn 자체의 "이 앞은 이제 안전하다"는 판단이
  실제 도로가 아직 휘어져 있는 상태에서 너무 이르게 내려짐.
- **왜 실제 사고/과속으로 이어지지 않았나**: 공교롭게도 이 구간엔 `cam`
  (카메라/구간단속 속도, `desiredSpeed` 산정용 `speed_n_sources`의 별개
  후보)이 t=4787.23부터 desiredSpeed를 **110km/h로 8초 넘게 고정**시켜
  min()에서 계속 이겼음 — vturn이 149까지 올라가 있어도 cam=110이 더
  작아 실제 목표속도는 110에 묶여 있었고, vEgo도 그 아래로 서서히
  감속(112.6→103.6km/h)했음. **cam이라는 무관한 제약이 우연히 vturn의
  조기 해제를 가려준 것으로 보임** — 이 구간에 카메라 단속이 없었다면
  vturn(103→149)이 min()에서 이겼을 것이고, 곡선이 아직 안 끝난 상태에서
  차가 재가속을 시도했을 가능성이 있음(이번 로그로는 가정만 가능,
  실증 못함 — cam이 항상 이렇게 유리하게 겹쳐줄 거란 보장 없음).
- **사용자가 제보한 "탈출 후 가속 지연"과는 방향이 반대**: 이 사례에서는
  vturn 자체는 오히려 "너무 빨리" 해제됨(1초). route2에서도 회복이 검출된
  6건 전부 0~1.65초로 빨랐던 것과 일치 — **지금까지 3개 route 어디서도
  "vturn/model이 탈출 후 재가속을 스스로 지연시키는" 증거는 못 찾음.**
  사용자가 체감한 "탈출 후 가속 지연"이 실재한다면, vturn/model 자체보다는
  (a) 이번처럼 cam/road/section 등 곡선과 무관한 다른 속도 제약이 곡선
  탈출 시점과 우연히 겹쳐 있거나, (b) vCruiseCluster 캡(기존 22차 기록,
  목표속도 헤드룸 없음) 같은 곡선 밖의 다른 메커니즘일 가능성이 이번
  3개 route 종합 결과로는 더 높아 보임 — **다음 세션에서 route(경로) 소스
  분석 재개 시 이 가설부터 확인 권장.**
- **정점 감속 부족(자동스캐너 재확인)**: apex t=4785.98 부근 max_gap
  +9.6km/h(t=4785.87) — route1(+8.1)/route2(+8.2 평균) 범위와 일치, 3개
  route 전부에서 재현(합계 3 route 관측).
- **사전감속(1)**: lead_time=7.75s로 route1(<3s)보다는 나음, route2 평균
  (11.1s)보다는 짧음 — 이 구간도 cam↔vturn 후보 교차(t=4769.8부터 여러 번
  cam/vturn 전환)가 있어 route1과 유사한 "경쟁 후보 flicker로 조기 안정화
  방해" 패턴 재확인.
- **코드 변경 없음(관찰/분석만)**.
- **다음 세션 조치 제안**: `vturn_speed()`의 "여러 예측지점 중 가장 엄격한
  지점" 로직이, 정점을 막 지난 시점에 예측 지평선(8.0s) 내 남은 지점들이
  이미 곡선이 풀리는 궤적으로 보이면 그 순간 바로 해제하는 구조로 보임 —
  "방금 지나온 곡률이 아직 유효한가"를 보는 대신 "앞으로 남은 예측경로가
  얼마나 곧은가"만 보기 때문. 재가속 쪽에도 진입 쪽과 유사한 완만한
  hold/hysteresis(예: 정점 통과 후 최소 유지시간)를 추가하는 방향 검토
  가치 있음 — 단 표본 1건, 재현성 확인 우선.

## [NEEDS_VALIDATION, 표본 3 route] 46차 종합 — "곡선구간 가감속" 3대 제보 증상 최종 정리

route1(`203f99d429`)/route2(`f3db6ca89d` 5세그)/route3(`866476e5c3`) 3개
패치이전 로그 종합 결과:

1. **진입전 사전감속 부족**: route1(고속순항→단일 급커브)에서 가장 뚜렷
   (유효 경고 3초 미만). 원인 후보로 `carrot_serv.py` L1051
   `abs(vturn_speed)<120` 게이트가 vturn 자체 불안정 구간에서 더 안정적인
   model 후보를 차단하는 것을 발견(13차 `119b101` 도입, 표본 1건).
   route2(연속 저속 커브)/route3에서는 덜 뚜렷하거나 판단 곤란 — **고속
   순항 중 갑자기 만나는 단일 커브 상황에서 특히 두드러지는 것으로 잠정
   판단.**
2. **정점 감속 부족**: **3개 route 전부에서 일관되게 재현** — route1
   +8.1km/h, route2 24/32건(75%) 평균+8.2/최대+18.1km/h, route3 +9.6km/h.
   **세 증상 중 가장 확실하게 재현된 문제.** `vturn_decel_rate=1.2m/s²`
   과소 여부/사전감속 지연으로 인한 거리부족 여부는 아직 분리 못함.
3. **탈출 후 가속 지연**: **route1은 세그 내 미종료로 미확인, route2
   6건 전부 0~1.65초로 오히려 빠름, route3에서는 도리어 "너무 빨리(1초
   내) 감속 해제"하는 반대 현상을 발견**(단, 우연히 겹친 `cam` 제약이
   가려서 실제 과속 재현은 안 됨). → **"탈출 후 가속 지연"은 vturn/model
   원인이 아닐 가능성이 이번 조사로 오히려 높아짐** — route(내비)/cam/
   vCruiseCluster 캡 등 곡선 밖 메커니즘 쪽으로 다음 조사 방향 전환 권장.

**패치 없음(전부 관찰/분석)**. toolkit 변경: `extract_log.py`
modelTurnSpeed 컬럼 추가, `work/curve_decel_scan.py`(스크래치, 커브 이벤트
자동탐지) 신규 — 다음 세션에 toolkit 정식 편입 검토.

**다음 세션 최우선 후보**:
(a) `abs(vturn_speed)<120` 게이트 재설계 설계/패치 (사전감속 부족 원인 1순위),
(b) 정점 감속 부족 원인을 "사전감속 지연 때문" vs "`vturn_decel_rate` 자체
과소" 중 어느 쪽인지 분리하는 실험(예: 사전감속이 충분했던 route2 케이스들
중에서도 정점 초과가 나는지 재확인 — 이미 그런 것으로 보임(사전감속 정상인
route2에서도 75% 초과), 즉 **decel_rate 자체도 과소일 가능성이 route2
데이터로 한 단계 높아짐** — 다음 세션에서 명시적으로 분리 분석,
(c) "탈출 후 가속 지연" 재조사 시 route/cam/vCruiseCluster 캡 쪽으로 초점
이동.
