...
## [63차 계속10 정정] 방안 E 재평가 — REJECTED 철회, 사용자 실제 의도 확인 후 재해석하니 오히려 설계 근거 강화됨

**정정 배경**: 위 "[63차 계속10]" 항목에서 "REJECTED" 판단은 "frac_rate=1.0
(급감속)"을 정답 기준선으로 놓고 방안E가 이를 억제하는 것을 리스크로
해석한 것이었음. 사용자가 실제 요구사항을 명확히 함: "끼어드는 차량이
내 차보다 빠른 속도로(가속하며) 끼어들었을 때는 급감속이 아니라
레이더 락온 이후와 같은(정상) 반응을 원한다" — 즉 **급감속 자체가
바로 사용자가 없애고 싶어하는 증상**이었음.

**재해석**: seg3의 raw_rate 극단값(-100~-339m/s)은 실제 물리적 접근이
아니라 **트랙이 더 가까운 새 물체(끼어든 차)로 전환되는 순간의 인공적
점프**였음(이는 위 원 항목에서도 이미 관찰함). 반면 `leadVLead` 기반
`ref_rate`는 이 구간 내내 거의 0~-1m/s대(위험한 접근 아님)를 가리켰고,
**이게 오히려 정확한 판단이었음** — 레이더 락온 후 실측 vRel이
+3.2~+7m/s(명확히 opening)로 확인돼 "끼어든 차가 실제로 더 빨랐다"는
사용자 실측 정보와 정합. **급감속을 유발한 진짜 원인은 raw dRel
미분의 트랙전환 노이즈였고, leadVLead는 이미 옳았다** — 즉 방안E는
정확히 "raw dRel은 튀지만 모델 속도추정은 위험하지 않다고 판단하는"
이 상황을 걸러내도록 설계된 것이고, seg3에서 그 역할을 제대로 수행함이
확인된 것.

**"안전장치가 자신이 보완할 신호에 종속되는 모순"이라는 원 비판은
58차1번(원거리 저확신 리드에서 leadVLead가 위험을 *과소*평가하는
다른 실패모드)의 프레임을 이번 케이스에 잘못 적용한 것 — 이번은 반대
상황(leadVLead가 맞고 raw dRel 미분이 트랙전환으로 튀는 상황)이었음.

**추가 안전 확인**: 방안E의 margin 로직상, leadVLead가 진짜 위험(빠른
접근)을 정확히 가리키는 경우엔 `ref_rate`도 함께 매우 음수가 되므로
`plausible_min=ref_rate-MARGIN`도 충분히 낮아져 raw_rate를 거의
그대로 통과시킴 — 즉 leadVLead가 "안전하다"고 말할 때만 raw dRel
튐을 억제하는 구조라, 진짜 위험(leadVLead도 위험 인지)까지 억제하는
방향은 아님(danger override 경로도 항상 무관하게 유지).

**결론**: **REJECTED 철회, 방안E 설계는 원안(63차 계속9) 그대로 유지,
실제 구현 진행.** PARAMS_REGISTRY.md 상태를 REJECTED → NEEDS_VALIDATION
(seg3 재생검증으로 방향성은 뒷받침됐으나 실제 acados MPC 통합/실차
검증은 아직)으로 갱신.
...
## [63차 계속10 (b)] 실제 통합 코드(e6a00ae)로 seg3/seg14 재생검증 — 로직단위 시뮬레이션과 일치 확인, 신규 관찰 추가

**목적**: 63차 계속10 (a) WIP "다음(최우선) 2번" — "이번 통합 코드로
seg3/seg14 raw CSV 재생검증 한 번 더(패치-이전 로직단위 시뮬레이션과
실제 통합 코드가 완전히 동일한 로직인지 최종 대조)".

**방법**: `long_mpc.py`(commit `e6a00aea`)를 재구현하지 않고, 실제 파일의
`update()` 중 vision_dRel_rate/discontinuity/방안E 블록(원본 778~921줄)을
**문자 그대로**(`open()`으로 읽어 슬라이싱, 수기 재입력 없음) 잘라내
`exec()`으로 하네스 클래스에 편입 — 상수값도 파일에서 정규식으로 직접
파싱해 사용(`replay_integrated_e_seg3_seg14.py`, `/home/claude/work/verify63/`,
toolkit 미편입 스크래치). 사용자가 재업로드한
`drive-download-20260824T072553Z-1-001.zip`에서 seg3/seg14 원본 rlog를
재추출(`extract_log.py`, commit `e6a00aea` 상태에서 추출 — 방안E가 이미
반영된 코드로 뽑은 CSV) 후 재생.

**결과 (frac_rate, `VISION_CLOSING_RATE_GATE_CAUTION/DANGER`=-2.2/-5.0
기준으로 실제 코드 977~997줄 공식 그대로 계산)**:
- **seg3**: 전체 세그 최대 frac_rate = **0.205** (t=260.945, 레이더 락온
  t=260.997 직전) — 63차 계속10 로직단위 시뮬레이션 결론("PATCHED_E 최대
  0.209")과 거의 일치(오차 0.004, 반올림/이산 dt 샘플링 차이 수준).
- **seg14**: aEgo 최저점(t=925.148) 시점 frac_rate = **0.671** — 63차
  계속9 로직단위 시뮬레이션 결론("aEgo 최저점에서 frac_rate=0.678")과
  거의 일치(오차 0.007).

**결론(핵심)**: 로직단위 시뮬레이션(수기 재구현)과 실제 `long_mpc.py`에
통합된 코드 사이에 **의미있는 drift 없음** — 방안E 설계가 커밋 시
정확히 의도대로 반영되었음을 실제 코드 재생으로 최종 확인. 소수점 셋째
자리 수준의 오차는 로직단위 시뮬레이션이 discontinuity 리셋 타이밍을
프레임 단위로 완전히 동일하게 재현하지 못했을 가능성(부동소수/샘플링)
정도로 추정, 결론에 영향 없음.

**[신규 관찰, 이전 로직단위 검증이 못 본 부분]** 63차 계속9/10의
로직단위 검증은 "aEgo 최저점 한 시점"의 frac_rate만 보고 "1.0 아님,
완만한 램프"로 결론지었으나, 이번 실제 코드 전체 구간 재생으로 보면
seg14의 discontinuity 구간(t=923.10~923.50, 7회 연속 재트리거) 동안
`_lead_acq_timer`가 반복적으로 0으로 리셋->재축적되며 **frac_rate가
그 사이(t=924.05~924.40)에 실제로는 최대 0.882까지 상승했다가 다시
0.227까지 떨어지는 등, "완만한 단조 램프"가 아니라 여러 번 오르내리는
톱니 패턴**임이 드러남. 여전히 UNPATCHED(즉시 1.0 포화, 923.65부터
925.30까지 유지)보다는 확연히 완화된 형태이고 방향(급격한 무감쇠 튐
방지)은 설계 의도와 일치하나, "aEgo 최저점 한 지점=0.678"이라는 단일
수치만으로는 이 구간 전체의 반응 프로파일을 대표하지 못함 — 실차
체감 시 이 톱니 패턴(0→0.88→0.23→0.85 식의 반복 등락)이 저크(jerk)로
느껴질 가능성은 별도로 관찰 필요(신규 관찰, NEEDS_VALIDATION, 실차
검증 시 참고).

**코드 변경 없음(ryu 미변경, devnotes만 갱신)**. 스크래치:
`work/verify63/seg3_seg14.csv`(commit `e6a00aea` 기준 재추출),
`work/verify63/replay_integrated_e_seg3_seg14.py`(toolkit 미편입),
`work/verify63/replay_result.pkl`.

## [64차, 실차 검증] 방안E(`e6a00aea`) 적용 후 첫 실주행 로그 — cutin 2건 급감속 없이 정상 반응 확인

**배경**: 사용자가 방안E 적용/push(`e6a00aea`) 이후 첫 실차 검증 로그
(`20260825_070702_0000031d--4ddb171bfb`, 14세그, 2026-08-25 07:07~07:19)
업로드. 사용자 제보: 세그4(07:09:33경)/세그10(07:15:35경) cutin 상황
발생, 급감속 없음. `extract_log.py`로 CSV 추출, meta.json 확인 결과
**commit `e6a00aea7421`(방안E 반영 상태) 그대로 기록됨** — 방안E가 실제
적용된 상태에서의 로그임을 확인.

**주의(경로 확인)**: 사용자가 말한 "세그4/세그10"과 폴더 접미사
숫자(`--4`/`--10`)가 실제로는 1개씩 어긋남 — 사용자가 말한 시각
(07:09:33/07:15:35)으로 역산하면 실제로는 폴더 `--3`/`--10`의 후반부
구간이 해당(전자는 접미사와 다름, 후자는 접미사와 일치). 이번 검증은
CSV `t` 축과 폴더 시작시각을 직접 대조해 실제 이벤트를 특정.

**검증 결과**:
1. **폴더 `--4`(t=336.99~349.6, 실질적으로 사용자가 말한 세그4 직후
   구간) — cutin 2건, qcamera로 실제 차로진입 확인**:
   - t≈338.5: 우측 차로에서 검은색 세단이 자차 차로로 cutin, dRel이
     30.4m→10.0m으로 급감(프레임 대조로 실제 근접 cutin 확정, 화면 전체를
     채울 정도로 근접). 이 순간 aEgo=-0.56m/s²에 그침(방안E 이전이었다면
     raw dRel 급감 자체로 frac_rate가 튀었을 가능성이 있던 패턴). 이후
     cutin 차량이 자차보다 빠르게 가속해 이탈(vRel -0→+1.5→+3.3→+3.8
     opening)하자 aEgo도 그에 맞춰 자연스럽게 회복(+0.87 峰) — 63차
     계속9/10에서 다루던 "끼어든 차가 더 빠른 경우" 시나리오가 실차에서
     재현되고 방안E 의도대로 과잉감속 없이 처리됨.
   - t≈346.8: dRel이 32.99m→10.3m으로 재차 급감(discontinuity성 점프),
     qcamera로는 실제 전방에 흰색 세단이 근접 주행 중임을 확인(오검출
     아님, 정체 진입으로 보임). aEgo 최저 -0.81m/s²로 완만하게만 반응,
     t=350.87에는 이미 +0.10으로 회복 — 63차 seg14류 discontinuity
     반복 패턴과 유사한 raw dRel 급변인데도 실제 aEgo 반응은 매끄러운
     단조 곡선(사용자 제보 "급감속 없음"과 일치).
   - 이 구간 최대 |jerk| = 9.02 m/s³(t=348.6) — 톱니형 급반전 없이 단일
     피크, 63차 WIP에서 우려했던 "seg14류 톱니 패턴"의 저크 체감은
     이번 표본에서는 관찰되지 않음.
2. **폴더 `--10`(t=709.9~723.4, 사용자가 말한 세그10과 접미사 일치) —
   신호대기 정체 진입, 급접근 후 완전정차**:
   - t=709.92~711.97 사이 dRel이 66.97m→25.5m→18.5m로 급감(전방 차량들이
     신호에 걸려 급정지, closing rate -6.8~-7.4m/s), aEgo 최저
     **-3.77~-3.95m/s²**로 이번 로그 전체에서 가장 강한 감속. qcamera로
     확인 결과 전방에 브레이크등 켜진 차량 행렬이 실제로 존재 —
     정탐(실제 급정지 차량 행렬), 오검출 아님.
   - 이후 vEgo가 17.6m/s→0까지 부드럽게 단조 감소(t=723.4 dRel=5.1m
     유지한 채 완전정차, 앞차와 안전거리 확보), 재가속/재감속 반복 없이
     한 번에 매끈하게 정차 완료. 최대 |jerk| = 6.28 m/s³(t=711.7),
     톱니 패턴 없음.
   - 이 이벤트는 dRel 절대변화량(66.97→18.5, 2초 내 48.5m)이 상당히
     커서 danger override(TTC<=2.5s) 근처였을 가능성이 있으나, aEgo
     -3.77~-3.95는 통제된 단일 피크로 나타났고 브레이크 페달 개입
     (`brakePressed`) 없이 ADAS 단독으로 안전하게 처리됨.

**종합 판단**: 사용자 제보("cutin 2건, 급감속 없음")와 로그/영상 실측이
**일치**. 방안E 적용 이후에도 (a) cutin 차량이 더 빠른 경우 과잉감속
없이 자연스럽게 처리, (b) discontinuity성 raw dRel 점프에도 매끄러운
단조 반응(톱니/저크 없음), (c) 실제 급정지 차량 행렬에는 여전히
필요한 만큼(-3.77~-3.95) 반응해 danger 상황 대응력 유지 — 63차
계속10 WIP "다음(최우선)" 항목 중 cutin 정상반응/톱니-저크 체감 확인은
이번 로그로 긍정적으로 확인됨. 단 "진짜 위험한 cutin 회귀 검증"과
"일반 원거리 접근 정상 유지"는 이번 로그 표본에 해당 시나리오가
명확히 없어 여전히 별도 표본 필요(미확정 유지).
