...
## [INVESTIGATING] MAX_SEGMENTS_PER_ROUTE=20 패치, 실기기 로그에서 여전히 40개 단위로 회전 — 반증 확인 (2026-08-20, 라우트 260819-5, route ba55f880d1)
- 증상: 260819-3/4/5 세 라우트에 걸쳐 이어 분석한 route ID `ba55f880d1`가
  seg0(260819-3, route3b 추정)부터 seg39(260819-5)까지 **끊김 없이
  40개 세그먼트**로 이어지다가 seg39 다음(125527+60s=125627) 새 route ID
  `dc8bdc7d4d`로 정확히 rotate됨. 세그먼트 파일명 타임스탬프가 1분
  간격으로 연속(재부팅 갭 없음)이라 디바이스 재부팅이 아니라 loggerd의
  `MAX_SEGMENTS_PER_ROUTE` 도달에 의한 정상 rotate로 보이는데, 이 값이
  20이 아니라 **여전히 40에서 도는 것처럼 보임**.
- 원인 (미확정): 아래 실기기 미반영 후보 중 확인 필요
  1) 이 로그가 기록된 시점(8/19 12:41~12:56)에 comma 3X가 f7b154638cf2
     패치를 반영해 재빌드/재부팅한 상태가 아니었을 가능성 (master 로컬
     git am+push는 원격 저장소 반영일 뿐, 디바이스 쪽 scons 재빌드·재부팅
     여부는 별개)
  2) `MAX_SEGMENTS_PER_ROUTE` 상수가 컴파일 타임 상수라 디바이스가 이전
     빌드를 그대로 쓰고 있었을 가능성
  3) (낮은 확률) rotate 로직 자체가 의도와 다르게 40 기준으로 동작할
     가능성 — 코드 리뷰(FIXED 항목 참고)상으로는 그럴 이유가 없어 우선
     순위 낮음
- 조치: 코드 자체는 변경 없음 (기존 FIXED 판단 유지, 아래 원본 항목
  참고). 다음 실주행 로그에서 route ID가 20개 단위로 끊기는지 재확인
  필요 — 만약 다음 로그도 계속 40개로 이어지면 디바이스 재빌드/재부팅
  여부를 사용자에게 직접 확인 요청.
- 근거 로그: route `ba55f880d1` = 260819-3(seg0~4 추정, x5seg) +
  260819-4(seg5~24, x20seg) + 260819-5(seg25~39, x15seg) = 총 40세그.
  260819-5 seg25~39 타임스탬프 12:41:27~12:55:27(1분 간격 연속),
  다음 route `dc8bdc7d4d` seg0 시작 12:56:27(정확히 1분 뒤, 재부팅
  갭 없이 즉시 rotate).
...
## [기타 확인] 라우트 260819-5 분석 — MAX_SEGMENTS_PER_ROUTE 반증(위 항목) 외 신규 이슈 없음 (2026-08-20, HEAD f7b154638cf2, 신규 커밋 없음)
