## 373차 -- [PARTIAL_ROOT_CAUSE -- 추가 검증 필요, 코드 미수정] 완만한 고속도로 커브 근거리(apexDist<150m) 구간에서 routeApexSpeed가 프레임마다 두 값 사이를 오가며 vTurnSpeed(150 안팎) 대비 낮은 값(84~90대)으로 관측 -- 362차 원거리 오탐과 동일 계열이나 367차계속4 게이트(>=150m) 사각지대

**배경**: 사용자가 "고속도로 완만한 커브가 긴 구간에서 vturn은 150 정도인데 route가 간간히 80~90으로 나온다(깜빡이 없음)"고 신고, 실차 dashcam zip(2026-09-12 14:31~14:36, seg12-17) + 대응 클립 영상 2개(14:32:44/14:35:10) 업로드.

**§24 dedup 확인**: FINDINGS.md 전체 검색 -- 근거리(apexDist<150m)에서의 fine 오실레이션을 특정한 기록은 없음(최초 문서화). 362차(원거리 고립 spike, 이미 367차계속4로 부분 대응)와 219/220차(apexIdx flicker, 미해결)/368-371차(L1807 src 플래핑, 이미 hold=4로 패치됨)는 모두 유관하지만 이번 증상과 메커니즘 구분 필요.

### 1. 증상 재현 확인 (실차 로그 + dashcam)
`extract_log.py --with-navi-paths --repo ryu`로 seg12-17(7,201행) 재추출, device `gitCommit=ea4c5f444997`(=`ryu` HEAD, 372차 시점, dirty=False, 드리프트 없음). 사용자가 준 클립 2개의 wall-clock을 `t`축으로 환산(seg12 시작 14:31:47=t=3183.2 기준)해 두 구간(t≈3225~3229, t≈3378~3381)을 특정, `verify_and_extract_frames.py`로 프레임 추출 -- **두 구간 모두 완만하게 좌측으로 휘어지는 왕복 분리 고속도로**로 시각 확인(급커브 아님, 사용자 신고와 일치).

### 2. 로그 상의 패턴 -- src=='route' 구간에서 routeApexSpeed 2값 진동
두 구간 모두 vEgo 92~99kph, vTurnSpeed 145~190(사용자가 말한 "150 정도"와 일치, 커브를 아직 강하게 인지하지 않은 상태), `src=='route'`. 이 구간에서 `routeApexMode`가 `matched`(신규 계산)/`held`(직전 값 유지, apexDist를 vEgo*dt로 감쇠) 패턴을 반복하는데, **연속된 두 `matched` 프레임이 서로 다른 routeApexSpeed를 내며(예: t=3225.809 apexDist=80.0 apexSpeed=94.88 -> t=3225.857 apexDist=80.0 apexSpeed=87.96, 47ms 간격), 이후 `held` 프레임들은 이 중 낮은 쪽 값을 그대로 고정**하는 패턴이 반복 관측됨(2번째 클립 구간에서도 동일 패턴: t=3378.06 apexSpeed=97.91 -> t=3378.11 apexSpeed=88.43). 즉 실제로 하위 파이프라인에 전달/유지되는 값은 대체로 낮은 쪽(84~90대)에 가깝다 -- 사용자가 "80~90 간간히"로 체감한 것과 일치.

### 3. naviPaths 원시좌표 대조 -- 47ms 간격 두 프레임 사이의 미세 흔들림
위 두 `matched` 프레임(t=3225.809/3225.857)의 `naviPaths` 원시 리샘플 좌표를 직접 비교 -- x는 거의 동일하나 **y가 프레임마다 0.05~0.15m 수준으로 계속 흔들림**(예: 10m 지점 y=0.67->0.77, 20m 지점 y=1.48->1.57). 즉 네비게이션 경로 자체가 매 계산 주기마다 서브미터 단위로 미세하게 다시 맵매칭/보간되고 있음.

### 4. `route_curvature_macro_fine()` 독립 재현 -- fine(짧은 chord)이 이 미세 흔들림에 훨씬 민감
`carrot_man.py`의 `calculate_curvature`/`route_curvature_macro_fine`(V_CURVE_LOOKUP_BP/VALS, ROUTE_CURVE_NEGLIGIBLE_THRESHOLD 포함 소스에서 직접 추출)을 devnotes 환경에서 verbatim 재구현해 위 두 naviPaths 스냅샷에 각각 적용: macro(40m chord, sample=4)는 두 프레임 사이 속도 변화가 자연스러운 반면(예: dist=90m macro_speed 204.6->210.6), **fine(20m chord, sample_fine=1)은 같은 dist=90m 지점에서 88.4->115.4로 훨씬 큰 폭으로 흔들림** -- 3.의 서브미터 좌표 흔들림만으로 fine 계산 결과가 크게 요동함을 직접 확인. 이 dist=80/90m 구간은 **150m 미만**이라 367차계속4가 추가한 `ROUTE_FINE_OVERRIDE_MIN_DIST_M=150.0` 게이트가 적용되지 않는 구간이다.

**주의(미완료 검증, §28 원칙)**: 위 3./4.의 verbatim 재구현은 `route_curvature_macro_fine()`까지만 재현했고, 이 함수의 출력(`speeds[]`)이 실제 `routeApexDist`/`routeApexSpeed`로 확정되기까지의 후속 apex 선택/클러스터링 로직(후보 리스트 구성, distance 기반 apex 선정 등)은 재구현하지 않았다 -- 그래서 내가 재현한 fine_speed 수치(58~115 범위)가 로그에 찍힌 실제 routeApexSpeed(84~99 범위)와 정확히 일치하지는 않는다. **따라서 이번 세션의 결론은 "근거리에서도 fine 계산이 naviPaths 미세 흔들림에 민감하게 반응해 매 프레임 다른 값을 낸다"는 메커니즘 존재 확인까지이며, 이것이 routeApexSpeed 최종값 진동의 100% 설명이라고 확정하지는 않는다(PARTIAL_ROOT_CAUSE)**. apex 선택 로직까지 포함한 전체 파이프라인 재현이 다음 세션 우선 후보.

**362차/367차계속4와의 관계**: 362차는 "300m 밖 위주 원거리, 인접 지점 뒷받침 전혀 없는 완전 고립 spike"였고 367차계속4가 그 대응으로 150m 이상만 게이트를 걸었다. 이번 373차 관측은 **150m 미만 근거리에서, 완전한 고립이 아니라 매 계산 주기 서브미터 좌표 흔들림에 의한 연속적 진동**이라는 점에서 메커니즘이 다르다 -- 150m 게이트를 단순히 낮추는 것은 147/148차가 fine을 도입한 원래 목적(근거리 실제 급커브 검출)을 해치므로 적절한 해법이 아닐 가능성이 높다(NEEDS_DESIGN_DECISION).

**코드 수정: 하지 않음(§27, analysis-only, 원인 파이프라인 미완결)**.

**미확인 사항**:
1. apex 선택/클러스터링 로직까지 포함한 전체 재현으로 로그 실측치(84~99 범위)와 byte-identical 재현 확인 필요 -- 다음 세션 최우선.
2. naviPaths 좌표가 47ms마다 서브미터 단위로 흔들리는 근본 원인(navd 맵매칭/보간 갱신 주기, GPS 노이즈 등) 자체는 이번 세션에서 추적하지 않음 -- "흔들림이 fine 계산에 미치는 영향"까지만 확인.
3. 이 근거리 진동이 desiredSpeed(최종 제어 출력)에 실제로 얼마나 반영되는지 정량 미측정 -- 이번 두 구간에서는 desiredSpeed 자체는 92~99 범위로 비교적 완만했으나(라우트 표시값 자체의 진동보다는 작음), 다른 corpus/구간에서도 이 폭이 유지되는지는 미확인.
4. 해법 방향(예: fine 결과에도 짧은 시간 hold/hysteresis 적용, naviPaths 자체를 소폭 스무딩, 150m 게이트를 근거리에도 다른 형태로 확장 등)은 Master 결정 필요.

**실차 검증**: 증상 자체는 이번 실차 로그+dashcam으로 재현 확인(§29 "증상의 실차 재현"). 수정안이 없으므로 "수정의 실차 검증"은 해당 없음.

**다음 작업**: WIP.md 373차 "다음 작업" 참고.

---

## 370차 -- [369차 NEEDS_DECISION 해결 -- Master 결정 B3 + 정량검증] L1807 히스테리시스를 hold=4프레임(0.20s) 시간기반 디바운스로 설계, 오프라인 재현에서 1프레임 토글 218건 전량(100%) 제거 확인

**기존 결론(369차)**: `carrot_man.py` L1807(`if v_ego_ms <= target_ms:`, INERT 분기)이 251건 플래핑의 실제 코드 위치이며, 히스테리시스를 추가할지/어떻게 설계할지(폭 vs 유지시간)는 Master 결정 필요(NEEDS_DECISION)로 열어둠.

**새로운 증거**:
1. 히스테리시스 두 축(A=값 기반 margin, B=시간 기반 hold) 후보 검토 -- grid 경계 apex_speed 스파이크 크기가 케이스마다 다름(341차 x20seg +1.6~+5.6kph, 369차 근거리 corpus +8.1kph)이 이미 관측돼 있어 고정 margin(A)은 크기 의존적이라 신뢰도 낮음. 크기 무관 필터인 B(시간 기반)가 근본적 -- 사용자가 **B3(hold=4프레임=0.20s)**로 확정.
2. `diag_required_decel_341.py`에 `--hold-frames=N` 옵션 추가(§21/§27, 신규 스크립트 아님, 관측 레이어만 추가) -- L1807 raw 조건을 프레임별 bool로 기록 후 N프레임 연속 디바운스(`apply_hold()`) 적용.
3. 369차와 동일 corpus(`22ebbb245d`, 3,296행, `commit=d5b34bb6b358`)로 재현: 평가 대상 925프레임(369차 569+356과 일치) 중 raw(hold=1, 현재 코드) 1프레임 토글 **218건** -> **hold=4 적용 시 0건(100% 제거)**.
4. raw 조건의 run-length 분포(1프레임 276/2프레임 38/3프레임 8/4프레임 17/5+ 41, 총 381 run) -- 1~3프레임 요동이 85%(322건)를 차지, hold=2/3이면 2~3프레임 요동(46건)이 여전히 "확정 전환"으로 오인됨 -- hold=4가 hold=2/3보다 뚜렷이 넓은 노이즈 대역을 걸러낸다는 정량적 근거.
5. 실측 `src`(route/비route) 컬럼 기준 1프레임 토글 228건 -- 368/369차가 보고한 135+116=251건과 크기가 비슷해(완전 일치는 아님) L1807 격리 재현이 실측 패턴을 상당 부분 설명함을 뒷받침.

**변경 이유**: 369차가 코드 위치는 확정했으나 설계 방향(폭/유지시간)은 미결이었음. 스파이크 크기의 케이스 의존성(341차/369차 비교)이 값 기반(A) 설계의 약점을 시사해, 크기 무관 시간 기반(B) 설계로 Master가 결정. hold 값 자체(4프레임)도 임의 선택이 아니라 run-length 분포 실측으로 뒷받침.

**새로운 결론**: L1807에 hold=4프레임(0.20s) 시간기반 디바운스를 추가하는 것으로 설계 확정(NEEDS_DECISION -> DECIDED). 단 이 검증은 L1807 조건만 격리 재현한 오프라인 근사(§29 -- `carrot_serv.py` 전체 arbitration까지 반영한 것은 아님, 실차 검증 아님)이며, 실제 코드 패치와 341차 원 corpus(x20seg) 기준 반응지연 부작용 검증은 다음 세션으로 이월. **상태: NEEDS_VALIDATION(반응지연 부작용) + 코드 패치 대기**.

**[371차 보강, 상태 갱신]** 위 "코드 패치 대기"가 371차에서 해소됨 -- `ROUTE_L1807_HOLD_FRAMES=4` 상수, `_route_apply_l1807_hold()` 메서드, L1807 삽입 지점 교체로 `carrot_man.py`에 구현 완료(패치 파일 전달, 사용자 적용/push 대기, §31). 신규 메서드가 위 8번(`apply_hold()`)과 완전히 동일한 알고리즘인지 None 섞인 랜덤 시퀀스 200개로 대조해 200/200 일치 확인. **단, 이 패치가 적용된 실제 `carrot_man.py`로 `22ebbb245d` corpus를 재생해 218건->0건을 직접 재현하는 것은 371차에서 하지 않음(알고리즘 동등성만 확인)** -- 다음 세션 최우선. **상태: NEEDS_VALIDATION(반응지연 부작용 + 패치코드 corpus 재검증) + 코드 패치 완료(미적용/미실차검증)**. 상세: WIP.md 371차.
...
**기존 결론(341차)**: 직선구간에서 관측된 route on/off 플래핑은 `routeApexIdx`가 10m grid 경계를 넘을 때 `routeApexSpeed`가 순간 상승(도로제한속도 근처까지 스파이크)하며 `v_ego<=target` INERT 판정을 정확히 1프레임 통과시키고 즉시 재진입하는 단일 메커니즘으로 확정(46건 중 43건 grid 전환, 40건 apex_speed 상승, 93%대 일치).

**새로운 증거**: 사용자 제보 신규 route(`22ebbb245d`, 2026-09-12 녹화, 곡선구간 위주)에서:
- `route->비route(1프레임)->route` 드롭아웃 135건 중 133건(98.5%) grid 전환 동시발생, 134건(99%) apex_speed 상승 동시발생.
- 대칭 방향(`비route->route(1프레임)->비route`) 블립 116건 중 113건(97%) apex_speed 하락 동시발생 -- 341차가 명시적으로 문서화하지 않았던 반대방향 대칭성도 이번에 확인.
- `routeApexIdx==-1`(후보소실, 327차/338차 candidate churn 계열) 관여 0건 -- 이번 재현은 341차 메커니즘 단독.

**변경 이유**: 341차는 직선구간 단일 route(x20seg)에서만 확정했었는데, 이번에 곡선구간 신규 route에서 동일 메커니즘이 훨씬 높은 밀도(251건, 최대 16.6초 동안 128회 전환)로 재현됨을 실측 확인 -- 메커니즘의 route-무관/도로형상-무관 일반성이 보강됨.

**새로운 결론**: 341차 결론 유지, 일반성 보강. `carrot_man.py` L1832 ACTIVE 진입 게이트에 히스테리시스/하한이 없는 것이 근본 원인이며, 341차 이후 코드 수정이 없어 여전히 열려있는 이슈. 원거리 150m 게이트(367차계속4)와는 독립적 문제(이번 corpus apexDist 대부분 0~140m, 150m 미만).

**검증**: 로그 분석 완료(3,296행, `d5b34bb6b358` 기준 드리프트 없음, 251건 수작업 프레임 대조). 시뮬레이션/실차: 해당 없음(정성/정량 실측 로그 분석).

**다음 작업**: `diag_required_decel_341.py` 정식 적용, 히스테리시스 설계 Master 결정.
...
**기존 결론(365차, 366차로 보강됨)**: `isolation_score` 게이트가 th=0.885~0.92 구간에서 오탐 억제 76~86%(362차/366차 확장 corpus, 29~42건)와 정탐 생존 85~91%(363차 corpus 대리/순수 필터, 1137~1233건)를 동시에 만족하는 안정적 평탄부. 오탐 corpus를 넓히면 억제율은 다소 하락(86.2%->76~81%)하지만 정탐 생존율은 corpus 확장/정제 후에도 84~91%대로 견고하게 유지됨을 366차가 확인.

**새 증거**: 367차가 09-01~09-03 신규 11개 route에서 확보한 정제된 물리적 이벤트(FP 22건, TP 4건, `dedup_physical_events_367.py` 산출)에 동일 `isolation_score()`(365차 verbatim 재사용, `apply_isolation_gate_367.py` 신규)를 적용. 11개 route를 `extract_log.py --with-navi-paths`로 재추출해 naviPaths를 복원, (route,t) 매칭 성공률 100%(22/22, 4/4).

| threshold | FP 억제율(22건) | TP 생존율(4건) |
|---|---|---|
| 0.885 | 40.9% | 50.0% |
| 0.90 | 36.4% | 50.0% |
| 0.905 | 36.4% | 50.0% |
...
TP 4건 상세(전부 R<13m급 실제 급커브, apexSpeed=5.0kph 포화):
```
2cbdaca9d2 t=159.27  R=10.7m  iso=0.811  (생존, <0.885)
2cbdaca9d2 t=201.87  R=12.1m  iso=0.996  (억제됨)
4e18e62932 t=244.67  R=9.5m   iso=0.791  (생존)
7e9c713c9d t=242.37  R=10.9m  iso=1.000  (억제됨)
```

**변경 이유**: 366차까지는 corpus가 넓어져도(오탐 29->42건) 정탐 생존율(1137~1233건 규모)이 84~91%대로 견고했는데, 이번에 처음 포함된 극단적 급커브(R<13m) TP 소표본에서 생존율이 50%까지 급락 -- 365차 핵심 가설("진짜 커브는 heading 변화가 인접 세그먼트로 분산되어 isolation_score가 0.5에 가까움")이 절반(4건 중 2건)에서 깨지는 최초 반례.

**새 결론(잠정, 추가 검증 전까지 미확정)**: ISOLATION 게이트(th=0.885~0.92)는 365/366차가 검증한 완만~중간 곡률 corpus(363차 R<30m 대리/순수 필터, 대부분 R=15~30m대로 추정)에서는 안정적이었으나, R<13m급 극단적 급커브에서는 정탐 생존율이 크게 무너질 수 있다는 신호가 처음 나왔다. 가능한 원인 가설(미검증): naviPaths가 10m 고정 격자로 리샘플되는데, 곡률 반경이 격자 간격과 비슷하거나 더 작아지면(R<13m vs 10m 격자) heading 변화가 실제로도 1~2개 세그먼트에 물리적으로 집중될 수밖에 없어 365차의 "분산=진짜 커브" 가정 자체가 이 곡률 영역에서는 성립하지 않을 수 있음. **다만 TP 표본이 4건뿐이라 게이트 폐기나 th 재조정 결론은 아직 내릴 수 없음(§28 성급한 결론 금지)** -- R 구간별(<15m / 15~30m / 30m+)로 isolation_score 거동을 분리한 추가 corpus 확보가 선행되어야 함.

FP 22건은 R=87.3~10196m로 폭넓게 분포(362차 오탐 corpus는 주로 R=88~92m대였음) -- 이 corpus 자체도 365/366차 오탐 corpus와 R 분포가 달라 억제율 하락(86.2%->36.4~40.9%)이 게이트 성능 저하인지 corpus 성격 차이인지 아직 분리되지 않음.

**코드 수정**: 하지 않음(검증 단계, ISOLATION 게이트 자체가 `ryu`에 반영된 적 없음).

**실차 검증**: 미실시(오프라인 재계산 한정).

**부가 확인(②)**: FP corpus 중 최대 클러스터(`2cbdaca9d2` t=673초, n_frames_in_cluster=54; t=978초, n=40)를 qcamera 프레임 + naviPaths 원본으로 직접 확인 -- 둘 다 단발성 오탐이 아니라 실제 IC(나들목)/터널 진입부의 60~75초짜리 지속 커브 구간으로 확인됨(사람 확인 완료). 8초 클러스터링 로직 자체는 정상 동작(연속 실제 커브를 올바르게 1개 물리적 이벤트로 병합) -- dedup 로직 결함 아님.

**다음 작업**:
1. FP 22건 / TP 4건을 R 구간별(<15m / 15~30m / 30m+)로 나눠 isolation_score 거동 재분석 -- 급커브 구간에서만 반례가 나오는지 확인
2. 가능하면 R<15m급 급커브 corpus를 추가 확보해 TP 표본 4건의 소표본 한계 보완
3. naviPaths 10m 격자 간격 vs 실제 곡률 반경의 관계(격자 해상도 한계 가설) 코드 레벨로 재확인 검토
4. FP corpus의 R 분포 차이(362차 corpus vs 367차 corpus)가 억제율 하락의 원인인지 분리 분석

## 366차 -- [365차 결론 보강, 코드 미수정 -- NEEDS_MORE_VALIDATION] 정탐 대리필터(routeApexSpeed<=45kph)의 실제 R<30m 재현 정확도 실측(precision 92.2%/recall 100%), 정제 표본으로 ISOLATION 게이트 평탄부 재확인

**기존 결론(365차)**: `isolation_score` 기반 ISOLATION 게이트가 th=0.885~0.92 구간에서 오탐 억제 86.2%와 정탐 생존 84~90%를 동시에 만족하는 안정적 평탄부를 보였으나, 정탐 표본을 고른 대리필터(`routeApexSpeed<=45kph`)가 363차 원 기준(`R<30m`)과 정확히 대응하지 않는 proxy라는 한계가 "미확인 사항 #2"로 남아 있었음.

**새 증거**: 363차 corpus(`0000039a--7b602ffb85` seg12-16, 5,999행)의 각 행에서 365차와 동일한 인덱싱(`j=round(routeApexDist/10)`, 동일 윈도우 조건 `j-1>=0`)으로 363차 원 `calculate_curvature()`를 verbatim 재적용해 실제 R을 직접 계산(`toolkit/sim_route_366_tp_proxy_r_crossvalidation.py`, 신규). 대리필터 통과 건수가 정확히 1233건으로 나와 365차 기록과 100% 재현(§28, 드리프트 없음 확인). 혼동행렬:

| 구분 | 값 |
|---|---|
| TP(실제 R<30m & 필터 통과) | 1137 |
| FP(실제 R>=30m & 필터 통과, 오염) | 96 |
| FN(실제 R<30m & 필터 탈락, 누락) | 0 |
| precision | 92.2% (1137/1233) |
| recall | 100% (1137/1137) |

임계값 스윕 결과 `routeApexSpeed<=25kph`로 좁히면 오염 0건(precision/recall 둘 다 100%, 1137/1137)으로 363차 R<30m 기준과 완전히 일치하는 순수 표본을 얻을 수 있음이 확인됨(45kph는 상한이 너무 느슨해 40~45kph 구간에서 오염 96건 대부분이 몰려 있음).

**변경 이유**: 365차의 "정탐 생존율 84~90%" 수치가 약 7.8% 오염된 모집단에서 나온 것인지, 순수 모집단에서도 동일 결론(평탄부)이 유지되는지 직접 확인이 필요했음(§28 성급한 결론 금지).

**재검증**: `sim_route_365_heading_isolation_gate.py --tp-speed-max 25.0`(순수 정탐 1137건)으로 재실행, 오탐 corpus도 이번에 확장 업로드된 `d1cd25bdf1` seg0-19(24,000행) 전체로 넓혀 29건->42건으로 확대:

| threshold | 오탐 억제율(42건) | 정탐 생존율(1137건, 순수) |
|---|---|---|
| 0.885 | 81.0% | 84.9% |
| 0.90 | 78.6% | 87.6% |
| 0.92 | 76.2% | 90.8% |

**새 결론**: (1) 대리필터(45kph)는 92.2% 정확 -- 완벽하진 않지만 365차의 정성적 결론(절벽 없는 평탄부)을 뒤집을 정도의 오염은 아님, 표본 순수화 후에도 평탄부 구조 유지됨을 확인해 **365차 결론이 보강됨**. (2) 다만 오탐 corpus를 29->42건으로 넓히자 억제율이 86.2%->76~81%로 낮아짐 -- 이는 365차 "미확인 사항 #1"(작은 표본, 일반화 미검증 우려)이 실제로 유효했음을 보여줌. 오탐 corpus 확장 시 억제율이 계속 낮아지는 경향이 있다면 th=0.885~0.92가 최종 채택 가능한 임계값인지 더 넓은 corpus로 확인 필요.

**코드 수정**: 하지 않음(검증 단계, ISOLATION 게이트 자체가 `ryu`에 반영된 적 없음).

**실차 검증**: 미실시(오프라인 재계산 한정).

**다음 작업**: WIP.md 366차 "다음 작업" 참고 (오탐/정탐 corpus 다변화 -> 최초 "new" 트리거 프레임 별도 재확인 -> 통과 시 게이트 반영 검토).

## 364차 -- [REGRESSION_CONFIRMED, 코드 미패치 -- NEEDS_DESIGN_REVISION] 363차 후속 "지속성 게이트(PERSIST)" 362차/363차 corpus 실측 -- `low_frac` 단일 파라미터로는 정탐 유지와 오탐 억제를 동시에 만족하는 임계값 부재 확인

**배경**: 363차가 RATIO 게이트(크기-비율)를 "채택 불가"로 판정하며 남긴 "게이트 재설계 필요" 후속. 364차 1부(WIP.md)에서 지속성 기반 대체안(PERSIST: 중심점 기준 완화된 임계값 `low_frac`을 만족하는 연속 구간 길이 `min_run` 조건)을 설계/정적 구현했으나 corpus 부재로 실측은 보류됐었음. 사용자가 362차 오탐 corpus(`d1cd25bdf1`, 24,000행)와 363차 급커브 corpus(`0000039a--7b602ffb85` seg12-16, 5,999행)를 제공해 이번 세션에서 실측.

**§24 dedup 확인**: FINDINGS.md 전체 `PERSIST`/`low_frac`/`지속성 게이트` 검색 -- 이번 항목 이전에는 364차 자체 기록이 없음(WIP.md에만 1부 존재). 363차 항목(RATIO)의 직접 후속이므로 아래에서 기존 결론/새 증거/변경 이유/새 결론을 명시.

**기존 결론(363차)**: 크기-비율 게이트(RATIO)는 오탐 억제 효과가 클수록 정탐 파괴도 함께 커지는 트레이드오프 관계이며, R<30m 급커브 기준 RATIO=0.3도 93% 손실, RATIO=0.5는 99% 손실 -- 단일 파라미터 재조정만으로는 해결 불가로 판정, 게이트 설계 자체의 재검토 필요.

**새로운 증거**(전부 `sim_route_364_gate_persistence_design.py --gate persist`, `min_run=2` 고정, `low_frac` 0.10~0.15 스윕):
1. 363차 급커브 corpus(R<30m, 1,140프레임) 정탐(실제 급커브) 생존율: `low_frac=0.10` 98%(1117/1140), `0.12` 96%(1098/1140), `0.13` 94%(1076/1140), `0.14` **21%**(243/1140).
2. 362차 오탐 corpus(R<50m 자동스캔, 170프레임) 오탐 생존율(=억제 실패율): `low_frac=0.10` 72%(123/170), `0.12` 69%(118/170), `0.13` 67%(114/170), `0.14` 66%(112/170).
3. `low_frac=0.13→0.14` 구간에서 오탐 생존율은 67%→66%로 사실상 변화 없음(억제 효과 거의 없음)인데 정탐 생존율은 94%→21%로 급락 -- 이 절벽 구간을 넘기는 것으로는 오탐 억제 이득을 얻지 못하고 정탐만 손실됨.
4. RATIO(363차) 기준선을 동일 363차 corpus에서 재확인(`sim_route_363_gate_sharp_curve_regression.py`) -- R<30m 급커브 생존율 RATIO=0.3 7%/RATIO=0.5 1%로 363차 원 결과와 일치(드리프트 없음).

**변경 이유**: PERSIST는 "이웃 1점과의 엄격한 비율"(RATIO) 대신 "완화된 임계값을 만족하는 연속 구간 길이"로 조건을 바꿔 RATIO의 정탐 파괴 문제를 완화하려 했다. `low_frac=0.10~0.13` 구간에서는 실제로 정탐 보존이 RATIO보다 크게 개선됐다(94~98% vs RATIO 1~7%). 그러나 동일 구간에서 오탐도 67~72%가 함께 살아남아, RATIO가 가졌던 "오탐은 확실히 억제"하는 장점을 잃었다. `low_frac`을 더 높여 오탐 억제를 강화하려 하면(`0.14`) 오탐은 거의 줄지 않고 정탐만 붕괴하는 절벽이 나타난다 -- 즉 정탐 corpus와 오탐 corpus의 `low_frac` 값 분포가 겹쳐 있어, 이 파라미터 하나의 임계값 조정만으로는 두 집단을 분리할 수 없음이 실측으로 확인됐다.

**새로운 결론**: PERSIST 게이트도(RATIO와 마찬가지로) 단일 파라미터(`low_frac`, `min_run=2` 고정) 조정만으로는 "정탐 유지 + 오탐 억제"를 동시에 달성하는 실용적인 임계값을 이번 corpus에서 찾지 못했다. RATIO보다는 정탐 보존 측면에서 개선됐으나 오탐 억제력이 부족한 상태(28~34% 억제)이고, 억제력을 높이면 RATIO와 유사하게 정탐이 붕괴한다. 게이트 코드는 `ryu`에 패치하지 않고 보류하며, 접근 자체(경로 구조 기반 원인 재분석, 거리 기준 지속성 조건, 시간축 이상 패턴 등)를 재검토해야 한다. 상세 방향은 WIP.md 364차 계속 "다음 방향 제안" 참고.

**검증**:
- 정적 분석: 완료(364차 1부에서 완료, 변경 없음)
- 로그 분석: 완료(363차/362차 corpus 각각 5,999행/24,000행, `naviPaths` 원본 기반)
- 시뮬레이션: 완료(RATIO 기준선 재확인 + PERSIST `low_frac` 스윕)
- 실차 검증: 미실시(게이트 코드 자체가 `ryu`에 반영된 적 없음, offline replay만)

**미확인 사항**:
- `min_run`을 2 외 값으로 바꾼 완전한 2차원 스윕은 미실시.
- 362차 오탐의 경로 구조적 원인(진행방향 정상성, 좌표 반전 등)은 이번 세션에서 직접 조사하지 않음 -- 다음 세션 1순위 제안.
- 거리(m) 기준 지속성 조건 재설계안은 코드로 존재하지 않음(제안 단계).

**362차 원 문제와의 관계**: 완만한 고속도로 커브 과감속(362차 원 finding) 자체는 여전히 미해결 -- RATIO에 이어 PERSIST도 채택 불가로 판정됨. 362차 finding 상태는 `ROOT_CAUSE_CONFIRMED, 코드 미수정 -- NEEDS_DESIGN_DECISION` 유지.

**다음 작업**: WIP.md 364차 계속 "다음 작업" 참고 (방향 확정 -> 경로 구조 재분석 또는 거리기준 재설계 -> 재검증 -> 패치).

## 363차 -- [REGRESSION_CONFIRMED, 코드 미패치 -- NEEDS_DESIGN_REVISION] 362차 계속2 "크기-비율 게이트"(RATIO=0.3/0.5) 회귀검증 결과 -- 실제 R≈20~35m급 급커브 다수를 오히려 억제함, RATIO 단독 설계로는 미채택 결정

**배경**: 362차 계속2가 "미확정 항목(단 1개)"으로 남긴 `RATIO` 값 확정. 원래 검증 대상이던 147/148차 corpus(`898edd0f96`, R≈27m급)는 §23(대용량 CSV 미보관)에 따라 재확보 불가였으나, 사용자가 대체 corpus(`0000039a--7b602ffb85` seg12-16, 2026-09-04, 경주/울산 IC 램프, R≈20~35m급 실제 급커브 -- qcamera로 단일 매끄러운 커브임을 시각 확인, desiredCurvature 노이즈 없이 단조증가)를 제공해 이번 세션에서 회귀검증 완료.

**§24 dedup 확인**: 이번 항목은 신규 이슈가 아니라 362차(원 finding)가 명시적으로 남긴 NEEDS_DESIGN_DECISION의 미확정 하위 항목(RATIO 값)에 대한 직접 후속 검증 -- 아래에서 기존 결론/새 증거/변경 이유/새 결론을 명시한다.

**기존 결론(362차 계속2, WIP.md)**: 크기-비율 게이트(좌우 인접 fine window 중 1개 이상이 `|curvature| >= RATIO * 중심점|curvature|`를 만족해야 채택, OR조건)로 362차 오탐 corpus(`d1cd25bdf1` 등, 완만한 고속도로 커브에서 신고된 27개 오탐 apex 지점)를 RATIO=0.3/0.5 둘 다 100% 억제. "0.3/0.5 둘 다 유효"로 판단, R~27m급 실제 급커브에서 정상 검출이 유지되는지는 원본 데이터 부재로 미검증 상태로 남김.

**새로운 증거**(전부 `sim_route_363_gate_sharp_curve_regression.py`, `route_curvature_macro_fine()` 원문 + 362차 계속2 확정 게이트 설계를 verbatim 포팅, §27):
1. 수동 선별 6프레임(t=2024.5~2032, IC 램프, NO-GATE 기준 R≈20~47m 급커브 검출) 중 "가장 급한 지점(global min R)"이 게이트 후에도 생존한 비율: RATIO=0.3 1/6(17%), RATIO=0.5 0/6(0%).
2. naviPaths 존재하는 corpus 전체(5,999프레임) 자동 스캔, NO-GATE 기준 R<50m 급커브 후보 1,584프레임: RATIO=0.3 518/1584(33%) 생존, RATIO=0.5 458/1584(29%) 생존.
3. 더 좁은 기준(R<30m, "진짜 급커브"만) 1,140프레임: RATIO=0.3 75/1140(**7%**) 생존, RATIO=0.5 17/1140(**1%**) 생존 -- R이 작을수록(더 급할수록) 생존율이 급격히 낮아짐.

**변경 이유**: 크기-비율 게이트는 "이웃 fine window 중 하나라도 중심점 곡률의 RATIO배 이상"을 요구한다. 362차 오탐(직선에 가까운 완만한 커브 위 고립 spike)은 이웃이 거의 0에 가까운 반면 중심만 튀는 패턴이라 게이트에 쉽게 걸린다. 그런데 실제 급커브(R≈20~35m)를 10m 간격 3점(fine, sample_fine=1)으로 근사하면, 원호 위 인접 윈도우들도 서로 다른 부분을 보게 되어 곡률값 자체가 자연스럽게 크게 흔들린다 -- 즉 "이웃 대비 상대적 낙폭이 크다"는 신호가 오탐과 정탐 모두에서 나타날 수 있고, 이번 실측에서는 정탐(실제 급커브) 쪽에서도 그 낙폭이 흔해 RATIO 조건을 못 만족하는 경우가 다수였다. 362차 계속2가 27개 오탐 지점 100% 억제만으로 "0.3/0.5 둘 다 유효"라 판단한 것은 통과 기준(오탐 억제) 한쪽만 확인한 것이었고, 회귀 기준(정탐 유지)은 이번이 최초 실측이다.

**새로운 결론**: RATIO 단독 크기-비율 게이트는 현재 형태로는 채택 불가. 오탐 억제 효과가 클수록(RATIO값이 높을수록) 정탐 파괴도 함께 커지는 트레이드오프 관계이며, 이번 corpus 기준 두 값 모두 정탐 파괴가 과도함(R<30m 급커브 기준 RATIO=0.3도 93% 손실, RATIO=0.5는 99% 손실). §27 최소변경 원칙에 부합하는 단일 파라미터 재조정만으로는 해결 불가능해 보이며, 게이트 설계 자체의 재검토가 필요.

**검증**:
- 정적 분석: 완료(게이트 로직 verbatim 포팅, Claude/ChatGPT 교차검증된 362차 계속2 설계와 diff 없음 재확인)
- 로그 분석: 완료(실차 로그 `0000039a--7b602ffb85` seg12-16, naviPaths 원본 좌표 기반, qcamera 시각 확인 경유)
- 실차 검증: 미실시(게이트 코드 자체가 `ryu`에 반영된 적 없음, offline replay만)

**미확인 사항**:
- 절대 곡률 임계값 병행/시간지속성 조건 등 대안 설계는 이번 세션에서 구현/검증하지 않음
- 147/148차 원래 corpus(`898edd0f96`, R≈27m)는 여전히 미확보 -- 이번 corpus(R≈20~35m)로 사실상 동급의 검증을 대체했다고 판단하나 완전히 동일한 corpus는 아님

**362차 원 문제와의 관계**: 완만한 고속도로 커브 과감속(362차 원 finding) 자체는 이번 세션으로 해결되지 않음 -- 후보 수정안(RATIO 게이트)이 채택 불가로 판정났을 뿐, 코드 패치는 여전히 없음. 362차 finding의 상태는 `ROOT_CAUSE_CONFIRMED, 코드 미수정 -- NEEDS_DESIGN_DECISION`을 유지(원인 확정은 불변, 수정안만 재검토 필요).

**다음 작업**: WIP.md 363차 "다음 작업" 참고 (게이트 재설계 방향 논의 -> 재검증 -> 패치).

## 365차 -- [362차 결론 보강, 코드 미수정 -- NEEDS_MORE_VALIDATION] 362차 고립 fine spike의 heading 레벨 메커니즘 규명 + "heading 집중도(ISOLATION)" 게이트 후보 초기 검증

**기존 결론(362차/363차/364차)**: 고립 fine spike는 확인됐으나 원인은 "인접 정합성 확인 없는 무조건 채택"이라는 코드 레벨 서술에 그쳤고, 이를 억제하려는 시도(RATIO=크기비율, PERSIST=지속프레임수)는 둘 다 실제 급커브(363차 corpus, R≈20~35m) 정탐을 광범위하게 파괴해 기각됨(FINDINGS.md 363차/364차).

**새 증거**: 362차가 특정한 두 오탐 지점(t=4426.417 dist=210m R=88.3m / t=4572.215 dist=210m R=92.0m, corpus `d1cd25bdf1`)의 `naviPaths` 원시 x,y,d 좌표를 직접 파싱해 각 10m 세그먼트의 heading(진행방향각)을 계산한 결과, **두 지점 모두 동일한 구조**를 보임:

```
t=4426.417 (d=130~280m 구간, heading 단위: 도):
d=130~210 (9세그먼트): heading 13.41~13.77 사이, 세그먼트간 변화 전부 <0.3
d=210->220 (1세그먼트): heading 13.72 -> 20.21  (변화 +6.497, 유일하게 큼)
d=220~280 (6세그먼트): heading 20.21~21.51 사이, 세그먼트간 변화 전부 <1.3

t=4572.215 (d=60~280m 구간):
d=60~210 (15세그먼트): heading 0.63~3.32 사이, 대부분 변화 <0.9
d=210->220 (1세그먼트): heading 3.32 -> 9.57  (변화 +6.243, 유일하게 큼)
d=220~280 (6세그먼트): heading 9.57~10.38 사이, 세그먼트간 변화 전부 <0.8
```

같은 corpus의 두 독립 오탐 지점이 **정확히 같은 dist=210m 격자**에서 **거의 같은 크기(6.2~6.5도)의 단일-세그먼트 heading 급변**을 보임. macro(sample=4, 40m 폭)로 같은 좌표를 재계산하면 R=309~8569m로 fine(R=88~92m)과 크게 갈린다 -- 40m 폭이 6.5도 꺾임을 넓은 baseline에 분산시켜 흡수하는 반면, fine의 20m 폭(sample_fine=1)은 이 국소 집중을 그대로 좁은 반경으로 계산한다.

**변경 이유**: 이 패턴(꺾임 앞뒤로 여러 세그먼트가 완전히 평평 + 꺾임 자체는 정확히 1개 세그먼트에 집중)은 "GPS 잡음"이나 "곡률 크기가 우연히 컸다"는 서술보다 훨씬 구체적인 기하학적 특징이다. 실제 도로는 완만하게(이 구간 전체로 보면 총 20도 안팎) 연속적으로 휘어지는데, navd raw waypoint 간격이 그 국소 구간에서만 유독 성기면 `resample_10m_np()`의 선형보간이 커브 전체의 각도변화를 그 한 세그먼트(raw waypoint 두 개를 잇는 직선 chord의 접합부, "vertex")에 몰아넣는다("polyline vertex aliasing" 가설).

**새 결론(초기, 추가 검증 필요)**: 곡률/속도 "크기"(RATIO)나 "지속 시간/거리"(PERSIST) 대신, **"이 지점의 heading 꺾임이 앞뒤 이웃 세그먼트 대비 얼마나 국소적으로 집중돼 있는가"**가 오탐/정탐을 구분하는 더 본질적인 특징일 수 있다. 이를 `isolation_score = 피크 꺾임각 / (진입 꺾임각 + 진출 꺾임각)`로 정량화(1.0=한쪽에 완전 집중=vertex 아티팩트 의심, 0.5=양쪽에 고르게 분산=진짜 커브 의심)해 `toolkit/sim_route_365_heading_isolation_gate.py`로 검증:

| threshold | 362차 오탐(29건) 억제율 | 363차 정탐(1233건, 대리필터) 생존율 |
|---|---|---|
| 0.80 | 96.6% | 8.4% |
| 0.85 | 89.7% | 12.2% |
| 0.885 | 86.2% | 83.9% |
...
RATIO/PERSIST가 보였던 "억제 개선 없이 정탐만 붕괴하는 절벽" 구조 없이, th=0.885~0.92에서 오탐 억제(86.2% 고정)와 정탐 생존(84~90%)이 동시에 높은 안정적 평탄부를 보인다 -- 이 지표가 RATIO/PERSIST보다 유의미하게 나은 분리력을 가질 가능성.

**미확인 사항(코드 패치 판단 전 필수 -- §28 성급한 결론 금지)**:
- 표본 크기 작음(오탐 29건, 그나마 유효 윈도우 확보되는 건 더 적음) -- 다른 corpus 일반화 여부 미검증, 과적합 가능성 배제 못함.
- 정탐 필터(`routeApexSpeed<=45kph`)는 363차의 "R<30m" 필터와 정확히 대응하는 지표가 아닌 대리(proxy) -- 363차 원 스크립트(`sim_route_363_gate_sharp_curve_regression.py`)의 실제 R 계산과 교차검증 필요.
- 오탐 29건 중 4건(apexDist=167.5/147.5/87.5/20.0, 10m 정수격자에 걸리지 않음 -- 최초 트리거 이후 접근 중 보간된 위치로 추정)은 `apexDist/10` 반올림 인덱싱이 실제 vertex 위치와 어긋나 낮은 점수(0.5~0.86)로 나왔을 가능성 -- 최초 "new" 트리거 프레임(정수 격자에 정확히 걸리는 시점)만 별도 재확인 필요.
- "vertex aliasing" 가설은 resample 이후 데이터(`naviPaths` 필드, 이미 10m 격자로 리샘플됨)만으로 방증한 것 -- navd가 실제로 보내는 raw(비-resample) waypoint 간격을 직접 계측한 것은 아님(현재 로그에 해당 필드 없음, 계측 패치 필요할 수 있음).
- apex 시간축(`routeApexIdx`/`routeApexDist`/`routeApexSpeed`의 프레임간 flicker 패턴, 219/220차 이월 항목)과의 상관관계는 이번 세션에서 다루지 않음.

**코드 수정**: 하지 않음(설계 검증 단계, ISOLATION 게이트 자체가 `ryu`에 반영된 적 없음).

**실차 검증**: 미실시(오프라인 재계산 한정, 게이트가 `ryu`에 없어 실차 검증 대상 자체가 아직 없음).

**다음 작업**: WIP.md 365차 "다음 작업" 참고 (정탐 필터 교차검증 -> 오탐 표본 확대 -> 통과 시 게이트 반영 검토).
...
### 4. E' 설계 확정 (358차 계속) -- 원 E안 폐기, capnp 필드 추가 불필요
사용자가 별도 세션 ChatGPT(지선생)와 독립적으로 `40ed6d9` 기준 호출
흐름을 재검증하도록 지시, Claude가 그 결과를 코드로 교차검증(§5/§35).
...
`static_freq_services`로 제한됨) -- 원 E안(carrotMan 메시지에 자체
age 필드 추가, capnp 스키마 변경 필요)을 **폐기**하고, capnp 필드
추가나 `carrot_man.py`/`carrot_serv.py`/`cereal/services.py` 어느
것도 건드리지 않고 소비처 5곳(`controlsd.py`/`lateral_planner.py`/
`cruise.py`/`selfdrived.py`/`carrot_functions.py`)에서만 로컬
`recv_time['carrotMan']` 기반 staleness 체크를 추가하는 방안으로
대체 -- 이하 **E'**로 표기. 이 패턴은 이미 저장소 내
`soundd.py:71`/`torqued.py:282`/`paramsd.py:320,357`에 선례 있음
(신규 패턴 아님, §27 최소변경에 더 부합).

`recv_time['carrotMan']` 초기값은 `SubMaster.__init__()`에서 0.0 --
freshness 판정에 `recv_time > 0` 가드 필수(첫 수신 전 stale 취급).

`carrot_man.py::broadcast_version_info()` 루프 순서 확인: `sm.update
...
try/except, 독립적으로 실패 흡수) -> `rk.keep_time()`. broadcast
네트워크 지연은 이번 프레임 carrotMan 송신 자체를 막지 못하고,
`carrot_navi_route()`/`update_navi()` 중 `pm.send` 이전 구간에서
발생한 예외만 outer `except: ... time.sleep(1)`(L1036-1039)로 이어짐
-- 358차가 지목한 Problem A 경로를 라인 단위로 재확정.

소비처별 stale fallback 설계 확정:
...
`plannerd.py`(`lateral_planner` 소속 프로세스, `sm = SubMaster([...,
...
**신규 발견(부수적)**: `cereal/custom.capnp`의 `desiredSpeed`(L29)/
...
latent 위험(§24 dedup 확인 -- FINDINGS.md 전체 검색 결과 기존 기록
없음, 최초 문서화). `vTurnSpeed`는 capnp 기본값(0)과 설계된 stale
fallback(0)이 같아서 문제 없음. "recv_time==0(미수신)도 stale 취급"
규칙을 `desiredSpeed`에 적용하면 이 latent 위험도 같은 패치로 함께
해소됨.

### 5. `CARROT_MAN_STALE_S` 임계값 실측 (358차 계속2, 실차 corpus 2건)
새 cereal 필드나 신규 실차 수집 없이 기존 `extract_log.py` 산출
CSV의 `carrotMan` 행 간 `t`(`logMonoTime/1e9`) 차이를 seg별로
그룹화해 계산하는 `toolkit/measure_carrotman_publish_gap.py`(신규,
319차 `group_orphan_episodes_319.py`의 `groupby('seg')['t'].diff()`
패턴 재사용, §21)를 작성, Master가 제공한 실차 corpus 2건에 실행.

corpus: `ryu` HEAD `40ed6d9`(357차, 무변경) 기준 스키마로 디코딩.

| corpus | seg 수 | carrotMan 행 | 유효 gap 샘플 | mean | p50 | p90 | p95 | p99 | max |
|---|---|---|---|---|---|---|---|---|---|
| x19seg(2026-09-09) | 19 | 22,798 | 22,779 | 0.0500s | 0.0500s | 0.0563s | 0.0593s | 0.0662s | 0.0849s |
| x20seg(2026-09-10) | 20 | 23,776 | 23,756 | 0.0500s | 0.0500s | 0.0548s | 0.0574s | 0.0644s | 0.0860s |

두 corpus(합산 46,535 gap 샘플, 39 seg) 전부 `dt>=0.5s` 임계값
초과 0건 -- `sleep1_suspect(1.5~2.5s)`/`unknown(>=2.5s)` 구간 관측
0건. 즉 **이번 corpus 기간(약 63분) 동안 `broadcast_version_info()`
예외 recovery(`time.sleep(1)`) 경로가 실제로 한 번도 발현되지
않았음**을 실측으로 확인(§29 -- 기존에 "실제 발현된 적 없음"으로
추정하던 것을 두 corpus에 대해 정량적으로 재확인, 다만 표본이 두
드라이브뿐이라 "발현된 적 없다"는 결론을 전체 운용기간으로 일반화할
수는 없음).

**한계(솔직히 기록)**: 이 실측은 두 가지를 확인해줄 뿐, `CARROT_MAN_
STALE_S` 확정값 자체를 실제 예외 사례로부터 도출해주지는 못한다.
1. 정상 운용 시 gap이 극도로 안정적(최대 0.086s, p99 0.066s)이므로
   0.3s 이상의 어떤 임계값을 선택해도 이 두 corpus 기준 false
   positive(정상 운행을 stale로 오판) 위험은 0%.
2. 그러나 예외 recovery(`sleep(1)`) 자체가 이 corpus에 없었으므로
   "실제 sleep(1) 발생 시 관측되는 gap 크기"는 여전히 이론값
   (~1.0~1.2s, 358차 WIP 추정)에 의존한다 -- 실측으로 대체되지 않음.
...
"전역 `frequency` 변경 시 새로운 SOFT_DISABLE 트리거를 만들 수 있다"는
점이 코드 레벨로 확정됨. 다만 실제 실차 장애 발생 사례는 아직 없음(§29).
대응 방향은 **E'**(소비처 5곳 로컬 staleness 체크, 위 4번)로 확정,
남은 것은 `CARROT_MAN_STALE_S` 값 하나.

**검토 중인 대응 후보** (E' 확정, 코드 미수정 상태, 다음 세션 결정 필요):
- A. 현행 유지 (근거 약화됨, 권장 안 함)
- B. `broadcast_version_info()` 단계별 try 격리 (E'와 병행 가능,
  독립적 개선)
- C. 예외 시 `sleep(1)` → 프레임 주기로 단축(+ 연속실패 backoff)
  (E'와 병행 가능)
- D. `update_navi()` 발행부만 얇은 try로 보호 (E'와 일부 중복)
- **E'. [채택]** 위 4번 설계. 소비처 5곳에 `recv_time['carrotMan']`
  기반 로컬 staleness 체크 추가. `CARROT_MAN_STALE_S` 확정값만 남음
  -- 위 5번 실측 한계상 완전한 경험적 확정은 어려우며, (a) 이론적
  worst-case(~1.0~1.2s)에 안전마진을 더한 값(1.5~2.0s, 기존 후보
  유지)으로 사용자가 결정하거나, (b) 예외가 실제 발생한 corpus를
  추가로 확보할 때까지 보류하는 두 선택지 중 사용자 판단 필요.

**검증**: 정적 분석(코드 직접 열람, `cereal/messaging/__init__.py`
alive/FrequencyTracker 로직 포함) 완료. 로그 분석 완료(위 5번, corpus
...
전부 같은 프레임에 B(새 apex)를 즉시 재획득하고 1프레임 이상
생존한다는 것까지는 확인됐으나, "그 재활성이 실제 route_active
재진입/제어개입까지 이어지는지는 스크립트 출력만으로는 미확인"으로
347차까지 계속 이월됨.

**새로운 증거(348차)**: `sim_route_348_active_reentry_trace.py`(신규 --
301차 A->LOST->B 추적 + 300차 `ActualLayer`의 `route_active` bool을
프레임별로 결합, §21/§27 -- 기존 로직 무변경) 로 7건 각각을 개별
트레이스:

| corpus | t(lost) | B 생존 | route_active 재진입 | 재진입 gap | 재진입 시 apex_dist | 같은 B? |
|---|---|---|---|---|---|---|
| x19seg | 634.00 | 0.51s/10f | O | +0.90s | 60.0m | **아니오(신규)** |
| x19seg | 635.46 | 0.30s/6f  | O | +0.96s | 50.0m | **아니오(신규)** |
| x19seg | 636.86 | 0.36s/7f  | O | +1.10s | 40.0m | **아니오(신규)** |
| x19seg | 638.35 | 0.36s/7f  | O | +1.15s | 30.0m | **아니오(신규)** |
| x19seg | 639.81 | 0.36s/7f  | O | +13.95s | 90.0m | **아니오(신규)** |
| x19seg | 655.66 | 5.00s/100f | O | +95.49s | 80.0m | **아니오(신규)** |
| x10seg | 162.76 | 0.95s/19f | O | +72.95s | 60.0m | **아니오(신규)** |

(탐색 범위를 60초 제한 없이 전체 로그 끝까지 확장한 결과. 60초 제한
기준으로는 x19seg 마지막 1건이 미재진입으로 나옴 -- 위 표는 무제한
탐색 결과.)

**변경 이유**: 346/347차가 "재활성 자체는 빠르다"까지만 관측하고
멈췄던 지점에서, 이번엔 `route_active` bool 자체를 프레임별로 저장해
"그 재활성이 언제 다시 ACTIVE를 만드는가"를 직접 추적. 추가로 그
재진입이 "같은 B가 살아있는 동안" 일어난 것인지, "B도 다시 끊긴 뒤"
일어난 것인지 구분(B_survive_frames 창 안/밖 여부)을 새로 계산.

**새로운 결론**:
1. **route_active 재진입 자체는 7/7건 전부 결국 발생한다**(무제한
   탐색 기준). 다만 개별 gap은 0.90s~95.49s로 편차가 매우 크다.
2. **0/7건에서 이 재진입이 즉시 재획득된 B가 살아있는 동안 일어나지
   않는다** -- B는 이번에도 짧게(0.3~5.0s) 생존한 뒤 반드시 다시
   끊기고, 그 후 한참 뒤(대부분 apex_dist가 원래보다 크거나 전혀 다른
   값, 즉 물리적으로 다른 지점)에야 새 apex가 INERT->ACTIVE 게이트를
   통과한다.
3. **"lost된 목표가 곧 회복되어 제어가 자연스럽게 이어진다"는 낙관적
   해석은 실측 7건 어디에서도 뒷받침되지 않는다** -- 재진입은 있지만
   그것은 항상 "그 커브의 회복"이 아니라 "다음(별개의) 커브"에 의한
   것. 즉 강제 RELEASE=lost가 발생한 그 특정 apex에 대한 제어 개입은
   7건 모두 끝까지 복구되지 않고 유실된 채로 남는다(그 이후의 route
   제어 자체가 끊기는 것은 아니며, 단지 이번에 놓친 그 목표는 되찾지
   못한다는 의미).
4. x19seg 6건 중 처음 5건(t=634.00~639.81, 연속된 grid 전환으로 보이는
   짧은 간격)은 재진입 gap이 0.9~14s로 비교적 빠르지만, 마지막
   1건(t=655.66, B 생존 5.0s로 6건 중 가장 김에도)은 gap이 95.49s로
   훨씬 김 -- B 생존시간과 재진입까지 걸리는 시간 사이에 뚜렷한
   상관관계는 관측되지 않음(표본 7건으로 결론 내리기엔 이름).

**한계(§28)**: (1) 표본 7건으로는 "재진입까지의 시간"이 corpus/도로
유형에 얼마나 의존하는지 통계적으로 확정할 수 없음. (2) 이 corpus들은
343차 패치 이전 채록이라(347차와 동일 한계) 343차 이후에도 이 패턴이
동일한지는 별도 확인 필요. (3) "재진입 시 apex_dist가 크다"는 것을
"실제로 다른(더 앞쪽) 커브"라고 해석했으나, qcamera 대조로 물리적
동일/이질 여부를 직접 확인하지는 않음(추론이지 실측 대조는 아님 --
gap이 73~95초에 달하고 apex_dist가 첫 lost 시점보다 크다는 간접
정황에 근거).

**실차 검증**: 미실시(오프라인 로그 재생 한정, §29).

**[349차 갱신] 한계 (3)의 qcamera 직접 대조 완료**: 위 한계 (3)에서
간접 추론으로 남겨뒀던 "재진입 시점이 실제로 다른 커브인지"를 gap이
가장 긴 2건(x19seg 6번째, x10seg)에 대해 `verify_and_extract_frames.py`
로 lost/reentry 두 시점의 qcamera 프레임을 자동 매칭 추출(diff 전부
0.024s 이내)하고, `vpPosPointLatNavi`/`LonNavi`로 두 시점 간 GPS
직선거리를 계산해 직접 확인.

| corpus | lost 시각/apex_dist | reentry 시각/apex_dist | gap | GPS 이동거리 | qcamera 육안 |
|---|---|---|---|---|---|
| x19seg 6번째 | t=655.36, 70m | t=751.16, 80m | +95.49s | 621.6m | 아파트단지 도로 -> 가드레일+산 배경의 완전히 다른 도로 |
| x10seg | t=162.46, 20m | t=235.71, 60m | +72.95s | 104.9m | 선행차에 가려진 건물 앞 콘 구간 -> 좌회전 차선 있는 대형 신호교차로 |

두 사례 모두 lost 지점과 reentry 지점의 화면이 완전히 다른 장소이며
GPS 거리도 100~600m 이상 떨어져 있어, 결론 3번("재진입은 그 apex의
회복이 아니라 다음 apex에 의한 것")의 간접 추론 부분이 qcamera+GPS
직접 증거로 확정됨(기존 결론 변경 없음, §24 -- 근거 보강). 나머지
5건(gap 0.9~14s, 연속 grid 전환 구간으로 판단)은 gap이 짧아 이번
직접검증 대상에서 제외.

**주의(컨테이너 재시작으로 인한 제약)**: 이번 검증에 사용한 비교
이미지 2장(`x19seg_349_lost_vs_reentry.jpg`,
`x10seg_349_lost_vs_reentry.jpg`)은 세션 컨테이너 초기화로 원본 프레임
파일이 유실되어 이 patch에는 포함되지 못함(§16). 수치(gap/GPS거리)와
qcamera 육안 확인 결과(장면 서술)는 초기화 전 세션 로그에 기록된 대로
보존해 반영. 이미지 자체가 필요하면 동일 corpus(x19/x10seg)로 재추출
가능.
...
## 347차 -- [실측 보강] 4개 추가 corpus(x19/x6/x10/x16seg)로 "강제 RELEASE=lost" 발생 빈도 재확인 -- corpus마다 발생 여부가 갈린다는 346차 가설을 6개 corpus(발생 3/미발생 3)로 보강, continuity 설계 전제는 401/401건(100%) 예외 없이 성립

**기존 결론(346차)**: x20seg 1개 corpus에서 강제 RELEASE=lost 0건 --
route1~4(300차, 15/15)와 정반대. "corpus(주행 상황)에 따라 이 패턴의
발생 여부가 크게 달라짐을 시사"하나 "단일 route 결과이므로 일반화
금지"로 유보.

**새로운 증거(347차)**: 335~339차가 이미 채록/분석했던 x19/x6/x10/
x16seg 4개 corpus(2026-09-09 채록, device `823943a6`, 343차 패치
이전)를 재업로드받아 `sim_route_346_lost_freq_single_route.py`(346차
스크립트, 코드 변경 없이 그대로 재사용)로 동일 분석 수행:

| corpus | mode=="lost" | apex_dist>0 비율 | 강제 RELEASE=lost |
|---|---|---|---|
| x20seg(346차) | 88 | 100% | 0건 |
| x19seg | 151 | 100% | **6건** |
| x6seg | 44 | 100% | 0건 |
| x10seg | 61 | 100% | **1건** |
| x16seg | 57 | 100% | 0건 |
| route1~4(300차) | -- | -- | 15건(15/15) |

**변경 이유**: 346차는 "0건 corpus"(x20seg)와 "15/15 corpus"(route1~4)
각 1개씩만 가지고 있어 어느 쪽이 전형적인지 판단할 수 없었다. 4개
corpus를 추가하자 강제 RELEASE=lost가 발생한 corpus(x19seg 6건,
x10seg 1건)와 발생하지 않은 corpus(x6seg, x16seg)가 각각 나와, 표본이
발생 3개(route1~4/x19seg/x10seg) : 미발생 3개(x20seg/x6seg/x16seg)로
균형 있게 늘었다.

**새로운 결론**:
1. **continuity 설계 전제("lost는 apex_dist>0 상태에서만 발생")가 이제
   5개 corpus 합계 401건(88+151+44+61+57) 전부에서 예외 없이 성립** --
   x20seg 1개(88건)에 그쳤던 346차보다 표본이 4.5배로 늘어난 채로 재확인.
2. **"강제 RELEASE=lost" 발생 여부는 corpus(주행 상황)마다 실제로
   갈린다**는 346차의 가설이, 우연일 가능성이 낮은 수준(발생 3 : 미발생
   3)으로 보강됨. 즉 "이 패턴이 위험하다/안전하다"를 코드 자체의
   속성만으로 일반화할 수 없고, 어떤 주행 상황(corpus)인지가 결정적
   변수임을 재확인.
3. **강제 RELEASE=lost가 발생한 7건(누적: route1~4의 15건은 이번
   집계에서 제외, x19seg 6+x10seg 1) 전부 같은 프레임에 즉시 재획득,
   전부 1프레임 이상 생존** -- 346차의 "재활성 자체는 빠르다(x20seg
   99%<1초)"는 결론과 방향이 일치. "그 재활성이 실제 route_active
   재진입/제어개입까지 이어지는가"는 여전히 미확인(다음 단계 과제,
   346차와 동일하게 이월).

**한계**: 이번 4개 corpus 전부 2026-09-09 동일 채록 세션(343차 패치
이전, device `823943a6`) 산출물이라 "촬영 시점/도로 유형"과 "코드
버전"이 완전히 분리되지 않은 confound 가능성이 있음(단, continuity
로직 자체는 코드적으로 동일하므로 결과 신뢰성에는 영향 없음). 표본
6개 corpus는 여전히 통계적으로 확정된 "발생률"을 낼 만큼 크지 않음.

**실차 검증**: 미실시(오프라인 로그 재생 한정, §29). 이 corpus들은
343차 패치 이전 채록이므로 343차 실차검증 용도로는 사용 불가(별도
과제로 계속 이월).
...
목표속도" / "apex가 좌회전 교차로였고 그 지점에서 신호로 정지" 두
시나리오의 release 여부를 코드 분석만으로 답한 뒤, 사용자가
`000003d4--59a8ae5773`(x20seg, 20260910 재촬영본)를 재업로드해 실측
검증 요청.

**신규 발견 -- 이 로그는 343차(speed_reached 삭제) 패치 미반영 상태**:
`check_device_build.py --compare-commit da7ab36f` 결과 device
gitCommit=`7b3dfec4`(336차)+dirty=True -- `da7ab36f`(343차)의 조상일
뿐 후손 아님. git 메타데이터만으로 단정하지 않고, 신규
`toolkit/verify_release_variant_344.py`로 실측 텔레메트리
(routeApexMode/Dist/Speed -- 코드가 이미 계산해 발행한 값 그대로,
재도출 아님)를 speed_reached 포함/제거 두 가설로 재생해 교차검증한
결과도 동일 결론: 3프레임(150ms) 이상 지속된 `src=='route'` run
118건 중 **19건이 dist_reached/apex_passed_or_lost가 아닌
speed_reached 단독 조건으로 정확히 그 프레임에 종료**됨(343차 패치가
실제 반영됐다면 이 경로 자체가 존재 불가). git 메타데이터/텔레메트리
재생이라는 서로 독립적인 두 방법이 동일 결론에 도달 -- **343차 패치가
이 로그 기록 시점 디바이스에는 아직 반영되지 않은 것으로 판단**
(§28 -- 확실성 등급: **측정 확인**, 다만 "패치가 왜 반영 안 됐는지"의
원인 자체는 미확인/사용자 확인 필요).

**시나리오 1(리드차량 서행/정지) 실측**: t=1290~1298s, 리드차량 거의
정지(`leadVLead` 0.2~1.2km/h) 상태로 접근 중, apex_dist 20~50m
(dist_reached 미충족)/apex_mode `matched` 유지 상태에서도 vEgo가
`apex_speed*1.05` 이하로 떨어질 때마다 route release(t=1294.2s/1297.1s,
각 3프레임). 341/342차 flapping 메커니즘의 실제 발현 사례로 판단됨 --
새로운 별개 메커니즘 아님(§24, 기존 항목 보강). 위 신규 발견에 따라
이 로그가 343차 이전 코드이므로, **343차 패치의 실제 효과(이 release가
사라지는지)는 여전히 미확인** -- 다음 실차 검증 필요.

**시나리오 2(좌회전 apex + 신호대기 정지) 실측**: t≈1406~1530s,
`xTurnInfo==1` 접근 중 route가 apex_dist 100m→10m까지 정상 개입 후
apex_dist가 물리적으로 0에 도달, 이후 약 100초 완전 정지(신호대기).
정지 내내 `apex_dist=0.0`(10m 이내, dist_reached 조건 항상 충족)이고
`apex_mode`는 `matched`<->`passed` flicker -- **이미 위(322차 계속,
원인 B) 문서화된 것과 동일 현상**(vEgo≈0 근처 부동소수점 잡음에 의한
`predicted<=0` 0-경계 판정, 로직 결함 아님, 신규 원인 아님, §24로
중복 생성하지 않음). 정지 내내 `src=='road'`, route가 arbitration에서
이기는 프레임 0건 -- dist_reached가 이미 상시 충족돼 이 flicker는
release 판정에 실질적 영향 없음. **직전 대화의 코드 기반 예측("정지선이
apex 10m 이내면 정지 중 release 유지")이 실측으로 확인됨** (확실성
등급: **측정 확인**, 단 apex의 물리적 위치와 실제 정지선 간 정확한
거리는 qcamera 대조 미실시).

**영향받는 실차 제어 로직**: 없음(`ryu` 코드 변경 없음, ANALYSIS_ONLY,
toolkit 신규 스크립트 1개 추가).

**검증**: 정적 분석(`py_compile`) + 재현성(`verify_release_variant_344.py`
동일 CSV 재실행, 동일 수치 재확인: mismatch 33/0.14%(WITH speed_reached)
vs 881/3.71%(WITHOUT), speed_reached 단독 종료 19/118건) + 로그 직접
대조(pandas). 실차 검증: 해당 없음(사후 로그 분석).

**미확인 사항**: 343차 패치의 사용자 로컬 적용/커밋/디바이스 재빌드
완료 여부. 좌회전 apex의 물리적 위치와 정지선 간 정확한 거리(qcamera
대조 미실시).

**다음 작업**: 343차 패치 반영 여부 사용자 확인 -> 반영 후 동일/유사
리드차량 서행 상황 재주행하여 시나리오 1 release 소멸 직접 재확인.
...
켜졌다 하는 증상")를 받고 t=1076~1085 구간에서 `required_decel_mss`가
임계값 근처에서 흔들려 ACTIVE/INERT 상태가 프레임 단위로 뒤집힐 것이라는
**가설(미확정)**을 남겼다("required_decel_mss 프레임별 직접 재구성해
인과 확정"이 340차의 다음 세션 최우선 과제).

**이번 세션 작업**: 동일 route(x20seg, 340차와 동일 파일 재업로드,
`extract_log.py` 재추출 시 23,776행으로 340차와 완전 일치 재현 확인)를
사용해 `toolkit/sim_route_273_active_gate_relax_sensitivity.py`의
`ContinuityApprox`/`confidence_from_streak`/게이트 산식(§27 -- 산식
자체는 변경하지 않고 그대로 재사용, §21)을 풀어써 새 관측 스크립트
(`toolkit/diag_required_decel_341.py`)로 `src` flapping 그룹13
(t=1075.99~1084.84, 실측 26회 전환, steeringAngleDeg 평균 0.26°=직선
확인)을 프레임별 대조했다.

**핵심 결과(코드추적+실측 직접 대조로 인과 확정, §28)**: 이 구간의
`src` 토글은 (a) `routeApexIdx`가 10m grid 경계를 넘어 감소
(예: 12->11->10->9->8...)하는 매 전환 프레임에서, (b) 그 순간
`routeApexSpeed`가 이전 프레임 대비 갑자기 큰 폭으로 상승(관측 사례
+1.6~+5.6kph, 대부분 도로제한속도 근접 값인 ~48~50kph로 점프)하면서,
(c) 순간적으로 목표속도(target)가 `vEgo`를 넘어서 INERT 판정 분기
(`v_ego<=target`)를 프레임 1개만 통과시키고, (d) 다음 프레임에서
`apexSpeed`가 다시 정상적으로 하강하며 `required_decel_mss`가 재상승해
즉시 ACTIVE(=route)로 복귀하는, **매우 규칙적인 단일 프레임
드롭아웃-재진입 패턴**임이 확인됐다.

이 인과관계는 실측 텔레메트리(내 근사 시뮬레이션이 아니라 실제 CSV의
`src`/`routeApexIdx`/`routeApexSpeed` 컬럼 직접 비교)로 전체 로그
(23,776행) 기준 통계적으로 검증됨:
- "route -> 비route(1프레임) -> route" 형태의 단일 프레임 드롭아웃:
  전체 46건.
- 그 중 `routeApexIdx`가 해당 프레임에서 실제로 변한(=grid 경계 전환)
  경우: 43건(**93.5%**).
- 그 43건 중 같은 프레임에서 `routeApexSpeed`가 상승(양수 delta)한 경우:
  40건(**93.0%**, 두 조건 결합 시 전체 46건 중 87.0%가 "grid 전환 +
  apex_speed 상승"으로 직접 설명됨).

**340차 가설과의 관계(정정)**: 340차는 "요동이 `required_decel_mss`를
임계값 근처에서 프레임마다 넘나든다"는 포괄적 가설을 세웠다. 이번
세션에서 확정된 것은 그보다 **더 구체적인 단일 메커니즘**이다 --
`required_decel_mss`가 임계값 부근에서 막연히 진동하는 것이 아니라,
grid 경계 전환 시 `apexSpeed`(목표속도) 자체가 순간적으로 도로제한
근처까지 튀어올라 `v_ego<=target` 조건이 정확히 1프레임 성립하는
것이 직접 원인이다. 즉 "9m 그리드 apex_speed 계단식 흔들림"(219차가
이미 문서화한 현상)이 매 grid 전환마다 **일시적으로 목표속도를
현재속도 위로 넘기는 스파이크**를 만들고, 이 스파이크가 INERT
판정 분기를 정확히 1프레임 통과시키는 것이 확정된 인과 경로다.

**FINDINGS.md 244차/288차와의 관계(§24 대조)**: 244차(터널 position-identity
vs 후보전환)/288차(margin blend 불일치)와도 다른, **세 번째 별개
경로**로 확인된다 -- candidate identity(routeCandidateCount)나
margin(1.1) 트리거가 아니라, grid 전환 시 apex_speed 후보 자체가
과도하게(도로제한속도 근처까지) 튀는 것이 원인이다.

**검증**: 정적 분석(스크립트 재사용 py_compile 없음, 단순 실행) +
독립 재추출 재현성(23,776행 340차와 완전 일치) + 로그 검증(전체 CSV
통계 기반, 43/46건·93.5%, 40/43건·93.0% 직접 카운트) + 시뮬레이션
(sim_route_273 게이트 로직으로 grid13 구간 프레임별 재생, gate 판정과
실측 src 일치 확인). **실차 재검증: 불필요**(기존 로그 재분석,
`ryu` 코드 변경 없음).

**미확인 사항**:
1. 나머지 6.5%(단일프레임 dropout 중 grid전환과 무관한 3건, t=828.74/
   849.94/1138.44)의 원인 -- 이번 세션 범위 밖(§27, 주 원인 규명이
   목적이었고 소수 예외 케이스는 별도 조사 필요).
2. 이 "매 grid 전환마다 apex_speed가 도로제한 근처로 튀는" 현상 자체의
   상류 원인(`route_curvature_macro_fine`/후보 산출 로직 어느 지점에서
   grid 경계마다 이런 값이 나오는지)은 이번 세션에서 code-level까지는
   추적하지 않음(다음 세션 과제, 하류 영향 경로 확정을 먼저 완료).
3. 이 flapping이 실제 종방향 제어 출력(가속/감속 명령)에 체감 가능한
   영향을 주는지는 여전히 미확인(340차가 이미 남긴 미확인 사항, aEgo
   기반 탐지 미실시).

**다음 작업**:
1. `route_curvature_macro_fine`/apex 후보 산출 경로에서 10m grid
   경계마다 apex_speed가 급상승하는 근본 원인 code-level 추적.
2. 수정 방향 논의(§27/§34 -- Master 확인 후, grid 경계에서 apex_speed에
   최소 유지/보간을 추가할지 여부는 코드 임의수정 금지 원칙상 사용자
   승인 필요).
3. `diag_required_decel_341.py`를 다른 flapping 그룹(8/9/11/12/14/16 등)
   에도 적용해 동일 메커니즘 비율 확인(현재는 그룹13 1건만 상세 대조).
...
오프라인 `apex_dist<0` 사례가 각 1~2건 있었으나 전부 `vEgo`≈5.5m/s대
저속 프레임이라 곡선속도제어 문맥과 관련 있는지 불확실(qcamera 대조
없이는 결론 보류)."

**정정(§24, 새로운 증거)**: 이 5건의 실측 `vEgo` 컬럼을 직접 대조한
결과, "5.5m/s대"는 실제로는 offline으로 산출된 `apex_speed`(곡선구간
목표속도, 194차/195차가 이미 확인한 `V_CURVE_LOOKUP_BP/VALS` saturate
현상과 동일 계열 -- 급한 곡률에서 5kph로 고정되는 것)였다. **실제
`vEgo`는 5건 모두 15.8~43.6km/h로 정상 주행 속도**였음(정차/서행 아님):

| route | t | offline apex_speed(kph) | 실제 vEgo(kph) | qcamera |
|---|---|---|---|---|
| x6seg | 1409.454902725 | 5.5 | 41.8 | 옥외 교차로, 도로 좌측 굴곡 |
| x10seg | 154.811047678 | 5.7 | 15.8 | 옥외, 좌회전 화살표+커브 |
| x10seg | 245.309976998 | 5.6 | 28.7 | 옥외 교차로, 회전 표지판 |
| x16seg | 1648.416400778 | 5.5 | 17.4 | 옥외, 커브경고 표지판 있는 실제 우커브 |
| x16seg | 1836.167584049 | 58.8 | 43.6 | 옥외, 완만한 다차로(급커브 아님) |

335차의 "곡선속도제어 문맥과 관련 있는지 불확실, qcamera 대조 없이는
보류"라는 판단 자체는 §28 원칙상 정당했으나(당시 미확인이었으므로),
근거로 든 속도 수치가 실제로는 offline apex_speed였다는 점은 정정이
필요함. qcamera 대조 결과 5건 중 4건은 실제 도로 곡선(교차로/커브
경고표지 등)과 위치·시점이 부합하는 정황이 확인됐고, 나머지 1건
(x16seg t=1836)은 완만한 도로로 offline apex_speed도 saturate 값이
아니어서(58.8) 애초에 이 유형과 무관했음이 재확인됨.

**신규 발견(결정적) -- x6seg 실제 production `routeApexMode='new'` +
`routeApexDist<0` 사례 5건**: 335차/337차는 offline 재현이 아닌 **실측
텔레메트리에서 직접 관측된 사례**를 x19seg 1건만 확보했었다. 이번
회차에서 x6/x10/x16seg 전체를 `routeApexMode=='new' AND
routeApexDist<0` 조건으로 전수 재스캔한 결과 **x6seg에서 5건**을
추가 확인(x10seg/x16seg는 0건):

| t | apexDist | src | vEgo(kph) | 환경(qcamera) |
|---|---|---|---|---|
| 1409.607477381 | -40.0 | vturn | 41.7 | 옥외 교차로 |
| 1501.554057463 | -40.0 | vturn | 17.9 | **지하주차장 내부** |
| 1505.201206372 | -40.0 | vturn | 7.9 | **지하주차장 내부** |
| 1506.30425704 | -40.0 | bump | 6.9 | **지하주차장 내부** |
| 1514.001617419 | -40.0 | vturn | 9.9 | **지하주차장 내부** |

5건 중 4건이 지하주차장 내부에서 발생했음을 qcamera로 직접 확인 --
실내 공간, 주차 차량, 조명 등이 명확히 보임. `ccPoseValid`/
`naviPointsActive`/`navdActive`는 이 4건 전부 `True`로 정상 기록돼
있어(즉 코드상 GPS/navi 상태 플래그만으로는 "실내 GPS 저하 상황"을
구분할 수 없음), 오직 qcamera 육안 대조로만 확인 가능했다(§28, 계측
필드만으로는 알 수 없는 사각지대).

**해석**: 지하주차장 4건은 실제 도로 곡선과 무관한 환경에서
`route_local_curve_merge()`가 의미 없는 근접 클러스터를 만들어낸
것으로 추정된다(코드 레벨 확정은 아님) -- 이는 330~337차가 다뤄온
"옥외 도로에서의 커브 tail 오탐" 메커니즘과 별개의 새로운 유발 조건
(실내/GPS 열화 환경)일 가능성이 있다. 단 이번 6건(x19seg 1건 +
x6seg 5건) 전부 `src`가 `route`가 아니었으므로(vturn/road/bump),
337차와 동일하게 이번에도 실제 `desiredSpeed` 출력에는 영향이 없었다.

**변경 이유**: 335차 WIP "다음 작업"(x6/x10/x16seg 저속 사례 qcamera
대조) 완료.

**현재 상태(미해결, 수정안 설계는 미착수)**: 실제 나쁜 사례가 x19seg
1건 + x6seg 5건으로 확대 확인됨. 지하주차장 4건의 정확한 유발
메커니즘은 코드 레벨로 추적하지 않음(§28, 필요시 `routeOrphanRawPath`
원본 좌표 검토로 추가 확정 가능). 수정안(§26/§27) 설계 착수 여부는
여전히 사용자 확인 대기 -- 331차 "0 클램프가 유일한 정답은 아님"
유보도 계속 유지.

**패치 범위**: 없음(이번 회차도 코드 변경 없음, 순수 실측/qcamera
대조 분석, `ryu` 소스 무변경).

**실차 검증**: 이번 회차 자체가 실차 로그(x6/x10/x16seg)+qcamera
기반 확인.

**관련**: WIP.md 338차/337차/335차/332차, FINDINGS.md 335차/332차/330차,
`toolkit/sim_route_331_ws_negative_downstream.py`/
`sim_route_332_ws_negative_real_corpus.py`/`verify_and_extract_frames.py`
(재사용, 신규 코드 없음).

---

## 335차 -- [실측 확인, 미해결·수정안 착수는 사용자 승인 대기] 332차가 "관측 안 됨"으로 이월했던 실제 `routeApexMode='new'` + `routeApexDist<0` 사례를 신규 route(x19seg)에서 최초 확인 -- offline replay와 diff=0.0

**기존 결론(332차)**: `ws<0` 라벨이 apex_dist로 전파되는 메커니즘은
오프라인 재생(x18seg 3645프레임 중 946건 `ws<0`, 그중 71% 음수 apex_dist
예측)으로는 구조적으로 100% 재현됐으나, 실제 production 텔레메트리
`routeApexDist`는 x18seg 20,498행 전체에서 음수가 단 0건이었다. 원인은
"근접(`ws<0`) orphan이 클러스터로 승격되고 + 마침 그 순간 'new' lock되는"
두 조건의 정렬이 이 corpus에는 우연히 없었기 때문으로 추적됐고, "미관측
== 무해 아님"이라며 다른 route에서의 확인을 이월했다(§28).

**새로운 증거(335차, 사용자 업로드 신규 route x19seg -- 327~334차가
반복 사용한 x18seg와 무관한 처음 보는 route, `toolkit/
sim_route_332_ws_negative_real_corpus.py` 재사용, 재구현 없음)**:
- x19seg(22,798행, orphan-raw-path 존재 1013프레임) 전수 스캔 결과,
  `t=481.858660709`에서 **실측 `routeApexMode='new'`, 실측
  `routeApexDist=-40.0`**을 확인. 같은 프레임의 offline replay 예측값도
  `-40.0`으로 **diff=0.0 완전 일치**.
- 이 프레임의 근접 클러스터 구성: `n_clusters=1`,
  `cluster_dists=[-40.0]` -- 근접 orphan이 다른 경쟁 클러스터 없이
  단독으로 승격되어 그대로 'new' lock의 대상이 됨(332차가 이월한 정렬
  조건이 정확히 이 형태로 충족됨).
- 원본 CSV로 전후 프레임 문맥 확인: 이 시점 전후 약 0.65초 동안
  `routeApexMode`가 `held→matched→passed→none→new(+124.0, 정상)→
  held→matched→held×3→lost→none×3→new(-40.0)→passed→matched→...`로
  두 차례 lock 상태가 교체되는 flicker 구간에 위치. `vEgo`≈16.3m/s
  (≈59km/h), `vTurnSpeed`는 124~135 범위로 flicker 동안 크게 변하지
  않음(즉 이 특정 사례에서 목표속도 자체의 급변은 없었음).
- 이 프레임을 "ACTIVE 상태였다고 가정"하고 330차 게이트 로직(`_route_
  cluster_continuity_step()` apex_dist 채택 + ACTIVE RELEASE 판정)에
  대입하면 `[ACTIVE] RELEASE`가 즉시 트리거된다 -- 330차가 코드추적으로
  예측한 결과 그대로. 단 이 프레임에서 실제로 ACTIVE 상태였는지는 로그에
  해당 상태 컬럼이 없어 **직접 확인은 안 됨**(§28, 확정 아님 -- 조건부
  결과로만 기록).
- 참고(등록하지 않음): 나머지 신규 route(x6seg/x10seg/x16seg) 3개에서도
  오프라인 `apex_dist<0` 사례가 각 1~2건 있었으나 전부 `vEgo`≈5.5m/s대
  저속 프레임이라 곡선속도제어 문맥과 관련 있는지 불확실(qcamera 대조
  없이는 §28에 따라 결론 보류).

**해석**: 330~332차가 코드추적/synthetic/오프라인재생으로 예측한 결함
전파 경로("`ws<0` → apex_dist 그대로 노출 → ACTIVE RELEASE 조기 트리거")가
**실제 production 텔레메트리에서 최초로 직접 관측**됨(offline 예측과
diff=0.0). 332차가 "미관측이지 무해 증명 아님"이라 표현했던 우려가
근거 있었음이 확인됨. 다만 이번 1건만으로는 (a) 실제 ACTIVE 상태에서
발생했는지, (b) 발생 빈도/재현성이 어느 정도인지가 확정되지 않아
수정안 설계 착수 여부는 여전히 사용자 승인 필요(§26 -- 지선생 331차
제안 "0 클램프가 유일한 정답은 아님" 유보도 계속 유지).

**변경 이유**: 332차 WIP "다음 작업 1번"(다른 route에서 실제 나쁜
`routeApexDist<0` 사례 탐색) 완료 -- 신규 route 업로드로 이월 조건 충족.

**현재 상태(미해결, 수정안 설계는 미착수)**: 실제 나쁜 사례 1건 확보.
수정안(§26/§27, 최소변경 원칙 -- 좌표계 정합 등 대안 포함 검토 필요)
설계 착수 여부는 다음 세션에서 사용자 확인 후 진행.

**패치 범위**: 없음(이번 회차도 코드 변경 없음, 순수 오프라인/실측 대조
분석, `ryu` 소스 무변경).

**실차 검증**: 이번 회차 자체가 실차 로그(x19seg) 기반 확인. 단
`routeLocalResampleUsed` 직접 계측(`5cba802`, 이미 `ryu`에 병합됨)
기준 로그는 아직 없음 -- 이번 4개 로그는 전부 그 이전(823943a6) 채록.

**관련**: WIP.md 335차/332차/331차/330차, FINDINGS.md 332차/330차,
`toolkit/sim_route_332_ws_negative_real_corpus.py`(재사용, 신규 코드
없음).

**337차 보강(qcamera 대조 + arbitration(src) 실측 확인, 기존 결론
변경 없음)**: t=478.0~485.0 구간(약 90프레임)을 `src` 컬럼으로 전수
대조한 결과, t=480.665601282 이후 t=485.0까지(문제의 t=481.858660709
포함) `src`가 계속 `vturn`으로 고정돼 있었음을 확인 -- **이 -40.0
'new' lock이 실제 `desiredSpeed` 출력에는 반영되지 않았다**(route가
이 구간 arbitration을 한 번도 승리하지 못함). 즉 "ACTIVE 상태였다면
RELEASE가 트리거됐을 것"이라는 조건부 서술을, "이 사례는 실제로는
vturn 승리로 무해했다"는 구체적 사실로 좁힐 수 있음. 단 결함 메커니즘
자체(`ws<0` orphan이 승격되고 마침 'new' lock되는 정렬)는 여전히
유효 -- 다른 시점/route에서 route가 승리 중이었다면 동일 메커니즘이
실제 급락을 유발했을 것이므로 수정 필요성 판단은 그대로 유지.
qcamera 대조로 해당 지점이 급커브가 아닌 완만한 굴곡이며, 직전
t=481.211848368~481.603084637에 정상 추적되던 커브가 `lost` 직후
0.2초 만에 그 잔여(tail) raw-path 포인트가 새 클러스터로 오인식된
것으로 보이는 정황을 확인(코드 레벨로 확정한 것은 아님, §28). 상세는
WIP.md 337차 참고. `route_active` 내부 상태는 `cereal/custom.capnp`에
전혀 발행되지 않는 순수 파이썬 상태임을 코드로 재확인 -- 계측 패치
없이는 구조적으로 영구히 직접 관측 불가.

**339차 보강(code-level 원인 확정, 기존 결론 변경 없음 -- 사용자 지시로
지하주차장 4건은 이번 회차 범위 제외)**: 이 사례(x19seg t=481.858660709)와
338차가 발견한 x6seg 옥외 사례(t=1409.607477381)를 `carrot_man.py`
(base `823943a6`) 코드와 직접 대조한 결과, **`apexDist=-40.0`은
`LOCAL_CURVE_WINDOW_BACK_M`(=40.0) 그 자체이며, 이 결함은 337차까지
새 결함으로 추정됐으나 실제로는 FINDINGS.md 330차가 이미 합성 재현으로
확정한 `route_local_curve_merge()`의 `distance_offset=ws` 언클램프
라벨링 버그(`route_crop_path_by_distance()`의 `d_start=max(0.0,d_start)`
클램프와 어긋남)의 인스턴스임이 확정됨**. 두 사례 모두 orphan(직전에
lost/강등된 apex의 잔여 포인트)의 위치가 ego 바로 근처(≈0m)였기 때문에
`ws = 0 - 40 = -40.0`이라는 가장 단순한 경계값이 정확히 산출된 것 --
"왜 하필 -40.0인가"에 대한 답이 이걸로 나옴. x6seg 사례도 x19seg와
동일하게 `src` 컬럼이 전 구간 `vturn` 고정이라 실제 `desiredSpeed`
출력에는 영향 없었음(재확인, 무해 여부는 신규 아님). 330차가 미확정으로
남긴 "이 라벨링 문제가 실제 도로에서 발생하는 빈도"에 대해, 서로 다른
2개 route에서 독립적으로 관측됐다는 것은 "orphan이 ego 근처에서 생기는
상황 자체가 드물지 않다"는 부분적 답을 제공. 상세는 WIP.md 339차 참고.
...
발견한 route_active 상이 2건 중 아직 미추적)를 동일 스크립트
(`sim_route_325_ep4_frame_trace.py`, 324차 `GridReplayState` 무변경
재사용, §27)로 프레임 단위 추적. 신규 재업로드된 원본 zip(18세그)을
재추출한 CSV로 실행했으며, 재추출 CSV가 기존 324차/325차 업로드분과
byte-for-byte 동일함을 먼저 확인(WIP.md 327차 1번 참고)한 뒤 진행.

**route_active 구간 실측(그리드별)**:
- 10m: t=720.189~720.538 (0.35초) -- 사실상 이 커브를 놓침
- 5m: t=712.137~721.736 (9.60초)
- 2.5m: t=712.036~721.736 (9.70초)

5m과 2.5m은 활성화 시각이 0.1초 차이로 사실상 동일 -- ep4에서 5m이
10m과 같은 편에 있었던 것과 대조적으로, 이 사례에서는 **5m 단독으로
이미 2.5m과 거의 동일한 결과**를 낸다.

**10m 실패 메커니즘(신규 규명)**: t=712.6~720.1(약 7.5초) 구간에서
10m의 `apex_mode`가 물리적으로 동일한 커브(도로 위 한 지점)를 향해
`apex_dist`가 270→260→250→...→130m로 정확히 10m 그리드 간격으로
계단식 하강하며 매번 `new`(신규 candidate)로 재등록됐다가
`cluster_count=1`(min_points=2 미달)인 채로 1~4프레임 뒤 즉시
`held`→`lost`→`none`으로 꺼지는 패턴이 약 0.7~1초 주기로 **14회
반복**됨. 즉 10m 해상도에서는 이 커브의 두 인접 리샘플 포인트가
`ROUTE_CLUSTER_MIN_POINTS=2`를 충족할 만큼 충분히 가깝게(또는 충분히
동시에) 잡히지 않아 단 한 번도 cluster로 승격되지 못한 채 약 7.5초를
허비. 반면 5m/2.5m은 같은 물리적 위치(175~180m 부근)에서
t=712.0~712.1대에 곧바로 `cluster_count>=2`로 안정화돼 즉시
ACTIVATE. 10m은 결국 t=720.14(apex_dist=130m)에서 한 차례
ACTIVATE에 도달하나 0.35초 뒤 `apex_lost`로 즉시 release -- 사실상
무의미한 개입.

**325차(ep4) 결론과의 관계 -- 세 번째 구분되는 메커니즘**: 324/325차가
지금까지 확인한 grid divergence 메커니즘은:
1. (325차 구간1) 재샘플 밀도별 apex_speed 추정 타이밍 차이 -- 5m도
   결국 통과, 순수 타이밍 문제.
2. (325차 구간2) 2.5m만의 고유 apex 분리 검출 -- 10m/5m 둘 다
   놓치는 별도 apex를 2.5m만 검출.
3. **(327차, 신규) 10m grid quantization에 의한 candidate churn** --
   apex 자체는 5m 이하에서는 문제없이 잡히는 "충분히 큰" 커브지만,
   10m 간격이 continuity/cluster 형성에 필요한 최소 조건(연속
   2포인트)을 충족시키지 못해 매 그리드 스텝마다 candidate가
   streak=1부터 처음부터 다시 시작 -- 234차/292차가 지목했던 "apex
   candidate continuity instability"(메모리에 기록된 기존 학습)의
   구체적 실차 재현 사례로 해석 가능.

이번 사례는 (2)와 마찬가지로 "5m으로 충분하다"는 판단에 대한 반례지만,
매커니즘은 다르다 -- (2)는 2.5m만 성공하고 5m도 실패하는 유형인 반면,
(3)은 5m이 이미 2.5m과 동등하게 성공하고 10m만 유일하게 실패하는
유형이다. 즉 "5m 이상이면 충분, 문제는 10m 자체"라는 해석에 힘을
싣는 사례.

**한계(§28/§29)**: 이번 규명도 ep3 1건 한정. 324차가 보고한 상이
2건(ep3/ep4)은 이제 둘 다 프레임 단위로 추적 완료됐으나, 이 corpus
(x18seg, 41 orphan 에피소드) 자체가 1회 주행분이라 세 메커니즘의
상대적 발생 빈도나 다른 corpus에서의 재현성은 미확인. **실차 검증
아님** -- offline replay 결과.

**다음 단계**: 326차가 설계한 "10m 기본 + 국소 2.5m 재샘플" patch의
"곡선 후보 판정 trigger 조건"(326차 미확정 사항 1번) 설계 시, 이번
메커니즘(10m에서 apex_mode가 `new`로 반복 재등록되며 매번
`cluster_count<2`로 꺼지는 패턴 자체)을 trigger 후보로 검토할 가치가
있음. 상세는 WIP.md "327차" 참고.

---

## 325차 -- [실차 실측 확정] ep4 `route_active` grid 상이의 원인은 하나가 아니라 두 개(속도추정 타이밍차 vs 2.5m 고유의 추가 apex 검출) -- "여전히 유의미"의 실체는 대부분 후자

**배경**: 324차가 발견한 "41개 orphan 에피소드 중 2건은 `route_active`
시퀀스 자체가 grid 간 상이"의 가장 큰 사례(ep4, seg--4, 62초, 10m 전체
미발동/5m 28프레임/2.5m 195프레임)를 `sim_route_325_ep4_frame_trace.py`
(324차 `GridReplayState` 무변경 재사용, §27)로 프레임 단위 추적.

**검증 신뢰성 확보**: `--validate-parity`로 324차
`summarize_episodes()` 결과(41개 에피소드의 cluster 승격/`orphEnd`/
`route_active_diverge` 판정)와 100% 일치 확인 -- 진단값 노출이 324차
연산 결과를 바꾸지 않았음을 보장한 뒤 트레이스 진행.

**실측 결과(x18seg, ep4, 962 orphan-tagged 프레임)**:

두 개의 독립된 divergence 구간이 존재하며 메커니즘이 다름:

- **구간1(t=730.836~733.336, ~2.5초, 동일 apex의 추정 타이밍 차이)**:
  10m은 이 구간 내내 `cluster_count=0`(min_points=2 미통과)이라
  `apex_mode="none"` -- "임계 미달"이 아니라 "판정 자체 미실행". 5m과
  2.5m은 물리적으로 같은 apex(~135~140m)를 대상으로 하지만, 분기
  프레임의 `apex_speed` 추정치가 재샘플 밀도에 따라 다름(5m=47.4kph
  vs 2.5m=41.6kph) -> `required_decel_mss` 5m=0.676(임계
  `AUTONAVI_SPEED_DECEL_RATE`=0.90 미달) vs 2.5m=0.979(즉시 통과)로
  갈림. 5m도 0.254초 뒤 짧게 통과하고 두 grid 모두 같은 시점에
  release -- 순수 타이밍 차이, 두 grid 다 결국 감속 판정에 도달.
- **구간2(t=770.690~779.091, ~8.4초, 활성시간 비대칭의 주된 기여분,
  2.5m 고유의 추가 검출)**: 10m/5m은 `cluster_count=1~3`으로 먼
  apex(320m->180m) 하나만 추적하며 `required_decel_mss`가
  0.065~0.21로 임계에 한참 못 미침(below_threshold 지속) -- 이 구간은
  5m도 10m과 같은 편에 위치(5m grid의 승격 효과가 나타나지 않음).
  반면 2.5m은 `cluster_count=3~4`로 **10m/5m 해상도에서는 뭉개지는
  더 가까운(160m->20m) 별도의 curve apex를 독립 cluster로 검출**해
  `required_decel_mss=0.916`으로 즉시 통과 -> 8초 이상 지속 활성.

**해석**: 324차가 "cluster 승격은 5m에서 포화, route_active
발동시간은 2.5m이 여전히 유의미"라고 정리했던 것 중, "여전히
유의미"의 실체가 최소 이 사례에서는 **단순 판정 타이밍 차이(구간1,
5m도 결국 도달)가 아니라 2.5m만의 순수한 추가 검출력(구간2, 5m은
아예 놓침)**임이 확인됨. 즉 "5m으로 충분하다"는 결론을 cluster 승격
지표(324차)만으로 내리면 안 된다는 324차의 유보가 실제로 근거 있는
유보였음을 뒷받침 -- 이번 사례에서는 오히려 2.5m 채택 쪽에 힘을
싣는 방향의 증거.

**한계(§28/§29)**: 이번 규명은 ep4 1건 한정(324차가 보고한 상이 2건
중 ep3은 아직 미추적). 구간2 유형(2.5m만 별도 apex를 분리 검출하는
패턴)이 이번 corpus 1회 주행에 국한되는지, 다른 corpus/커브에서도
재현되는지는 미확인. **실차 검증 아님** -- offline replay 결과.

**다음 단계**: ep3도 동일 방식으로 추적, 구간2 유형의 재현성 확인용
추가 corpus 필요, 이후 5m/2.5m 중 production 반영 여부 판단(§27/§31).
...
검증작업 우선") + 지선생(ChatGPT) 제안(317차 결과 재정리 -> 국소
5m/2.5m 재검증(chord=20m 고정) -> false positive 분리검증 -> 순서대로
ANALYSIS_ONLY/실차계측/production)을 반영해 진행. 지선생 제안이 인용한
318차 수치를 코드로 재확인하는 과정에서 벤치마크 자체의 오류를 발견.

**318차 벤치마크 재검증(정정)**: `perf_route_269_curvature_batch_
optimize.py`의 `benchmark()`/`self_test()`가 `distance_interval`
인자를 항상 10.0으로 고정 호출하고, 포인트 생성 함수(`make_straight`
등)도 `step=10.0` 고정임을 코드/직접실행으로 확인. `long_route_121`
시나리오의 실제 point spacing을 측정한 결과 10.0m(5m 아님), 총연장
982.6m(600m 아님)로 확정 -- **이 시나리오는 5m 그리드가 아니라 그냥
더 긴 10m 경로였다.** 318차의 "1.9배/+0.09ms" 수치 자체(계산량 증가
비율)는 유효하나 "경로 길이 2배"로 재해석해야 하며, "10m->5m 그리드
전환 부하"라는 명명은 오분류였다. **318차 WIP/FINDINGS 원문은 §14에
따라 수정하지 않음 -- 이 항목이 재해석만 추가한다.**

**신규 국소 윈도우 벤치마크**: `sim_route_321_local_window_grid_
benchmark.py` 신규 작성(317차 Part1 설계 -- 밀도만 올리고 chord는
물리적으로 고정 -- 그대로 이식, WINDOW_PAD_M=60m 재사용). 10m
baseline(sample=4/macro chord 80m, sample_fine=1/fine chord 20m,
n=13) vs 국소 5m(sample=8/macro chord 80m 동일, sample_fine=2/fine
chord 20m 동일, n=25) 비교 -- 비율 약 1.7~2.0x, 절대증가 약
0.04~0.06ms/**트리거 1회당**(조건부이므로 매 프레임이 아님). 전체-route
10m baseline(318차 straight_61, n=61) 프레임 비용 대비 약 25% 수준.

**Part 3(신규) -- 실제 함수 조합 false positive 검증**: 317차 Part1은
거리-속도 프로파일 추상화였던 것과 달리, `resample_10m_np`(carrot_
man.py 408행 재이식)+`calculate_curvature`+`route_find_clusters`(317차
이식본 재사용) 실제 함수 조합으로 검증. radius/arc_len 스윕(10~60m x
3~20m) + 노이즈 offset 스윕(0.2~3.0m)으로 "10m에서 candidates=1
(orphan)"이 되는 최소 조합을 먼저 캘리브레이션(임의 추정 안 함, §33):
- 진짜 좁은 커브(R=30m, 호길이 3m): 10m orphan -> 국소 5m에서
  cluster로 승격 **PASS**.
- 단발 GPS 노이즈(원본 정점 1개 0.3m 이탈): 10m orphan -> 국소 5m
  에서도 orphan 유지(오탐 재유입 없음) **PASS**.
317차 Part1(추상화 수준) 결론이 실제 함수 조합에서도 재현됨.

**코드 구조 확인(중요, 317차 한계 재해석)**: `carrot_man.py::
carrot_navi_route()`를 직접 읽어 `relative_coords`(10m 리샘플 이전
원본, 1145행 할당)가 `route_find_clusters()` 호출/orphan 판정(1369행,
동일 함수 내부) 시점까지 **재대입/해제 없이 동일 함수 스코프에 그대로
살아있는 지역변수**임을 확정. 317차가 "실측 검증에는 relative_coords를
cereal에 발행하는 계측 patch가 필요"라 한 것은 **과거 로그로 사후
검증**하는 데 필요한 조건이지, **국소 재샘플 기능 자체가 동작하는 데
필요한 전제는 아니었다** -- 즉 코드 구조상 신규 로깅 없이도 구현
가능함을 이번에 처음 구분해 확정(317차의 "실측 검증 불가능" 결론
자체를 뒤집는 것은 아님 -- 그건 여전히 유효, "구현 가능 여부"만
별개로 새로 밝힌 것).

**한계(§28)**:
1. 벤치마크는 이 컨테이너(클라우드 서버 CPU) 기준 상대비교 -- C3
   디바이스 절대시간 아님(269/271/318차와 동일 한계).
2. "조건부" 실제 트리거 빈도(Hz)는 미측정 -- 317차가 이미 남긴
   별개 과제, 이번에도 미해소.
3. Part3의 raw 폴리라인은 완전 합성(raw_step=1m 고정)이며 실제 GPS/
   navi TCP 좌표의 노이즈 특성(간헐 결측, 다중 튐, 비등간격 등)을
   반영하지 않음 -- 실 corpus 검증을 대체하지 않는다.
4. 실제 patch 설계(트리거 시점/윈도우 자르는 방식/continuity tracker
   연결)는 이번 세션 범위 밖, 사용자 승인 필요(§31).

**영향받는 실차 제어 로직**: 없음(`ryu` 코드 변경 없음, devnotes
toolkit 신규 스크립트 1개만 추가, ANALYSIS_ONLY).

**검증**: 정적분석(py_compile, 독립 clean clone 재확인)/시뮬레이션
(self-test+벤치마크 4회 반복+Part3 캘리브레이션 스윕 및 2케이스 PASS)
완료, 로그검증은 x17seg 재추출 재현 확인(318차 재검증 목적, Part3
자체는 합성), 실차검증 미실시. 상세는 WIP.md "321차" 참고.

## 320차 -- [정정, 신규 실측 없음] **바로 아래 319차 항목의 핵심 결론 철회** -- 319차가 "원거리(~510m)=lookahead 경계 아티팩트"로 판단한 것은 314차가 이미 실측으로 반증한 결론과 정면 상충, 319차가 "근접 신규 아형"으로 명명한 것도 310차가 이미 "교차로 회전부"로 명명한 것과 동일 현상 -- 318차 "다음 작업 1번"은 310~316차로 이미 완료되어 있었음을 뒤늦게 확인

**중요**: §14 원칙에 따라 아래 319차 항목 자체는 수정/삭제하지
않는다. 이 항목이 그 결론을 정정한다.

**경위**: 319차는 318차 "다음 작업 1번"(persistent 232건 표본추출
qcamera 대조)을 재구성하며 WIP.md 상단(317/318차)만 확인하고 그
아래(314~316차)를 확인하지 않은 채 "미기록"이라 판단, 처음부터
그룹화 로직을 새로 만들어(`group_orphan_episodes_319.py`) 8곳을
qcamera로 대조했다. 이 patch가 push된 직후, WIP.md 전체를 다시 훑어
314~316차 원문을 확인한 결과 다음이 드러났다:

- **310차**가 근접 후보 최상위 사례를 이미 qcamera로 "교차로
  회전부"로 명명함 -- 319차가 "지하주차장 진출입로 신규 아형"이라
  명명한 것(seg6 t=856.5, dist_mean≈0)과 같은 부류의 현상으로 판단됨.
- **311~313차**가 근접+이동중 57건을 물리적 위치 **4곳**
  (seg3/seg4/seg14/seg16)으로 이미 확정하고 4곳 전부 qcamera 대조를
  마쳤다. 319차가 근접 후보로 뽑은 seg14(ep50, t=1322~1339)/seg16
  (ep53, t=1462~1472)은 311/312차가 이미 대조 완료한 물리적 위치
  (seg14 t=1329.72~1334.82, seg16 t=1465.92~1471.63)와 시간대가 거의
  정확히 겹친다 -- **신규 정보 아님**.
- **314차가 "원거리=lookahead 아티팩트" 가설을 이미 실측으로
  반증**했다. seg6 대표사례에서 production `routeApexMode`가 500m
  부터 30m까지 끊김없이 정상 추적(matched)함을 확인했고, "500m 밖은
  qcamera 가시거리(100~150m) 밖이라 원래 안 보이는 것뿐"이라는
  방법론 교훈까지 명시적으로 기록되어 있다. **319차가 프레임 2장만
  보고 내린 "아티팩트" 결론은 이와 정면 상충 -- 철회한다.**
- **315차**가 원거리+이동중 19건 전부를 5개 물리적 사건(seg6/
  seg14-t1358/seg4/seg3/seg13→seg14경계)으로 이미 설명했다. 319차가
  뽑은 원거리 12개 에피소드(seg4/5/7/8/9/13)는 seg 번호로 볼 때 이
  5개 사건의 시간축 파편(세그먼트 파일 60초 경계로 인한 재분할)일
  가능성이 높다. 그중 seg13→seg14 경계 사례는 이미
  `ROUTE_CLUSTER_MIN_POINTS=2` 게이트 결함의 실측 확정 사례로
  기록되어 있다.
- **316차**가 4곳 전체에 `harsh_brake_events`/
  `steering_oscillation_detector`를 체계적으로 적용해 전부 0건임을
  재확인했다.

**결론**: 318차 "다음 작업 1번"은 이 시점부로 **310~316차로 이미
완료된 것으로 확정, 추가 조치 없이 종료**한다. 319차의 실측 자체
(orphan 3433건 재확인, 53개 에피소드 산출, 8곳 프레임 육안 확인)는
유효한 관찰이나, 그 "원거리=아티팩트" 해석만 철회된다. 319차가 목록에
없던 것으로 지목한 seg1(주차차량 협착, t=570~577s)과 seg6 t=856.5
(진입로 횡단)은 310~316차의 4곳/5개 사건 목록에 없는 구간이긴 하나
둘 다 "커브 아님"이라는 부정적 결과라 실익이 낮고, 추가 조사는
우선순위 낮음으로 이월한다.

**toolkit 관련**: 319차가 추가한 `group_orphan_episodes_319.py`는
삭제하지 않는다(§36, 삭제 금지 대상은 아니나 이미 push된 파일이므로
그대로 둠). 다만 `toolkit/README.md`에 "이 목적(근접/원거리 후보
분류)에는 `sim_route_310_provisional_streak_real_corpus.py
--check-cruise --check-comfort`가 이미 검증된 정식 도구이며 우선
사용할 것"이라는 안내를 추가한다(아래 toolkit 변경 참고).

**재발 방지(§4/§33)**: 다음 세션부터는 WIP.md 상단 1~2개 회차만 보고
"해당 작업 기록 없음"이라 판단하지 않는다. 특정 주제(예: 오늘의
"근접+이동+활성 orphan 후보")에 대한 기록이 있는지 확인할 때는
`grep`으로 관련 키워드(seg 번호, 필드명 등)를 WIP.md/FINDINGS.md
전체에서 검색한 뒤 "기록 없음"을 판단할 것.
...
## 319차 -- [부분 실측, 신규 toolkit 스크립트 1개] 317/318차가 이월한 "근접+이동+활성 orphan 후보군 qcamera 육안 대조" 작업 -- **컨테이너 리셋으로 원래 그룹화 로직(232건/84건/73건/16곳)이 devnotes 미기록 상태로 전량 소실, 재현 불가 확정** -- 그룹화/계층화 기준을 코드로 명시 고정한 신규 baseline(53개 에피소드)으로 재구성 후 8곳 육안 대조

**배경**: 318차 WIP "다음 작업 1번"(persistent 232건 중 일부 표본추출해
qcamera 대조)에 착수한 세션이 있었으나, 그 세션이 devnotes에
아무것도 기록하지 않은 채(§21/§22 위반) 컨테이너가 리셋되어 작업
전체가 소실됨. §29 원칙에 따라 "확인했다"고 보고된 결과 전체를
미검증으로 재분류하고, 그룹화 로직 자체를 신규 재구성.

**재현 시도 및 실패**: 소실 전 세션이 언급한 수치(orphan 3433건 →
316개 에피소드 → persistent 232건 → 근접+이동+활성 73건)를 재현하기
위해 두 가지 그룹화 방식을 시도:
- 시간 gap 0.5s 기준: 150개 에피소드
- 거리 연속성(10m) + gap 2.0s 허용: 114개 에피소드

둘 다 원래 수치(316)와 불일치. 정확한 원본 로직(위상 재앵커링 처리
방식 등)이 어디에도 저장되지 않아 **재현 불가 확정**. orphan 프레임
총수(3433)만 x17seg corpus(020ea86, 20171행) 재추출로 재확인됨.

**신규 baseline**: `toolkit/group_orphan_episodes_319.py` 신규 작성
(같은 seg 내 시간 gap<=1.0s 기준 그룹화, dist_mean/vEgo_mean/
cruise_frac 기준 5개 stratum 분류 — 코드 docstring에 기준 명시,
재발 방지 목적). x17seg corpus 적용 결과:

- orphan 프레임: 3433건 (재확인, 이전과 일치)
- 에피소드: 53개 (이전 316/150/114와 모두 불일치 -- 그룹화 기준
  차이로 인한 결과이며, 이 스크립트가 새 기준선)
  - near_moving_active(<=200m, vEgo>0.5, cruise>=50%): 14건
  - far_moving_active(>=400m, vEgo>0.5, cruise>=50%): 12건
  - moving_inactive(cruise off): 23건
  - stopped_inactive: 2건
  - other: 2건

**육안 대조 8곳 (qcamera, x17seg corpus)**:

| 위치 | dist_mean | 판정 |
|---|---|---|
| seg1 t=570.2 (ep5) | 197m | 주차차량으로 협착된 직선 도로, 커브 아님 |
| seg1 t=574.1 (ep7) | 176m | 동일 유형 |
| seg1 t=577.5 (ep9) | 156m | 동일 유형(교차로 인근) |
| seg14 t=1330 (ep50) | 123.5m | 완만한 도로 곡선 있으나 이 시점 프레임만으론 판정 애매 -- 에피소드 길이 60프레임(t0=1322.2~t1=1339.3)이라 apex가 다른 시점일 가능성, 재확인 필요 |
| seg16 t=1467 (ep53) | 53.3m | 가로수길 완만한 좌커브 -- 실제 커브로 판단 |
| seg6 t=856.5 (ep36) | 0m | 지하주차장 진입로(차량이 진입로를 가로지름) -- 도로 곡선이 아니라 **진출입 교차점 아형**, 신규 발견 |
| seg4 t=755.7 (ep23) | 510m | 프레임 내 아무것도 안 보임 -- lookahead 경계 아티팩트 가설 부합 |
| seg8 t=963.9 (ep40) | 510m | 동일 -- 직선도로, 아티팩트 가설 부합 |

**해석**:
1. 원거리(약 440~510m, 대부분 정확히 510m) 그룹은 이번에도 예외 없이
   프레임상 아무 것도 보이지 않는 직선 도로 -- lookahead 최대거리
   경계에서 발생하는 아티팩트라는 가설이 새 표본에서도 계속 부합.
2. 근접 그룹 중 상당수(이번 14건 중 7건)가 `dist_mean≈0`으로,
   실제로는 "커브"가 아니라 지하주차장/시설 진출입로처럼 **경로가
   거의 직각으로 갈라지는 지점**인 것으로 보임 -- 232건 재조사 당시
   없었던 신규 분류 후보. 코드 관점에서 이 아형이 route curvature
   로직에 어떤 영향을 주는지는 아직 미조사.
3. 나머지 근접 그룹(dist 140~200m대, seg1)은 이전 소실 세션이
   보고했던 것과 질적으로 동일하게 "주차차량 협착으로 인한 오탐"
   패턴을 유지.
4. seg14/seg16처럼 실제 도로 곡선이 있는 사례도 존재하나, 에피소드
   전체 구간(수십 프레임)을 대표하는 단일 시점만 봐서는 apex 위치
   판정이 불완전함 -- 에피소드 전체 프레임을 훑는 방식으로 보완 필요.

**한계(§28)**:
- 신규 baseline(53개 에피소드)은 소실된 원본(316개)과 절대 수치가
  다르므로, 이전에 "232건 persistent"로 보고됐던 결론과 직접 비교
  불가.
- 8곳만 확인(near_moving_active 14건 중 6건, far_moving_active
  12건 중 2건) -- 전수 확인 아님.
- seg14 ep50은 단일 시점 프레임만으론 판정 불충분.
- `dist_mean≈0` 진출입로 아형이 실제 route 제어(감속/HUD 등)에
  주는 영향은 이번에 조사하지 않음.
...
## 318차 -- [실측 완료, 기존 269차 perf 도구 재사용] 10m->5m 곡률 그리드 전환 부하를 프레임당 ms로 정량화 -- optimized(현재 production 269차 패치 반영본) 기준 61-point(10m/600m)=0.09~0.10ms/frame vs 121-point(5m/600m)=0.18~0.19ms/frame, 절대증가폭 약 0.09ms/frame(비율 약 1.9배, 이 컨테이너 CPU 상대비교 -- C3 임베디드 절대치 아님)

**배경**: 317차 체크포인트(8843c4e)가 "다음 작업 2번"으로 남긴
"곡률 loop 자체의 실제 프레임당 실행시간을 프로파일링해 '부하 2배'가
실제로 몇 ms인지 숫자로 확인"을 수행한다. §21 원칙대로 새 스크립트를
작성하기 전에 `toolkit/README.md`를 먼저 확인한 결과,
`perf_route_269_curvature_batch_optimize.py`가 이미 정확히 이 목적
(baseline/optimized 곡률 계산의 `n_points=61`/`n_points=121`
시나리오 벤치마크)으로 존재함을 확인 -- **신규 도구 작성 없이
기존 도구를 그대로 재실행**해 답을 구했다.

**한 일**:
1. `perf_route_269_curvature_batch_optimize.py --self-test` 재실행
   -- 10/10 PASS 재확인(이 세션 컨테이너에서도 baseline/optimized
   출력이 완전 동일함을 재검증, §28 -- 오래된 결과를 그대로 신뢰하지
   않음).
2. `--benchmark --iters 20000`을 4회 반복 실행해 타이밍 안정성(분산)
   확인.
3. 실 x17seg corpus(20171행)의 `routeNaviPointsLen` 컬럼 분포를
   확인해 합성 시나리오의 `n_points=61` 가정이 근거 있는지
   교차검증 시도 -- 중앙값 65(61과 근접)이나 164 등 더 큰 값도
   상당수(4666행) 존재. **단, `routeNaviPointsLen`은 raw navi
   points 개수이지 곡률 루프에 실제로 들어가는 `resampled_points`
   개수(=path를 10m로 리샘플한 후의 점 개수, `route_lookahead_m=
   600.0`이면 이론상 약 61개로 수렴)와 다른 필드이므로 직접 비교는
   불가 -- 계측 공백으로 남김(아래 한계 참고).

**결과(4회 반복, ms/frame)**:

| 시나리오 | n_points | optimized(1회) | optimized(2회) | optimized(3회) | optimized(4회) |
|---|---|---|---|---|---|
| straight_61 | 61 | 0.1063 | 0.0991 | 0.0970 | 0.1043 |
| single_turn_61 | 61 | 0.1090 | 0.1014 | 0.0972 | 0.1028 |
| s_curve_61 | 61 | 0.0891 | 0.0930 | 0.0890 | 0.0903 |
| long_route_121 | 121 | 0.1899 | 0.1862 | 0.1837 | 0.1908 |

61-point 3개 시나리오 평균 ≈0.098ms/frame, 121-point(long_route_121)
평균 ≈0.187ms/frame -- **비율 약 1.9배**(포인트 개수 비율 121/61≈1.98과
거의 일치, 즉 이 루프는 포인트 개수에 거의 선형으로 스케일).
**절대 증가폭은 프레임당 약 0.09ms**(이 컨테이너 CPU 기준).

**해석**:
- 269차가 이미 적용한 최적화(np.interp 배치화 등) 덕분에, 전체 600m를
  10m 대신 5m로 전부 재샘플해서 매 프레임 돌리더라도 이 곡률 계산
  자체의 절대 부하 증가는 이 컨테이너 CPU 기준으로 미미한 수준(서브
  밀리초)이다. "600m 전체 5m 재샘플은 부하가 걱정된다"는 애초
  우려(317차 체크포인트가 사용자 발언으로 기록)가 이 지표만으로는
  강하게 뒷받침되지 않는다.
- 다만 317차가 이미 채택한 "orphan 지점 주변 국소 윈도우만
  재샘플"(전체 600m가 아니라 편도 약 120m 윈도우, WINDOW_PAD_M=60m
  가정 시)은 이 "전체 5m 최악의 경우"보다도 훨씬 저렴하므로, 이번
  측정 결과는 317차 설계 방향의 부하 우려를 오히려 낮추는 방향의
  보강 근거가 된다(직접 그 윈도우 케이스를 벤치마크하지는 않음,
  아래 한계 참고).
- **이 수치는 C3 임베디드 ARM 디바이스의 절대 실행시간이 아니다**
  (269차/271차가 이미 명시한 한계와 동일) -- 클라우드 서버 CPU에서의
  **상대적** 알고리즘 복잡도 스케일링(선형 증가)만 신뢰할 수 있다.

**한계(§28)**:
1. 클라우드 컨테이너 CPU 기준 상대 비교 -- C3 디바이스 절대 실행시간
   미확인(269차/271차부터 이월된 동일 한계, 이번에도 미해소).
2. 317차가 실제로 설계한 "orphan 주변 국소 윈도우(약 120m) 재샘플"
   케이스를 직접 벤치마크하지 않음 -- 이번 측정은 "전체 600m를
   5m로" 최악의 경우만 다루므로, 국소 윈도우 케이스는 이보다 더
   저렴할 것이라는 추론일 뿐 직접 측정치는 아님.
3. `routeNaviPointsLen` 실측 분포(중앙값 65, 최대 164)가 실제
   `resampled_points` 개수와 어떻게 대응하는지 이번 세션에서
   확인하지 못함 -- 실제 production이 항상 n≈61 근처에서 동작하는지,
   때때로 그보다 훨씬 긴 path를 처리하는지는 별도 계측 필요.
4. `carrot_navi_route()`가 호출되는 실제 주기(Hz)를 이번 세션에서
   확인하지 않음 -- "프레임당 0.09ms 증가"가 초당 몇 회 반복되는지
   몰라 전체 CPU 점유율 증가폭은 계산하지 않았음.

**영향받는 실차 제어 로직**: 없음(`ryu` 코드 변경 없음, 기존 toolkit
스크립트 재실행만 수행, ANALYSIS_ONLY).

**검증**:
- 정적 분석: 해당 없음(신규 코드 없음, 기존 스크립트 재사용)
- 로그 검증: `routeNaviPointsLen` 분포 확인(x17seg 20171행)
- 시뮬레이션: `--self-test` 10/10 PASS 재확인, `--benchmark` 4회
  반복 실행으로 수치 안정성 확인(분산 작음, 위 표 참고)
- 실차 검증: 미실시(C3 디바이스 프로파일링 없음)

**다음 작업**:
1. 실제 C3 디바이스(또는 그와 유사한 임베디드 환경)에서 동일
   벤치마크를 재실행해 절대 실행시간 확인.
2. 317차가 설계한 "orphan 주변 국소 윈도우(약 120m)" 케이스를
   `perf_route_269` 방식으로 직접 벤치마크(n_points≈12~24 수준 예상,
   전체 600m 케이스보다 훨씬 저렴할 것으로 예상되나 미측정).
3. `carrot_navi_route()` 실제 호출 주기(Hz) 확인 후, "프레임당 증가폭
   x Hz"로 총 CPU 점유율 증가폭 계산.
4. `routeNaviPointsLen`과 실제 `resampled_points` 개수의 관계를
   코드 레벨(또는 신규 계측)로 확인해 n≈61 가정의 실측 근거 보강.

## 317차 -- [합성 self-test PASS + 실측 일부] 사용자 제안 "orphan 국소 재샘플" 아이디어 -- 밀도만 올리고 fine chord(20m)는 고정하는 방식이 "폭 있는 진짜 좁은 커브"와 "폭0 노이즈"를 구조적으로 구분해냄을 합성 검증. x17seg는 routeSource=tcp_navi라 navRouteNavd(원본 폴리라인) 0건 -- "국소 재샘플" 원안 자체는 이 corpus로 실측 불가 확정, 대신 그리드 위상 민감도(같은 메커니즘의 다른 예측)를 실측해 orphan 316개 중 84건(26.6%) phase-resolved 확인

**배경**: 306차가 코드 구조로 확정한 "route_find_clusters()의
min_points=2 게이트가 고립된 1포인트 좁은 커브(orphan)를 노이즈로
오인"하는 문제에 대해, 사용자가 이번 세션에서 개선 방향을 제안했다 --
"600m 전체를 매 프레임 5m로 재샘플하는 것은 부하가 크니, orphan이
탐지된 지점 주변만 국소적으로 5m(또는 더 촘촘하게) 재샘플해서 진짜
커브인지 확인하자". 이번 세션은 이 아이디어를 (1) 개념 수준에서
합성 검증하고 (2) 실측 가능한 범위를 x17seg corpus(20171행, `020ea86`)
로 확인한다(신규 `toolkit/sim_route_317_orphan_local_fine_resample.py`).

**Part 1 -- [합성 self-test, 실측 불필요, 코드 논리로 확정]**: carrot_man.py의
실제 커브 검출 구조(macro chord=2×4×10=80m, fine chord=2×1×10=20m,
1165/1217~1263행)를 코드 대조로 정리하면, "포인트 밀도"(리샘플
`distance_interval`)와 "곡률 chord 길이"(3점 곡률 계산의 인덱스
스텝×interval)는 서로 독립된 두 축이다 -- 306차가 확정한 orphan은
"밀도"가 아니라 "chord로 잡은 curvature 판정이 정확히 몇 개
그리드 인덱스에 걸치는가"의 문제다. 따라서 **chord는 고정한 채
밀도만 국소적으로 올리면**, 물리적으로 폭이 있는 진짜 커브는 더 많은
인덱스에 걸리게 되어 min_points=2를 통과할 확률이 올라가는 반면,
폭이 아예 없는(0m) 순수 1프레임성 스파이크는 어떤 밀도에서도 표본
2개가 들어갈 "구간" 자체가 없어 여전히 orphan으로 남아야 한다는
것이 이론적 예측이다. 이를 폭(L=0/4/8/12/16/20/30m) x 간격
(10/5/2.5m) x 위상(그리드 시작점 1m 단위 스윕) 3중 스윕으로 검증한
결과:
- **L=0(순수 노이즈)**: 10/5/2.5m 전부 클러스터 성립률 0% 유지
  (self-test 단언 통과 -- 밀도를 올려도 노이즈가 절대 "구제"되지 않음).
- **L>0(폭 있는 커브, 4~30m)**: 간격을 10m→5m→2.5m로 좁힐수록 위상
  무관 클러스터 성립률이 단조 증가(예: L=8m일 때 10m에서 0%였던 것이
  5m에서 60%, 2.5m에서 100%로 개선). 전 구간(폭>0, 총 24개 조합)에서
  "밀도를 올렸는데 오히려 성립률이 떨어지는" 역행 사례 0건.
- **결론(개념 검증 한정)**: "밀도만 올리고 chord는 고정"하는 접근은
  원리적으로 노이즈 재유입 없이 진짜 좁은 커브를 구제할 수 있는
  판별자로 유효하다. 단, 이건 메커니즘 자체의 타당성 검증이며 x17seg의
  실제 orphan이 이 케이스에 해당한다는 뜻은 아니다.

**Part 2 사전 확인 -- [실측, 중요한 제약 발견]**: 사용자 원안("orphan
주변 **원본** 폴리라인을 가져와 국소적으로 5m 재샘플")을 x17seg로
실측 검증하려 했으나, 다음이 실측으로 확인됨:
- x17seg 17세그 전체가 `routeSource=="tcp_navi"`(navi 앱이 TCP
  7712 소켓으로 좌표를 직접 밀어넣는 경로, 182차 계측 필드).
- `cereal/log.capnp`의 `navRouteNavd`(NavRoute, 원본 lat/lon 폴리라인을
  담을 수 있는 표준 필드) 이벤트가 이 corpus 전체(17세그 중 샘플
  확인분)에서 **0건**.
- `carrot_man.py::carrot_navi_route()` 705행이 리턴하는
  `resampled_points`(이미 10m로 리샘플된 결과)만 `carrotMan.naviPaths`
  로 발행되며(carrot_serv.py 1357행), 10m 리샘플 **이전** 원본
  `relative_coords`/`path`는 이 corpus 어디에도 로그되어 있지 않다.
- **결론: "orphan 주변 원본 폴리라인을 국소적으로 재샘플"이라는 원안은
  이 corpus로는 실측 검증이 구조적으로 불가능하다.** 이미 10m로
  손실된 정보를 사후에 되살릴 방법이 없기 때문(10m 포인트 사이를 5m로
  보간해봤자 원본이 직선이었다고 가정하는 것과 동일 -- 실제 좁은 코너
  형상을 복원 못함). 진짜 검증하려면 `relative_coords`를 새 계측으로
  cereal에 발행하는 `ryu` 패치(§31 사용자 승인 필요) + 그 계측이 들어간
  빌드로 신규 주행이 필요.

**Part 2 대안 실측 -- [실측 완료, 같은 가설의 대리 검증]**: 원본
폴리라인 없이도 이미 로그된 데이터만으로 검증 가능한, 같은 메커니즘의
다른 예측이 있다 -- 219/220차가 이미 확정한 "리샘플 그리드가 매 프레임
현재위치 기준으로 재앵커링된다"는 사실을 이용하면, 차량이 이동하며
프레임마다 10m 그리드의 위상(phase)이 계속 바뀐다. 즉 "이 프레임에서
orphan이었던 지점이, 그리드 위상이 다른 인접 프레임(§267 tolerance
10m 이내로 같은 물리적 지점으로 매칭)에서는 실제로 cluster(2점 이상)로
잡힌 적이 있는가"를 `routeOrphanSingletonDist`/`routeClusterCount`/
`routeApexDist` 컬럼만으로 실측 가능하다. 결과:
- orphan 프레임 3433개(전체 20171행 중) → 연속성+거리 기준(§265/267
  CONTINUITY_MATCH_TOLERANCE_M=10m 그대로 재사용)으로 316개 에피소드로
  그룹화.
- **phase-resolved(인접 프레임에서 같은 위치가 cluster로 잡힌 이력 있음):
  84건(26.6%)**.
- **persistent(에피소드 구간 내내 한 번도 cluster로 안 잡힘): 232건
  (73.4%)**.

**한계(§28, 명확히 구분)**:
- Part 1(합성)은 개념 검증일 뿐 -- x17seg 실제 orphan 316건이 이
  메커니즘 때문이라는 직접 증거는 아니다.
- Part 2의 "phase 해소" 판정은 거리 근접성(10m tolerance) 기반 근사
  매칭이며, 완전히 다른 두 인접 커브를 같은 커브로 오인할 가능성을
  배제 못한다(에피소드 길이가 짧고 접근속도가 빠를수록 위험 커짐).
- phase_resolved 26.6%는 "국소 재샘플을 했다면 그만큼 풀렸을 것"의
  직접 증거가 아니라, "그리드 위상이 문제의 일부"라는 정황일 뿐이다
  (Part 1 메커니즘과 방향은 일치).
- persistent 232건(73.4%)이 모든 위상에서 왜 안 풀리는지(진짜
  폭<10m급 좁은 커브인지 vs road_limit/커브 자체가 애초에 미약한
  노이즈성 후보인지)는 이번 세션에서 원인 분리하지 않음 -- 235차가
  이미 지적한 "threshold 부근 미약 후보 산재" 패턴과 겹칠 가능성 있음.
...
**검증**:
- 정적 분석: `py_compile` 통과
- 로그 검증: 완료(x17seg 20171행, `020ea86`, Part2 phase-resolution
  실측)
- 시뮬레이션: 완료(Part1 self-test PASS, 노이즈 0%유지 + 폭>0 24개
  조합 전부 밀도↑ 성립률 단조증가 단언 통과)
- 실차 검증: 해당 없음(코드/파라미터 변경 없음, ANALYSIS_ONLY)

**다음 작업**:
1. persistent 232건 중 일부를 표본추출해 qcamera 대조 -- 진짜
   폭<10m 좁은 커브(예: 교차로 코너)인지, 235차류 "미약 후보 노이즈"
   인지 분류.
2. "국소 재샘플" 원안을 진짜로 검증하려면: `relative_coords`(10m
   리샘플 이전 원본)를 cereal에 발행하는 계측 patch를 `ryu`에 추가할지
   사용자 승인 필요(§31) -- 승인되면 다음 세션에서 패치 설계 + 신규
   주행 요청.
3. phase-resolved 84건 중 일부를 표본추출해, cluster로 해소된 그
   인접 프레임의 실제 apex_speed가 orphan 프레임 대비 얼마나
   다른 감속을 명령했을지(=실제로 놓쳤다면 어느 정도 손실인지)
   정량화(316차가 harsh_brake/조향진동으로 이미 확인한 "실제 불편
   증거 0건" 결론과 이어붙일 수 있는지 검토).

## 316차 -- [실차 검증 완료] 근접+이동중 ADAS engaged 물리적 위치 4곳(seg3/seg4/seg14/seg16) 전체에서 harsh_brake_events/steering_oscillation_detector 0건 확인 -- "production이 route 후보를 놓쳤다"(계측 사실, 참)와 "그로 인해 실제 불편이 생겼다"(§28/§29)는 계속 별개 단계에 머묾. 검증 과정에서 --check-comfort 옵션 초판의 cruiseEnabled 필터링 누락 버그를 발견 및 즉시 수정

**배경**: 310차부터 이월된 다음 작업("근접+이동중 57건 중
harsh_brake_events/steering_oscillation_detector 교차검증")을 마무리.
311/312차가 seg14/seg16 두 곳에서 대화식 python으로 개별 확인했던
"실제 불편 신호 없음(0건)" 결과를, 313차가 정식 편입한
`--check-cruise`/`--cluster-gap-s` 군집화 결과(물리적 위치 4곳:
seg3/seg4/seg14/seg16)에 일괄 적용해 재현 가능한 스크립트로
일반화하는 것이 목표였다.

**방법**: 동일 x17seg zip 재추출(20171행, `020ea86` 확인, 3회 이상
연속 재현성 유지). `sim_route_310_provisional_streak_real_corpus.py`에
`--check-comfort`/`--comfort-pad-s` 옵션을 신규 추가(§21/22 --
`analysis_helpers.py`의 기존 `harsh_brake_events`/
`steering_oscillation_detector` 함수 재사용, 새 탐지 로직 아님). 위치
군집화로 얻은 각 물리적 위치의 `[start_t-pad, end_t+pad]`(기본
pad=5.0초) 구간에 두 함수를 적용해 급브레이크/조향진동 건수를
위치별로 집계한다.

**[초판 버그 발견 및 즉시 수정]**: 시간 패딩만 적용한 초판을 seg3
클러스터(t=673.57~676.53s)에 실행한 결과 harsh_brake **8건**이
잡혔다. 원시 CSV로 개별 프레임을 직접 대조한 결과 이 8건은 전부
`cruiseEnabled=False`(t=676.57s에 ADAS가 이미 비engaged로 전환된 뒤)
구간에서 발생한 것이었고, `verify_and_extract_frames.py`로 추출한
qcamera 프레임에서도 적신호 교차로에 운전자가 수동으로 정지하는
장면임을 확인했다(t=676.5~682.0s, vEgo 13.9m/s -> 4.2m/s로 감속,
`src=vturn`, `cruiseEnabled` 전 구간 False). 즉 초판은 ADAS와 무관한
운전자 수동조작 이벤트를 이 위치의 "불편"으로 오탐할 위험이 있었다
-- 패딩으로 넓힌 시간창 안에서 `cruiseEnabled`가 도중에 False로
전환되는 경우를 걸러내지 않은 것이 원인. 패딩 구간 내에서도
`cruiseEnabled=='True'`인 프레임만 검사 대상으로 남기도록 즉시
수정했다.

**수정 후 결과 (물리적 위치 4곳, cruiseEnabled=True 프레임만 검사)**:

| 위치 | 검사구간 | 프레임(True/제외) | harsh_brake | steering_osc |
|---|---|---|---|---|
| seg3 t=673.57~676.53s | 668.57~681.53s | 64/196 | 0건 | 0건 |
| seg16 t=1465.92~1471.63s | 1460.92~1476.63s | 228/86 | 0건 | 0건 |
| seg14 t=1329.72~1334.82s | 1324.72~1339.82s | 303/0 | 0건 | 0건 |
| seg4 t=722.93~724.52s | 717.93~729.52s | 161/0 | 0건 | 0건 |

seg14/seg16은 311/312차가 대화식으로 얻었던 "0건" 결과를 정식
스크립트로 재현한 것이고, seg3/seg4는 이번 세션에서 처음으로
체계적으로(스크립트 기반, cruiseEnabled 필터 포함) 확인되었다.
...
-- 특히 315차 seg13->seg14 확정 사례) 단계에 머물러 있고, "그로 인해
실제 문제가 생겼다"(§28/§29, 미확인) 단계로는 나아가지 못했다. 이는
310~312차의 개별 결론과 일관되며, 이번 세션은 그 결론을 스크립트
기반으로 재현 가능하게 만들고 seg3/seg4로 범위를 확장했다는 점에서
증분적이다.

**한계(§28)**:
1. 단일 로그(17세그먼트) 표본, harsh_brake_events/
   steering_oscillation_detector의 기본 임계값(`accel_drop_thresh=-0.8`,
   `min_reversals=3` 등)에 따른 결과이며 더 민감한 임계값에서는 다른
   결과가 나올 가능성을 배제하지 못함(코드 레벨 임계값 검증 없음).
2. `--comfort-pad-s` 기본값(5.0초)은 311/312차가 수동으로 앞뒤를
   넓혀 확인했던 관행을 그대로 편입한 것으로, 이 값 자체의 타당성을
   별도로 검증하지는 않음.
3. seg14/seg16이 완만한 저속 커브(조향각 최대 10도/26.9도)라는
   312차 지적과 동일하게, "완만해서 애초에 감속이 불필요했다"는
   대안 설명은 이번에도 배제하지 못함.
...
- 실차 검증: **완료**(§29 -- harsh_brake/조향진동 부재를 실차 로그로
  직접 확인한 것이므로, 311/312차와 동일하게 "실차 로그 기반 검증
  완료"로 표기. 단 "production apex 공백이 무해하다"는 결론은
  이 단일 corpus/이 임계값 기준에 한정됨, 위 한계 참고)

---

## 315차 -- [실차 최초 실증] `ROUTE_CLUSTER_MIN_POINTS=2` 게이트 결함(306/307차 가설)의 x17seg corpus 내 최초 실측+qcamera 확정 사례 -- seg13->seg14 경계(원거리 510m 진입 ~ 근접 0m 통과, 약 55초간) 전 구간에서 production `routeApexMode`가 단 한 번도 'matched' 승격 못 함, qcamera로 실제 우회전 커브(경고표지판 포함) 확인

**배경**: 314차가 "원거리+이동중" 19건 중 seg6 대표 사례(streak=184)
1건만 재검증하고 나머지 18건을 이월했다. 이번 세션은 그 18건을 마저
표본검증해 "310차 가설(lookahead 끝단 샘플링 아티팩트)이 완전히
틀렸는지, 일부는 진짜 아티팩트가 남아있는지"를 일반화하는 것이
목표였다(314차 WIP "다음 작업" 1번).

**방법**: 동일 x17seg zip을 재추출(20171행, `020ea86` 확인, 재현성
확인). `sim_route_310_provisional_streak_real_corpus.py`로 원거리+
이동중 19건 전체를 다시 나열하고, 물리적으로 겹치는 사례들을 위치
기준으로 묶어 5개 물리적 사건으로 재분류했다:

1. seg6(500m->30m, streak=184) -- 314차가 이미 검증 완료(재확인만,
   신규 아님).
2. seg14 t=1358.97(510m 고정, streak=16) -- production이 즉시
   `matched` 승격, qcamera로 우회전 커브 확인(정상 동작 사례).
3. seg4(510m->490m, streak=9/8/8 연속 3개 episode) -- 초반 flicker
   후 t=727.47부터 `matched` 안정화(정상, 292차 continuity 패턴과
   유사).
4. seg3(160m~210m, streak=5/7) -- 312/313차가 이미 확인한 근접 다중
   후보 구간(seg3 t=661~681s)의 원거리 꼬리일 뿐, 기존 결과 원용.
5. **신규**: seg13(t>=1304s, streak=3 x 5episode) -> seg14(t<=1338s,
   streak=4/3/2/2/2/2 x 6episode) 경계를 넘나드는 연속 후보.

**[핵심 신규 발견] seg13->seg14 경계 사례 상세 검증**:

`routeOrphanSingletonDist`를 시간순으로 추적한 결과, 이 후보는
`sim_route_310...py`의 episode 재구성 로직(streak가 재시작되면 새
episode로 취급)이 11개의 별도 episode로 쪼개 놓았을 뿐, 실제로는
**t=1304.07s(dist=510m)부터 t=1339.22s(dist=0m, 통과)까지 약 35초간
연속으로 존재한 하나의 물리적 후보**임을 원시 CSV 직접 대조로
확인했다(거리가 510m에서 10m 간격으로 단조 감소, 끊김 없음).

이 구간 전체(t=1304~1339, seg13 뒷부분+seg14 앞부분, 1100+ 프레임)를
`routeApexMode`/`routeClusterCount` 기준으로 전수 조사한 결과:

- `routeApexMode` 집합: `{'none', 'new'}` 뿐 -- **'matched'/'held'가
  단 한 번도 나타나지 않음** (마지막 프레임 t=1358.97에 'new'가
  뜨지만 이는 dist=500m인 **완전히 다른 후속 커브**이며 이 후보와는
  무관, 아래 참고).
- `routeClusterCount`: 이 구간(1100+ 프레임) 중 0이 아닌 프레임은
  **1건뿐**(그마저도 다음 후보의 것).
- `routeOrphanSingletonCount`: 132개 프레임에서 1(간헐적으로만 관측
  값이 찍힘 -- carrot_man.py 로직상 매 프레임 채워지진 않음), dist가
  510m -> 0m까지 10m 간격으로 끊김없이 단조 감소.

**즉, min_points=2 게이트가 이 고립 후보(orphan, 1포인트 클러스터)를
근접 구간(300m 이내)에 들어와서도 끝까지 노이즈로 오인해 제거했고,
그 결과 production은 접근~통과 전 구간에서 이 커브에 대해 apex를
전혀 잡지 못했다.**

**qcamera 대조**(`verify_and_extract_frames.py`, t=1322.2/1328.2/
1333.2/1337.4/1339.2 -> dist 환산 약 240m/160m/80m/30m/0m):
- 240m(t=1322.2): 넓은 다차선 도로, 이 시점 화면만으로는 커브 형상이
  아직 명확하지 않음(원거리 특성상 314차 방법론 교훈과 일치).
- 160m(t=1328.2): 도로가 우측으로 굽어지는 형상 + 우측에 커브 경고
  표지판(노란 화살표형) 시야에 확인됨.
- 80m(t=1333.2): 도로 우회전 굴곡과 표지판이 뚜렷하게 확인됨.
- 30m(t=1337.4): 도로 경계석이 우측으로 굽어지는 형상까지 근접
  확인됨.

**결론**: 306/307차가 코드+합성 재현으로 확정한 가설("min_points=2
게이트가 고립된 1포인트 좁은 커브를 노이즈로 오인해 제거할 수 있다")의
**x17seg corpus 내 최초 실차+qcamera 실증 사례**다. 이전까지 이 가설의
유일한 실측 근거는 309차가 별도 route1~4 corpus에서 찾은 ep108
(orphan 1건) 뿐이었는데, 이번 사례는 이와 독립된 별도 corpus/장소에서
나온 두 번째 실증이며, 특히 **접근 시작(510m)부터 통과(0m)까지 전
구간에서 단 한 번도 승격되지 못했다는 점**에서 ep108보다 훨씬 완전한
형태의 결함 재현이다.

**한계(§28, 과대해석 경계)**:
1. 이 구간 vEgo는 12.0~17.0m/s로 완만히 변화(가속 후 소폭 감속)했고,
   `aEgo` 최솟값 -2.76m/s^2는 t=1358.97(구간 종료 후, `leadStatus`=
   True/`leadDRel`=45m인 리드 차량 관련)에 발생해 **이 커브 자체와는
   무관**하다. 이 구간 내내 harsh brake/조향 진동 등 "실제 운전자
   불편" 신호는 확인되지 않았다(`steeringAngleDeg` -10.0~4.6도, 급격한
   변화 없음). "min_points=2 게이트가 이 특정 커브에서 실제로 문제를
   일으켰는가"는 이번 세션 범위 밖이며 여전히 "아니오"에 가깝다 --
   이번 발견의 의미는 어디까지나 "게이트 결함이 실차에서 발생함을
   확인"이지 "그로 인한 불편이 확인됨"이 아니다.
2. 표본 1개 corpus(x17seg)뿐이며, 이 커브 지점이 다른 로그에서도
   재현되는지는 미확인.
3. `ryu` 본체 변경 없음(§27), ANALYSIS_ONLY.

**부수 관찰**: 314차는 seg17(`20260908_071258...--17`) rlog.zst가
잘려있어 one-shot/stream_reader 폴백 모두 실패했다고 기록했으나,
이번 세션 재추출에서는 정상적으로 972행이 추출됨(원인 불명 -- 컨테이너
환경 차이인지 파일 자체 문제였는지 재현 불가, §33 원칙대로 사실만
기록하고 임의 해석하지 않음).

**검증**:
- 정적 분석: 완료(`py_compile` PASS, 기존 스크립트 재사용만 함)
- 로그 검증: 완료(x17seg 20171행 재추출 -- 재현성 확인, seg13->seg14
  경계 1100+ 프레임 전수 조사)
- 시뮬레이션: 해당 없음(실측 로그 + qcamera 직접 대조)
- 실차 검증: apex 미승격 자체는 실측 확인, "실제 불편" 신호는
  확인되지 않음(위 한계 1)

**Devnotes**: `FINDINGS.md`(이 항목), `WIP.md`(315차 항목) -- toolkit
신규/수정 없음(기존 `sim_route_310_provisional_streak_real_corpus.py`/
`verify_and_extract_frames.py` 그대로 재사용, §21).

**다음 작업**:
1. seg3/seg4의 나머지 원거리 사례(위 3, 4번)는 이미 기존 결과로
   설명되므로 추가 조사 불필요 -- "원거리+이동중 19건" 표본검증은
   이번 세션으로 사실상 완료(5개 물리적 사건으로 전부 설명됨).
2. 근접+이동중 57건 중 harsh_brake_events/steering_oscillation_detector
   교차검증(310차 다음 작업 2번, 계속 이월 중, 여전히 미착수).
3. min_points=2 게이트 자체의 수정(설계안 검토)은 "실제 불편 증거 0건"
   상태이므로 여전히 서두를 근거가 약함 -- 사용자 계획 ②단계 결정
   보류 중.
4. (낮은 우선순위) seg17 rlog.zst 추출 성공/실패가 재현되지 않는 원인
   규명 -- 이번 조사 대상과 무관, 필요 시 별도 세션.

## 314차 -- [정정, §24/310차 결론 보강] "원거리+이동중 19건 lookahead 끝단 샘플링 아티팩트" 가설 반증 -- seg6 대표 사례(streak=184, dist 500m->440m)는 실제로는 진짜 우회전 커브였고, production apex_mode가 처음부터 끝까지(500m->30m) 정상 승격·추적함을 실측+qcamera로 확인. 310차의 "커브 없음" 판정은 500m 밖은 원래 육안으로 안 보인다는 점을 놓친 검증방법론 오류였음

**기존 결론(310차)**: seg6 t=872.79~881.94s(streak=184, provisional dist
500m->440m) 후보를 qcamera로 대조한 결과 "프레임 전 구간에 커브 없음,
다차선 직선 도로 + 정체 차량만 확인" -> "naviPaths lookahead 끝단(약
500m) 샘플링 아티팩트 의심(코드 레벨 확인 미실시)"이라고 기록,
"근접 진짜 커브 증거" 목록에서 제외.

**계기**: 314차는 310차 WIP "다음 작업" 1번(이 가설의 코드 레벨 확인,
310->311->312->313차까지 4세션째 이월된 최우선 항목)에 착수.

**새로운 증거**:
1. `extract_log.py`에 307차 계측 필드 4개(`routeNaviPointsLen`/
   `routeNaviStartIdxIn`/`routeNaviStartIdxOut`/`routePathLen`, cereal
   `@52~@55`)를 추가로 반영(310차가 같은 계측 묶음 중 이 4개를 누락)해
   x17seg를 재추출. 해당 구간(t=872~882s) 전체에서 `routeNaviPointsLen`
   =93(고정, 도중 84로 한 번 교체), `routePathLen`=18->12로 감소하는
   패턴을 발견해 최초엔 "버퍼 소진" 가설에 부합하는 듯 보였다.
2. 그러나 `extract_log.py --with-navi-paths`로 실제 리샘플 폴리라인
   (10m 간격 x,y,거리)을 직접 뽑아 대조한 결과, **같은 프레임들에서
   리샘플 거리가 매번 590.00m까지 완전히 채워져 있음**(`resample_10m_np`
   출력 60개 점, 마지막 거리 항상 590.00m) -- `routePathLen` 감소는
   원시 GPS 포인트 개수(간격이 넓은 raw waypoint 수)가 준 것일 뿐,
   리샘플된 600m lookahead 자체는 한 번도 소진되지 않았다. **"버퍼
   소진" 가설은 이 사례에서 반증됨**.
3. 리샘플 폴리라인 좌표를 직접 검토한 결과, arc-distance 0~70m
   구간에서 진행방향이 한 번 꺾이고(첫 번째, 근접 커브), 70~510m는
   거의 직선(x 55~58 사이 미세 변동, GPS 노이즈 수준)이다가,
   **510~590m 구간에서 진행방향이 거의 90도 급격히 꺾임**(x: 57.47m
   -> 3.08m, 동시 y가 466->517m로 계속 증가) -- 명백한 실제 도로
   형상(우회전)이며 샘플링 잡음으로 보기 어려운 규모.
4. 이 후보의 production 추적을 이어서 확인한 결과, **t=883.02s부터
   `routeApexMode`="matched"로 승격**돼 이후 정차(vEgo->0, 신호대기로
   추정, t=884~893s apexDist 430m 고정 유지) -> 재출발 후에도 거리가
   500m대에서 30m대까지 **한 번도 끊기지 않고 연속 감소**(t=883
   ~944s), 접근 중 속도도 11.6m/s(apexDist=130m)에서 7.3m/s
   (apexDist=30m)로 자연스럽게 감속하는, 전형적인 "커브 접근 감속"
   패턴을 보임.
5. apexDist≈30m 시점(seg7, seg-local t=42.79s)의 qcamera 프레임을
   직접 확인 -- **우회전 화살표 노면표시 + 실제로 오른쪽으로 꺾이는
   도로 형상이 명확히 존재**(스크린샷 확보). 310차가 "커브 없음"이라
   판정한 것은 t=872~882s 시점(그 시점 apexDist는 아직 430~500m)의
   qcamera만 봤기 때문 -- **500m 밖의 커브는애초에 그 시점 화면에
   보일 수 없는 거리**(도심 대시캠 가시거리는 대체로 100~150m
   이내)였다는, 검증방법론 자체의 허점이었다.

**변경 이유**: 위 4개 증거 모두 "샘플링 아티팩트로 인한 오탐"이
아니라 "정상적으로 조기(500m) 감지된 진짜 커브가 production까지
정상 승격·추적된 사례"임을 가리킨다. 310차의 원인 판정(아티팩트
의심)과 "근접 진짜 커브 증거 목록에서 제외" 조치는 **철회한다**.

**새로운 결론**: 이 사례는 아티팩트가 아니라 **route 로직이 의도대로
동작한 정상 사례**다. "원거리+이동중 19건" 전체에 대한 일반화는
아직 이르다(이번 세션은 19건 중 1건만 이 방식으로 완전 재검증) --
그러나 최소한 "500m 밖에서 qcamera에 커브가 안 보이면 아티팩트"라는
310차의 검증 기준 자체가 틀렸다는 점은 이번 사례로 명확해졌다.

**방법론 교훈(재발 방지)**: 원거리(>=150m) 후보를 qcamera로 검증할
때는 **후보가 처음 잡힌 시점이 아니라, 그 후보가 apex_mode=matched로
승격된 뒤 apexDist가 충분히 작아진(예: <100m) 시점**의 qcamera를
봐야 한다. 처음 잡힌 시점의 화면에 커브가 안 보이는 것은 "아직 멀어서
안 보이는" 정상 상태와 "애초에 커브가 없는" 아티팩트 상태 둘 다에서
동일하게 관찰되므로, 이 두 경우를 구분하지 못하는 검증 기준이었다.

**한계(§28)**:
1. 19건 중 이번에 완전 재검증한 것은 1건(seg6, streak=184)뿐 --
   나머지 18건은 미착수. 진짜 아티팩트가 있다면(있을 수도 있음)
   "승격되지 않고 provisional/orphan 상태로 계속 남는 경우" 또는
   "승격은 됐지만 apexDist가 작아진 뒤에도 qcamera에 커브가 없는
   경우"로 구분 가능할 것이나, 이번 세션에서 실제로 관찰하지는
   못했다.
2. 이 사례가 사실은 "실제 불편했는가"(harsh_brake/조향진동 등)까지는
   확인하지 않았다 -- 이번 세션의 검증 범위는 "커브가 실재하는가/
   production이 정상 추적했는가"로 한정.
3. `ryu` 본체 변경 없음(§27), 이번 세션은 계측 컬럼 4개 추가 +
   실측 로그 분석뿐 -- ANALYSIS_ONLY.

**검증**:
- 정적 분석: 완료(`py_compile` PASS, `extract_log.py` 수정분)
- 로그 검증: 완료(x17seg 19199행, seg1-16 재추출 -- seg17은 rlog.zst
  파일 자체가 잘려있어 스트리밍 폴백도 실패, 이번 조사 대상 구간
  (seg6-7)과 무관해 범위 밖으로 둠, §33 아래 "미확인 사항" 참고)
- 시뮬레이션: 해당 없음(실측 로그+qcamera 직접 대조)
...
- seg17(`20260908_071258...--17`) rlog.zst가 잘려 있어 one-shot
  decompress 실패 -> `decode_rlog.py`의 stream_reader 폴백도 실패
  (`zstandard.backend_c.ZstdError`가 `except zstandard.ZstdError`에
  안 걸리는 것으로 보임, 원인 미확인 -- 이번 세션 조사 대상과 무관해
  범위 밖으로 두고 seg1-16만으로 재추출·분석함, §27 최소변경 -- 다음
  세션에서 seg17이 필요해지면 별도로 원인 규명).

## 313차 -- [정정, §24/311·312차 인용 수치 보강] `--check-cruise` 옵션 프레임 단위 정밀화로 311/312차 "cruiseEnabled=True 23건/물리적 위치 4곳" 수치가 실제로는 "True 25건(위치 4곳은 동일, seg3 클러스터만 7건->9건)"이었음을 확인

**배경**: 313차는 312차 WIP "다음 작업" 1번(`sim_route_310_provisional_streak_real_corpus.py`에 `--check-cruise`/위치 군집화를 정식 옵션으로 편입)에 착수. 311/312차는 이 분류를 대화식 python 후처리로 수행했다.

**한 일**: `--check-cruise`(cruiseEnabled 구간 전체 값 True/False/mixed 분류) + `--cluster-gap-s`(같은 seg 내 시간 gap 기준 물리적 위치 군집화) 옵션을 스크립트에 정식 추가하고, x17seg corpus(`020ea86`, 20171행, 311/312차와 동일 corpus)로 재실행.

**차이 발견**: 근접+이동중 57건에 대해:
- 311/312차 인용: True 23건 / False 29건 / mixed 5건
- 313차 프레임 단위 재계산: **True 25건 / False 31건 / mixed 1건**

원시 CSV(`cruiseEnabled` 컬럼, 20Hz)를 seg3 t=672~677.5s 구간에서 직접 대조한 결과, 전환 시점(673.371s False->True, 676.573s True->False)이 정확히 확인되며 313차 프레임 단위 분류가 이 원시값과 정확히 일치한다(t=673.22~673.52 episode만 전환이 겹쳐 mixed, 그 앞뒤 episode는 모두 순수 True/False). 즉 **313차 수치가 원시 로그와 일치하는 정확한 값**이며, 311/312차가 어떤 근사 방식으로 23/29/5를 얻었는지는 이번 세션에서 재현하지 못했다(코드가 대화식 스니펫이라 세션 종료 후 소실, §28 -- 원인 확정 불가, 사실만 기록).

**영향 범위**: **312차의 핵심 결론("ADAS engaged 후보가 물리적으로 4곳에 불과함")은 무영향** -- 위치 군집화 재실행 결과도 동일하게 4곳(seg3/seg4/seg14/seg16)이 나왔다. 차이는 seg3 클러스터의 episode 개수(7건->9건)와 총 True 개수(23->25)뿐이며, qcamera 대조가 이미 완료된 4개 물리적 위치 자체나 "실제 불편 신호 0건"이라는 결론에는 영향이 없다.

**한계(§28)**: 311/312차 원본 대화식 코드가 남아있지 않아 오류의 정확한 지점(어떤 근사/샘플링을 썼는지)은 규명 불가. 향후 유사 작업은 처음부터 정식 스크립트 옵션으로 만들어 재현 가능하게 할 것(이번 313차 조치가 그 자체).

**검증**: 로그 검증 완료(x17seg 20171행 원시 CSV 직접 대조, seg3 t=672~677.5s 전 프레임 육안 확인). 실차 검증: 해당 없음(계측 정확도 문제이지 차량 거동 문제 아님).

## 312차 -- [실차 로그 정황증거 마무리] ADAS engaged(cruiseEnabled=True) 23건이 실제로는 서로 다른 물리적 위치 단 4곳으로 수렴함을 확인 -- 4곳 전부 qcamera 대조 완료(311차 3곳 + 이번 세션 seg14 1곳), seg14에서 두 번째 "실제 커브+ADAS개입+apex공백" 사례 확인(단, 여기서도 harsh_brake/조향진동 0건)

**배경**: 311차는 근접+이동중 57건 중 `cruiseEnabled=True`인 23건을
추렸으나 그중 3개 위치(seg3/seg4/seg16)만 qcamera 대조를 마치고
"나머지 20건 미확인"으로 남겼다. 이번 세션에 이어서 확인했다.

**이번 세션에서 발견한 것(방법론적 정정)**: 23건을 seg+시간 근접
(gap>5s 기준)으로 군집화한 결과, 실제로는 **서로 다른 물리적 위치가
단 4곳뿐**이었다(episode 수가 위치 수를 과대표기 -- 하나의 커브에
차량이 접근하면서 `routeProvisionalStreak`가 여러 번 끊겼다 이어지며
episode가 여러 개로 쪼개지기 때문). 즉 "23건 중 20건 미확인"이라는
311차의 남은 작업 규모는 실제로는 "물리적 위치 1곳(seg14) 미확인"
정도로 축소된다. 이 사실 자체가 향후 후보 우선순위를 셀 때 episode
개수가 아니라 위치(군집) 개수를 기준으로 삼아야 함을 보여준다.

**4개 위치 최종 대조 결과**:
1. seg3 t=674.04~676.23s (episode 7건, max_streak=8) -- 311차 확인:
   넓은 직선 도로 + 원거리 교차로, 리드 차량 존재, 커브 없음.
2. seg4 t=722.93~724.52s (episode 3건, max_streak=6) -- 311차 확인:
   직선 도로, 커브 없음.
3. seg16 t=1465.92~1471.63s (episode 7건, max_streak=5) -- 311차 확인:
   실제 커브 지형 + `cruiseEnabled=True`(대부분) + apex 전 구간
   `none`. harsh_brake/조향진동 0건. `cruiseEnabled`가 클러스터 직전
   False->True 전환된 정황 있어 "커브 통과 후 재개입" 가설 미확정.
4. **seg14 t=1329.72~1334.82s (episode 6건, max_streak=3) -- 이번
   세션 신규 확인**: qcamera 프레임 3장(t=1329.72/1331.07/1333.50/
   1334.72)에서 가로수 늘어선 편도 다차선 도로가 완만하게 좌측으로
   굽는 형태를 육안 확인(우측에 커브 경고 표지판으로 보이는 황색
   쉐브론 표지 함께 확인). `routeApexMode`는 이 구간 전체(t=1327~
   1340s로 넓혀도 동일) `none` 고정. `cruiseEnabled=True`,
   `leadStatus=True`(리드 차량 `leadDRel`이 약 32m에서 34.5m로
   완만히 벌어지는 정상 추종 상태) 유지. **`aEgo`가 이 구간 내내
   0.0~0.4 범위의 약한 가속으로, 커브 진입부에서도 감속이 전혀
   발생하지 않고 `vEgo`가 13.3->14.6m/s로 계속 상승**했다.
   `steeringAngleDeg`는 구간 후반(t>=1334.8s)부터 서서히 커져
   최대 -10.0도까지 도달(조향각 자체가 크지 않아 완만한 커브임을
   뒷받침). `harsh_brake_events`/`steering_oscillation_detector`
   (§21 재사용, 넓힌 창 t=1327~1340s 포함)는 이번에도 **둘 다 0건**.

**해석**: seg16에 이어 seg14도 "production이 커브를 완전히 놓쳤다"는
사실은 계측상 명확하지만(apex_mode 전 구간 none), 실제 감속이 전혀
없었음에도(오히려 계속 가속) 급브레이크/조향진동 등 "불편했다"는
신호가 없다. 조향각 최대치(10도)가 크지 않다는 점을 함께 보면, **이
정도로 완만한 커브는 애초에 감속이 필요 없었을 가능성**이 다른
설명으로도 성립한다 -- "놓친 candidate = 실제 개입 필요"라는 단순
등식은 이번 로그의 seg14/seg16 두 사례 모두에서 성립하지 않았다.

**누적 결론(310~312차 종합)**: 이번 x17seg 단일 로그(17세그먼트)
내에서 "근접+이동중" 진짜 커브 후보로 qcamera 확인된 위치는 4곳
(cruiseEnabled=True 기준)이며, 그중 실제 커브 지형은 2곳(seg14,
seg16) -- 둘 다 apex_mode 전 구간 공백이지만 harsh_brake/조향진동
증거는 전무하다. "306/307차 가설(고립된 좁은 커브 노이즈 제거
문제)"을 뒷받침하는 근거는 "production이 이 후보들을 놓쳤다"는
계측 사실까지는 확실하나, "그로 인해 실제 불편이 발생했다"는 근거는
이번 로그 표본 전체에서 여전히 0건이다(§28, §29).

**한계(§28)**:
1. 단일 로그(17세그먼트) 표본 -- 4개 위치 모두 완만한 커브/저속
   구간이라, 급커브 상황에서의 재현 여부는 미확인.
2. seg14/seg16 모두 "감속 없이도 문제없었다"는 것이 도로 형상 자체가
   완만해서인지, 다른 이유(예: 곡률이 lookahead 계산상 임계값 이하)
   인지는 코드 레벨 확인 없이는 구분 불가.
3. 원거리+이동중 19건(310차부터 이월)은 이번 세션에서도 다루지
   않음 -- 다음 작업으로 이월.
4. 근접+정차중/원거리+정차중 버킷은 이번 세션 대상 아님(310차에서
   이미 자명 케이스로 확인).

**검증 상태**: 실측 확인(§28) -- CSV 원본 프레임 직접 조회 +
qcamera 프레임 육안 대조(신규 3프레임 위치 x 3장=9장, seg14).
`harsh_brake_events`/`steering_oscillation_detector`는 기존 검증된
함수 재사용. **실차 검증 아님** -- 오프라인 로그 분석.

**Devnotes**: WIP.md 312차 항목(이 내용 요약), FINDINGS.md 이 항목.
`toolkit/README.md`/`CHANGELOG.md`는 이번에도 신규 스크립트 없어
변경 없음(311차부터 이월된 `--check-cruise`/위치 군집화 옵션 정식
편입은 여전히 다음 작업).
...
남아있던(FINDINGS.md 293차/296차) `lost_with_candidates_present`
1건(ep108, route4 t=4017.4, 주택가 좁은 도로/교차로 인접)의 클러스터링
코드 레벨 추적을 이번 세션에서 착수·완료.

**핵심 발견 1 -- 구조적으로 코드만으로 확정(수학적 필연, 실측 불필요)**:
`carrot_man.py::_route_cluster_continuity_step()`(749~822행)을 전체
분기 추적하면, 반환값 `apex_speed`가 `None`이 되는 경로는 **정확히
하나**뿐이다 -- 그 프레임의 `clusters`(=`route_find_clusters()`가
min_points=2 필터를 통과시킨 결과)가 완전히 빈 리스트인 경우. `clusters`
가 하나라도 있으면(812행 `if clusters:`) reset_reason이 "passed"든
"lost"든 무관하게 그 클러스터 대표점으로 즉시 재탐색에 성공해 유효한
apex_speed를 반환한다. 따라서 293차 분류 스크립트(`sim_route_292_
continuity_root_cause.py`)가 "lost_with_candidates_present"로 잡아낸
현상(cutoff 프레임에 `routeCandidateCount`(stage0 raw candidate 개수,
1206행)>0인데 최종 apex_speed가 0/None)은 **"raw candidate는 있었지만
route_find_clusters()가 min_points=2 미만이라며 전부 걸러내 최종
clusters=[]가 된 경우"로만 설명 가능**하다는 것이 코드 자체로 증명된다
-- 이것 말고는 다른 코드 경로가 없다.

**핵심 발견 2 -- 구체적 트리거 가설(§28, 정황증거 + 합성 재현, 아직
실 corpus 미확정)**: 그렇다면 "raw candidate는 있는데 클러스터가 전부
필터링되는" 공간 패턴이 무엇인지 추가로 추적했다. 147차가 넣은 fine
곡률 서브샘플(`carrot_man.py` 1094~1139행, `ROUTE_CURVATURE_FINE_
SAMPLE=1`, 10m 그리드에서 chord 20m로 "교차로 우회전 같은 좁은 코너"를
명시적으로 잡기 위해 추가된 기능)은, 물리적으로 짧은 커브(좁은 반경의
교차로 코너 등)의 경우 `distance_interval=10m` 그리드에서 "도로제한
속도보다 느린 지점"이 **정확히 1개 그리드 포인트에만** 찍히게 만들 수
있다. 그런데 247차 design doc이 명시한 `route_find_clusters()`
min_points=2 게이트의 설계 의도 자체가 "단발성 노이즈 후보 하나만으로
apex가 성립하지 않도록" 하는 것이다 -- 즉 **147차가 잡으려던 신호(좁은
실제 커브)와 251차가 걸러내려던 노이즈(단발성 오탐)가 10m 그리드
해상도 관점에서 구분 불가능한 동일한 모양(고립된 1개 포인트)을 가질 수
있다**는 것이 이번 가설의 핵심.

**검증(신규 `toolkit/sim_route_306_ep108_cluster_isolation.py`,
296차가 이식해둔 production 함수를 그대로 import, §21/22)**:
- 실험1: "고립된 짧은 커브" 하나만 존재하는 합성 시나리오에서 dip
  길이(연속 그리드 포인트 수)를 1~5로 스윕. **dip_len=1(물리적 폭
  <10m)일 때만** raw candidate(1개)가 있음에도 최종 apex_speed=None
  (mode="none") -- dip_len=2 이상(20m+)부터는 즉시 min_points=2를
  통과해 정상적으로 apex 확보. self-test 5/5 PASS.
- 실험2: 293/294차가 실제로 검출한 상황(이미 정상적으로 locked된
  apex가 존재하다가, 다음으로 접근하는 것이 고립된 1포인트 커브인
  경우)까지 2프레임으로 재현. mode="passed"(기존 apex를 물리적으로
  통과)로 정상 전이되는 바로 그 프레임에, 새로 접근한 고립 커브의
  raw candidate(1개)가 존재함에도 apex_speed=None으로 떨어짐을 확인
  -- 293차 분류 기준(`routeCandidateCount>0` + `apexSpeed` 0/None)과
  정확히 동일한 조합이 재현됨. self-test PASS.

**한계(§28, 명확히 구분)**:
- 발견 1(구조적 필연)은 **코드 읽기만으로 확정**된 사실이며 추가 검증
  불필요.
- 발견 2(fine-sample 좁은 커브가 실제 트리거라는 가설)는 **정황증거 +
  합성 재현**까지만 확인됨. ep108 그 프레임의 실제 raw candidate
  배열(간격/거리, 몇 개였는지, 서로 몇 m 떨어져 있었는지)은 이번
  세션에서 확인하지 못했다 -- 293/294차가 쓴 route4 zip은 §23 대용량
  CSV 정책상 devnotes에 커밋되지 않았고, 이번 컨테이너에도 재업로드
  되지 않아 원본 데이터 접근이 불가능했다. 따라서 "ep108이 실제로
  이 메커니즘 때문이었다"는 아직 **확정이 아니라 가장 유력한 가설**
  상태다(§28 -- 추측만으로 원인을 확정하지 않는다).
- 표본 자체가 1건(ep108)뿐이라 이 메커니즘이 293차 corpus의 다른
  `lost_no_candidate` 36건 중 일부에도 관여하는지는 별개 확인 필요
  (그쪽은 이번 세션 범위 밖 -- 294차가 이미 다른 각도(맵 곡률 오탐)로
  분석 중인 이슈와 겹칠 수 있음, 302~294차 "ep47/48/101/105 직선도로
  오탐" 항목과의 관계도 향후 확인 필요).

**영향받는 실차 제어 로직**: 없음(이번 세션은 `ryu` 코드 변경 없이
기존 코드 읽기 + `ryu-devnotes/toolkit` 신규 스크립트만 추가, §27/§28
ANALYSIS_ONLY 유지).

**검증**:
- 정적 분석: 완료(코드 전체 분기 추적)
- 로그 검증: 미실시(원본 route4 corpus 미보유, 재업로드 필요)
- 시뮬레이션: 완료(합성 시나리오 self-test 5/5 + 2/2 PASS)
- 실차 검증: 해당 없음(코드/파라미터 변경 없음)

**다음 작업**:
- 사용자가 293/294차에 썼던 route4 zip(또는 최소 t=4017.4 부근
  세그먼트)을 재업로드하면, 그 프레임의 실제 `routeCandidate0/1/2`
  텔레메트리(거리/속도, 최대 3개까지는 원본 값 확인 가능)로 발견 2
  가설을 직접 대조 확정할 수 있음.
- 재업로드가 어려우면, 이 메커니즘이 확정된 것으로 간주하고 바로
  개선 설계(예: min_points=2를 "그리드 해상도가 놓칠 수 있는 좁은
  커브"에 한해 완화하는 방안, 단 §31 사용자 승인 필요 + 노이즈 재유입
  위험 재평가 동반)로 넘어갈지 여부는 사용자 판단 필요.
- (293차부터 이월, 이번 세션과는 별개 이슈) ep47/48/101/105 직선도로
  오탐 코드 트레이스는 여전히 미착수.
...
9.5m/#9→#10 0.0m, 전부 a3b3373495)에 대해 "같은 커브였다"를 넘어
"B를 A의 연속 track으로 승계해도 되는가"를 판단할 연속성 feature를
찾는 것이 302차 다음 작업 2번이었다. 사용자가 제시한 문서의 STEP1
(3쌍 프레임 단위 재추적)/STEP2(연속성 feature 수치화)를 이번
세션(303차) 범위로 진행.

**방법**: 신규 `sim_route_303_ab_continuity_features.py`. 301차
`build_stream()`/`run_route()`, 302차 `resolve_point()`/`load_gps()`/
`haversine()`/`classify()`를 무변경 재사용(§21). 각 쌍에 대해
prev.B(재획득 프레임) ~ next.A_last(다음 이벤트 LOST 직전 마지막
매칭) 사이 Δtime/Δdistance(apex)/Δspeed(apex)를 301차 이벤트 필드로
계산하고, ΔGPS_bearing(신규)은 302차가 구한 GPS fix의 `bearingDeg`
차이(원형 wraparound)로 계산. 동시에 그 구간 전체를 프레임 단위로
dense 나열(mode/apex_dist/apex_speed/raw candidate 개수/cluster
개수·크기/miss_frames).

**실측 결과**:

| 쌍 | Δtime | Δdistance(apex) | Δspeed(apex) | GPS haversine | ΔGPS_bearing | prev.B_cluster_size | next.miss_frames | prev.B_survive |
|---|---|---|---|---|---|---|---|---|
| #4→#5 | 0.70s | -20.0m | +6.9kph | 6.8m(SAME_CURVE) | 0.0deg | 2 | 6 | 4.91s(중도 이탈) |
| #7→#8 | 3.40s | -60.0m | -8.8kph | 9.5m(SAME_CURVE) | 0.9deg | 2 | 6 | 3.40s(중도 이탈) |
| #9→#10 | 0.20s | +0.0m | +11.4kph | 0.0m(SAME_CURVE) | 0.0deg | 3 | 6 | 2.45s(중도 이탈) |

**핵심 관찰**:
1. **Δdistance/Δspeed는 3쌍 모두 부호와 크기가 제각각** -- GPS로는
   동일한 물리적 커브로 확정된 3쌍인데도, naviPaths apex 거리값
   자체는 -60m~0m로 큰 편차를 보인다. 특히 #7→#8은 Δdistance=-60m로
   사용자 제시 문서 STEP2 예시("거리가 확 튀면 승계 부적절")의
   "나쁜 예"에 해당하는 크기이지만, GPS 판정은 오히려 가장 명확한
   SAME_CURVE(0.9도라는 매우 작은 bearing 변화까지 동반) 중 하나다.
   **즉 naviPaths apex 거리값의 연속성(Δdistance)은 "동일 물리
   대상 여부" 판별에 신뢰할 수 있는 feature가 아닐 가능성이 있다**
   -- naviPaths가 매 프레임 HD맵 곡률 재계산에 따라 그리는 "가장
   먼저 검출한 커브 후보"의 위치 자체가 흔들릴 수 있음을 시사(원인
   자체는 이번 세션에서 코드 레벨로 추적하지 않음, §28).
2. **ΔGPS_bearing은 3쌍 모두 0.0~0.9도로 매우 작다** -- 최소한
   진행방향(heading) 급변은 없었다. 다만 1Hz 샘플이라 프레임 단위
   미세 회전은 관측 범위 밖(302차 한계와 동일).
3. **신규 발견: #4→#5 구간에서 naviPaths 기반 프레임 시각이 2.05~
   2.07초 간격으로 3회 연속 점프**(578.36~579.06까지는 정상 0.05~
   0.1s 간격, 이후 579.06→581.11→583.21→585.33→585.36으로 대형
   점프). `build_frames()`(300차)가 `naviPaths` 필드가 비어있거나
   `nRoadLimitSpeed<=0`인 행을 건너뛰기 때문으로 추정되나, 어느
   쪽이 원인인지는 이번 세션에서 분리하지 않음. **이 구간의
   `miss_frames`(301차 지표, 최종 6 도달)는 균등한 20Hz 프레임이
   아니라 실제로는 최대 2초 이상 벌어진 드문 샘플들의 누적이라는
   뜻** -- 301차/300차가 "6프레임"을 시간으로 환산할 때 암묵적으로
   가정했을 수 있는 "프레임=일정 간격"이 이 구간에서는 깨진다.
   #7→#8/#9→#10 구간에서는 이런 대형 점프가 관찰되지 않음(정상
   간격 유지) -- #4→#5에 국한된 현상인지 route 전역 패턴인지 미확인.

**중요 한계(§28, 반드시 인지)**:
- 표본 3쌍뿐이라 일반화 불가. Δdistance/Δspeed가 "승계 조건" feature로
  부적합해 보인다는 관찰(위 1번)도 반례 하나(#7→#8)에 기반한 잠정
  해석이지 확정된 결론이 아니다.
- #4→#5의 프레임 점프 원인 미조사(naviPaths 갱신 간헐성 / road_limit
  <=0 구간 / 세그먼트 경계 중 미분리) -- 이번 세션의 원래 목표(STEP1/2)
  와는 별개로 발견된 신규 이슈이며, 다음 세션에서 별도로 추적 필요.
  기존 FINDINGS.md 항목 중 이 현상과 관련된 선행 기록이 있는지는
  이번 세션에서 검색하지 않음(§24 -- 다음 세션에서 먼저 검색할 것).
- STEP3(LOST 재정의 설계)/STEP4(설계 후보)/STEP5(counterfactual 3방식
  비교)는 착수하지 않음 -- 위 한계로 "B 승계 조건"이 데이터로
  뚜렷하게 확정되지 않은 상태에서 다음 단계로 넘어가는 것은 성급하다고
  판단(원안 문서 자체도 "조건 입증 후 다음 단계" 순서였음).
- `carrot_man.py` 여전히 무변경(ANALYSIS_ONLY, §29).

**결론**: 302차가 GPS로 확정한 3쌍의 SAME_CURVE는 여전히 유효하지만,
"이 B를 A의 연속 track으로 승계해도 되는가"에 대한 판별 feature로
naviPaths 자체 거리/속도값의 연속성(Δdistance/Δspeed)은 이번 표본
에서 신뢰할 만한 신호를 보이지 않았다. ΔGPS_bearing(작음)만이
일관된 신호였으나 표본 3건으로는 임계값을 확정할 수 없다. 코드
설계(STEP3 이후)는 표본 확대 또는 다른 feature 조합 재탐색 후로
이월.

---

## 302차 -- [실측 확정] A->LOST->B 연쇄 4쌍(a3b3373495 #1→#2/#4→#5/#7→#8[신규]/#9→#10) GPS 좌표 대조 -- 3쌍(#4→#5, #7→#8, #9→#10)은 SAME_CURVE(<=15m), #1→#2는 NEXT_CURVE 경계(20.0m, 판정 유보)

**배경**: 301차가 밝힌 15건의 A->LOST->B 경계 중 시간상 연쇄된 쌍
(#1→#2, #4→#5, #9→#10)이 실제로 같은 물리적 커브인지 GPS로 확정하는
것이 298차부터 이월된 과제였다. 사용자가 ChatGPT와의 협업 결과(302차
방향 제안: GPS 좌표 대조)를 공유하며 그대로 진행을 지시.

**방법**: `extract_gps.py`로 4개 route zip(rlog.zst, 이번 세션에
재업로드됨)에서 실측 `gpsLocation`(1Hz)을 추출. naviPaths의 로컬
(x,y)(analysis_helpers.parse_navi_paths, §21 재사용)를 apex 거리값과
정확히 매칭(원본 d 배열에서 exact/near match, 10m 간격)해 해당
apex의 차량-로컬 좌표를 복원하고, 그 프레임과 가장 가까운 실측 GPS
fix의 위치+`bearingDeg`로 단일 회전투영해 절대 lat/lon을 구함(신규
`sim_route_302_ab_gps_correlation.py`). **carrotMan 자체 추정 위치/
헤딩(`xPosLat/xPosLon/xPosAngle`)은 사용하지 않음** -- 162차가 이미
회전 중 dead-reckoning으로 헤딩이 최대 11초 고정되는 문제를 확인했기
때문(사용자 지시로 명시적 배제).

**실측 결과**:

| 연쇄 | prev.B -> next.A_last_matched gap | haversine 거리 | 판정 |
|---|---|---|---|
| #1.B -> #2.A | 0.26s | 20.0m | NEXT_CURVE(경계) |
| #4.B -> #5.A | 0.70s | 6.8m | SAME_CURVE |
| #7.B -> #8.A | 3.40s | 9.5m | SAME_CURVE(**신규 식별** -- 301차까지는 이 쌍을 연쇄로 명시하지 않았음) |
| #9.B -> #10.A | 0.20s | 0.0m | SAME_CURVE |

(gap은 "이전 이벤트가 LOST를 선언한 시각" -> "다음 이벤트가 A를
마지막으로 정상 매칭했던 시각" 기준. 판정 임계값: SAME_CURVE<=15m,
NEXT_CURVE<=80m, 그 이상 DIFFERENT -- 이번 세션에서 문서화한 신규
기준, PARAMS_REGISTRY.md 등재 예정)

**핵심 관찰**:
1. **15건 중 시간상 연쇄로 식별된 쌍은 전부 4개(모두 a3b3373495
   route 내부)이고, 그 중 3쌍(#4→#5/#7→#8/#9→#10)이 15m 이내로
   SAME_CURVE 판정됨.** 즉 이 3건에서는 "LOST -> 재획득된 B"가
   실제로는 A와 동일한 물리적 커브였다 -- 추적이 순간 끊겼다가 같은
   대상을 다시 잡은 것이지, 다른 커브로 착오 전환된 것이 아니다.
2. **#1→#2(20.0m)만 NEXT_CURVE 경계에 걸림.** SAME_CURVE 임계값
   15m를 5m 초과 -- 이 정도 오차는 투영 방식의 한계(1Hz bearing
   지연, naviPaths 로컬 프레임 자체의 근사 오차, GPS 안테나 오프셋
   미보정)로 설명 가능한 범위이나, 명확히 SAME_CURVE로 단정하지는
   않는다(판정 유보).
3. **#7→#8은 301차에서 명시적으로 언급되지 않았던 신규 연쇄** --
   301차는 #1→#2/#4→#5만 "시간상 연쇄"로 짚었으나, 이번 세션이
   전체 15건에 대해 같은 route 내 인접 쌍을 프로그램적으로 스캔한
   결과 #7→#8(gap 3.40s, 둘 다 no_curve 라벨)도 동일 패턴으로
   드러남.
4. **horizontalAccuracy는 15건 전 구간에서 0.0으로 기록**되어(295차
   기존 발견과 동일 재확인) GPS 품질 게이팅에 쓸 수 없었다 -- 판정에
   accuracy 기반 GPS_UNCERTAIN 강등은 이번 corpus에서 한 번도
   발동하지 않음(수치상 통과일 뿐 실제 신뢰도가 검증된 것은 아님).

**중요 한계(§28, 반드시 인지)**:
- 이 판정은 "같은 물리적 커브인가"에 한정되며, "그러므로 lost ->
  seamless switch로 코드를 바꿔야 한다"는 결론까지 자동으로 이어지지
  않는다. 301차가 이미 확인했듯 SAME_CURVE로 밝혀진 이 3쌍의 B조차
  5초 생존을 완주하지 못했다(생존시간 2.2~4.9s) -- "같은 커브를
  다시 잡았지만 그 track 자체도 오래 유지되지 못한다"는 별개 문제가
  남아있다.
- 투영 정확도는 1Hz GPS bearing에 의존하며, 프레임별 개별 오차
  검증(예: 알려진 정지 랜드마크 대조)은 하지 않았다 -- #1→#2의
  20.0m가 "실제로 다른 지점"인지 "투영 오차"인지는 이 결과만으로
  완전히 분리되지 않는다.
- SAME_CURVE/NEXT_CURVE/DIFFERENT 임계값(15m/80m)은 이번 세션에서
  새로 정한 것으로, 기존 세션에서 검증된 상수가 아니다(§27 -- 최소
  변경 원칙상 실측 없이 세부값을 더 정교화하지 않고 이번엔 이
  기준으로 1차 판정만 수행).
- `carrot_man.py` 여전히 무변경(ANALYSIS_ONLY, §29).

**결론**: 4개 연쇄 쌍 중 3개(75%)가 SAME_CURVE로 확인됨 -- "강제
RELEASE 후 재획득된 B가 노이즈가 아니라 같은 커브"라는 가설이 이
표본에서는 우세하다. 다만 표본이 4쌍(그것도 전부 한 route 안)뿐이라
일반화하지 않는다(§28). `carrot_man.py` seamless switch 설계 검토는
이 결과를 근거로 다음 세션(303차 후보)에서 논의하되, 코드 패치는
여전히 보류.
...
15건(24.6%)이 seamless forced-release(같은 프레임 재탐색 성공한 유효
apex를 버리고 무조건 RELEASE)였고, 15건 전부 `new_apex_needs_decel=True`
(재탐색된 apex가 실제로 감속 대상)라는 것까지 실측으로 확인됐으나,
qcamera 육안 대조는 미실시 상태였다.

**새로운 증거(298차)**: `verify_and_extract_frames.py`로 15건 전체
프레임(전후 문맥 포함 45장) 추출 + 육안 확인 결과:
- **7/15(46.7%)만 화면상 실제 커브로 확인**(#1,2,6,11,12,13,14 --
  상세 목록/판정 근거는 `evidence/route_297_seamless_release_qcamera/
  classification.md` 참고)
- **5/15(33.3%)는 명백히 커브 없음**(#3,7,8,9,10) -- 그중 1건(#3)은
  터널 내부(전후 프레임 모두 완전 직선)로, 293/295차의 터널 GPS
  열화 가설과 동일 유형 위치에서 나온 사례(정량 검증은 여전히 불가,
  정성적 정합성만 확인).
- 3/15(20.0%)는 원거리 목표라 화면상 명확히 판정 불가(불분명으로
  남김, #4,5,15).
- **부수 관찰**: #1/#2, #4/#5, #9/#10 세 쌍은 같은 물리적 위치를
  6~7초 간격으로 두 번 잡은 것으로 보임 -- 15건이 15개의 독립된
  상황이 아닐 수 있음(GPS 좌표 직접 대조로 확정은 다음 세션 과제).

**변경 이유**: 297차가 "감속 대상인가"(물리량, `new_apex_needs_decel`)
만으로 개선 방향의 타당성을 시사했는데, "재탐색된 candidate 자체가
실제 커브인가"라는 별도 축이 검증되지 않은 채로는 그 타당성 판단이
불완전했다.

**새로운 결론(297차 결론 자체는 무효화하지 않고 보강)**: 297차의
"24.6%가 seamless forced-release, 100%가 감속 대상"이라는 실측
수치는 그대로 유효하다. 다만 이 15건 중 실제로 **유효한 커브**에서
발생한 비율은 46.7%(불분명 포함 최대 66.7%)로, 297차가 시사했던
"재탐색된 apex가 감속 대상이면 ACTIVE 유지" 개선안의 실질 효용은
그보다 낮을 수 있다 -- 나머지 33.3%(최대 53.3%)는 애초에 candidate
소스 자체가 노이즈인 지점이라, 그 개선안을 그대로 적용하면 "잘못된
candidate에 대해서도 계속 ACTIVE로 붙잡아두는" 부작용이 생길 수
있다. 다음 세션에서 코드 변경을 설계한다면 "감속 대상 여부"뿐 아니라
"candidate 신뢰도"까지 함께 고려하는 방향이 필요하다(§31 승인 전
설계 단계에서 반드시 반영).

**미해결/한계**: 표본이 15건(그중 3쌍이 같은 물리적 위치일 가능성)에
불과해 46.7%/33.3% 비율 자체를 일반화하지 않는다(§28). GPS 좌표
직접 대조, naviPaths 곡률값과의 정량 교차검증은 다음 세션 과제로
남김.

---

## 297차 -- [실측 확정] "seamless forced-release" 실 corpus 빈도: 61개 ACTIVE 에피소드 중 15건(24.6%), 전부 mode=lost, 전부 재탐색된 새 apex가 실제로 감속 대상이었음 -- 296차의 코드 레벨 예측을 실측으로 확정

**기존 결론(296차)**: `carrot_navi_route()`가 "passed"/"lost" +
동일 프레임 재탐색 성공 시 유효한 새 apex 데이터(`apex_dist`/
`apex_speed`)를 버리고 무조건 RELEASE시킨다는 것이 코드 레벨로는
확정됐으나, "실측 빈도가 얼마인가"는 293차의 어떤 자료로도 답할 수
없는 미확정 상태로 남아있었다(293차 "lost_with_candidates_present
1/39"는 이 현상과 무관한 별개 통계라는 것만 확인됨).

**새로운 증거(297차, route1~4 corpus, 2026-09-06 16:56~18:03,
75009 naviPaths 프레임)**: `sim_route_296_active_reacquire_gap.py::
RouteStateMachine`을 신규 어댑터(`sim_route_297_reacquire_gap_real_corpus.py`)
로 실 corpus에 직접 통과시킨 결과:

- 전체 61개 ACTIVE 에피소드 중 **15건(24.6%)**이 seamless
  forced-release로 종료(경로별 39건 중 10건/12건 중 3건/7건 중
  1건/3건 중 1건).
- 15건 **전부 mode="lost"** -- "passed + 동일 프레임 재탐색"(자연
  통과 직후 재탐색) 조합은 이 corpus에서 한 건도 관측되지 않음(296차
  self-test는 이 두 케이스를 합성으로만 확인했었음 -- 실측 분포는
  이번이 처음).
- 15건 **전부 `new_apex_needs_decel=True`** -- 재탐색된 새 apex가
  매번 실제로 `v_ego_ms > apex_speed/3.6`(감속 대상)이었다. 즉
  RELEASE가 버린 값은 예외 없이 "당장 감속에 유효하게 쓰일 수 있었던
  목표"였다.
- `immediate_reentry`는 15건 전부 False -- 296차가 self-test 단계에서
  이미 지목한 지표 자체의 구조적 한계(streak 리셋 -> confidence=0.0 ->
  eff_apex_speed=v_ego_kph 그대로 -> INERT 게이트가 항상 통과 실패)가
  실측에서도 그대로 재현됨, 새로운 정보 아님(296차 경고의 확인일 뿐).

**변경 이유**: 293차 통계로는 상/하한을 전혀 줄 수 없었던 이 현상의
실측 빈도를, 296차가 설계한 상태기계를 실 corpus에 직접 적용해
처음으로 수치화함.

**새로운 결론**: "재탐색된 새 apex가 감속 대상이면 ACTIVE를 유지하는
것이 낫지 않은가"라는 사용자/ChatGPT의 설계 우려는, 적어도 이
corpus에서는 매 발생 사례(15/15, 100%)에서 실측으로 뒷받침된다 --
"어쩌다 한 번 감속 불필요한 재탐색까지 섞여 있어 이득이 희석되는"
상황이 아니라, 관측된 모든 경우가 유효한 감속 대상이었다. 다만
표본이 특정 4개 route(283차부터 반복 사용된 동일 corpus, 총
2시간여 주행)에 한정되므로 다른 주행 조건(고속도로 전용, 시내
저속 등)에서도 동일 비율이 유지되는지는 별도 확인 필요 -- 이번
결론을 "이 코드 경로가 일반적으로 이 정도 빈도로 발생한다"로
일반화하지 않는다(§28, 재현조건 추가 확보 전까지 표본 한정 결론으로
유지).

**미해결/한계**: 15건 지점의 qcamera 육안 대조(실제 커브 여부 확인)는
아직 미실시 -- 293/294차가 다른 통계에서 "candidate 후보의 상당수가
화면상 커브 없음"(294차, lost_no_candidate 35건 중 18건)을 이미
확인한 전례가 있어, 이 15건도 일부는 맵 candidate 소스 자체의 노이즈일
가능성을 배제할 수 없다 -- 다음 세션 최우선 후보로 이월.

**부수 발견(devnotes toolkit 자체의 gap, 별도 기록)**:
`analysis_helpers.py::recompute_route_curvature_speed()`(147/158차)가
279차에 프로덕션에 재도입된 `mapTurnSpeedFactor` 곱셈을 반영하지
않음을 코드 대조로 확인 -- 이 함수를 쓰는 기존 68개 toolkit
스크립트는 전부 암묵적으로 factor=1.0을 가정한 결과라는 뜻(그
가정이 깨지는 것은 아니고, "factor≠1.0인 corpus를 그 함수로
분석하면 실제 프로덕션과 어긋난다"는 새로운 주의사항). 이 corpus는
실측 factor=1.10이라 297차는 이 gap을 우회하는 별도 함수
(`recompute_route_speeds_with_factor()`)를 신규 스크립트에만 추가해
사용 -- 기존 함수 자체는 무변경(§27, 68개 호출부 영향 회피).
...
있었지만 지표로는 안 보인다"를 구분할 방법이 없어 보류했던 터널
가설(ep9/ep10, `lost_no_candidate`)을 `horizontalAccuracy` 컬럼
추가로 재검증 시도.

**핵심 발견**:

1. **`horizontalAccuracy`는 이 corpus 전체에서 상수 0.0**. 터널
   구간(t=525~545)과 개활 고속도로 2개 구간(t=570~590, t=1160~1180)
   모두에서 예외 없이 0.0 -- 추출기 버그가 아니라 raw capnp
   이벤트(`gpsLocation.horizontalAccuracy`) 자체가 0.0임을 직접
   확인. route1~4 전체 샘플링에서도 동일. `source` 필드는 전부
   `qcomdiag`(Comma 3X 내장 QCOM 모뎀 GPS/diag 소스) -- 이 소스가
   해당 필드를 채우지 않는 구현/드라이버 특성으로 판단됨.
   `satelliteCount`도 항상 0, `hasFix`는 항상 True로 고정(터널
   진입/이탈과 무관) -- 셋 다 이 디바이스에서는 판별력 없음.

2. **재해석**: 293차는 "터널 가설을 증거 불충분으로 확인도 기각도
   하지 않는다"였다. 295차는 이를 한 단계 더 좁힌다 -- "이
   `horizontalAccuracy` 지표 자체가 이 하드웨어에서 항상 0이라
   원천적으로 판별에 쓸 수 없다"는 것이 확정됨(더 이상 "표본 부족"
   문제가 아니라 "이 지표가 이 디바이스에서 무의미하다"는 구조적
   결론). 터널에서 GPS가 실제로 열화되었는지는 **여전히 미확인** --
   다만 이 특정 방법으로는 더 이상 검증을 시도할 이유가 없다.

3. **부수 관찰(탐색적, n 작음 -- 결론 아님)**: 같은 대조에서
   `verticalAccuracy`/`speedAccuracy`는 프레임마다 실제 값이 변함.
   터널 구간(speedAccuracy mean=0.40, n=14)이 개활 구간(mean=0.07~
   0.08, n=20)보다 높게 나타났으나, 단일 route·소수 샘플이라 통계적
   의미를 부여하기엔 이르다(§28 -- 재현조건 확인 전 단계).

**한계**: 이 결론은 route1~4(Comma 3X, 이 특정 빌드/디바이스)에만
적용된다. 다른 GPS 소스(예: ublox)를 쓰는 디바이스라면
`horizontalAccuracy`가 정상적으로 채워질 수 있으므로, "이 필드
자체가 쓸모없다"가 아니라 "이 디바이스 조합에서는 쓸모없다"로
한정해서 읽어야 한다.

**다음 필요 작업**:
- 터널 GPS 열화 가설을 계속 추적하려면 `verticalAccuracy`/
  `speedAccuracy` 프록시를 더 넓은 corpus·더 많은 터널/개활 표본
  쌍으로 검증하거나, 다른 진단 경로(예: `positionDtSinceFix` 재검토,
  navi 앱 자체 로그)를 찾아야 함 -- 사용자 결정 필요.
- (293차부터 이월) ep108 클러스터링 코드 레벨 추적
- (294차부터 이월) ep47/48/101/105 "직선 도로 허위 candidate" 패턴
  코드 레벨 추적

**관련**: 293차(터널 가설 최초 제기 및 보류), 234차 계속5(동일한
"신규 컬럼은 패치 적용 이후 로그에만 채워짐" 제약 패턴).

---

## 294차 -- [ANALYSIS_ONLY, 코드 변경 없음] 293차 `lost_no_candidate` qcamera 대조 나머지 28건 완료 -- 전체 35건 중 과반(18건, 51%)이 화면상 직선/커브 없음으로 확인, "margin 완화 부작용" 프레임에서 "route candidate 소스 노이즈" 가설로 무게중심 이동

**배경**: 293차가 우선순위 상위 8건만 육안 대조하고 남긴 `lost_no_candidate`
나머지 28건(및 293차의 "36건" 표기와 실측 재확인 결과 "35건" 간 1건
불일치)을 전부 대조 완료.

**핵심 발견**:

1. **35건 전수 육안 대조 최종 집계**:
   - 터널 구간: 2건 (ep9, ep10 -- 293차)
   - 뚜렷한 실제 커브 확인(소실이 타당): **13건** (ep22,23,24 / 30,31,32
     / 41 / 46 / 67 / 71 / 82,83 / 91)
   - **커브 없음/직선(원인 미상): 18건(51%)** (ep12,13,14,15 / 16,17 /
     36 / 37 / 40 / 47,48 / 52,53 / 73,74,75 / 101 / 105)
   - 판단 보류(역광 등): 2건 (ep66, ep79)

2. **"직선인데 candidate 발생" 패턴의 지역적 편중**: ep47, ep48, ep101,
   ep105 네 건 모두 **도심 아파트단지 인근 직선 대로**에서 발생. 46차
   이후 산발적으로 의심되어 온 "지도 곡률 데이터 노이즈" 가설이 이번
   전수 대조로 처음 통계적 무게(18/35, 단일 최다 그룹)를 얻음.

3. **재해석**: 289차는 margin(1.1→1.05) 완화가 "continuity 소실"을
   늘릴 수 있다는 우려를 제기했고, 292/293차는 이 39건(→35건)이
   대부분 진짜 소실임을 확인했다. 이번 294차 결과는 그 "진짜 소실"의
   성격을 한 단계 더 분해한다 -- 39%(13/35)만 실제 커브 관련 소실이고,
   과반(51%, 18/35)은 애초에 화면상 커브가 없는 구간에서 candidate가
   생성되었다가 사라진 것으로 보인다. 즉 margin 파라미터 자체보다
   **route candidate 생성/클러스터링 단계에서 직선 도로 위 허위 후보를
   걸러내지 못하는 문제**가 `lost_no_candidate` 그룹의 더 큰 비중을
   차지할 가능성이 있다.

4. **미해결 -- "36건" vs "35건" 카운트 불일치**: 293차 WIP/FINDINGS는
   `lost_no_candidate`를 36건으로 기록했으나, 이번 세션에서
   `sub_cause.str.startswith('lost_no_candidate')` 필터로 직접 집계한
   결과는 35건(293차 리뷰 7건 + 294차 신규 28건)이었다. 원인 미확인 --
   추측으로 정정하지 않음(§28).

**한계**: 이 발견은 여전히 qcamera 화면 육안 대조 수준이며,
`route_find_clusters()`/navi 맵 곡률 추출 코드가 실제로 왜 직선
구간에서 후보를 만드는지는 코드 레벨로 추적하지 않았다. 표본이
"apex 소실 사례"에 국한되어 있어(생존 편향), 전체 candidate 생성 중
직선 구간 허위 후보의 비율이 얼마인지는 이 데이터만으로 알 수 없다 --
"소실된 것 중 51%가 직선"이지 "직선에서 X%가 허위 후보"라는 rate는
아니다.

**다음 필요 작업**:
- (a, 293차부터 이월) `horizontalAccuracy` 컬럼 `extract_log.py` 추가
  후 터널/직선-candidate 양쪽 재검증
- (b, 293차부터 이월) ep108(`lost_with_candidates_present`) 클러스터링
  로직 코드 레벨 추적
- (c, 신규) ep47/48/101/105 패턴을 `route_find_clusters()` 코드 레벨로
  추적 -- 직선 도로에서 candidate가 생성되는 조건 규명
- "36건 vs 35건" 카운트 불일치 원인 확인

**관련**: 293차(이 이슈의 최초 실측 확인, 8건 표본), 289차(margin
완화 what-if, 이 39건→35건 재분류의 출발점), 292차(4-way 세분류
설계), 46차(도심 직선 구간 "지도 곡률 노이즈" 최초 산발 의심 -- 이번
294차로 처음 다건(4건) 패턴 확인).
...
하강률) 기반으로 `A_CHANGE_COST`를 200→완화값으로 낮추는 게이트를 구현,
합성 시나리오 기준 부호전환 0.25s 단축/gap 13% 개선을 확인했으나 실측
검증은 route `6310bba9b8`(176차/176차계속, 직진 cruise 감속) 1건뿐이었고
그마저 폐루프 절대치 괴리로 완전히 결론짓지 못한 상태였음.

**새로운 증거(271차)**: `0000039a--7b602ffb85` seg12-16 corpus에서
177차 검증 corpus와 다른 상황(곡선 route→vturn 연속 감속, t=2117~2127,
leadStatus 전구간 False, vEgo 21→12kph)을 신규 발굴해 오픈루프(매 프레임
실측 상태 강제 리셋, 폐루프의 누적오차 문제 회피)로 재검증. acados 실솔버
사용.

**변경 이유**: 기존 176차/177차 실측은 corpus 1건(직진 감속)에 국한돼
"이 패치가 다른 상황(곡선 구간)에서도 일관되게 작동하는가"가 미확인
상태였음. 269차가 CPU 최적화 크로스리뷰 중 새 corpus에서 적합한 구간을
찾아 271차가 이어서 검증.

**새로운 결론**: 오픈루프 solver 예측(a_pred_next) vs 실측
(a_actual_next) 오차가 baseline(200 고정, 177차 이전 재현) 평균
+0.1122 m/s²(RMSE 0.1588)에서 현재 프로덕션(완화 게이트 활성) 평균
+0.0619 m/s²(RMSE 0.1369)로 개선(평균오차 약 45%, RMSE 약 14% 감소) --
**직진 감속(176차)에 이어 곡선 감속(271차)에서도 완화 게이트가 실측
감속 궤적에 더 가깝게 예측한다는 것을 독립 corpus로 재확인**. 200프레임
중 186프레임(93%)에서 baseline과 실제 다른(더 빠른) 감속 명령이
발동됨 -- 완화 게이트가 이 구간 전반에서 지속적으로 작동했음을 의미.
177차 실측검증 항목("route `6310bba9b8` 재검증" 1건)에 corpus 다양성이
추가된 것으로, 177차의 파라미터/설계 자체를 바꾸는 결론은 아님(NEEDS_
VALIDATION 상태 유지 -- 실차 미검증은 여전함, 177차가 이미 배포된
로직이라는 점은 별개).

**실차 검증**: 미실시(오프라인 재계산 한정, 177차 패치 자체는 이미
배포되어 있으나 이 검증 자체가 실차 확인은 아님).

**관련**: FINDINGS.md 174차/176차/176차 계속/177차,
`toolkit/sim_acados_causeB_270_real_replay.py`(271차 신규).
...
**[267차 추가 실측 -- CONTINUITY_MATCH_TOLERANCE_M 10m/15m 불일치 재검증]**
사용자 결정("이번엔 FINDINGS 기록만, 재검증은 보류")이 이후 뒤집혀 267차에서
사용자가 `0000039a--7b602ffb85` seg12-16 원본을 재업로드, 실측 재검증 완료.
`sim_route_260_confidence_signals.py`에 `--tolerance` CLI 플래그 추가(§27
최소변경, 하드코딩값 15.0을 오버라이드 가능하게만 확장 -- 미지정 시 기존
260차와 100% 동일 동작) 후 동일 corpus를 10.0m/15.0m 양쪽으로 재실행.

- 전체(윈도우 무관) streak 분포: 15.0m 기준 65개 track(streak=1: 36개
  55.4%, 2-5: 23개 35.4%, 6-20: 1개 1.5%, 20+: 5개 7.7%, 260차 원 수치와
  정확히 일치 -- 재현성 확인) vs 10.0m 기준 66개 track(streak=1: 36개
  54.5%, 2-5: 23개 34.8%, 6-20: 1개 1.5%, 20+: 6개 9.1%). **streak=1
  비율은 15.0m/10.0m 사이에 거의 차이 없음(55.4% vs 54.5%)** -- 260차가
  실측한 "노이즈/실커브 분리력" 결론 자체는 10m로 바꿔도 유지된다.
- 그러나 track 단위로 보면 **완전히 무관한 변화는 아니다**: 15.0m에서
  단일 track(t=2041.46~2045.41, streak=80, 하나의 지속 접근 구간)이던
  것이 10.0m에서는 **두 개의 track으로 분리**된다(t=2041.46~2042.45
  streak=21 + t=2042.50~2045.41 streak=59). 두 조각 모두 streak=21/59로
  "20+" 버킷에 들어가 버킷 분포상 눈에 띄지 않지만(20+ 5→6로만 반영),
  **분리가 일어나는 그 프레임에서는 streak가 1로 리셋**된다 -- 266차
  confidence blend를 이 지점에 그대로 적용하면, 실제로는 하나의 지속되는
  커브 접근 중간에 confidence가 순간적으로 0으로 떨어져
  `eff_apex_speed`가 잠깐 `v_ego_kph` 쪽으로 튀는(=감속 명령이 순간
  풀리는) 프레임이 발생할 수 있다는 뜻이다. 15.0m였다면 이 지점은 애초에
  분리되지 않아 streak가 계속 이어졌을 것.
- **결론**: streak *분포*(버킷 비율) 관점에서는 10m/15m 차이가 260차
  결론을 뒤집지 않지만, confidence blend가 소비하는 것은 분포가 아니라
  "이 프레임의 실제 streak 값"이므로 **10m 프로덕션 값을 쓰면 15m 분석
  스크립트가 보지 못한 종류의 중간 리셋(지속 커브 중 순간적 streak=1)이
  발생할 수 있음을 이번 실측으로 처음 확인**. 266차 patch가 이미 프로덕션
  `CONTINUITY_MATCH_TOLERANCE_M=10.0`을 그대로 사용하므로 이 patch
  자체에는 반영이 필요 없으나(분석 스크립트만 15m를 썼던 것뿐), **이번
  발견은 confidence blend의 corpus A/B 재검증(WIP 266차 다음 작업 1번)
  때 이런 중간 리셋 프레임이 실제로 몇 건 존재하고 체감 가능한
  flicker/감속 순간해제로 이어지는지를 반드시 함께 확인해야 함**을
  시사한다 -- 이 항목이 다음 corpus A/B 재검증의 체크리스트에 추가됨.
- 스크립트: `sim_route_260_confidence_signals.py --tolerance 10.0`
...
**계기**: 250차가 요청한 "234차/244차 원본 corpus(`0000039a--7b602ffb85` seg12-16) 재업로드"에 사용자가 응해, 이번 세션에 해당 route(터널 구간에 apex candidate가 실제로 존재하는 유일한 known-good 로그)가 확보됨.

**검증 방법**: `extract_log.py --with-navi-paths`로 5999행 재추출(234차/244차와 동일 행수, 재현성 확인) 후 `sim_route_234_spatial_apex_continuity.py --skip-gate`(248차 옵션)로 기본모드/skip-gate 모드 양쪽을, 전체 로그 + 3개 구간(터널 t2190-2225/IC gore t2108-2112/S커브 t2116-2122.2)에 대해 실행.

**결과(>40m 프레임간 점프, stage3 최종)**:
| 구간 | 기본모드(gate 포함) | `--skip-gate` |
|---|---|---|
| 전체(5999행) | 3건 (active 306) | **1건** (active 570) |
| 터널(t2190-2225) | 0건 | **0건** |
| IC gore(t2108-2112) | 0건 | **0건** |
| S커브(t2116-2122.2) | 3건 | **0건** |

**결론**: (1) 터널 구간은 stage2/3만으로 gate와 동등하게(0건) 억제됨 -- 247차 §11의 핵심 질문에 대한 최초의 긍정적 실측 답. (2) 245차까지 "단일-apex 트래킹 구조의 근본 한계로 완전 0 달성이 어려울 수 있는 잔여"로 판단됐던 S커브 잔여 3건(234차 계속8/9/10 기록)이 gate 제거 후 오히려 0건으로 개선됨 -- gate가 이 구간에서는 후보 선택을 오히려 불안정하게 만들었을 가능성(원인 프레임별 분석은 미실시, 다음 과제). (3) 전체 로그 기준 active frame이 gate 제거로 86% 증가(306->570)하면서도 잔여 flicker는 오히려 감소(3->1) -- gate 삭제가 "노이즈 억제"와 "정상 개입 보존" 양쪽에서 기본모드와 동등하거나 우수함을 지지.

**검증**: 오프라인 로그 재계산(기존 toolkit `sim_route_234_spatial_apex_continuity.py` 재사용, 신규 코드 없음) + 재현성 확인(행수 일치). **실차 검증: 미실시**. `ryu` 코드 변경 없음(시뮬레이션 전용 세션).

**다음 작업**:
1. S커브 구간 gate 제거 후 개선(3->0) 원인의 프레임별 분석(어떤 후보가 gate에 의해 걸러졌다가 continuity 매칭을 방해했는지).
2. 247차 검증계획 3번(통과 확인됐으므로 INERT/ACTIVE 상태머신 정식 코드 반영 착수 여부 사용자 확인) -- §31 사전승인 필요.
3. 247차 검증계획 5번(246차 원거리 apex freeze, 239차 self-elimination 케이스도 새 상태머신에서 해소되는지) 착수.

**관련**: 233차(터널 GPS 노이즈 최초 확인), 234~238차(severity gate 도입), 244차(터널 구간 CASE_B 노이즈 메커니즘 규명), 247차(§11 검증계획 수립), 248차(`--skip-gate` 옵션 구현+합성검증), 249차(비터널 실차 corpus 첫 검증), 250차(대체 터널 corpus 탐색 실패, apex 비활성 2건).
...
**14건 상세(ratio 오름차순)**:
```
file                                          t     vEgo  apex_v apex_d ratio  vTurn@ho desired@ho
dashcam_1788421451911                      152.4   17.5    5.0  290.0  0.285    19.0     20.0
20260901_105450_..._fc9313110d              1946.6  18.6    5.7   60.0  0.306   -79.0     79.0
dashcam_1788421451911                       130.2   10.5    5.0  430.0  0.477    19.0     20.0
dashcam_1788421451911                       232.7   24.7   13.0   20.0  0.525    24.0     24.0
20260903_071654_..._7e9c713c9d              289.4   14.9   10.9   20.0  0.729    22.0     22.0
20260901_060756_..._15ef45a546              800.3   50.2   46.8   50.0  0.932    72.0     72.0
20260902_060614_..._a57501e67f              238.8   49.9   47.6   80.0  0.953    84.0     84.0
20260902_181435_..._4e18e62932              680.8   44.8   43.3   60.0  0.966    56.0     56.0
20260903_071654_..._7e9c713c9d              290.4   18.1   18.0   20.0  0.990    21.0     21.0
20260903_071654_..._7e9c713c9d              268.7   15.4   26.9   30.0  1.746    29.0     29.0
20260901_103650_..._2fde2e3826             1364.1   22.2   49.4   50.0  2.226   157.0    157.0
20260901_060756_..._15ef45a546              853.1   19.1   46.6   70.0  2.444   -50.0     50.0
20260902_060614_..._a57501e67f              761.2   18.9   46.6   70.0  2.463   -51.0     51.0
20260902_073700_..._e635e188cf             1660.7    3.7   37.3  170.0 10.016    73.0     73.0
```

**vEgo로 나눠보면**:
- **vEgo<30kph 표본(n=11)**: ratio가 0.285~10.016 전체 대역에 넓게 분포. `apex_dist=290m/430m`(dashcam, vEgo=17.5/10.5kph)나 `vEgo=3.7kph`(ratio=10.016 최댓값 케이스) 등은 시내 저속 주행/교차로 회전/정체 구간으로 추정되며(§ FINDINGS 기존 `curve_exit_no_accel_scan_v2`/`steering_oscillation_detector` 항목의 "정차·저속 시내 회전 오탐" 패턴과 정황 일치), route apex 후보 자체가 하이웨이 커브 사전감속과 다른 성격(교차로 회전 curvature, 정체 중 후보 흔들림 등)일 가능성이 높다. 이 부분집합을 "route→vturn 핸드오프 설계"의 근거로 쓰기엔 부적합.
- **vEgo>=30kph 표본(n=3)**: `ratio=0.932/0.953/0.966`, median=0.953, 범위폭 0.034로 **매우 좁음**. vEgo 44.8~50.2kph, apex_speed 43.3~47.6kph 대역으로 실제 고속 주행 중 커브 접근 상황과 부합.
- 즉 240차가 관측한 "표본 14건의 큰 분산"은 **표본 자체가 이질적(저속 시내 vs 고속 하이웨이)이었던 것이 주 원인일 가능성**이 있다 -- gate 없는 구코드에서 handoff 시점이 "물리적으로 불규칙"하다기보다는, 서로 다른 두 종류의 상황을 하나로 합쳐서 본 결과일 수 있다.

**raw `vTurnSpeed_at_handoff` 부호 확인**: 14건 중 3건(위 표 `vTurn@ho`가 음수인 행)에서 handoff 직후 첫 프레임의 raw vTurnSpeed가 음수로 표시됨(-50/-79/-51). 기존 FINDINGS 항목("raw vTurnSpeed CSV 필드 특이사항", src=vturn 확정 직전/직후 프레임에서 부호가 순간 반전되는 현상, 기능 버그 아님)과 일치하는 패턴. 세 건 모두 `confirm_t`가 handoff 후 1.85~1.9초(2초 window 거의 끝)에서 별도로 수렴 조건을 만족했음을 확인 -- `confirmed=True` 판정 자체는 이 표시 프레임의 부호 반전과 무관하며 유효하다. 다만 `vTurnSpeed_at_handoff`(nxt 프레임 raw 값) 필드 자체를 단독 지표로 쓰면 오해 소지가 있어 toolkit README에 주의사항 추가.

**결론(240차 CRITICAL 결론 유지, 보강만)**:
1. 240차의 핵심 판단 -- "이 11개 로그(223차 이전, gate 없음)로는 지시서가 요구하는 gate ratio 값을 확정할 수 없다" -- 는 **변경 없음**. n=3은 어떤 값도 확정할 근거가 못 된다(§28 원칙).
2. 다만 "고속(vEgo>=30kph) 상황으로 한정하면 분산이 실제로 작아 보인다"는 것은 **주목할 만한 방향성**(가설 수준) -- 향후 223차 이후 빌드(gate 존재)로 재수집 시에도 동일하게 vEgo 대역을 나눠서 보는 것을 권장.
3. 저속(vEgo<30kph) 표본을 향후 유사 분석에서 자동으로 걸러낼지(예: `scan_route_vturn_handoff_ratio.py`에 vEgo 하한 기본값을 두는 것)는 표본이 더 쌓인 뒤 사용자와 논의.

**vEgo 저속(<30kph) 표본의 신호대기/정차 개입 확인(대화 중 사용자 지적, brakePressed/cruiseEnabled로 교차검증)**: vEgo<30kph 11건 각각에 대해 handoff 시점 ±8초 구간의 `brakePressed`/`cruiseEnabled`/`vEgo` 추이를 확인:
```
file                                          t_ho    vEgo_ho ratio  minV(±8s) brakePressed cruiseEnabled=False
dashcam_1788421451911                        152.4    17.5   0.285    5.7      True          True
20260901_105450_..._fc9313110d              1946.6    18.6   0.306    0.0      True          True
dashcam_1788421451911                        130.2    10.5   0.477   10.4      True          True
dashcam_1788421451911                        232.7    24.7   0.525   23.2      True          True
20260903_071654_..._7e9c713c9d               289.4    14.9   0.729    0.0      True          True
20260903_071654_..._7e9c713c9d               290.4    18.1   0.990    0.0      True          True
20260903_071654_..._7e9c713c9d               268.7    15.4   1.746    0.0      True          True
20260901_103650_..._2fde2e3826              1364.1    22.2   2.226   20.9      False         False
20260901_060756_..._15ef45a546               853.1    19.1   2.444   12.1      True          True
20260902_060614_..._a57501e67f               761.2    18.9   2.463    8.8      True          True
20260902_073700_..._e635e188cf              1660.7     3.7  10.016    0.0      True          True
```
**11건 중 10건**에서 handoff 전후 8초 이내 `brakePressed=True`와 `cruiseEnabled=False`(운전자 개입/해제)가 함께 관측되고, 다수는 같은 구간 최저속도가 0에 가까움(신호대기 정차로 추정) -- **저속 표본 분산이 컸던 원인이 route/vturn 로직 자체가 아니라 신호대기 등 무관한 변수(운전자 개입, 정차)와 섞여 있었기 때문일 가능성이 높다는 사용자 가설이 데이터로 확인됨**. 예외 1건(`20260901_103650`, t=1364.1, vEgo=22.2kph, ratio=2.226)은 brake/cruise-off 둘 다 없어 신호대기가 아닌 다른 원인(정체 흐름 중 저속 등)으로 추정 -- 별도 확인 필요, 이번 세션에서는 원인 미규명.

**결론(보강)**: 향후 이런 handoff ratio 분석에서는 vEgo 하한 필터만으로는 부족하고, `brakePressed`/`cruiseEnabled` 등으로 "신호대기/운전자 개입 구간"을 명시적으로 배제하는 필터가 더 원인에 가까운 정제 방법일 수 있다(vEgo는 결과이자 대리 지표일 뿐). `scan_route_vturn_handoff_ratio.py`에 이 필터를 정식 반영할지는 표본이 더 쌓인 뒤(223차 이후 로그 확보 시) 판단.

**실차 검증**: 미실시(전부 오프라인 로그 재분석, 231~240차와 동일 성격).

---

## 240차 -- [CRITICAL, 실측 확정] 2026-09-05 업로드 실차 로그 11건 전부가 223차(2초 hold 신설) 이전 빌드에서 채록됨 -- "handoff ratio + 2초 hold 재검토" 지시서 4/5번은 이 로그로 검증 불가, 1~2번도 다른 성격의 표본

**[282차 갱신, 진행상황 참고]** 이 항목이 지적한 "hold 재검토" 자체는
255~280차 사이 아키텍처가 크게 바뀌면서 방치돼 있었으나, 281차가 소스
추적+합성 시뮬레이션으로 재착수 -- 266차 confidence blend 도입 이후
hold의 원 목적(노이즈 재래치 방지)이 구조적으로 중복됐다고 판단, 282차에서
사용자 확정으로 `ROUTE_RELEASE_HOLD_S`를 0.0으로 변경(실차 검증 전,
PARAMS_REGISTRY.md 참고). 이 항목이 원래 요구했던 "223차 이후 빌드로
재플래시 후 새 로그 확보"는 이번에도 여전히 미완료 -- 282차 패치가
실제로 실행된 로그가 나와야 최종 결론 가능(§29).

**배경**: 사용자 지시서(2026-09-05, "Route→Vturn Handoff Ratio + Route Release 2초 Hold 재검토")에 따라 업로드된 11개 실차 로그(`20260901_060756_00000376`~`dashcam_1788421451911`, 2026-09-01~09-03 채록)를 분석하기 전, §3 원칙(새 세션 시작 시 GitHub 최신 상태 확인)에 따라 `ryu`/`ryu-devnotes`를 재클론하고 `check_device_build.py`로 11개 로그 전부의 실제 device 빌드를 확인.

**확인된 사실**:
```
파일                                                device gitCommit   해당 차수         dirty
20260901_060756_00000376--15ef45a546_x14seg        8958189            179차 후속2       True
20260901_103650_0000037a--2fde2e3826_x18seg        6c00b9c            182차 계측        True
20260901_105450_0000037b--fc9313110d_x8seg         6c00b9c            182차 계측        True
20260902_060614_0000037e--a57501e67f_x12seg        ea97831            193차 진단        True
20260902_073700_00000380--e635e188cf_x19seg        0194815            194차             True
20260902_075600_00000381--abe1d2bb34_x6seg         0194815            194차             True
20260902_133350_00000383--3e6ec098ab_x8seg         a772453            199차             True
20260902_181435_00000387--4e18e62932_x12seg        2b44b65            207차             True
20260903_071333_00000388--9c49b5f8d5               2b44b65            207차             True
20260903_071654_00000389--7e9c713c9d_x18seg        4514e97            214차             True
dashcam_1788421451911                              7519a3a            221차             True
```
전부 `ryu` HEAD(`fc98eaa`, 237차)의 조상 커밋이며(`git merge-base --is-ancestor` 확인), **223차(`ee1f5f8`, "Route 감속 로직 전면 재설계: 무상태 감속식 + route_active 상태기계") 이전** 빌드다. `git log -S"ROUTE_RELEASE_HOLD_S"`로 확인한 결과 **`ROUTE_RELEASE_HOLD_S`(2초 hold) 상수 자체가 223차에서 신설**됐으므로, 이번 11개 로그 전부는 **이 코드 경로가 아예 존재하지 않는 빌드**에서 채록됐다. 또한 전부 `dirty=True`(빌드 시점 워킹트리에 미커밋 로컬 변경 존재)라 gitCommit 해시로 확인되는 커밋조차 실제 실행 코드와 100% 일치한다고 단정할 수 없다(178차 선례와 동일 성격의 불확실성, `check_device_build.py` 주석 참고).

**영향**:
1. **지시서 §4/§5(2초 hold 검증)는 이 11개 로그로 원천적으로 검증 불가.** hold 로직 자체가 코드에 없으므로 "hold 중 candidate가 같은 커브 재검출인지 다음 커브인지" 같은 질문 자체가 성립하지 않는다.
2. **지시서 §1~§3(handoff ratio)도 234~239차가 논의해온 것과는 다른 성격의 표본.** 234~239차의 severity gate(`ROUTE_SEVERITY_GATE_RATIO`, 234차 도입)도 223차 이후에 추가된 것이라 이번 로그들엔 없다. 즉 여기서 관측되는 "route→vturn 전환"은 gate로 인한 조기소거가 섞이지 않은, gate 없는 구코드에서의 자연 전환(주로 apex 도달 릴리즈로 추정)이며, 239차가 정의하려는 "self-elimination을 의도된 핸드오프로 재해석했을 때의 ratio"와는 다른 물리적 현상을 보고 있는 것이다.
3. `vturn_speed()` 핵심 물리(82차 이후 불변, 239차 확정)는 이번 로그 전부(2026-09-01~03, 82차 이후)에 여전히 적용 가능하므로 vTurnSpeed 실측치 자체의 신뢰도는 유지된다. 문제는 route 쪽 아키텍처(상태기계/gate 유무) 쪽이다.

**시도한 예비 분석(신규 toolkit `scan_route_vturn_handoff_ratio.py`, 참고용, 결론 근거로 사용 금지)**: stage0(게이트 없음, 각 로그의 실제 배포 코드와 동일 방식)로 naviPaths 재구성 후 `src` route→vturn 직접전환 + 2초 내 vTurnSpeed 수렴("confirmed") 기준으로 11개 로그 전체 스캔 -- 145건 후보 중 confirmed 44건, 그 중 apex_dist>15m(순수 사전감속으로 추정) 14건. ratio(confirmed, far-from-apex) 분포: median=0.960, P25=0.576, P75=2.106, min=0.285, max=10.016 -- **표본 14건에 분산이 매우 커서(0.285~10.0) 지시서가 요구하는 "적정 gate ratio 제안"의 근거로 쓸 수 없음.** 이 넓은 분산 자체가 "gate 없는 구코드에서는 route→vturn 전환 시점이 물리적으로 일정하지 않다"는 정황일 수 있으나, 표본 부족과 아키텍처 불일치가 섞여 있어 원인 분리 불가.

**결론**: §33("GitHub 상태가 예상과 다를 경우... 임의로 진행하지 않는다") 적용 -- 지시서 원안(가설 0.90 등 gate ratio 확정, 2초 hold A/B/C안 비교)을 이번 로그로 완결하지 않고, 사용자에게 보고 후 다음 중 하나를 확인받아야 함: (a) 223차 이후(가급적 현재 HEAD `fc98eaa`=237차, `dirty=False`) 빌드로 재플래시 후 새로 촬영한 로그로 재분석, (b) 이번 구코드 로그를 "gate 없는 자연 전환 베이스라인" 참고자료로만 활용하고 본 지시서 결론은 보류, (c) 다른 이미 존재하는 최신 빌드 로그가 있다면 그것으로 우선 재분석.

**실차 검증**: 미실시(이 항목 자체가 "검증 불가 상태 확인"이 결과물 -- 사용자 확인 대기).

---

## 239차 -- [CRITICAL, 코드 확인+시뮬레이션 확정, 실차 미검증] 237차 severity gate(`ROUTE_SEVERITY_GATE_RATIO=0.70`)가 "자기소거(self-elimination) 진동" 구조적 결함을 가짐 -- 237차 patch 실차 적용 보류 권고

**기존 결론(237/238차)**: candidate 레벨(apex_dist 점프 172->47건) 및 out_speed 레벨(overshoot 1건->0건) 모두 개선되어 POSITIVE로 판정, 실차 적용 대기 상태였음.

**새로운 증거**: `carrot_man.py`(237차 HEAD, `fc98eaa`) L888-891의 게이트 조건이 매 프레임 **라이브(현재) vEgo**를 기준으로 재평가됨을 코드로 확인:
```python
v_ego_kph = v_ego_ms * 3.6   # self.sm['carState'].vEgo, 매 프레임 최신값
candidates = [k for k in candidates if speeds[k] < max(v_ego_kph, 1.0) * ROUTE_SEVERITY_GATE_RATIO]
```
사용자 지적으로 발견: target(=apex_speed) 고정, vEgo가 target을 향해 감속 중이라면, 게이트 통과 조건 `target < 0.70*v_ego`는 `v_ego > target/0.70`(≈target×1.4286)일 때만 성립한다. 즉 **route 자신이 유발한 감속이 진행되어 vEgo가 이 경계값 아래로 내려가는 순간, 그 감속을 유발한 candidate 자신이 게이트에서 탈락**한다 -- 감속이 성공할수록 자기 존재 근거가 사라지는 구조.

**시뮬레이션(toolkit `RouteSim223` + 실제 게이트 수식 재현, 자연스러운 감속/재가속 궤적 대입, 세부 가정치는 §28에 따라 확정적 부분과 분리)**:
- vEgo=105, target=70 고정 궤적 대입 -> **v_ego=100.18kph(정확히 target/0.70 근방)에서 즉시 RELEASE** 발생(확정적, 실코드 그대로).
- release 후 재가속(vCruise 방향, 가정치 1.2m/s²) 반영 시, ENGAGE(재감속)->자기소거->RELEASE->재가속을 반복하는 **리밋사이클 진동**이 나타나며, 평형속도가 목표(70kph)가 아니라 `target/0.70`(≈100kph) 근방에 형성됨 -- 시뮬레이션 트레이스 1건에서 커브 실제 도달 시점(apex_dist=0)까지도 vEgo=102.2kph로 목표 대비 +32kph 초과 상태였음.
- ENGAGE/RELEASE가 `v_ego≈target/0.70` 근방에서 반복되는 메커니즘 자체는 게이트 수식에서 직접 도출되는 확정적 성질. 다만 이 진동이 실제로 apex 도달 시점까지 목표속도 미달을 초래하는 "정도"는 재가속률/`ctrl_end_s` 등 가정치에 의존적이라 실차와 정량적으로 동일하다고 단정할 수 없음(RouteSim223 단순화 한계, 238차 문서 한계3번 동일).

**영향 재해석**: 238차가 "POSITIVE"로 판정했던 active-frame 82%감소(2383->430)가, 실제로는 노이즈 억제가 아니라 이 자기소거 구조로 인한 **정상적으로 필요한 감속 개입의 조기 중단**을 상당 부분 포함하고 있을 가능성. 238차 WIP/README 결론 자체가 이 재해석으로 정정 필요.
...
**253차 계속 갱신(run-length 수치 재검증, 부분)**: 253차 세션 전반부(컨테이너 초기화로 유실)에서 발견됐던 route-active run 개수 이상치("실측 61건 vs 시뮬 163건", 당시 스크립트는 RELEASE-hold 버그가 있던 판)를, 사용자가 재업로드한 `dashcam_1788583013065.zip`(route_ac 세그0-15만 유효 -- 세그16 rlog.zst 0바이트 손상, route_ad는 이번 업로드에 미포함, 최초 유실분과 완전히 동일한 데이터는 아님)로 재추출한 CSV(19229행) 위에서, RELEASE-hold 수정본(`sim_route_252_active_state_full.py`)을 재사용하는 신규 스크립트 `analyze_route_253_active_run_length.py`로 재실행. 결과: **실측 68건 vs 시뮬 51건**(short-run<2s: 49건 vs 35건) -- 수정 전 관측됐던 "시뮬이 실측의 약 2.7배" 과다분절 현상이, 수정 후에는 재현되지 않고 오히려 방향이 역전(시뮬이 실측보다 적게 분절)됐다. 이는 최초 관측된 이상 분절이 새 상태머신 자체의 잔존 결함이 아니라 RELEASE-hold 누락이라는 스크립트 버그의 인공물이었다는 가설을 뒷받침하는 정황 증거다. **단, 다음 두 가지로 이 결론을 "239차 완전 종결"로 승격하지 않는다(§28)**: (1) 이번 corpus는 세그 구성이 원본 유실분과 다르고 route_ad가 빠져 있어 동일 데이터 기준 A/B 비교가 아니다 -- 재추출된 실측 68건 자체가 이전 61건과 다른 이유가 스크립트 수정 때문인지 데이터가 달라졌기 때문인지 완전히 분리되지 않는다. (2) 이 corpus는 vEgo 최대 61.9kph로, 239차 원 CRITICAL의 실제 재현 조건(vEgo≈105kph/target≈70kph급 고속 커브 접근, 위 시뮬레이션 절 참고)을 한 프레임도 포함하지 않는다 -- 즉 이번 재검증은 "일반적인 저~중속 주행에서 새 상태머신이 인위적으로 과다분절하지 않는다"는 것만 확인했지, 원 CRITICAL이 지목한 고속 시나리오 자체의 실측 반증은 여전히 미실시다. run 정의 자체의 한계(실측 `src`는 전체 arbitration 승자 소스, 시뮬 `sim_src`는 route 후보 유무만 반영 -- 완전히 동일한 신호가 아님)는 스크립트 docstring에 명시. 실차 검증: 미실시.

**255차 갱신(전체 25세그 원본으로 재검증 -- run-length 방향 재역전, freeze는 일관)**: 사용자가 동일 파일명 `dashcam_1788583013065.zip`을 재업로드했고, 이번엔 253차/253차계속이 요청했던 **완전한 원본 25세그(route_ac 세그0-19+route_ad 세그0-4)가 전부 무결 상태**로 포함됨(rlog.zst 전부 0바이트 아님). `extract_log.py`로 재추출한 CSV가 29126행(route_ac 24030행+route_ad 5096행)으로 253차 WIP가 기록했던 원본 수치와 정확히 일치 -- 원본과 동일한 데이터임을 확인.

이 corpus로 `check_device_build.py` 실행 결과, 실제 채록 device 빌드는 커밋 `a70deea3`(**243차**, dirty=True)로 확인됨. `git show a70deea3:selfdrive/carrot/carrot_man.py`로 직접 확인한 결과 이 빌드는 `ROUTE_SEVERITY_GATE_RATIO = 0.90`(242차가 본 CRITICAL 완화 목적으로 0.70->0.90 상향한 버전)이 **여전히 코드에 존재**한다 -- 즉 이 corpus 전체의 실측 `src`/`routeOutSpeed`는 247/251/252차가 게이트를 완전히 삭제하기 이전 코드가 만든 결과이며, 아래 비교들은 전부 "243차 실측(0.90 게이트 존재) vs 252차 시뮬(게이트 완전 삭제)" 구도다(기존 246차/253차 비교들도 동일 구도였으나 device 빌드를 명시적으로 확인한 것은 이번이 처음).

1. **`analyze_route_253_active_run_length.py` 재실행(전체 25세그)**: **실측 96건 vs 시뮬 113건**(short-run<2s: 71건 vs 90건). 253차계속의 부분-corpus 결과(실측 68 vs 시뮬 51, 시뮬<실측)와 **방향이 다시 역전**(이번엔 시뮬>실측, 즉 원래 253차 전반부가 관측했던 방향과 유사) -- 253차계속이 "버그 인공물 가설을 뒷받침"이라 평가했던 결론은 완전한 원본 corpus로는 재현되지 않는다. 이 지표(run 개수)가 corpus 구성(세그 범위)에 매우 민감하다는 것이 이번에 확인된 사실이며, run 정의 차이(실측 `src`=전체 arbitration 결과 vs 시뮬 `sim_src`=route 후보 유무만 반영, 완전히 동일한 신호가 아님)가 근본 원인일 가능성이 여전히 유력하다. **§28 원칙상 이 수치만으로 "새 상태머신이 과다/과소 분절한다"는 결론은 내리지 않으며, 253차계속의 "인공물 가설 뒷받침" 평가도 이번 결과로 취소한다(기존 항목은 삭제하지 않고 이 갱신으로 대체 해석만 추가).**
2. **246차 freeze 재확인(`sim_route_252_active_state_full.py`, 변경 없음)**: 실측 12건 -> 시뮬 0건 -- 기존 결론(9->0, 1->0)과 일관, 표본만 증가. 이 지표는 corpus 확장에도 방향이 안정적이었다.
3. **239차 원 CRITICAL 고속 재현조건(vEgo≈105/target≈70kph) 재스캔**: route_ad 포함해도 corpus 최대 vEgo=61.9kph로 여전히 미포함 확인(253차계속과 동일 결론).
4. **핸드오프 ratio 신규 스캔 시도(`scan_route_vturn_handoff_ratio.py`)**: raw 후보 4건, confirmed 1건(ratio=8.15). JSON 상세 확인 결과 4건 중 3건이 `v_ego_kph` 5.5~18.2kph(저속 구간, 고속 커브 접근 시나리오 아님)이고 이 중 2건은 `vTurnSpeed_at_handoff` 원시값이 **음수**(-81, -94)로 세션~222에 플래그된 "vturn 원시값 순간 불안정" 스레드와 유사한 패턴으로 보임. ratio 분포(0.27~8.45)도 기존 239차 예비분석 표본(0.617/0.854/1.002)과 판이하게 달라 **이번 4건은 N값 추정 표본으로 채택하지 않음** -- 표본 오염 의심이 해소되기 전까지 handoff ratio 누적 목록에 합산 보류.

**실차 검증**: 미실시(전부 open-loop 재생/오프라인 분석, §29).

---

## 239차 -- [설계 방향 합의, 실측 근거 아직 불충분] "route=사전감속 전용, vturn=커브 실주행" 역할 분리 설계안 -- ratio는 실측 route→vturn 핸드오프 사례로 결정하기로 함

**배경**: 위 자기소거 문제를 사용자가 오히려 "route→vturn 핸드오프 장치"로 재해석 -- 0.70(또는 다른 값)의 게이트를 "결함"이 아니라 "route가 목표속도의 N%까지 사전감속시킨 뒤 vturn에게 넘기는 의도된 핸드오프 트리거"로 재정의하자는 설계 방향 제안. §27 최소변경 원칙에 부합(기존 자기소거 메커니즘을 뒤집지 않고 재해석/활용).

**N값 결정 방법(사용자 제안, 채택)**: 임의로 0.90 등을 정하지 않고, 실차 로그에서 `src`가 `route`->`vturn`으로 전환되는 사례를 모아 그 순간 `routeApexSpeed/vEgo` 비율의 분포를 실측으로 산출.

**예비 분석(seg12-16, `route_v6.csv`, 1개 로그)**:
- route->vturn 전환 raw 25건 -> 8초 이내 재전환을 같은 커브로 병합 -> 6개 에피소드(vEgo<10kph 1건 제외, 5개 유효)
- 핸드오프 후 2초간 vTurnSpeed가 apex_speed±15kph 이내로 수렴하는 "만족스러운 핸드오프"만 필터링 -> **3건**: ratio=0.617 / 0.854 / 1.002
- **결론: 표본 3건으로는 어떤 숫자도 정당화 불가.** 추가로, 현재 device엔 severity gate가 없어 이 로그의 route->vturn 전환은 "ratio 도달"이 아니라 apex_dist=0(도달) 또는 candidate 소실(도로제한속도 변경 등 무관한 원인)로 발생 -- 즉 이 데이터는 "route가 어떤 ratio에서 자연스럽게 넘기는가"를 아직 깨끗하게 측정하지 못함(원인이 뒤섞여 있음). 나머지 2건(비수렴)은 vTurnSpeed 원시값 순간 급등 문제(46/50차, f94a7d2)와 겹쳐 보여 별도 필터 없이는 ratio 추정에 노이즈로 작용.

**vturn 로직 호환 로그 기준 확정**: git log 전수 확인 결과 `vturn_speed()` 핵심 물리 로직(safe_time, lookahead horizon, TARGET_LAT_A, 대칭버퍼)은 **82차(2026-08-26, `451a3b9`) 이후 변경 없음**. arbitration 참여/트레일링 판정도 **`f94a7d2`(2026-08-23) 이후 변경 없음**. 따라서 **2026-08-26 이후 촬영된 로그**는 지금과 동일한 vturn 로직으로 간주 가능(그 이전은 lookahead/safe_time이 달라 혼합 금지). route 쪽 apex_speed는 naviPaths 원시데이터에서 `recompute_route_curvature_speed()`로 오프라인 재구성 가능해 route 버전 차이 영향을 덜 받음(238차와 동일 방식) -- 단 vTurnSpeed 수렴 확인은 원시 텔레메트리에 의존하므로 82차 컷오프가 실질적 기준.

**다음 작업**: 사용자가 2026-08-26 이후 촬영된, 커브가 많이 포함된 긴 로그(국도 굴곡/IC·JC/고속도로 커브 구간)를 추가로 업로드하면, 재사용 가능한 toolkit 스크립트(`scan_route_vturn_handoff_ratio.py`, 아직 미작성 -- 다음 세션에서 seg12-16 로직을 일반화해서 작성 예정)로 에피소드-ratio 분포를 축적. 표본이 쌓이면 (a) 자기소거 버그 자체의 구조적 수정(latch 방식) 여부와 (b) 데이터 기반 ratio 확정을 함께 진행.

**실차 검증**: 미실시.

---

## 236차 -- [VALIDATED via 실측 telemetry] 2단 굴곡 apex 후보 점프는 실제 제어 출력에 무해 -- "1차 곡선 사전감속 충분성"이 핵심 지표라는 사용자 재프레이밍 확인

**배경**: 235차가 "가까운 완만곡선↔먼 급곡선 apex 점프"의 물리적 실재를 확정한 뒤, 사용자가 이 점프 자체를 안정화하는 다중 apex 트래킹 재설계는 우선순위가 낮다고 판단 -- "1차 곡선에서 route 사전감속이 충분히 걸렸다면, vEgo가 이미 낮아진 채로 2차 급곡선에 진입하므로 별도 재감속이 불필요하다"는 방향 제시.

**검증 방법**: 235차와 동일 로그(`route_v4.csv`, `seg12-16`)의 t=2114~2135 구간 원시 telemetry(`vEgo`/`aEgo`/`brakePressed`/`src`/`desiredSpeed`/`routeOutSpeed`/`routeApexIdx`/`routeApexDist`/`routeApexSpeed`/`vTurnSpeed`)를 프레임 단위로 전수 대조(재구성 시뮬레이션 아닌 실제 published 값).

**결과(POSITIVE)**:
1. 1차 곡선(t=2116.05~2122.10): route가 `apexIdx=15→1`(150m→10m)로 정상 카운트다운하며 vEgo를 75.0→62.3kph로 실제 하강시킴 -- 사전감속 정상 작동.
2. 235차가 지목한 잔여 점프 3건(t=2120.20/2120.46/2120.75)이 이 1차 곡선 추적 구간 안에 있음에도, `desiredSpeed`/`routeOutSpeed` 출력은 프레임마다 단조 감소로 전혀 끊기지 않음 -- apex 후보 선택 단계의 순간 노이즈가 실제 제어 출력까지 전파되지 않음을 확인.
3. 갭 구간(vturn 단독 제어)에서도 vEgo가 62.3→45.6kph로 매끄럽게 추가 감속.
4. 2차 급곡선 진입 시점(route 재포착, `apexDist=0`, target=49kph)에 **vEgo가 이미 45.6kph로 target 이하** -- 별도 재감속 불필요, 실제로도 route가 무리하게 추가 개입하지 않고 vturn과 함께 자연스럽게 처리.

**결론**: 이 로그 기준으로 사용자 재프레이밍이 정확히 성립. 다중 apex 트래킹 재설계는 불필요 판단, 작업 중단. 대신 "route가 apex를 처음 포착한 시점 vEgo"와 "apex 도달(RELEASE) 시점 vEgo/apex_speed 차이"를 자동 계측하는 것이 핵심 검증 지표가 되어야 함(신규 toolkit 필요 여부는 사용자 결정 대기).

**한계**: 검증 로그 1건(seg12-16)뿐 -- 다른 곡선 조합/로그에서도 "1차 사전감속 충분 → 2차 무해" 패턴이 일반적으로 성립하는지 미확인. apex_idx 노이즈를 흡수하는 정확한 코드 경로(램프리미터 등)는 이번 세션에서 코드 레벨로 확인 안 함(§28 호출흐름 추적 미실시, telemetry 대조로만 확인).

**상태**: VALIDATED(이 로그에 한해) -- ryu 코드 변경 없음, 다중 apex 트래킹 재설계 중단 결정. 다음 방향(자동계측 toolkit/추가 로그 검증/현재 설계로 patch 진행) 사용자 결정 대기.

**상세**: WIP.md "236차" 항목 참고.

---
## 235차 -- [ANALYSIS_ONLY-NEEDS_USER_DECISION] S커브 구간(t=2116~2122.2) 잔여 stage3 점프 3건, 원인은 GPS노이즈가 아니라 실제 2단 굴곡(가까운 완만커브+먼 급커브) 병존

**대상**: 234차가 발견한 severity gate(v_ego*0.70)+spatial cluster+apex
continuity 4단계 A/B 시뮬레이션(`sim_route_234_spatial_apex_continuity.py`)에서
S커브 구간(t=2116~2122.2)에 남은 stage3(+continuity) 잔여 점프 3건
(t=2120.20/2120.46/2120.75)의 dashcam+곡률배열 대조.

**관측**: 233차가 확정한 "터널 구간 점프=GPS/positionDtSinceFix 노이즈
아티팩트"와 이번 3건은 시각적 증상(근/원거리 후보 교대)은 비슷하나
근본 원인이 다름. 세 시각 전부에서 naviPaths 곡률 재구성 배열(10m
그리드)이 동일 패턴: 근거리(70~90m)에 완만한 국소최소(45~50kph대),
원거리(170~190m)에 더 급한 국소최소(31~38kph대)가 별도로 존재하고
그 사이(100~160m)는 다시 45~52kph대로 완화됨. dashcam도 t=2121.5부터
우회전 경고 쉐브론 표지판이 나타나 카메라 시야 밖(전방 150~200m)에
별도의 더 급한 커브가 실존함을 뒷받침.

**원인**: 단일 apex가 아니라 그리드 위에 물리적으로 구분되는 2개의
굴곡(가까운 완만 커브 + 먼 급커브)이 동시에 severity gate 통과 후보로
잡힘. vEgo 하강에 따라 v_ego*0.70 임계값이 프레임마다 미세하게
흔들리면서 "어느 굴곡이 먼저 게이트를 통과하는 후보인가"가 근/원거리
사이에서 교대로 바뀜. stage2(spatial cluster)는 두 굴곡을 별개
클러스터로 정확히 구분하지만, stage3(continuity) 예측매칭이 굴곡
전환 시점마다 실패(miss_frames 초과)하며 재탐색이 반대쪽 굴곡으로 튐.

**234차/233차와의 관계(§24, 기존 결론 유지·보강)**: 234차 계속7이
"IC gore/S커브"로 명명한 것이 은유가 아니라 문자 그대로 맞았음을
이번 대조로 확인. 233차 터널 구간 점프(GPS 노이즈 아티팩트, positionDtSinceFix
톱니파 상관)와는 근본적으로 다른 성격 -- 이 3건은 노이즈가 아니라
"실제로 존재하는 두 번째 커브를 어느 시점에 후보로 인정할지"의
구조적 모호성. 234차 stage0~3 점프 감소 수치(전체 172/60/16/3 등)는
그대로 유효 -- 이 발견은 잔여 3건의 "정체"를 규명한 것으로 기존
수치를 수정하지 않음.

**한계(NEEDS_VALIDATION로 분류한 이유)**:
1. 원거리 급커브(170~190m)가 이 로그 뒷부분(t>2122)에서 실제 vEgo
   급감속/조향각 변화로 이어지는지 대조 안 함.
2. 검증 로그가 1건(seg12-16)뿐 -- 다른 winding road 로그에서도 동일
   패턴(근거리 완만+원거리 급커브 병존)이 재현되는지 미확인.
3. 단일-apex 트래킹 구조의 구조적 한계인지, 아니면 clustering/continuity
   파라미터 튜닝(gap_m, tolerance_m 등)으로 추가 억제 가능한지는
   미검증 -- 다중 apex 재설계 없이 파라미터만으로 해소 가능성 남아있음.

**상태**: ANALYSIS_ONLY-NEEDS_USER_DECISION -- ryu 코드 변경 없음. 다음
방향(다중 apex 트래킹 재설계 착수 여부, 또는 현재 설계를 구조적 한계로
수용하고 그대로 patch 진행) 사용자 결정 대기.

**상세**: WIP.md "235차" 항목 참고.
...
최초 실차로그 기반 검증. naviPaths 재파싱 없이, 223차가 curve 후보 선택
로직(`candidates[0]`, 179/196차 방식)을 그대로 재사용했다는 점을 이용해
222차가 이미 추출해 둔 로그의 `routeApexIdx/routeApexDist/routeApexSpeed`
(193/194차 계측, 실측 grount truth)를 새 상태기계에 그대로 입력으로 재사용.
신규 `toolkit/replay_route_223_vs_baseline.py`로 재생.

**입력**: 사용자 재업로드 17세그 로그(`0000038c--2cbdaca9d2`, 2026-09-03
16:20~16:37, 20,399행) — 222차가 이미 분석했던 바로 그 드라이브.
`check_device_build.py`로 재확인한 실제 온디바이스 gitCommit은 `7519a3a`
(221차)와 일치하나 **dirty=True**(빌드 시점 커밋 안 된 로컬 변경 존재,
178차와 동일 성격의 한계 — 223차 계속4 WIP "다음 작업 3"이 언급한 디바이스
로컬 dirty 상태와 관련 가능성, 코드 자체가 무엇이 달랐는지는 미확인).
`turnSpeedControlMode`는 로그에 프레임별로 기록되지 않아 전 구간 mode
on(2/3)으로 가정(스크립트 한계 1번), `autoNaviSpeedDecelRate=1.00`/
`autoNaviSpeedCtrlEnd=7.0`(222차 시점 추정값)을 가정치로 사용.

**핵심 결과 1 (CONFIRMED) — 222차 원 버그 재현 안 됨**: 222차가 실측한
"정지→재출발 전이구간에서 liveRouteSpeed가 vEgo를 최대 55kph 초과, 8초+
고정 유지" 패턴은 223차 오프라인 재계산에서 재현되지 않음. 해당 위치들에서
apex_dist가 10m 이하로 좁혀지는 즉시 RELEASE(`out=None`)되어 구코드처럼
장시간 고정되는 현상이 사라짐 — 설계 의도(apex 도달 시 즉시 원복)대로 작동.

**핵심 결과 2 (신규 발견, NEEDS_INVESTIGATION) — apex 진입 "전"에 정지하면
RELEASE 자체가 걸리지 않음**: t=434.2~519.7(80.8초) 구간, 실측 vEgo가
0~2kph(정지)인데 apex_dist가 **40.0m로 고정**(차량이 멈춰있으니 물리적으로
당연 — 곡선 진입점까지 40m 남은 채 정지). `ROUTE_APEX_REACHED_DIST_M=10.0`
조건이 걸리지 않아 `route_active=True`가 정지 내내 유지되고, `out_speed`는
apex_speed(≈45.2~47.3kph)를 그대로 계속 발행(80.8초간 44.996~47.270kph).
같은 구간 구코드 실측(`liveRouteSpeed`)은 20.0~34.8kph로 훨씬 낮게 유지됐음
— 즉 **223차가 훨씬 더 오래(80초+, 222차 최악 사례 94초와 비슷하거나 그
이상 규모) 정지 중에도 route 상한이 실제 속도(0)와 무관하게 커브 안전속도
근처로 열려 있는 시간대**가 여전히 존재. 성격은 222차와 다름(222차는 ceiling
+ 램프리미터 lo 하한이 원인, 이번은 RELEASE 조건이 "apex 도달"만 보고
"정지 여부"를 보지 않음이 원인) — **out_speed는 상한값이라 이 자체가 가속
명령을 만들지는 않으나(arbitration min() 구조), 정지→재출발 순간 route가
실질적 브레이크 소스 역할을 못 하는 시간대가 재설계 후에도 남아있다는 뜻**.
사용자 설계문서 §10/§11 의도(apex "도달"=hold 대상)가 "정지"까지는
다루지 않은 것으로 보임 — 사용자 확인 필요(§33, 판단 보류).

**핵심 결과 3 (신규 발견, NEEDS_INVESTIGATION) — apexIdx flicker(219/220차
미해결) 노이즈가 무상태 설계에서 그대로 출력에 노출됨**: t=649~656(고속
순항, vEgo≈75~86kph) 구간에서 apex_dist가 350~500m 사이를 매 프레임(0.05초)
흔들리고(219/220차가 이미 확정한 "리샘플 그리드 재앵커링" 원인과 동일
메커니즘) apex_speed도 84~100kph 사이로 요동 — 223차 `new_out_speed`가
매 프레임 85→99→86→97kph처럼 들쭉날쭉 출력됨(전체 로그에서 5kph+ 튀었다가
3kph+ 복귀하는 단발 패턴 103건). 같은 구간 구코드 `liveRouteSpeed`는
85.1→86.5로 완전히 매끈 — 132/172/173차 프레임간 램프리미터가 이 노이즈를
전량 흡수하고 있었음을 이번에 실측으로 재확인. 223차는 설계상 "무상태"로
램프를 전량 삭제했으므로 이 흡수 기능도 함께 사라짐. **다행히 이번
드라이브에서는 노이즈가 vEgo보다 낮은 쪽(=실제 감속 명령)으로 단발성으로
튀는 사례가 0건**(전수조사, `new_out_speed < vEgo-1kph`이면서 앞뒤 프레임은
아닌 "고립된 감속 프레임" 탐지 0건) — 이번 로그 기준으로는 급제동을
유발하지는 않았으나, apex_speed가 road_limit_speed 근처(노이즈가 실제
vEgo보다 낮아지는) 값으로 나오는 다른 도로/구간에서는 다를 수 있음
(참고: 219/220차 apexIdx flicker 게이트 재설계 자체는 별개로 여전히 미결).

**한계**: mode 프레임별 미기록(전 구간 on 가정), decel_rate/ctrl_end
가정치, 디바이스 dirty=True. 결과 2/3은 실차 재현이 아니라 오프라인
재계산이므로 실제 arbitration/제어 영향까지는 이 검증만으로 확정 불가.

**다음 작업**: (1) 결과 2 — "정지 중 RELEASE" 조건 추가 여부 사용자 결정
필요(예: vEgo<=일정 임계값이면 apex_dist 무관 즉시 RELEASE), (2) 결과 3 —
219/220차 apexIdx flicker 게이트 재설계를 이번 발견과 묶어 우선순위 상향
검토, (3) 실차 검증은 여전히 미실시 — 위 2건 모두 사용자 판단 후 처리.

**도구**: `toolkit/replay_route_223_vs_baseline.py`(224차 신규, README/
CHANGELOG 갱신 필요).

**상세**: `WIP.md` "224차" 항목 참고.
...
못 잡는다"는 결론을 바탕으로, 중단된 이전 세션이 작성해둔 `carrot_man.py`
패치안(apex_idx debounce(A) + rolling-max 게이트(B))을 검증.

**방법**: (1) `verify_220_gate_regression.py`(이전 세션 작성)의 합성 시나리오
A/B/C/D 실행. (2) 219차와 동일한 `x18seg_215cha_route.csv`(commit `4514e97`)의
`naviPaths`/`routeApexIdx`/`routeApexDist`/`routeApexSpeed`/`liveRouteSpeed`/
`vEgo`/`aEgo`/`brakePressed`를 t=990~1046 구간에서 프레임 단위로 직접 재생.

**핵심 결론 1 (rolling-max(B) 회귀)**: 합성 시나리오 A(완만한 굽이길, 불연속
없음)에서 신규 게이트가 212/400(53%) 프레임에서 오무장(OLD는 0/400). 실차
로그에서도 동일 현상 확인 — t=990~1004 정상 접근 구간에서 **같은
`routeApexIdx`를 유지한 채로** `routeApexSpeed`가 ±15~30kph 오르내림(리샘플
그리드 재앵커링에 의한 정상 곡률 잡음, idx flicker 아님). rolling-max(1.0초
누적낙차) 방식은 "정상적으로 완만하게 계속 내려가는 굽이길"과 "계단식 불연속
하강"을 구조적으로 구분하지 못함 — 두 경우 모두 "1초 내 누적낙차 초과"라는
동일한 시그니처를 남기기 때문. 임계값을 25kph 이상으로 올리면 합성 A/C는
분리되나(A 최대 24.0 vs C 최대 73.5kph) 마진이 얇고 실차 곡률 변동폭에 좌우돼
근본해결로 보기 어려움.

**핵심 결론 2 (apex_idx debounce(A)만으로는 199차 게이트가 작동하지 않음, 단
이유가 "실패"가 아니라 "불필요해짐")**: 219차가 실측한 t=1005.4~1013 flicker
원시 시퀀스(`routeApexIdx` 0→18→3→17→2→16→1→21→20→19… 요동)에 (A)만(B
미적용) 적용해 재생한 결과, debounce를 통과한 안정화 시퀀스는 프레임당 낙차가
15kph를 한 번도 넘지 않음(chaos 구간 흡수 후 104→42kph가 약 8초에 걸쳐
매끄럽게 하강) → OLD(199차, 미변경) 게이트 적용해도 armed 0회. "debounce가
OLD 게이트를 작동시켜준다"가 아니라 **"debounce가 타겟 자체를 매끈하게 만들어
게이트가 감지할 불연속 자체가 사라진다"**가 정확한 해석.

**핵심 결론 3 (가장 중요, 219차 결론 자체에 대한 반례 — 미확정)**: 위 t=1005~
1025 구간의 실제 주행 결과를 대조: `brakePressed=False`(전 구간),
`aEgo` 최대 약 -1.8 m/s²(급제동과 거리가 먼 수준), `liveRouteSpeed`(boost
미장착 상태의 실제 출력)가 104→57kph까지 이미 매끈한 계단식(132차 "route
카운트다운")으로 하강하며 vEgo를 앞서 유도. **운전자 개입/급제동 없음.**
219차가 "게이트 감지 사각지대(문제)"로 확정한 이 사례가, 실제로는 boost 없이도
문제없이 지나간 구간이었을 가능성 — 219차의 "armed=0회=문제"라는 결론은 게이트
작동 여부만 확인했을 뿐 실제 주행 결과(급제동/aEgo)와는 대조되지 않았었다.

**219차 항목과의 관계**: 219차의 "게이트가 계단식 하강을 구조적으로 못 잡는다"는
관찰(사실관계) 자체는 이 세션에서도 재확인되어 유효하다. 다만 "그것이 실제
문제(boost가 필요했던 상황)였다"는 219차의 해석은 이 세션에서 실측된 aEgo/
brakePressed 데이터로 아직 뒷받침되지 않으며, 재검토가 필요하다 — 219차 항목을
삭제/수정하지 않고 이 항목으로 보강한다(§24).

**상태**: ANALYSIS_ONLY-NEEDS_USER_DECISION — ryu 소스 코드 변경/패치 확정
없음(이전 세션이 작성한 patch는 로컬에만 존재, push 안 됨). 사용자가 다음 3안
중 "보류"를 선택 — 방향 결정 대기:
1. 219차 결론 재검토(aEgo/brake 기준으로 진짜 문제 구간이었는지 먼저 확인)
2. 18세그 CSV 전체에서 실제 급제동(`brakePressed=True`/aEgo 큰 음수) 구간을
   찾아 그것으로 debounce-only 가설 재검증
3. (선택됨) 보류 — 체크포인트만 기록, 사용자가 직접 다음 방향 결정

상세는 WIP.md "220차" 참고.

**273차 갱신(같은 현상, 다른 로그·다른 목적에서 재확인)**: route B
실측(`000003b0--794e227a32`, 272차 corpus)에서 동일 패턴을 다시 확인 --
`routeApexIdx`가 불변(예: idx=7, dist=70.0 고정)인 구간에서 `routeApexSpeed`
가 40.3->39.3->42.7->41.8->40.7->39.7->38.8->37.8kph로 프레임마다
±3kph씩 흔들림. 220차는 이 현상을 rolling-max 게이트(199차 보완안)의
오무장 원인으로 지목했었는데, 273차는 이번엔 반대 방향 문제로 이 현상을
지목한다 -- 258차 ACTIVE 거리게이트(`required_decel>=AutoNaviSpeedDecelRate`)
계산에 이 노이즈가 그대로 들어가면 게이트가 "겨우 열릴락 말락"하는
경계에서 불안정해질 수 있다는 가설(§28, 아직 정량 검증 안 됨 -- 273차
감도분석 스크립트가 baseline 재현에 실패해 정량화는 다음 세션 이월,
WIP.md/toolkit/README.md 273차 참고). 219/220차와 273차 모두 같은 근본
원인(리샘플 그리드 재앵커링에 의한 apex_speed 잡음)이 서로 다른 후속
로직(rolling-max 게이트 / 258차 거리게이트) 양쪽에 영향을 줄 수 있음을
시사 -- 근본 원인 자체를 고치는 게 두 문제를 동시에 완화할 가능성이
있으나, 이 근본원인 수정은 별도 과제로 아직 착수되지 않음.

---

## 219차 — [ANALYSIS_ONLY-NEEDS_USER_DECISION] t=1004~1030류 사례 199차 discontinuity boost 미작동 원인 확정: 게이트가 "누적은 크지만 프레임당은 작은" 계단식 하강을 구조적으로 감지 못 함

**대상**: 218차가 미확인으로 남긴 "199차 boost가 t=1004~1030 사례에서 실제로
왜 안 먹혔는지"를 실측 재생으로 규명.

**방법**: 215차 18세그 CSV(`x18seg_215cha_route.csv`, commit `4514e97`)를
`toolkit/diag_route_boost_arm_219.py`(기존 `sim_route_217_ceiling_vcruise_ab.py`
의 `Branch` 클래스 재사용)로 t=1000~1040 구간 프레임 단위(20Hz) 재생.

**핵심 결론**: 199차 게이트(`ROUTE_APEX_SPEED_DISCONTINUITY_THRESH_KPH=15.0kph`,
직전 프레임 대비 apex_speed 낙차 비교)는 이 구간에서 **단 한 번도 무장되지
않음**(armed 프레임 수=0). 구간 내 관측된 최대 단일 프레임 낙차는 10.75kph
(t=1005.38)로 임계값 미만. 원인은 `routeApexIdx`가 매 프레임 인접 후보(10m
간격)로 계속 전환되면서(179차/215차가 실측한 apexIdx flicker와 동일 메커니즘)
apex_speed가 "한 프레임 급락"이 아니라 **잘게 쪼개진 계단식 하강**으로
나타나기 때문 -- 누적으로는 104kph→42kph대(~60kph)까지 크게 떨어지지만 각
스텝은 게이트 임계값 미만이라 구조적으로 감지 불가. **"부스트가 무력화됐다"가
아니라 "이 케이스는 애초에 199차 게이트의 감지 대상이 아니다"**가 정확한
원인 -- 회귀나 버그가 아니라 199차 설계 자체의 사각지대.

**실측 소요시간**: `liveRouteSpeed` 재구성값이 스파이크(t=1005.38, 104.0kph)
에서 50kph 미만으로 내려오기까지 0.70 m/s² 기준 **23.19초**(218차의 "25초
이상" 기술과 근사 일치). 218차 결정(0.70→1.00 m/s²) 적용 시 **20.04초**로
일부 단축(약 14%)되나 armed 프레임 수는 여전히 0 -- boost 발동이 아니라
base ramp 자체가 빨라진 결과일 뿐.

**부수 관측**: 같은 구간에서 vEgo가 37km/h대→3km/h대(근정차)로 자연 감속하는
동안 `liveRouteSpeed`는 60~90kph대를 유지 -- 215차가 발견한 "정차 중에도
route 목표속도가 시간기준으로만 하강" 현상과 동일 패턴이 이 케이스에도 함께
관측됨(별개 원인인지 동일 근본구조의 다른 발현인지는 미확정).

**상태**: NEEDS_VALIDATION(실차) — 이 세션은 오프라인 로그 재생 분석만 수행,
ryu 소스 코드 변경 없음. 재설계 방향(게이트 기준 변경 / flicker 안정화 /
하강램프 자체 재설계)은 사용자 결정 대기 중, WIP.md 219차 "다음 작업" 참고.
...
**방법**: `carrot_man.py` L905~1038의 실제 코드 구조(raw 클리핑 → 199차 boost → 172/173차 비대칭 램프리미터 → 162/167차 position-uncertainty 게이트)를 그대로 재현하되, raw 클리핑 단계만 OLD(202차 고정 150)/NEW(205차 동적 상한) 두 갈래로 병렬 계산(`toolkit/sim_route_205_vego_cap_ab_206.py`).

**결론(NEGATIVE)**: 문제의 스파이크/고원 구간(t=418.4~423.2)과 이후 북대전IC 접근 구간(t=423~498) 전체에서 OLD와 NEW가 **완전히 동일**(would_bind 37.1%→37.1%, 프레임별 출력값 소수점까지 일치, 이 구간 960프레임 중 1프레임도 다르지 않음). 원인은 이 실패모드의 성격 자체에 있음 — apex_idx가 먼 후보로 오선택될 때, 손상되는 것은 raw out_speed뿐만이 아니라 `routeApexSpeed`(목표속도) 자체도 함께임(스파이크 구간 내내 raw==apex_speed, 둘 다 ~297~298로 동일 값). 205차 공식의 `max(v_ego_kph, apex_speed)` 항은 이 상황에서 항상 오염된 apex_speed 쪽이 더 크므로 그 값에 지배되고, vEgo 기반 하한은 단 한 번도 작동하지 않음.

**205차 자체 검증(WIP.md 참고)과의 괴리**: 205차가 수치 검증에 쓴 4개 합성 시나리오(예: "raw=298/vEgo=55/apex=50 → 55")는 raw와 apex_speed를 서로 독립된 신호로 가정했다. 그러나 실제 이 실패모드에서는 이 둘이 같은 근본원인(candidates 리스트가 apex_idx 전환에 따라 먼/빠른 후보로 슬라이딩)으로 동시에 오염되므로 분리되지 않는다 — 합성 시나리오가 실제 실패 데이터의 상관구조를 반영하지 못한 것이 검증 결과가 서로 어긋난 원인.

**전체 로그 스캔 결과**: 8세그 전체(7,098 활성 프레임) 중 OLD≠NEW인 프레임은 56개뿐, 전부 t=524 부근의 경미한 사례 1건(raw=75.1/apex_speed=67.3처럼 두 값이 실제로 8kph가량 벌어지는 정상적 후보전환 상황) — 이 지점에서는 205차 공식이 설계 의도대로 동작함을 확인했으나, 202/203차의 핵심 실패모드와는 무관한 위치.

**참고(205차 자체의 기여와 202차의 기여를 분리)**: OLD(202차 150고정) 단독으로도 이 로그에서 이미 상당한 개선을 보였음 — apex 최근접 프레임(t=461.08) 기준 목표 대비 격차 4.3kph(202차 패치 이전 원본 baseline 분석의 ~170kph 격차 대비 큰 개선). 이 개선은 202차의 150 cap 자체에 의한 것이며, 205차가 이 로그에서 추가로 기여한 부분은 사실상 0.

**대응 방향**: 203차가 이미 제시했던 근본 대응(①`carrot_navi_route()`에 candidate_count/candidates 전체 실측 계측 추가, ②상승측 게이트 설계 자체 재검토)이 여전히 유효한 다음 과제. 205차 패치 자체를 되돌릴 필요는 없음(§27 최소변경 관점에서 다른 상황(경미한 후보전환)에는 실제로 도움이 되고 부작용도 없음 확인됨) — 다만 "202/203차 문제를 해결했다"는 기대는 이 로그 기준으로 성립하지 않음을 사용자에게 명확히 보고해야 함.

**실차 검증**: 미실시(시뮬레이션 재현 단계, 205차와 동일하게 실차 미검증 상태 유지).

**관련**: 205차(패치 원 커밋), 202차(baseline 스파이크 최초 규명, 150 cap 도입), 203차(candidates 리스트 텔레메트리 부재로 인한 구분 불가 문제 최초 제기).

**전달 파일**: `toolkit/sim_route_205_vego_cap_ab_206.py`

## 203차 — [PARTIALLY_RESOLVED] `hi=vEgo` 앵커링 방향은 유효하나, "apex_idx 급변" 단독 신호로는 허위스파이크/정상 연속곡선을 구분 불가

**대상**: 202차가 제안한 상승측(hi) 디바운스 게이트 설계의 세션 내 시뮬레이션 검토.

**결론 1(유효, 정량 재확인)**: `hi=math.inf`(현재) 대신 `hi=vEgo_kph`로 앵커링하면 북대전IC(t=450~498, 960프레임)에서 route 후보값이 훨씬 더 자주 arbitration에서 이길 조건을 만족함(would_bind A 37.1% → B 98.9%, 재현 스크립트 기준). 스파이크(t=418.4~419.5) 구간에서도 out_A는 150(cap)에 붙잡히고 out_B는 vEgo(≈64.3~64.5)에 고정 — 방향은 확실히 유효.

**결론 2(신규 발견)**: 202차가 "단발성 스파이크"로 표현한 현상은 실제로 t=418.62~423.18(4.6초, 92프레임) 동안 지속되는 "고원"이며, t=423.23에 진짜 apex(idx=21, dist=230m)가 잡히는 순간부터 raw가 단조 감소로 전환됨. 디바운스 N=92(4.6초)로 맞추면 disarm 시각이 t=423.23과 정확히 일치 — `hi` 게이트가 실제로 관여해야 하는 구간의 길이를 정확히 특정함.

**결론 3(회귀 위험, 실측 확인)**: 동일한 "apex_idx 변화 + raw≥150 + raw 급등(전프레임 대비 +20kph 이상)" 신호를 8세그 전체(29회 발생)에 적용하면, t=382~393 구간(조향각 ±14°, vEgo 52.0→60.8km/h 실가속, src=road/desiredSpeed=200=사실상 무제한 — **진짜 연속곡선 통과 후 가속 구간**)에서도 신호가 반복 발생. N=92 디바운스 적용 시 이 구간 armed(hi=vEgo) 비율이 10.1/11초. Armed 상태에서는 route 후보가 정확히 현재 vEgo로 고정되므로, **실제 가속 상황에서 route가 그 후보를 그대로 arbitration에 제출한다면 가속을 억제하는 후보가 될 위험**이 실측으로 확인됨 — 202차가 예상한 "커브 통과 후 서행"보다 더 근본적인 문제.

**근본 원인**: `carrot_navi_route()`의 텔레메트리는 apex_idx/dist/speed(선택된 후보 1개)만 노출하고(193/194차 계측), 매 프레임 실제 candidates 리스트(개수·값 전체)는 노출되지 않음. 이 때문에 "후보 리스트가 실제로 비었다가 다시 채워짐(진짜 재평가)"와 "후보 리스트는 안 비었지만 최근접 후보가 잠깐 다른 인덱스로 바뀜(정상적 후보 갱신, 연속곡선에서 흔함)"을 구분할 신호가 텔레메트리에 없음.

**대응 방향(미착수, 사용자 결정 대기)**:
1. `carrot_navi_route()`에 candidate_count 실측 계측값 신규 추가(ryu 코드 변경 + 실차 재수집 필요) — 가장 직접적이나 배포/재수집 사이클 필요
2. 상승측 게이트 접근 자체를 재검토(예: `hi=vEgo` 이진 스위치 대신 완만하지만 유한한 상승 램프 등)
3. 이번 8세그 로그 하나로는 "진짜 직선복귀"와 "연속곡선 가속탈출" 두 시나리오 모두를 포함한 회귀검증이 불충분 — 다른 로그로 재검증 필요

**관련**: 202차(baseline 스파이크 최초 규명), 173차(상승측 무제한 램프 도입 배경), 197차(apex 선택 candidates[0] 설계).

**전달 파일**: `toolkit/sim_route_hi_vego_anchor_203.py`, `toolkit/sim_route_hi_debounce_sweep_203.py`

## 202차 — [ROOT_CAUSE_CONFIRMED] 201차 원인 B 상류 메커니즘: `_route_speed_prev` baseline이 298까지 점프하는 원인 = apex_idx 전환 스파이크 + 173차 상승측 무제한 램프

**대상**: 201차가 규명한 원인 B(북대전IC 커브, `desiredSource`가 239프레임 내내 `"route"`가 되지 못함)의 상류 원인. 사용자가 "baseline이 왜 ~230까지 높게 형성되는지" 재질문.

**결론**: 오래된 stale 값 누적이 아니라, 북대전IC 진입 불과 26초 전(t=418.6)에 발생한 단발성 "직선 오판정 스파이크" 1회가 직접 원인.

**메커니즘(199차 8세그 로그, 9,598프레임, t=137.72~617.58 전체 시뮬레이션 기준)**:
1. t=418.42~418.59: apex_idx 전환으로 raw out_speed가 200→298까지 급등(진짜 근접 커브 후보가 순간 사라지거나 먼 지점으로 바뀌며 "직선"으로 오판정).
2. 173차가 도입한 상승측 무제한 램프(`hi=math.inf`, `carrot_man.py`)가 즉시 발동 — `_route_speed_prev`가 raw를 그대로 추종해 298까지 점프.
3. 하강 램프 상한(`accel_limit_kmh` = `AutoNaviSpeedDecelRate`(0.70)×3.6 = 2.52km/h/s)로는 298→55(북대전IC 목표속도)까지 96초가 필요한데, 실제 남은 시간은 26~30초뿐 — 물리적으로 따라잡기 불가능.
4. 그 결과 북대전IC 커브(t=450~498, 239프레임) 전 구간에서 route 후보값이 실제 arbitration 승자 이하로 단 한 번도 내려오지 못함(201차 원인 B와 정합).

**정량 확인**: 8세그 전체(480초)에서 이런 "raw≥290 스파이크"는 이 1회만 발생, 위치가 정확히 북대전IC 진입 26초 전 — 우연이 아니라 이 실패 사례의 직접 원인.

**대응 조치**:
- **(즉시 적용, 202차)** route 최대속도 sentinel/상한을 300.0→150.0으로 명시적 분리(`ROUTE_MAX_SPEED_KPH`, `carrot_man.py`). 이 패치는 스파이크의 **최대값을 298→150으로 낮출 뿐**(196km/h 낮춤 → 하강 시간도 (150-55)/2.52≈38초로 단축, 여전히 26~30초보다는 김 — 완전 해결 아님), 상승측 무제한 램프 자체와 apex_idx 전환 스파이크 발생 원인은 그대로 남아있음. **비활성 sentinel(300.0, `_route_out_speed`/`carrot_serv.route_out_speed` 초기값)은 의미가 달라 변경하지 않음** — 자세한 내용은 WIP.md 202차 참고.
- **(미착수, 203차 예정)** 근본 대응: 상승측(hi)에 디바운스 게이트 추가 — apex_idx가 전환되거나 raw가 급등(전 프레임 대비 +100kph 이상)하는 프레임에서는 상승 무제한을 즉시 적용하지 않고 최소 확인 절차(연속 프레임 유지 등)를 거치는 설계. 173차가 상승 무제한을 도입한 원래 목적("apex 통과 후 즉시 원복", 160차 설계 의도)과 충돌하지 않는지 회귀검증 필수 — 진짜 apex 통과 시나리오와 이번 같은 "허위 직선 스파이크"를 구분하는 것이 핵심 설계 과제.

**실차 검증**: 미실시(baseline 메커니즘 분석 + 150 상한 패치 모두 시뮬레이션/정적분석 단계).

**관련**: 201차(원인 B 최초 규명), 173차(상승측 무제한 램프 도입), 179~181차(apex 선택 relative severity gate — 이번 스파이크의 apex_idx 전환과는 별개 이슈, gate 자체는 계속 유효).
...
**기존 결론(194차 계속3, 오류)**: "abe1d2bb34/e635e188cf 두 로그 모두 로그 전 구간 vEgo가 15.4km/h를 넘지 않음(완전한 도심 저속/정체 주행)" — 이 전제로 ③MPC 추종지연/④다른 source 덮어쓰기 검증이 불가능하다고 결론.

**정정 사유**: 194차 계속3은 사용자가 제공한 `.txt` 형태 CSV(재추출본 아님)를 사용했는데, 이 파일이 잘렸거나 손상됐던 것으로 추정됨. 195차에서 `e635e188cf`(19세그) 원본 rlog를 `extract_log.py`로 직접 재추출한 결과 **최고 48.6km/h**까지 나오는 정상적인 시내 주행 로그였음(22,799행, t=543.0~1682.9). 사용자 스크린샷 3장을 GPS 앵커링(`gpsLocation.unixTimestampMillis` 오프셋)으로 rlog t축에 매핑 후 `qcamera.ts` 프레임을 직접 대조해 재확인 — 배경 건물·신호등·차량 배치 전부 일치, 앵커링 정확도 확정.

**새 결론**: route가 실제 arbitration에서 이기는 구간(예: t=706~759)에서 apex_dist 단조감소→out_speed 하강→vEgo 실제 감속→apex 근처 정지까지 7단계 체인이 설계대로 작동함을 실측 확인(WIP.md "195차" 참고). 단 `abe1d2bb34`는 이번 회차에서 재검증하지 않음 — 동일하게 손상 의심되므로 필요시 재추출 필요.

**교훈**: 사용자가 제공한 `.txt`/CSV가 `extract_log.py`로 방금 재추출한 것이 아니라면(예: 이전 세션 산출물을 재사용하거나 형식 변환된 파일), 행 수·컬럼·max값 등을 원본 rlog와 교차검증 없이 그대로 신뢰하지 않는다 — 이번처럼 3배 이상 차이(15.4 vs 48.6km/h)가 났음에도 초기엔 미포착됨.

---

## `곡선_가감속_코딩.txt` 설계문서 vs 현재 코드(160차~194차) 대조검토 — 195차

**검토 회차**: 195차 (코드 변경 없음, 대조검토만)

**배경**: 사용자가 제공한 route 감속 재설계 방향 문서(`곡선_가감속_코딩.txt`, 7개 항목)가 실제로는 **160차에서 이미 반영된 바로 그 설계문서**임을 `carrot_man.py` L687 주석에서 확인(주석이 이 파일명을 직접 인용함). 상세 항목별 대조는 WIP.md "195차" 참고.

**핵심 발견**: 문서 7항목 중 6개는 이미 코드에 부합. 1개(6/7번, "카메라 로직과 완전히 동일" 취지)만 부분적으로 어긋남 — route는 132차/172-173차가 도입한 **하강측 전용 비대칭 램프리미터**(`accel_limit_kmh` 기반, 카메라/회전엔 없음)를 여전히 갖고 있음. 이 램프는 131차가 실측·재현한 별개 버그(lookahead 윈도우 경계 스냅으로 인한 단일프레임 Δ-24~-25kph 급락, Hypothesis C SUCCESS)를 막기 위한 의도적 장치이며, 제거 시 그 버그가 재발할 위험이 있음.

**추가 근거**: 194차 계속3이 실측한 "route가 이길 때 desiredSpeed가 raw routeOutSpeed보다 평균 +15.6~+19.8km/h 높게 유지되는 gap"의 정확한 원인이 바로 이 하강측 램프임을 이번 대조검토로 확정(194차 계속3 당시엔 "172/173차 비대칭 램프리미터의 설계된 결과"로만 기록, 195차에서 "카메라와 다른 유일한 지점"이자 "문서 6/7번과의 관계"까지 명확화).

**상태(195차 당시)**: NEEDS_VALIDATION(사용자 결정 대기) — (a) 현행 유지 (b) 램프 제거+apex_dist/speed 계산 자체 평활화로 131차 문제 재해결 (c) 현행 유지+예외 명시. 코드 미착수(사용자 승인 전 패치 금지 원칙).

**196차 갱신 — (c) 채택, VALIDATED로 격상**: 사용자가 (c)를 승인 — 현행 하강측 전용 비대칭 램프리미터를 그대로 유지하고, `곡선_가감속_코딩.txt`(사용자 원본 설계문서)에 8번 항목으로 공식 예외를 명시함. 근거: 이 램프는 131차가 실측한 lookahead 경계 급락(Hypothesis C SUCCESS)을 막는 의도적 안전장치이지 문서 7번이 말하는 "방해하는 무관한 코드"가 아니며, 160차 설계 핵심(apex 거리 기반 `calculate_current_speed`)을 훼손하지 않음. 상승측은 173차에서 이미 무제한(`hi=math.inf`)으로 풀려 있어 "apex 통과 후 즉시 원복"도 이미 만족됨. 따라서 하강측 램프 제거는 131차 문제를 다른 방식으로 먼저 재해결한 뒤에만 정당화되며, 현재 증거만으로 (b)를 진행하는 것은 성급하다고 판단해 보류. `carrot_man.py` 코드 주석(L687-720, L780-819)은 재검토 결과 이미 "물리공식은 카메라와 동일 + 하강측 램프는 132/172/173차의 별도 출력 안정화 보호장치"로 정확히 기술되어 있어 **추가 코드/주석 변경 없음**. (b)는 향후 실차에서 이 limiter가 실제 감속을 과도하게 지연시키는 문제가 구체적으로 확인될 때만 재검토.
...
(187차 신규 도구)를 처음 실행. 187차 문서화 사례 재확인과 함께 신규
후보 2건이 추가로 탐지되어 대시캠/naviPaths 원본으로 개별 검증.

### 확정 유형3 (2건 — 187차 재검증 + 신규 A)

**187차 사례(t=1365.71~1376.56)**: 재추출 CSV로 재실행한 결과
start_t/end_t/steer_peak 등 187차 문서 값과 정확히 일치 — 152차 유형3
정의(naviPaths 원본 폴리라인 좌표 자체에 급회전 형상 미포함) 그대로
재확인.

**신규 A(t=1336.76~1346.65)**: 정지 후(vEgo 최소 0.01m/s, 약 3초 정지)
급우회전(steeringAngleDeg -3.7°→-171.8°), `rightBlinker=True`. 대시캠
프레임(t=1342.46/1344.06) 확인 결과 신호등 있는 교차로에서 정지 후
우회전 실측. naviPaths 재계산 결과 far_window(50~250m) 전체가
200~288km/h로 한 번도 저속 지점이 없음 — 급회전 형상이 폴리라인
자체에 전혀 담기지 않은 전형적 유형3. **187차 사례와의 차이**: 이번엔
`rightBlinker=True`였음(187차는 blinker 없이 발생) — blinker 유무와
무관하게 naviPaths 자체의 한계로 발생함을 교차 확인.

두 사례 모두 152차 결론("곡률 계산 로직 문제가 아니라 naviPaths 입력
데이터 자체의 한계", chord 샘플 간격을 줄여도 해결 불가)을 재확인.
c3-ms-dev 현재 HEAD(166/167차 헤딩보정 패치 반영 이후)에서도 그대로
유효 — 유형3은 166/167차 패치 범위와 무관한 별개 미해결 과제.

### 오탐 후보 — 유형3으로 확정하지 않음 (신규 B)

**신규 B(t=1352.76~1361.91)**: v1 도구가 3번째 후보로 탐지했으나,
대시캠 프레임(t=1360.31/1360.60) 확인 결과 교차로가 아니라 수풀을 낀
일반 도로 커브였음. naviPaths 원본을 직접 검산한 결과 d=80~100m
구간에서 speed_cap이 정상적으로 5km/h까지 떨어져 있어 **폴리라인
자체는 실제 곡률을 담고 있었음** — 152차가 정의한 "폴리라인 좌표
자체에 형상이 없는" 유형3에 해당하지 않음. 그 앞뒤(d=50~70m,
d=110~250m)가 길게 직선이라 v1의 median(중앙값) 판정이 200km/h로
나와 min_recomputed_speed_kph=150 임계값을 잘못 통과한 것으로 확인.

**결론**: 이 건은 route 내비게이션 로직(`carrot_man.py`)이나 naviPaths
데이터 자체의 결함이 아니라, **devnotes 분석도구(`scan_type3_
curvature_blindspot.py` v1)의 median 단독 통계 판정이 갖는 한계**임.
ryu 프로덕션 코드/naviPaths 입력에는 문제 없음.

### 유형3 하위 분류 (188차로 확정)

이번 분석으로 유형3에 최소 두 가지 서로 다른 원인이 섞여 있었음이
드러남:

1. **진짜 blind spot**(152차/187차/신규A): naviPaths 원본 폴리라인
   좌표 자체가 급회전 형상을 완전히 누락. 상류 내비게이션 앱/GPS
   맵매칭 단계의 한계로 추정(원인 미착수, 152차/187차부터 이어지는
   과제).
2. **탐지기 통계 판정의 한계**(신규 B, 188차 신규 발견): naviPaths에는
   곡률이 존재하나 짧은 커브 구간이 긴 직선 구간 사이에 끼어 median
   계산 시 희석됨. route 실제 동작과는 무관하며 분석도구
   `scan_type3_curvature_blindspot.py`에서만 발생하는 현상.

### 도구 개선 및 회귀테스트

②번 문제를 걸러내기 위해 `type3_curvature_blindspot_scan_v2()`
(2단계 판정, `low_cap_run_m`/`low_cap_ratio_thresh`/`low_cap_eval_start_m`)
신규 추가. 개발 중 `low_cap_eval_start_m` 없이 1차 구현했을 때 187차
사례 초반부(1365.71~1367.76)가 오탐 제외되는 회귀 실패가 있었음 —
`near_field_guard_m`(50m) 바로 다음 d=50~80m 구간에서도 187차 사례
naviPaths가 순간적으로 저속을 보이는 별도의 근접노이즈 패턴이 있음을
확인(원인은 확정하지 못함, NEEDS_INVESTIGATION — 근접 앵커전환
노이즈가 50m 경계에서 정확히 끊기지 않고 번지는 것으로 추정만 함).
`low_cap_eval_start_m=80.0`으로 이 구간을 2단계 판정에서만 제외해
해결. 회귀테스트 3건(187차 기존/신규A/신규B) 전부 기대대로 통과
확인(상세는 WIP.md 188차 참고).

**toolkit 변경**: `analysis_helpers.py::type3_curvature_blindspot_scan_v2()`
신규, `scan_type3_curvature_blindspot.py` `--v2` 플래그 추가(v1 그대로
유지, 회귀 없음). ryu 프로덕션 코드 변경 없음. 상세는 WIP.md 188차,
`toolkit/CHANGELOG.md` 참고.

**NEEDS_INVESTIGATION**: near_field_guard_m(50m) 경계 바로 다음
구간(50~80m)의 근접노이즈 번짐 현상 자체의 근본 원인(맵매칭/리샘플링
단계 중 어디서 발생하는지)은 미규명 — `low_cap_eval_start_m=80.0`은
이번 3건 데이터로 정한 경험적 값이며 근본 해결은 아님.


## 187차 — [실차 재현 확인] 152차 유형3(naviPaths 폴리라인 원본 좌표 자체에 급회전 형상 미포함)의 신규 실차 사례 — 우회전 교차로에서 route 사전감속 전혀 미작동

**배경**: 사용자가 우회전 교차로에서 route 사전감속이 전혀 동작하지
않았다는 스크린샷(HUD 10:48:41, vturn=25, route=146.4, 우회전 화살표
안내 중)과 해당 시각 포함 seg14/seg15 zip을 제공. 182차/186차가 다룬
"naviPaths 완전 드롭아웃" 재현인지부터 우선 배제하며 조사.

**배제 확인(기존에 이미 해결/계측된 원인 유형 전부 해당 없음)**:
`naviPointsActive`=True/`navdActive`=True/`dtRouteInactive`=0.0/
`routeSource`=tcp_navi 전 구간 유지(182/186차 드롭아웃 아님).
`positionDtSinceFix` 최대 ~1.05초(임계값 3.0초 미만)/`ccPoseValid`
전 구간 True(162/163차 GPS bearing 정체, 166/167차 헤딩보정 발동
조건 자체가 없었음). `compare_navpos_vs_gps.py`로 추정위치-실측GPS
이격 재확인 시 min=0.3m/max=11.2m/mean=5.1m(n=600) — 162차형
데드레커닝 drift(최대 28m 누적) 없음.

**확정된 원인 — 152차 유형3 재현**: 스크린샷 순간(t=1370.06) naviPaths
원본 텍스트 확인 결과 d=30~290m 구간 내내 횡오프셋 y가 28~29m 근처에
고정된 사실상 직선(실제 조향각은 동일 구간 -4도→-160도 급변,
dashcam으로 실제 우회전 교차로 확인). TBT(`xTurnInfo`)도 전 구간 2를
유지하다 t=1374.66에야 1로 리셋되며 383m 밖 다음 턴으로 건너뜀 —
152차 이벤트 A와 완전히 동일한 패턴(폴리라인 원본 좌표 자체에 이
지점의 급회전 형상이 담겨있지 않고, TBT도 이 교차로를 별도 타겟으로
포착한 적이 없음). 152차 결론("곡률 계산 로직 문제가 아니라 naviPaths
입력 데이터 자체의 한계")이 현재 c3-ms-dev HEAD(166/167차 헤딩보정
패치 반영 이후)에서도 그대로 유효함을 재확인 — 유형3은 166/167차
패치 범위와 무관한 별개 미해결 과제로 확정.

**추가 사례(190차, 동일 route 0000037a 전체 구간 스캔으로 확인)**:
동일 route의 다른 구간에서 동일 패턴 2건 추가 확인.
- t=995.11~1005.95: 횡단보도 교차로, steer_peak 231.0°,
  `liveRouteSpeed` 240.6~297.5(제약없음대) 고정, `src=[gas,vturn]`
  (route 후보 자체가 승자가 된 적 없음).
- t=1439.01~1445.81: 광장형 교차로/T자로, steer_peak 198.8°,
  `liveRouteSpeed` 167.3~196.0(제약없음대) 고정, `src=[bump,vturn]`.

두 사례 모두 naviPaths 원본 텍스트가 해당 구간에서 횡오프셋이 사실상
평탄하게 유지됨을 직접 확인 — 152차/187차와 동일한 "naviPaths 원본
좌표 자체에 급회전 형상 미포함" 메커니즘. `scan_type3_curvature_
blindspot.py --v2` 전체 라우트(25분, routeA+routeB) 전수 스캔으로
발견(190차, WIP.md 참고). 스캔이 함께 찾아낸 나머지 4건(routeA
1457.71/1483.51/1662.36, routeB 1940.76)은 qcamera+CSV 원인 재확인
결과 유형3이 아닌 스캐너 오탐으로 판정(190차 WIP.md 상세 참고,
scan_type3 v2의 신규 오탐 메커니즘 2종 발견으로 이어짐).

**192차 추가 확인(v3 정차게이트 회귀 재현)**: 190차가 오탐으로 판정한
4건 중 1483.51(routeA)/1940.76(routeB)은 191차가 도입한
`type3_curvature_blindspot_scan_v3()`(정차프레임 후보 배제+병합 차단)
적용 후에도 REJECT로 전환되지 않고 여전히 accepted로 남음을 원본
routeA/routeB CSV 재추출로 확인 — 이벤트 길이만 축소(routeA
50.50s→27.00s, routeB 57.44s→9.49s+1.59s 2건 분리)되었을 뿐 정차
전 접근 구간 자체가 median 기준 "유형3처럼" 남는 구조적 한계로 판단.
동시에 기존 확정 유형3 4건(995.11/1336.76(→1339.36으로 시작부만
트리밍, steer_peak_t 불변)/1365.71/1439.01)은 v3에서도 전부 accepted
유지 — 회귀 없음. 1457.71/1662.36 오탐 2건은 이번 회차 대상 아님,
변화 없음(상세는 WIP.md "192차" 참고).
**의의**: 152차 이후 devnotes에 처음으로 확보된 유형3의 뚜렷한 실차
재현 사례. 152차가 제안했던 "유형3 전용 탐지 도구"(steeringAngleDeg
급변 + naviPaths y값 정체 조합, blinker 비의존) 작성 시 이번
seg14/seg15를 1차 검증셋으로 사용 가능.

**toolkit/ryu 변경**: 없음(`compare_navpos_vs_gps.py`/
`verify_and_extract_frames.py` 기존 도구 재사용). 상세는 WIP.md
"187차" 참고.
...
**[핵심 발견 1] 디바이스 빌드 아이덴티티 검증 실패**:
- `extract_log.py` meta.json의 "repo commit: ef016dbaa4cf"는 분석 컨테이너 checkout일 뿐(기존 원칙 재확인).
- 신규 `check_device_build.py`(178차 신규, toolkit 편입)로 rlog InitData 직접 디코드:
  - device gitCommit: `0ef6b6a35e57f6c444e30cfe534b6ab7eb3817a1`
  - device gitCommitDate: `2026-08-31 18:09:27 +0900`
  - device gitBranch: `c3-ms-dev`
  - device **dirty: True**
- 이 커밋 해시는 `ryu` origin 히스토리(unshallow fetch 후에도) 어디에도 없음. `git fetch origin <hash>` 시도도 "not our ref"로 거부됨(GitHub API는 이 세션에서 rate-limit으로 추가 확인 못 함).
- gitCommitDate(18:09:27 KST)는 177차 패치 push 시각(2026-08-31 09:01:04 UTC = 18:01:04 KST)보다 늦어 **시간 순서상 모순은 없음**(177차 push 이후 빌드된 것 자체는 가능). 하지만 해시가 origin에 존재하지 않고 dirty=True라, "177차 커밋을 포함한 빌드"라고 적극적으로 확정할 근거는 없음 -- 로컬 미푸시 커밋 위에 빌드했거나, 워킹트리에 추가 로컬 수정이 있는 상태로 빌드했을 가능성 등 여러 시나리오가 남음.

**우회전 이벤트 특정(교차검증 완료)**:
- CSV: `rightBlinker`=True + `steeringAngleDeg` 최대 -209°(우회전) + `xTurnInfo` 2→1 전환 + `xDistToTurn` 153m→0 감소 → route t≈400~419 구간.
- 클립: HUD 시계 오버레이(18:16:17~18:16:44) + "vturn 30" 표시 + 회전거리 안내("153m"→"30m"→"10m") + 회전 완료 후 가속(34km/h 회복)로 log t 매핑 교차검증 완료(t≈359~366 부근 24초 정차 신호대기 존재 확인, 이 구간 포함해 시간축 정합 확인).

**[핵심 발견 2] 리드차량 존재로 패치 게이트 대부분 미적용**:
- t=400~419 구간 `leadStatus` 대부분 True(클립에서도 흰색 승합차 근접 컷인 육안 확인, 앞서 t=316 부근에서도 리드차량 "14m" 표시).
- leadStatus=False 구간은 t≈410.24~413.05(약 2.8s, 회전 정점 부근)뿐. 177차 패치의 완화 게이트(`elif self.source=='cruise'`, 즉 리드가 없을 때만)는 코드상 이 좁은 구간에서만 이론상 발동 가능.
- 공교롭게도 aEgo 감속→가속 부호전환(사후복원의 핵심 지표) 시점이 t≈412.44로 바로 이 구간 내부에 위치 -- 하지만 이것만으로 "패치가 작동해서" 라고 결론내리기엔 근거 부족(아래).

**검증 불가로 판단한 이유(4가지 복합)**:
1. 빌드 아이덴티티 불일치(위) -- 이 로그가 177차 코드를 실행했는지 자체가 불확실
2. 게이트가 열린 구간이 전체 회전 대응 구간(~19s) 중 2.8s뿐 -- 대부분 기존 리드기반(`j_lead` interp) 로직이 지배
3. 리드기반 로직도 자체적으로 `A_CHANGE_COST` 200→20 완화 가능(기존 로직, 177차 이전부터 존재) -- 관측된 "빠른 회복"이 177차 신규 로직 때문인지 기존 j_lead 로직 때문인지 이 로그 하나로 분리 불가
4. 비교 기준(패치 OFF/이전) 베이스라인 로그 부재로 A/B 비교 불가 -- 176/177차가 참조한 실측 route `6310bba9b8`가 이 세션에도 캐싱 안 돼 있어 재검증 못함(177차부터 이월된 미해결 항목)

**결론**: 이번 실차 로그로는 177차 패치 반영 여부/효과를 확정할 수 없음. 관측된 사전감속/사후복원 자체는 정성적으로 정상 동작(급격한 스냅백 없이 매끄럽게 복원)으로 보이나, 이것이 177차 신규 로직 덕분인지 기존 로직만으로도 충분한 결과인지는 미판별.

**toolkit 변경**: `check_device_build.py` 신규 편입(README/CHANGELOG 갱신 완료).

**다음 세션/사용자 확인 필요**:
1. 디바이스 빌드 절차 확인 요청 -- `git am` 후 rebuild인지, 로컬 파일 직접 수정 후 빌드인지(dirty 원인), 가능하면 디바이스에서 `git log -1 --format='%H %ai %s'`와 `git status --short` 직접 확인해 공유 요청.
2. 패치 효과를 깨끗하게 보려면 (a) 리드 없는 우회전 로그, 또는 (b) 동일 지점(대덕대로/탑립동 부근) 패치 적용 전/후 비교 로그 필요.
3. `sim_acados_causeB_real_replay.py` 실측 재검증(route `6310bba9b8`) 여전히 대기중(177차 이월, zip 재업로드 필요).
...
## 179차 — [PATCH_APPLIED(실차) + 시뮬레이션 검증 완료, 실측 로그에서 상반된 두 특성 발견] apex 선택기준 "가장 급한 지점"→"가장 가까운 지점" 변경 — 연속 실제커브 케이스는 명백히 개선, 근접 미세잡음 vs 원거리 실제급커브 케이스는 역효과 가능성

**배경**: 사용자 지시로 `carrot_man.py::carrot_navi_route()`의 apex 선택
기준을 157/160차부터 유지되던 `min(speeds)` 전역탐색("가장 급한 지점")에서
"가장 가까운 지점"(distances가 항상 오름차순이라는 production 불변식을
이용, `speeds[k] < nRoadLimitSpeed`인 최소 index)으로 변경. `py_compile`+
`git am` 검증 후 실차 코드(c3-ms-dev `4f90922`)에 이미 적용/push 완료.

**검증 1(사용자 업로드 실주행 로그, route 00000374, 우려되는 특성 발견)**:
`replay_route_camera_style_vs_baseline.py`에 `--apex-mode both` 옵션을
신규 추가해 동일 실측 로그(t=736.8~782.7, **cruiseEnabled=False 전구간
— ACC 미체결, 순수 알고리즘 반응 비교일 뿐 실제 제어 영향 검증 아님**)를
sharpest/nearest 두 방식으로 나란히 오프라인 재생.
- t≈736.8~739(초입 저속 급회전): nearest가 sharpest보다 더 보수적(13.0 vs
  18.2km/h) — 근접 지점 자체가 이미 심한 커브라 문제 없음.
- **t≈753.5~759.3(우려 사례)**: 실제 급커브(curv≈-0.08~-0.10, 60~100m
  전방)가 있는데도 nearest가 10m 앞의 floor 임계값(0.001) 바로 위 미세
  곡률(curv≈-0.001~-0.02, 거의 직선/잡음 수준)에 apex를 고정 —
  sharpest(32.8~38.0km/h, 실제 급커브를 정확히 겨냥해 서서히 감속) 대비
  최대 **9.72km/h 더 높은(덜 안전한)** 값을 냄(t=758.69). 전체 450 유효
  프레임 중 129건이 1km/h+ 차이.
- **원인**: "가장 가까운 지점" 게이트가 "감속이 조금이라도 필요한 첫
  지점"으로 구현돼 있어, floor를 살짝만 넘는 미세 곡률(사실상 노이즈)도
  apex로 채택됨 — 진짜 급커브가 그보다 조금 더 멀리 있어도 그 급커브는
  "자기 차례"(=가장 가까운 지점이 될 때)까지 기다려야 하므로, route
  기능의 존재 목적인 "사전감속"이 지연될 위험.
- **주의(한계)**: 실측 `nRoadLimitSpeed`를 CSV가 기록하지 않아 오프라인
  재계산은 200.0 가정 사용(148/161차부터의 기존 한계) — 절대값(sharp/near
  모두 실측 liveRouteSpeed와 스케일 다름)은 참고용, **sharp vs near의
  상대적 차이만 신뢰 가능**. 실제 device에서는 진짜 nRoadLimitSpeed
  기준으로 게이트가 걸리므로 정확한 문턱 교차 지점은 다를 수 있으나,
  "근접 미세잡음이 원거리 실제커브를 이긴다"는 메커니즘 자체는
  road_limit_speed 값과 무관하게 구조적으로 성립.

**검증 2(사용자 가설 — 카카오맵 스크린샷 기반, 연속 좌회전 시나리오,
시뮬레이션으로 확정 검증, POSITIVE)**: 사용자 주장 — "1차 커브보다 2차
커브가 조금이라도 더 급하면, sharpest 기준으론 1차 진입 시 곡률에 비해
과속 상태로 들어가지만, nearest 기준으론 1차부터 충분히 감속되고 2차도
그 시점부터 재계산되므로 감속 시간이 충분하다"는 가설을 `sim_route_camera_style_decel.py`
에 신규 유닛테스트 3건으로 정식 추가해 검증(10/10 PASS):
1. curv1=0.010(1차,120m)/curv2=0.011(2차,260m, 1차보다 살짝만 더 급함)
   시나리오에서, sharpest는 1차 진입 1m 직전에도 68.2km/h(1차 고유
   안전속도 54.0km/h보다 14.2km/h 과속 — **1차가 사실상 무시됨**, 이는
   160차 자체 기록에 "1차는 사실상 무시됨(문제로 취급 안 함)"으로 이미
   있던 현상과 정확히 일치하나 그 문서는 "1차 고유속도 도달 여부"를
   검증한 적이 없었음).
2. nearest는 동일 지점에서 54.0km/h — 1차 고유 안전속도(54.04km/h)에
   ±1km/h 이내로 정확히 도달.
3. **1차를 통과해 윈도우에서 사라진 뒤(pos=130~259m) 2차 접근 구간에서는
   sharpest/nearest가 0.01km/h 이내로 완전히 수렴** — nearest가 1차를
   제대로 처리하면서도 2차 대응력은 전혀 희생하지 않음을 확인. 사용자
   가설이 정확함.

**종합 판단**: 179차 nearest 변경은 "연속된 두 실제 커브(2차가 조금 더
급함)" 케이스에서는 160차부터 있던 실질적 결함("1차 무시")을 명백히
해소하지만(검증 2), "근접한 floor-경계 미세잡음 vs 조금 더 먼 진짜 급커브"
케이스에서는 반대로 역효과를 낼 수 있음(검증 1) — **두 특성이 공존**하며
근본 원인은 동일함: "가장 가까운" 게이트가 진짜 위험도(곡률 크기)를 전혀
고려하지 않고 "floor를 넘겼는가"만으로 판정하기 때문. 검증 2의 curv1=0.010은
floor(0.001)보다 훨씬 크고 명백히 실제 커브이므로, 게이트에 "최소 심각도"
기준(예: 목표속도가 도로제한속도 대비 일정 비율 이상 낮을 때만 유효한
apex 후보로 인정)을 추가해도 검증 2 케이스는 깨지지 않으면서 검증 1
케이스는 개선될 가능성이 있음 — **사용자 판단 대기 중, 이번 회차에서는
코드 추가 변경 없이 발견사항만 기록**.

**toolkit 변경(이번 회차)**: `sim_route_camera_style_decel.py`에
`carrot_navi_route_camera_style_nearest()` 신규 + 유닛테스트 3건 추가
(10/10 PASS). `replay_route_camera_style_vs_baseline.py`에 `--apex-mode
{sharpest,nearest,both}` 옵션 추가(기본 sharpest, 기존 호환 유지).
route_00000374.csv는 devnotes 미보관(대용량 정책) — 재현 필요 시
`/mnt/user-data/uploads/20260831_182250_00000374--c0c7a9c087--11.zip`
(사용자 로컬 보관 가정) 재추출.

**다음 단계**: (1) 최소 심각도 게이트 추가 여부 사용자 확인 후 필요 시
179차 패치 보강, (2) cruiseEnabled=True 상태에서 실제 연속 커브를 지나는
로그로 재검증(이번 로그는 ACC 미체결이라 실제 제어 영향 미검증), (3) 가능하면
route 00000374 지점(카카오맵 스크린샷 위치)의 재주행 로그로 검증 2 시나리오
실측 대조.
...
`carrot_man.py::carrot_navi_route()`에 실제 반영. `ROUTE_APEX_RELATIVE_SEVERITY_RATIO
= 0.85` 상수 추가, apex 선택을 "candidates(감속필요 지점) 중 severity(k)
>= ratio * sharpest_severity인 가장 가까운 지점, 없으면 게이트 없는
nearest(candidates[0])로 폴백" 방식으로 교체. `py_compile`/`ast.parse`
통과, `git am`(base=4f90922) 성공, 사용자가 origin `c3-ms-dev`에 push
완료(`8958189`) — 재클론으로 원격 반영 직접 확인함.

**외부 검토에서 제기된 우려 및 코드 추적 검증**: 패치 리뷰 과정에서
"`candidates`/`gated`의 index 순서가 실제로 거리순(가까운→먼)인지 보장이
있는가"라는 지적이 나옴. `carrot_navi_route()` 내 `distances[]` 생성부
(604~618행)를 직접 추적한 결과:

```python
distance = 10.0
for i in range(len(resampled_points) - sample * 2):
    distance += distance_interval   # 10.0m씩 단조 증가
    ...
    distances.append(distance)
```

`distance` 변수가 루프마다 `distance_interval`(10.0m)만큼 단조 증가하며
순서대로 `append`되므로 `distances[]`는 **구조적으로 항상 오름차순**
(index 0 = 가장 가까운 지점)임이 재확인됨. 이 불변식은 179차 커밋 주석에
이미 명시돼 있던 전제였고(FINDINGS.md 179차 항목 참고), 이번에 대안1
적용 후 그 전제가 여전히(그리고 애초부터) 실제 코드와 일치함을 재추적
확인한 것 — 우려는 기우로 판명, `candidates[0]`/`gated[0]`는 항상
"조건을 만족하는 가장 가까운 지점"이 맞음.

**아직 안 된 것(정직하게 기록)**: 오프라인 replay 재검증
(`replay_route_camera_style_vs_baseline.py`에 relative_gated 모드 추가,
cruiseEnabled=True 실측 로그 기준)과 실차 주행 검증 모두 미착수 —
0.85(=15% 이내 차이는 무시) 값이 실제 감속 timing에 미치는 영향은 코드
추적만으로는 결론 낼 수 없음.

**다음 단계**: (1) 실차 적용 후 잡음 무시 + 진짜 커브 반응 유지 여부
실측 확인, (2) 실차 적용 전 오프라인 replay 재검증(권장, 미착수), (3)
카카오맵 연속커브 지점 재주행 로그 확보 시 대조 검증.

**범위**: `selfdrive/carrot/carrot_man.py`만 변경(toolkit 변경 없음).
patch 파일: `0001-route-apex-relative-gated.patch` (base `4f90922`,
c3-ms-dev, 커밋 `8958189`로 push 완료).

## 180차 계속 — [toolkit 신규] replay_route_camera_style_vs_baseline.py에 relative_gated/both_relative 모드 추가, 합성 스팟체크 POSITIVE (실측 재검증은 CSV 재확보 대기)

**진행**: 외부 검토에서 권고된 "오프라인 replay A/B 비교"를 위해
`replay_route_camera_style_vs_baseline.py`에 `--apex-mode relative_gated`
(179차 후속2/180차 프로덕션 게이트, road_limit_speed=200.0/
relative_severity_ratio=0.85 고정)와 `both_relative`(nearest vs
relative_gated 나란히 비교) 신규 추가. 기존 `both`(sharpest vs nearest)
전용으로 하드코딩돼 있던 A/B 비교 출력부를 `modes[0]/modes[1]` 기반으로
일반화(코드 중복 없이 재사용).

**합성 스팟체크(실측 CSV 없이 함수 직접 호출)**: speeds=[199.5, 199.0,
198.5, 40.0], distances=[10, 20, 30, 300] (근접 3점=floor 근접 잡음,
원거리 1점=실제 급커브) 시나리오로 3개 모드 전부 호출:
- sharpest: 81.2km/h (실제 급커브를 정확히 겨냥)
- nearest: 199.5km/h (근접 잡음에 낚여 사실상 무반응 — 179차 문제 재현)
- relative_gated: 81.2km/h (**sharpest와 동일** — 잡음 게이트로 걸러내고
  실제 급커브 선택)

의도대로 배선이 동작함을 확인. 단 이는 함수 호출 배선 검증일 뿐, 실측
로그 기반 검증은 아님.

**아직 안 된 것**: 179차 문제 로그(`route 00000374`, t≈753.5~759.3)로
`both_relative` 실측 A/B 재검증 — 원본 zip이 devnotes 대용량 정책상
미보관, 이번 세션 업로드 폴더에도 없어 재업로드 필요(다음 세션 최우선).
...
## 301차 -- [ANALYSIS_ONLY, `ryu` 본체 무변경] 300차 "lost" 15건 A→LOST→B 경계 계측 -- miss_frames=6 결정론적 확인, B 5초 생존율 0/15, #1→#2/#4→#5 연쇄 확인(GPS 대조는 여전히 미완)

**배경**: 300차가 강제 RELEASE 15/15 전부 `mode="lost"`("passed" 0건)임을
확인한 뒤, ChatGPT가 제시하고 사용자가 검토를 지시한 다음 질문에 답하기
위한 세션:

> "lost가 발생한 순간, 기존 Apex A가 정말 죽은 것인가? 아니면 A를 잠깐
> 놓쳤지만 B가 이미 유효한 새 Apex였는가?"

**한 일**: 신규 `sim_route_301_lost_boundary_trace.py` 작성(§21 -- 기존
`ContinuityState`/`route_find_clusters`(296차)/`ActualLayer`(300차) 전부
무변경 재사용, 이벤트 탐지 로직도 300차 것을 그대로 import해 이중 구현
방지). 각 `mode="lost"` 이벤트에 대해 계측만 추가:
- A 마지막 정상 매칭(`matched`) 프레임의 t/거리/속도/vEgo
- LOST 선언까지 걸린 미스 프레임 수(`miss_frames_at_lost`)와 경과시간
- LOST 순간 재획득된 B의 거리/속도/클러스터 크기
- LOST 이후 `--persistence-horizon`(기본 5초) 동안 B가 `matched`/`held`로
  계속 이어지는지(생존시간/생존프레임)

route1~4 실측 corpus(297~300차와 동일 corpus, meta.json commit
`d2f47d1` 일치 재확인) 전체 실행, **정합성 자체검증**: 이번 스크립트가
탐지한 강제 RELEASE 15건/lost 15건이 300차 기록(15건, 전부 lost)과
정확히 일치(route별 10/3/1/1건도 동일) -- 300차 결과가 재현 가능함을
다시 확인.

**실측 결과(15건 전체, §28 -- 관측만, 게이트/패치는 아직 만들지 않음)**:

| # | route | t | label | miss_frames | A→LOST gap(s) | A(dist/spd) | B(dist/spd/cluster) | B 생존(s/frames) |
|---|---|---|---|---|---|---|---|---|
| 1 | a3b3373495 | 319.06 | real_curve | 6 | 2.25 | 300.0/87.8 | 250.0/60.8/2 | 4.45/9 |
| 2 | a3b3373495 | 325.61 | real_curve | 6 | 6.29 | 230.0/85.6 | 60.0/82.7/3 | 2.20/5 |
| 3 | a3b3373495 | 536.76 | no_curve | 6 | 2.25 | 330.0/85.0 | 270.0/77.6/2 | 3.50/30 |
| 4 | a3b3373495 | 578.36 | unclear | 6 | 2.30 | 310.0/81.1 | 250.0/74.2/2 | 4.91/18 |
| 5 | a3b3373495 | 585.36 | weak_curve | 6 | 6.31 | 230.0/81.1 | 60.0/76.7/2 | 2.24/5 |
| 6 | a3b3373495 | 749.11 | real_curve | 6 | 6.25 | 150.0/54.6 | 40.0/53.5/9 | 2.25/5 |
| 7 | a3b3373495 | 1001.06 | no_curve | 6 | 2.30 | 190.0/39.3 | 150.0/46.6/2 | 3.40/68 |
| 8 | a3b3373495 | 1006.76 | no_curve | 6 | 2.30 | 90.0/37.8 | 50.0/34.3/2 | 2.60/52 |
| 9 | a3b3373495 | 1225.81 | no_curve | 6 | 2.30 | 220.0/65.8 | 170.0/53.4/3 | 2.45/9 |
| 10 | a3b3373495 | 1228.31 | no_curve | 6 | 2.30 | 170.0/64.8 | 120.0/54.6/3 | 2.30/6 |
| 11 | 01742d6c1c | 1804.36 | real_curve | 6 | 4.30 | 140.0/57.6 | 60.0/52.9/3 | 2.25/5 |
| 12 | 01742d6c1c | 2144.71 | real_curve | 6 | 4.30 | 150.0/59.1 | 80.0/54.3/2 | 2.25/5 |
| 13 | 01742d6c1c | 2232.82 | real_curve | 6 | 2.31 | 110.0/49.1 | 80.0/33.2/6 | 2.24/5 |
| 14 | c8d2619479 | 2517.51 | real_curve | 6 | 0.29 | 30.0/5.5 | 90.0/26.0/3 | 2.99/20 |
| 15 | bf794c0073 | 3961.06 | unclear | 6 | 2.30 | 90.0/49.9 | 60.0/42.1/6 | 3.65/73 |

**핵심 관찰**:

1. **`miss_frames_at_lost`가 15/15 전부 정확히 6**(=`ROUTE_APEX_MISS_TOLERANCE_FRAMES`
   그대로) -- "predicted<=0"(=passed 경로)이 개입해 lost가 조기 확정된
   사례는 없음. `_route_cluster_continuity_step()`의 lost 분기가
   설계대로 결정론적으로만 작동함을 재확인(300차 결과의 메커니즘 설명).
2. **B가 `--persistence-horizon`(5초) 끝까지 살아남은 사례 0/15.**
   평균 생존시간은 real_curve/weak_curve 그룹 2.61초, no_curve
   그룹 2.85초로 **오히려 "노이즈"로 라벨된 쪽이 근소하게 더 길게
   생존** -- B의 생존시간 단독으로는 "이게 진짜 재획득 대상이었는가"의
   판별 지표가 되지 못함(299차가 `cluster_size` 등 4개 지표로 얻은
   결론과 같은 한계 패턴 -- n=15로는 구분력 있는 단일 지표를 못 찾음).
3. **cluster_size도 이번 표본에서 방향성이 약함**: real_curve 평균
   3.8, no_curve 평균 2.4로 299차와 같은 약한 경향은 보이나, #6(real_curve,
   cluster=9, 생존 2.25초/5프레임)처럼 큰 예외가 존재.
4. **#1→#2, #4→#5 연쇄가 시간상으로 확인됨**: #2의 `A_last_matched_t`
   (319.32)가 #1의 LOST 시각(319.06) 0.26초 후이고, #4→#5도 마찬가지
   패턴(#4 LOST=578.36, #5 A_last=579.06, 0.7초 후) -- 즉 #1/#4의 B가
   곧바로 #2/#5의 A로 이어져 쓰였다. 다만 **거리/속도 값 자체는 상당히
   다름**(#1 B: 250m/60.8kph vs #2 A_last: 230m/85.6kph) -- 같은
   물리적 커브를 가리키는지, 그 사이 실제로 다른 candidate로 갈아탄
   것인지는 이 스크립트(거리·속도·continuity만 봄, GPS 미사용)로는
   구분 불가. **298차부터 이월된 "GPS 좌표 대조" 미해결 항목은 이번
   세션으로도 해소되지 않음** -- #9→#10(1225.81→1228.31, 2.5초 간격)도
   같은 패턴 후보로 추가.
5. **#14(c8d2619479)는 나머지 14건과 성격이 달라 보임**: A_last가
   `dist=30m, speed=5.5kph`로 apex 통과 직전(거의 정지에 가까운 저속
   목표)이었고, 0.29초 만에 LOST → B(`dist=90m, speed=26.0kph`, 훨씬
   먼/빠른 다음 목표)로 전환. "동일 커브 재탐색 실패"라기보다 "직전
   커브를 거의 다 통과한 뒤 다음 커브로 정상 전환"에 가까운 패턴으로
   보임(정성적 관찰, qcamera 라벨 `real_curve`와도 배치되지 않음).

**중요 한계(§10/§28, 반드시 인지)**: 이 스크립트는 거리/속도/continuity
상태만 관측하며 GPS 좌표를 다루지 않는다. 따라서 "①진짜 동일 커브를
잠깐 놓친 seamless switch 대상"과 "②GPS/map noise로 엉뚱한 candidate를
새로 잡은 경우"를 이 결과만으로 확정할 수 없다(원 질문에 대한 완전한
답은 아직 아님). 15건 중 어느 것도 5초 생존을 완주하지 못했다는 사실은
"seamless switch(강제 RELEASE 제거)"를 적용해도 그 결과로 얻는 ACTIVE
트랙 자체가 오래가지 않을 수 있음을 시사하나, 이 역시 open-loop
관측이며(300차와 동일 한계) 실제 폐루프 거동을 증명하지 않는다.

**검증**:
- 정적 분석: 완료(`py_compile`/`ast.parse` PASS)
- 로그 검증: 완료(route1~4 실 corpus, 15/15 이벤트 재현 -- 300차와
  route별 건수까지 정확히 일치하는 정합성 자체검증 포함)
- 시뮬레이션: 해당 없음(open-loop 재생, 위 한계 참고)
- 실차 검증: 해당 없음(ANALYSIS_ONLY, `ryu` 코드 무변경, §29)

**미확인/남은 것**:
- #1/#2, #4/#5, (신규 후보)#9/#10의 GPS 좌표 대조로 "동일 물리적 커브
  재탐색 2회" 가설 확정(298차부터 이월, 이번 세션도 미해결 -- 이
  스크립트는 GPS 컬럼을 다루지 않음, 302차 후보 작업).
- A→LOST 경과시간이 2.25~6.31초로 편차가 큰 이유(naviPaths 프레임
  도착 간격이 `ROUTE_SPEED_LOOP_DT=0.05s` 가정과 다를 수 있음, 원인
  미추적).
- B 생존시간/cluster_size 모두 real/noise 구분력이 약하다는 결과가
  n=15를 넘는 표본에서도 재현되는지(299차와 동일한 표본 한계).
- 코드 변경(`carrot_man.py`) 여전히 미착수 -- 300차와 동일하게 이번
  세션도 관측만, 패치는 계속 보류가 맞다는 판단 유지.
