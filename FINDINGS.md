...
- **코드 원인**: `carrot_man.py`의 `vturn_speed()`는 모델이 예측한 전방
  ...
  구조적 문제.
- **패치 (`carrot_man.py`, commit `1827c1e`, 아직 ryu에 push 안 됨 —
  patch 파일로 전달, 사용자가 `git am`으로 적용 예정)**:
  ...
  불필요.
- **미해결/다음 단계**:
  1. **실차 검증 없음** — 이번 세션은 신규 로그 업로드 없이 기존
     FINDINGS 분석 + 사용자 체감 보고 기반으로 진행. 적용 후 유사
     조여드는 커브 구간 재주행 로그로 aEgo/운전자 개입 여부 재확인 필요.
