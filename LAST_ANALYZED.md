...
2026-09-03 (222차, c3-ms-dev): 신규 커밋 없음(분석 세션, repo HEAD
`7519a3a`=221차와 동일). 221차 최초 실차 검증 — 17세그 로그(20,399행,
t=16:20~16:37) 재추출로 (1) routeOutSpeed 텔레메트리가 ceiling 적용
이전 raw 값임을 확인(제어 영향 없음), (2) vEgo ceiling이 정지→재출발
전이구간(~8.3초)에서 172/173차 램프리미터 하한에 의해 무력화되는 반례
발견 및 정량화(liveRouteSpeed가 vEgo보다 최대 55kph 초과 유지). 221차의
"out_speed<=vEgo 항상 보장" 증명은 정상주행 중에는 유효하나 이 전이구간
