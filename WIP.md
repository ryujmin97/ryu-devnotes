...
vRel 부호 단독 게이트는 근거 약함)을 이어받아, "danger override는 항상
최우선 유지"를 전제로 세그4-1처럼 실제로는 위험(거리 부족)하지만
"리드가 벌어지는 중"이라 체감상 과잉감속으로 느껴지는 케이스의 승차감만
개선하는 3개 대안을 설계·비교했음(구현/시뮬레이션 착수 전, 순수 설계
논의 단계).

**용어 정정(중요)**: 이 논의 중 "핸드오버"(MPC가 추종하던 리드 대상이
바뀌는 순간, 예: 82m→9.9m/47.7m→18.6m처럼 dRel이 한 프레임 만에 뚝
떨어지는 현상)라는 표현을 썼으나, 61차 devnotes에서 이미 동일 현상을
"discontinuity(불연속 급락)"으로 명명해 사용 중임을 확인 — **앞으로는
"핸드오버" 대신 "discontinuity"로 용어 통일**(`DREL_DISCONTINUITY_DROP_
THRESH`, 61차 방안C와 동일 개념).

**대안 3개 (전부 danger override(TTC<=2.5s)는 무조건 우회 없이 실측값
그대로 최우선 작동 전제)**:
1. **방안F — x_lead(MPC 목표거리 입력) 블렌딩**: 기존 discontinuity
   감지(61차 방안C 재사용)로 점프 감지 시, MPC에 넣는 x_lead를 점프
   전 값→실제값으로 0.5~1.0s에 걸쳐 블렌딩. 세그4-1 원인(입력값 자체의
   순간 점프)을 가장 직접 겨냥하나 구현·검증 범위가 가장 큼. 리스크:
   danger 게이트가 이 블렌딩을 즉시 무시하고 실측값을 쓰게 하는 회로
   필수(안 그러면 진짜 위험한 급접근에서도 초반 여유거리로 오인).
2. **방안G — 핸드오버(discontinuity) 직후 저크비용(a_change_cost)
   한시적 강화**: 목표거리 크기는 안 건드리고 도달 속도만 늦춤(MPC
   jerk 비용 가중치 일시 상향). 코드 변경이 가장 작고(파라미터 1개),
   MPC 입력 자체를 조작하지 않아 부작용 범위가 좁음. 단 acados가
   "완만 vs 안전" 트레이드오프를 내부적으로 어떻게 저울질하는지 튜닝
   실험 필요.
3. **방안H — vRel 부호/추세를 이진 게이트가 아닌 연속 가중치로 사용**:
   기존 `margin_accel_weight`/`ttc_accel_weight`처럼 vRel이 opening
   이면서 커질수록 반응을 서서히 낮추는 `vrel_w`를 추가해 `min(dist_w,
   ttc_w, vrel_w)`로 결합. 세그4-1(벌어지는 중+거리부족)은 완전 무시
   대신 완화만, 세그7 초기(계속 closing)는 vrel_w=1로 기존과 동일 —
   사용자 원안 취지를 가장 직접 반영하나 새 weight 임계값 튜닝·검증
   기간이 김.

**추천 순서(논의 시점 기준)**: G(가장 가볍고 리스크 작음, 우선 시도
후보) > F(가장 근본적, 범위 큼) > H(원안 취지 가장 충실, 튜닝 필요).
**다음 방향은 사용자 결정 대기 — 세 방안 중 어느 것을 먼저
프로토타입할지 아직 미확정.**
...
## 65차 (체크포인트 — 세그4/세그7 승차감 이슈 jerk/TTC 실측, 세그4-2 재분류)

**배경**: 64차 로그(동일 14세그) 중 사용자가 직접 지목한 "차선변경 후
변경차로 리드에 급감속후 원복" 체감 3건(세그4 첫/둘째, 세그7)을 프레임
단위로 재검토. 세그4 둘째는 사용자가 "우회전 상황"이라고 정정 —
차선변경 로직과 무관하므로 **분석 대상에서 제외**(회전 예고등/교차로
진입 시나리오, `leadStatus` 깜빡임과 브레이크 개입 모두 회전 자체에서
기인할 가능성이 높아 60차2 lane-change suppression 부작용 후보에서
빠짐).

남은 두 건(세그4-1, 세그7)에 대해 "차선변경 조건으로 감속 완화"
제안이 나와, 코드 검토 + raw CSV 재추출(jerk/TTC 포함)로 먼저 실증
검토함.

**코드 검토 결과 (완화 레버 존재 여부)**:
- `d7c2b0d`(60차 계속, vLead 직접보정 유예)/`4ea63c3`(61차 계속 방안C,
  discontinuity 감지)는 전부 "v_lead 직접보정 유예"만 다룸 — 실제
  거리(dRel)가 진짜 좁혀지거나 점프한 이번 두 건과는 애초에 적용 대상이
  다름(이미 결론 남).
- `dynamicTFollowLC`(기본값 100=1.0, 사실상 비활성)+hold/blend 로직은
  "차선변경 *중* t_follow를 좁혀 민첩하게 완료"가 목적이라 방향이
  반대. 종료 후 ease-back 안전장치가 있으나 기본 설정에선 무의미(t_follow
  점프 자체가 없음).
- `LEAD_ACQ_RAMP_TIME=5.0s` 신규리드 proactive floor는 초반 0→시간에
  따라 강해지는 구조라, 초반 급감속의 "원인"이 아니라 이미 초반을
  약하게 만드는 안전장치임을 확인.
- 결론: 이번 두 건에 적용할 기존 "차선변경 조건부 완화" 레버가 없음
  — MPC 코어(desired_follow_distance vs 실측 lead_0_obstacle solve)의
  순수 반응.

**실측 데이터 (route14seg.csv 재추출, jerk/TTC 계산)**:
- **세그4-1 (t=336~341)**: 최고감속 -1.59m/s²(재계산, 기존 보고
  -1.49와 근사치 차이), max\|jerk\|=4.52 m/s³, min TTC=3.55s(danger
  2.5s와는 여유, caution 6.0s는 넘음). **t=338.32→338.37 사이 dRel이
  17.0m→9.9m로 프레임 점프(discontinuity), 동시에 leadVRel도
  -4.79→+1.5로 부호 반전(closing→opening)** — 신규 리드가 등록된
  순간부터 이미 "벌어지는 중"이었는데도 그 직후 -1.59까지 감속.
  **`desired_follow_distance` 역산 검증**: 점프 시점 v_ego≈13.49m/s,
  v_lead≈14.99m/s(=v_ego+vRel)로 계산하면 desired_distance≈17.0m —
  실측 dRel(9.9m)보다 여전히 ~7m 부족. 즉 리드가 이미 벌어지고 있다는
  사실을 공식 자체가 이미 반영(`get_stopped_equivalence_factor(v_lead)`
  로 목표거리를 깎아줌)했는데도 여전히 목표거리 미달 — **"리드가
  가속중이면 무시" 규칙을 여기 그대로 적용하면 실제로는 아직 위험할
  정도로 가까운(9.9m, 약 0.73s 거리) 상황에서 반응을 꺼버리는 셈이라
  안전 관점에서 위험**. 짧은 감속 자체는 버그가 아니라 정상적인
  거리회복 로직으로 잠정 판단.
- **세그7 초기 discontinuity (t=519.82, dRel 47.7m→18.6m)**: 이 순간
  leadVRel은 -3.31→-0.75로 부호는 그대로 closing 유지(벌어지지
  않음). 이후로도 계속 closing 지속, **min TTC=2.47s로 danger
  임계(2.5s) 바로 아래** — 실제 위험구간에 걸쳐 있었던 정탐. 사용자
  제안(가속중이면 무시)은 이 케이스엔 애초에 해당 안 됨(계속 감속중
  이었음), 완화 대상 아님.
- **세그7 후반 신규 발견 (t=526~528, [RISK_IDENTIFIED, NEEDS_VALIDATION])**:
  초기 discontinuity와는 별개로, 저속(dRel 5~7m 유지) 근접추종 구간에서
  leadVRel이 -1.4~+1.0 사이를 반복 진동하며 aEgo가 재차 -2.53m/s²까지
  감속(이번 로그 중 세그7에서 가장 강한 감속, max\|jerk\|=6.36 m/s³).
  discontinuity/차선변경과 무관한 별도 패턴(저속 근접 gap 오실레이션
  후보) — 다음 세션 조사 대상으로 신규 등록.

**결론**: "차선변경 중이면 상대속도(vRel)로 게이트"라는 제안은 세그4-1/
세그7 초기 케이스 둘 다에서 실증적 근거가 약함(세그4-1은 이미
공식이 vRel을 반영하고도 거리 부족, 세그7 초기는 애초에 계속
closing). vRel 부호만으로 게이트하면 절대거리가 위험한데 "잠깐
벌어지는 중"인 상황까지 억제해버릴 위험. **재조사 방향 전환**:
세그4-1/세그7 초기는 보류(정상 동작으로 잠정 결론), 대신 세그7
후반 신규 오실레이션 패턴을 다음 최우선 조사 대상으로 전환.

**코드 변경 없음(devnotes만 갱신, 체크포인트)**.

**세션 종료 아님 — 체크포인트 저장.**

## 64차 (체크포인트 — 방안E 첫 실차검증, cutin 2건 정상 확인, 회귀항목 일부 미확정)

**배경**: 63차 계속10에서 대기 중이던 방안E(`e6a00aea`) 실차 검증의
첫 로그(2026-08-25 07:07~07:19, 14세그) 도착. 사용자 제보: 세그4/세그10
부근 cutin 2건 발생, 급감속 없음.

**검증 결과(상세는 FINDINGS.md "[64차, 실차 검증]" 항목 참고)**:
- 폴더 `--4`(사용자가 말한 세그4 직후 구간)에서 cutin 2건 모두
  qcamera로 실제 확인, aEgo 최저 -0.81m/s² 수준으로만 반응 —
  63차 계속9/10이 다루던 "끼어든 차가 더 빠른 경우" + "discontinuity성
  dRel 점프" 둘 다 실차에서 과잉감속 없이 매끄럽게 처리됨(최대
  |jerk|=9.02 m/s³, 톱니 패턴 없음).
- 폴더 `--10`(세그10, 접미사 일치)에서는 신호대기 차량 행렬에 급접근 →
  aEgo -3.77~-3.95m/s²로 이 로그 중 가장 강하게 반응했으나 단일 피크로
  단조 감속, 안전하게 완전정차(정탐, qcamera 확인). 톱니/재가속-재감속
  반복 없음.
- 사용자 제보("cutin 2건, 급감속 없음")와 정확히 일치.

**미확정으로 남은 항목(63차 계속10 WIP에서 이월, 변경 없음)**:
1. **진짜 위험한 cutin(leadVLead도 위험을 인지하는 경우) 회귀 검증** —
   이번 로그엔 해당 시나리오 표본 없음, 여전히 필요.
2. **일반 원거리 접근에서 58차1/26차 원래 효과 유지 여부** — 이번
   로그로 확인 안 됨, 여전히 필요.
3. `VISION_RATE_REF_MARGIN=5.0` 값 자체는 여전히 seg3 1개 사례 기준
   설계 추정치 — 다른 route로 마진 적절성 교차검증 여지 계속 유효.

**다음(최우선)**: 위 1/2/3번 중 하나를 커버하는 로그 확보 시 이어서
검증. 특히 (1) 실제 위험한 cutin(반응 지연 여부), (2) 원거리(dRel
85~120m)에서 반응 강도가 예전과 동일한지가 남은 핵심 확인 사항.

**코드 변경 없음(devnotes만 갱신)**.
...
2. ~~이번 통합 코드로 seg3/seg14 CSV 재생검증 한 번 더~~ → **완료**
   (아래 "63차 계속10 (b)" 참고, `git am`/push와 별개로 재확인).
3. **실차 검증(최우선, 변함없음)**: (a) cutin(끼어든 차가 더 빠른 경우)
   재현 시 레이더 락온 후처럼 정상 반응하는지, (b) **회귀 검증 필수** —
   진짜 위험한 cutin(leadVLead도 위험을 인지하는 경우)에서 반응이
   지연되지 않는지, (c) 일반 원거리 접근 상황에서 58차1/26차 원래
   효과가 그대로 유지되는지, (d) **[신규]** seg14류 discontinuity
   반복 재트리거 구간에서 frac_rate가 0→0.88→0.23 식으로 톱니치는
   패턴이 실차에서 저크(끊김)로 체감되는지 — 아래 (b) 참고.
4. `VISION_RATE_REF_MARGIN=5.0` 값 자체는 seg3 1개 사례 기준 설계
   추정치 — 다른 route로 마진 적절성 추가 교차검증 여지 있음.

**세션 종료 아님 — 중단지점 저장.**

## 63차 계속10 (b) (체크포인트 — 실제 통합 코드로 seg3/seg14 재생검증 완료, drift 없음 확인 + 신규 관찰 1건)

**배경**: 위 (a) WIP "다음(최우선) 2번" 수행 — `long_mpc.py`(commit
`e6a00aea`)를 재구현하지 않고 실제 `update()` 778~921줄을 문자 그대로
`exec()`으로 편입한 재생 하네스(`work/verify63/replay_integrated_e_seg3_seg14.py`)
로 seg3/seg14(사용자 재업로드 zip에서 원본 rlog 재추출) 전체를 재생.

**결과**: seg3 최대 frac_rate=0.205(63차 계속10 로직단위 결론 0.209와
거의 일치), seg14 aEgo 최저점(t=925.148) frac_rate=0.671(63차 계속9
로직단위 결론 0.678과 거의 일치) — **로직단위 시뮬레이션과 실제 통합
코드 사이 유의미한 drift 없음 확정.**

**신규 관찰(중요)**: seg14 discontinuity 반복 재트리거 구간
(t=923.10~923.50, 7회) 동안 `_lead_acq_timer` 리셋->재축적이 반복되며
frac_rate가 실제로는 **0→0.88(t=924.25)→0.23(t=924.70)→0.85(t=925.30)
식으로 여러 번 오르내리는 톱니 패턴**임을 확인 — 이전 로직단위 검증은
"aEgo 최저점 한 시점"만 봐서 "완만한 램프"로 결론냈으나 전체 구간은
그보다 복잡함. UNPATCHED(즉시 1.0 포화 유지) 대비 완화된 것은 여전히
맞지만, 이 톱니 패턴 자체가 실차에서 저크로 체감될 가능성은 별도
확인 필요(상세는 FINDINGS.md "[63차 계속10 (b)]" 항목 참고).

**코드 변경 없음(ryu 미변경, devnotes만 갱신)**.

**다음(최우선, 위 3번과 동일, 변경 없음)**: 실차 검증 — (a)(b)(c) 기존
항목 + (d) 톱니 패턴 체감 여부 추가 관찰.
