...
**판단**: 아직 "이거다" 할 붕끗 사례를 확정 못함 — TTC danger 이벤트가
route당 1건뿐으로 표본이 매우 작고, 그마저도 설계 전제(완만한
closing)와 정확히 일치하는지 애매함. **다음 단계로 (a)
`congestion_stop_launch_lurch_scan()` 신규 함수를 WIP 설계 그대로
구현(정체 상태 추적 deque + TTC danger 이벤트 겹침 판정)해서 후보를
체계적으로 좁히거나, (b) 지금 나온 2건부터 바로 qcamera 프레임
대조로 실제 붕끗 체감 여부를 영상 확인하는 두 갈래 중 사용자 결정
필요 — 다음 메시지에서 사용자에게 제안.**

**코드 변경 없음(devnotes toolkit도 아직 미변경, 분석 스크립트는
인라인 실행만)**. ryu 코드도 미변경(2번 구현은 여전히 착수 전).
