...
그대로 재사용해 112차 threshold 패치(-1.8→-2.5)의 실측 검증 범위를
확장. 신규 스크립트 작성 없음(README 우선 확인 원칙 준수).

**핵심 발견**:
1. **smooth(1028): 완전 PASS.** 구threshold(-1.8) 기준 26프레임(1.247s
   조기구간) 발동 예상되던 게 신threshold(-2.5)에서는 0프레임 —
   112차 패치가 이 라우트의 저속게이트 오탐을 실측으로 완전히 제거함을
   확인. "급감없이 부드러움"이라는 사용자 체감과 일치.
