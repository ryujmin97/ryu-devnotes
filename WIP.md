...
5. (신규, 260819-5 세션에서 발견, 우선순위 높음 — 사용자 확인 필요)
   MAX_SEGMENTS_PER_ROUTE=20 패치가 원격 브랜치(f7b154638cf2)엔
   반영됐지만, 260819-5 로그 시점 실기기는 여전히 40개 단위로
   rotate하는 것으로 보임. **사용자에게 확인 필요: 이 로그(8/19
   12:41~13:00) 기록 당시 comma 3X가 f7b154638cf2 이후로 재빌드/
   재부팅됐었는지?** 재부팅 안 됐던 거라면 다음 실주행부터 재확인,
   재부팅했는데도 40개로 이어졌다면 rotate 로직 자체 재검토 필요.
