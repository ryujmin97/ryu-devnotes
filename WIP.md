...
- 저장 시각: 2026-08-21 (20차 계속, 사용자 "저장" 요청 — 도구 1~4/5
  실전 검증 + push 완료 상태에서 체크포인트. **다음 세션 최우선은
  seg6/seg12 qcamera 프레임 시각 검증**, 아래 "다음 세션에서 이어갈
  것" 1번 참고 — 세그먼트 폴더 재업로드 필요.)
...
1. **[신규, 최우선] extract_dashcam_frames.py로 seg6/seg12 시각 검증** —
   260821 로그의 seg6(`20260821_063354_...--6`, t≈443.5~454, 급점프
   8건이지만 aEgo 무변화 — 노이즈였는지 화면 확인)와 seg12
   (`20260821_063954_...--12`, t≈797.8~799.5, aEgo -1.9m/s² 반응 —
   진짜 리드 접근이었는지 화면 확인)를 qcamera 프레임 대조로 검증.
   **원본 zip을 지운 상태라 사용자가 이 두 세그먼트 폴더만 재업로드
   해야 진행 가능** — 다음 세션 시작 시 이 항목부터 확인.
2. **road<->vturn / road<->route 쌍의 min() 히스테리시스 설계 착수**
   — 실측 규모(107건/34건)가 확인됐으니 이제 실제 설계 단계로.
   9~13차 model 게이팅 패치와 유사한 접근(추세 기반 배제) 검토,
   또는 다른 방식 가능.
3. **curve_lead_dRel_jump_events would_trigger 휴리스틱 개선** —
   점프 이후 N프레임 동안 dRel/vRel이 물리적으로 일관되게 접근하는지
   후속 체크 추가 설계. (1번 시각 검증 결과가 이 설계에 참고자료가 됨)
4. 도구 5/5는 사실상 이번 세션에서 첫 실행 완료로 판단되나, 진짜
