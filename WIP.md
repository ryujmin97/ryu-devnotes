...
- 저장 시각: 2026-08-22 (37차 계속 3 — 패치 c3-ms-dev/c3-ms-test 양쪽
  적용+push 완료, 실차 검증 전 단계에서 체크포인트. 다음 세션은 아래
  "37차 계속 3 (진행 중)" 섹션의 "다음 단계"부터 이어감)
...
- **다음 단계(최우선)**:
  1. ~~사용자가 `git am`으로 `C:\dev\ryu`(c3-ms-dev)에 패치 적용 + push~~ →
     **완료**. `git am`이 처음엔 `c3-ms-test`(당시 체크아웃된 브랜치)에
     ...
     없이 적용은 됐으나, 원래 목표인 `c3-ms-dev`가 아니었음. 34차
     A/B 비교 오염 방지를 위해 **양쪽 브랜치 모두에 반영 필요**로
     판단해 `c3-ms-dev`에 `git cherry-pick b5a1209`(→ `21effa1`) +
     양쪽 `git push origin` 완료 확인. 최종:
  ...
  3. **[남음] 회귀 검증 필수**: `SCC_FALLBACK_DPATH_GATE=2.0m`가 정상적인
     ...
     유지되는지 확인.
  4. 검증 통과 후 → 34차(c3-ms-dev vs c3-ms-test A/B 실차 비교, 아직
     미착수) 원래 과제로 복귀 — 이번 37차 패치가 양쪽에 동일하게
     반영됐으므로 A/B 비교 축(클램프+중앙값 필터 지연)엔 영향 없음.
