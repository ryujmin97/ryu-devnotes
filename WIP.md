...
- 저장 시각: 2026-08-22 (38차 — "앞차_민감" 로그 분석/패치 작성 완료,
  실차 검증 전 단계에서 체크포인트. 아래 "38차" 섹션의 "다음 단계"부터
  이어감. 37차 항목은 여전히 실차 검증 대기 중으로 아래 그대로 유지.)

## 38차 (진행 중) — 앞차 가속도 민감반응, TTC 게이트 패치 작성 완료, 실차 검증 대기
- 상세 원인/조치는 FINDINGS.md 38차 항목 참고. 요약: 거리비율 기반
  `MARGIN_ACCEL_GATE`가 고속 구간에서 사실상 상시 무감쇠였던 사각지대를
  `ttc_accel_weight()` 신설 + `min()` 결합으로 보완.
- 패치 `0001-long_mpc-TTC-aLead-damping.patch` (base `21effa1`) 생성,
  `git am` 검증(임시 클론) + ast 문법 통과. **아직 Master 로컬(C:\dev\ryu)에
  미적용.**
- **다음 단계(최우선)**:
  1. Master가 `git am C:\dev\patch\0001-long_mpc-TTC-aLead-damping.patch`
     적용 후 `git push` (base는 `c3-ms-dev` HEAD `21effa1`이어야 충돌 없음 —
     그 사이 다른 patch가 먼저 push됐다면 rebase 필요).
  2. 실차 검증: 이번 로그 재현 상황(안전거리+완만한 가감속)에서 승차감
     개선 체감 + **회귀 검증**(실제 위험 cut-in/급접근 시 반응 지연 없는지,
     이번 로그엔 위험 프레임 표본이 없어 미검증).
  3. `LEAD_ACCEL_TTC_GATE_FULL=12.0s` 값 실차 기준 재조정 필요할 수 있음.
