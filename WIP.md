...
- 저장 시각: 2026-08-21 (32차 — 곡선 오탐 검증용 `곡선_로그.zip`
  3개 세그 분석. 결과: 문턱 재설계(-2.2/-5.0) 자체가 새로 만드는
  오탐 리스크는 확인 안 됨(발견된 글리치들은 기존에 이미 특성화한
  패턴과 동일 범주). **다만 별개의 더 심각한 사전 존재 결함을
  발견**: `203f99d429--8`에서 지속적 곡선(2.7초) 동안 dRel이
  vRel과 무관하게 물리적으로 불가능한 속도로 드리프트(93→38.6m인데
  vRel은 -1.7~-3.4m/s뿐, 6~10배 괴리) — **기존 baseline
  문턱(-5.5/-10.0)에서도 이미 DANGER까지 도달**. 문턱을 낮췄기
  때문이 아니라 원래 있던 결함. FINDINGS.md 32차, PARAMS_REGISTRY
  신규 이슈 행으로 기록. **문턱 재설계 패치는 이 발견 때문에
  철회할 이유 없음 — 다만 새 결함을 어떻게 다룰지 사용자 판단
  필요한 상태에서 저장.**)

## 다음 세션(또는 다음 메시지)에서 이어갈 것 (32차 기준, 최우선)
0. **선행 확인**: 26차 patch(`5cc0900`)는 로컬 `C:\dev\ryu`에
   `git am` 이미 적용됨/origin 미push (29차 확인). 새 문턱 패치는
   그 위에 적층.
1. **사용자 확인 필요 (두 갈래)**:
   (a) 30/31차 문턱 재설계(`GATE_CAUTION=-2.2`/`GATE_DANGER=-5.0`)
       는 32차 발견과 무관하게 그대로 패치 진행해도 되는지 —
       근거는 이미 충분(FINDINGS 31차). "진행" 답변이면 바로 2번.
   (b) 32차에서 새로 발견한 "지속적 곡선 dRel-vRel 불일치 드리프트"
       (`203f99d429--8` 사례)를 이번 세션에서 같이 다룰지, 별도
       과제로 미룰지. 다루기로 하면: 대시캠 프레임 대조로 원인
       확인(seg8 t=6579.9~6582.4) → consistency-check 설계(N프레임
       dRel 변화량 vs vRel 적분값 비교) → 별도 패치. 이건 문턱
       패치보다 범위가 크므로 별도 세션으로 미루는 것을 권장했었음.
2. 확정되면 `long_mpc.py`의 `VISION_CLOSING_RATE_GATE_CAUTION=-2.2`,
   `VISION_CLOSING_RATE_GATE_DANGER=-5.0`으로 패치 → `git
   format-patch` → `C:\dev\patch\` 전달 → `git am` 안내(26차 patch
   위에 적층 전제).
3. 원본 CSV(`/home/claude/work/seg7_12.csv`,
   `/home/claude/work/seg19.csv`, `/home/claude/work/extra.csv`,
   `/home/claude/work/curve2.csv`)는 컨테이너 로컬이라 이 세션
   끝나면 사라짐 — 재검증 필요 시 zip 재업로드 필요
   (`앞차_카메라_인식_미감속.zip`,
   `20260821_115242_000002e5--1723e8b850--19.zip`,
   `카메라_인식_추가.zip`, `곡선_로그.zip`, 전부 이미 한 번
   올렸던 파일).
4. 완료 후 26차 WIP 원래 다음 과제(아래 26차 섹션 "다음 세션에서
   이어갈 것")로 복귀.

## 32차 (완료, 위 최우선 과제로 이어짐) — 곡선 오탐 검증 중 문턱과
