...
- 저장 시각: 2026-08-20 (8차, vturn 지평선 2차 확대(6.5s→8.0s) 패치
  구현 완료 후 체크포인트)
- HEAD (c3-ms-dev): **로컬 `c4e3093`** (아직 GitHub `ryu`에 push 안
  됨 — patch 파일 `/mnt/user-data/outputs/0001-carrot_man-vturn-6.5s-8.0s.patch`
  로 전달, 사용자가 `4c15987` 위에 `git am` 적용 예정). GitHub 반영된
  최신은 여전히 `4c15987`.

## 8차 완료분 — vturn 지평선 2차 확대 6.5s → 8.0s (patch 전달, 미push)
- 1차(6.5s, `4c15987`) push 확인 직후, 사용자가 근거 사례(260819-7
  seg6, 조임 지속시간 8.6s)에 더 가깝게 맞추기 위해 8.0s로 재확대 요청.
- 사용자 질문에 답변: `vturn_lookahead_horizon_s`는 "감속에 걸리는
  시간"이 아니라 "커브 후보를 몇 초 앞까지 스캔할지" 지평선이고, 방지턱과
  동일한 거리기반 서서히-감속(v_i²=v_f²+2ad) 프로파일 자체는
  `vturn_decel_rate`(1.2 m/s²)/`vturn_safe_time`(1.0s)이 담당한다는 점을
  구분해서 설명함. 이번 변경은 전자(지평선)만 대상.
- 패치 구현 완료 (`carrot_man.py`, commit `c4e3093`): `vturn_lookahead_
  horizon_s` 6.5 → 8.0. 문법 체크(`py_compile`) 통과.
- push 대상 devnotes: `FINDINGS.md`, `PARAMS_REGISTRY.md`, `WIP.md`.
- ryu 패치: `/mnt/user-data/outputs/0001-carrot_man-vturn-6.5s-8.0s.patch`
  (`4c15987` 위에 `git am` 적용).

## 7차 완료분 — vturn 커브 사전감속 지평선 1차 확대 (push 완료, `4c15987`)
- 사용자 요청: "곡선 진입 전 사전 감속시간이 부족해서 충분히 감속이
  안된 상태에서 곡선에 진입하다보니 곡선내에서 급감속이 있었음. 원인
  찾고 vturn_lookahead_horizon_s를 2초 정도 더 늘려서 해결" (신규
  로그 업로드 없이 기존 FINDINGS.md 분석 + 사용자 체감 재기술 기반).
- **기존 근거 재확인**: `[INVESTIGATING] 조여드는 커브 중간에 vturn
  감속 진행 중 운전자 브레이크 개입` (260819-7 seg6, 표본 1건) — 곡률이
  8.6초에 걸쳐 서서히 증가하는 커브에서 vturn 감속률 자체는 매끈했지만
  aEgo -3.41m/s² 도달 직후 운전자가 추가 브레이크 개입, 이때도 곡률은
  계속 증가 중이었음. 사용자 이번 보고와 정확히 일치하는 패턴.
- **코드 원인 확정**: `carrot_man.py` vturn_speed()가 모델 예측 궤적 중
  `vturn_lookahead_horizon_s`(기존 4.5s) 이내 지점만 보고 그중 가장
  엄격한 필요속도를 취함 → 정점까지 걸리는 시간이 지평선보다 긴 커브에서는
...
1. **`c4e3093` patch를 `git am` 적용 + push 확인부터** (`4c15987` 위에
   쌓는 패치). 완료되면 이 항목 제거하고 WIP HEAD를 push된 새 커밋
   해시로 갱신.
2. **vturn 지평선 실차 검증** (1차 6.5s, 2차 8.0s 모두 미검증) — 패치
   적용 후 유사 조여드는 커브 구간 재주행, aEgo 프로파일 및 운전자
   개입 여부 확인.
3. **8.0s < 8.6s 한계 재확인** — 근소한 차이(0.6s)만 남음. 검증 결과
   부족하면 추가 미세 조정 또는 vturn_decel_rate/vturn_safe_time 조정
   검토(우선순위 낮음).
4. 지평선 확대 부작용 확인 — 지평선이 4.5s→8.0s로 거의 2배 늘어난 만큼,
