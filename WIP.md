...
교차검증 중 (b)를 먼저 진행하도록 지시(세그 선택: seg3, "cutin 급감속"
클립 — r1-3, 62~63차에서 이미 "radar 조기 락온, 방안C만으로 충분"으로
검증된 케이스).

**결과 요약(상세는 FINDINGS.md "[63차 계속10]" 항목 참고)**: seg3 raw
CSV로 `long_mpc.py` process_lead() 로직을 그대로 복제한 재생검증
(`work/route63c/sim_e_seg3.py`) 결과, **REF_MARGIN=5.0이 seg3의 진짜
cutin danger 응답까지 심하게 억제함을 확인 — 방안E 현재 설계 REJECTED.**

핵심: aEgo 최저치 직전 vision-only 구간(radar 락온 t=260.997 이전)에서
PATCHED_C는 frac_rate 1.00 도달하는데 PATCHED_E는 최대 0.209에 그침.
근본원인은 이 구간에서 **leadVRel(모델 자체 추정)이 +1~+3.2m/s(opening)
로 실제 위험을 완전히 놓치는데**(실측 dRel은 84m→20m/1.9s로 실제
평균 -32.8m/s 급접근, dPath 시종 -0.08~-0.59m로 동일차로 확인) 방안E의
`ref_rate`가 바로 그 실패하는 leadVLead에서 계산돼, **안전장치
(vision_dRel_rate)가 보완하려는 대상 신호(leadVLead)에 안전장치 자신의
민감도가 종속되는 구조적 모순**이 드러남. 이 구간은 frac_ttc(leadVRel
기반)도 ttc_now=999로 위험을 못 잡아 frac_rate가 사실상 유일한 안전망
이었던 것도 확인 — 억제 리스크가 더 크게 다가옴.

**다음(사용자 결정 대기, 최우선)**: (a) 방안E를 이 설계 그대로 구현하지
않고 보류, (b) ref_rate 산정 기준을 leadVLead가 아닌 다른 신호로
재설계, (c) 방안E 자체 폐기하고 다른 방향(예: dPath는 이미 63차
계속4~6에서 확인 완료 — seg14는 인접차선 아님 결론남) 검토. 사용자
방향 결정 시 그에 따라 (a) 착수 여부/방식 재조정.

**코드 변경 없음(ryu 미변경, devnotes만 갱신)**. `work/route63c/seg3.csv`,
`work/route63c/sim_e_seg3.py`(스크래치, toolkit 미편입).

**세션 종료 아님 — 체크포인트 저장, 사용자 응답 대기.**

## 63차 계속9 (체크포인트 — 4단계 완료: 방안 E 설계 + 로직단위 시뮬레이션 검증, 실제 코드 패치는 다음 단계)

**4단계 완료**: 3단계 결론(실제 cut-in이지만 raw dRel 미분값 자체가
물리적으로 불가능한 크기로 튐)을 바탕으로 방안 E 설계.

**핵심 근거(신규 발견)**: seg14 raw_rate(클램프 전)와 `vEgo-leadVLead`
(모델이 이미 추정한 상대속도 기반 참고 closing rate)를 프레임별로
직접 비교 — **raw_rate는 -235~-6 m/s로 요동치는데 반해
`vEgo-vLead` 기반 참고치는 시종일관 -0.5~-3.2 m/s로 훨씬 안정적**.
즉 raw dRel 미분(거리값 자체의 프레임간 변화)은 신뢰도가 낮고,
`leadVLead`(모델이 이미 산출한 속도 추정치, 58차1의 v_lead 직접보정이
참조하는 것과 동일 계열 신호)가 훨씬 안정적인 기준선 역할을 할 수
있음이 이번에 정량 확인됨.

**설계(방안 E)**: 기존 `raw_rate_clamped = max(raw_rate,
-VISION_CLOSING_RATE_MAX_PLAUSIBLE)` 절대값 클램프에 더해, **`vEgo -
leadVLead` 기반 참고 closing rate 대비 상대적 타당성 클램프**를 추가:
```
ref_rate = -(vEgo - lead.vLeadK)          # 모델 기반 참고 closing rate
plausible_min = ref_rate - VISION_RATE_REF_MARGIN   # 신규 상수, 예: 5.0 m/s
raw_rate_clamped = max(raw_rate, -VISION_CLOSING_RATE_MAX_PLAUSIBLE, plausible_min)
```
DANGER override(TTC<=2.5s, `process_lead()`의 별도 즉시반응 경로)는
이 클램프와 완전히 무관하게 항상 그대로 유지 — 진짜 급접근/급정지는
이 변경과 상관없이 즉시 반응.

**로직단위 시뮬레이션 검증** (`work/route63b/sim_e.py`, seg14 raw
데이터로 실측 재현, `REF_MARGIN=5.0` 기준): 기존 방식은 t=922.75부터
`frac_rate`가 즉시 1.0으로 포화돼 t=923.65(aEgo=-0.01)부터 925.30까지
계속 1.0 유지. **방안E 적용 시 t=923.25부터 서서히 상승, aEgo
최저점(t=925.148)에서 frac_rate=0.678(1.0 아님)** — 급격한 계단식
포화 대신 완만한 램프로 바뀜. 3단계 결론(방향은 정당, 크기가
과도했음)과 정합적인 결과.

**주의/한계(다음 실제 패치 작업 시 검토 필요)**:
1. 이번 검증은 **이 raw CSV를 코드 밖에서 순수 재현한 것**이지,
   실제 `long_mpc.py`/acados MPC 파이프라인 통합 검증이 아님(기존
   세션들과 동일한 한계).
2. `VISION_RATE_REF_MARGIN=5.0m/s`는 이번 1개 사례 기준 설계
   추정치 — 다른 route(특히 진짜 급접근 danger 사례가 아닌, 일반
   빠른 접근 사례)로 마진이 과하게 좁아 정상 반응까지 죽이지 않는지
   교차검증 필요.
3. `leadVLead`(모델의 속도 추정) 자체도 완벽하지 않을 수 있음 —
   이번 사례에서는 훨씬 안정적이었지만, `leadVLead`가 함께 잘못된
   극단 케이스(예: 모델이 속도도 크게 오판하는 경우)에 대한 회귀
   검증도 필요.
4. 63차 계속3의 방안C(신규등록 게이트 재사용, `_lead_acq_timer`
   리셋)는 폐기하지 않고 **병행 유지** 권장 — 방안E는 rate 자체의
   신뢰도를 높이는 접근이라 서로 배타적이지 않음.

**다음(사용자 결정 대기)**: (a) 방안E를 `long_mpc.py`에 실제 구현 →
`git am` 검증 → 패치 전달, 또는 (b) 다른 route로 방안E 마진값 먼저
교차검증(패치 전 시뮬레이션 우선 원칙), 또는 (c) 63차 전체를 여기서
일단락하고 다른 과제(58차 잔여 항목 등)로 전환.

**코드 변경 없음(설계+시뮬레이션만, `work/route63b/sim_e.py` 스크래치,
toolkit 미편입)**.

**세션 종료 아님 — 1~4단계 전부 완료, 사용자 결정 대기.**

## 63차 계속8 (체크포인트 — 3단계 완료: qcamera 프레임 대조, 실제 cut-in 확정 + 가속이탈 시각 확인)

**3단계 완료**: `verify_and_extract_frames.py`로 seg14 t=922.5~926.0
구간 10프레임 추출(전부 OK, diff<0.01s). 육안 대조 결과:

- **t=922.5~923.6**: 흰색 택시가 우측 차로/갓길 쪽에서 원거리에 위치,
  화면상 크기 변화 완만.
- **t=924.0~924.5**: 택시가 뚜렷이 커지고 **자차 차로 중앙으로 들어와
  있음** — 육안으로 cut-in(차로 진입) 완료 확인. dPath가 시종 작았던
  것과 종합하면, 이 시점 이전에 이미 횡방향 진입은 거의 끝나 있었고
  (dPath 데이터가 커버하는 t=921부터 이미 -0.17~-0.49로 작음), 남은
  건 순수 종방향 접근이었던 것으로 재해석.
- **t=925.15(급감속 최저점)**: 택시가 차선폭 상당 부분을 채울 만큼
  근접 — **실제로 가까운 상황이었음**(허위 근접 아님).
- **t=925.35~926.00(레이더 락온 이후)**: 택시가 화면상 크기를 유지
  하거나 오히려 살짝 작아짐(멀어짐) — CSV의 vRel 양전환(+3.2~+4.3m/s)
  과 시각적으로 정합. **사용자 제보("끼어드는 차량이 내차보다
  가속")가 영상으로도 확인됨.**

**종합 판단(3단계 최종)**: 이번 이벤트는 **허위 검출/노이즈가 아니라
실제 cut-in + 종방향 급접근 + 직후 가속이탈의 완전한 실제 시나리오**.
2단계에서 확인한 "dPath 연속(트랙 스왑 아님)"과 이번 "실제 접근/이탈
육안 확인"을 종합하면, 63차 계속3~4에서 세웠던 "인접차선 오검출"
가설은 완전히 폐기. **근본 문제는 대상 오인식이 아니라, 실제로
역동적으로 움직이는(cut-in 중인) 물체를 vision이 추적할 때 depth
추정이 매끄럽지 않고 불연속으로 튀는 것** — 반응 자체(감속)의 방향은
정당했으나, `_vision_dRel_rate`가 raw dRel의 프레임간 튐(최대
11.5m/frame, 63차 계속7 참고)을 그대로 흡수해 `frac_rate`/`frac_ttc`가
필요 이상으로 계단식·과잉으로 튀었을 가능성이 높음(정도의 문제이지
방향의 문제가 아님).

**방안 재설계 방향(4단계로 이월)**: 63차 계속3~4의 "discontinuity
감지 후 억제"(방안C/D) 프레임 자체가 전제("노이즈니까 억제")부터
재검토 필요 — 실제 접근 상황이므로 무작정 억제하면 진짜 위험 반응을
늦출 위험. 대신 **26차에 이미 `long_mpc.py`에 구현된 클램프
(`VISION_CLOSING_RATE_MAX_PLAUSIBLE=30.0 m/s`)+3프레임 중앙값 필터**
(현재는 `_vision_dRel_rate` 직접보정 경로에서만 저역통과 입력으로
쓰임)를 discontinuity 트리거/frac_rate 계산 경로에도 일관되게 적용해
"방향은 유지하되 크기의 튐만 완화"하는 쪽이 이번 실측과 더 부합할
가능성 — 4단계에서 구체 설계.

**세션 종료 아님 — 중단지점 저장, 바로 4단계(방안 재설계)로 이어감.**
