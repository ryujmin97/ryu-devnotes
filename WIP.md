...
- 사용자가 seg6/seg12 폴더 재업로드 → `extract_dashcam_frames.py`로
  프레임 매칭(오차 11~27ms) 및 육안 확인 완료.
- **seg6**: 곡선 우측 가장자리 버스가 리드 후보로 순간 혼입되는
  패턴으로 원인 특정 — 무해한 노이즈 확정.
- **seg12**: 전 구간 동일 흰 승용차가 실제 리드, 마지막 프레임에서
  브레이크등 육안 확인 — 진짜 위험 반응(aEgo -1.9m/s²) 재확인.
- **공통 원인 패턴 확정**: 곡선 구간에서 도로 가장자리 대형/정차
  차량이 리드 후보에 일시 혼입되는 것이 dRel 급점프의 실제 메커니즘
  (노이즈/진짜위험 둘 다에 나타날 수 있음). `would_trigger_ttc_danger`
  개선 설계 시 이 패턴을 명시적으로 반영할 근거 마련됨.
- 증거 이미지 5장 push 완료. push 대상: FINDINGS.md, WIP.md
  (코드 변경 없음).
...
2. **road<->vturn / road<->route 쌍의 min() 히스테리시스 설계 착수**
   ...
   또는 다른 방식 가능.
3. 도구 5/5는 사실상 이전 세션에서 첫 실행 완료로 판단되나, 진짜
