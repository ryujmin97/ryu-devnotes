...
- 저장 시각: 2026-08-22 (39차 — "저속_앞차" 로그 분석/패치 작성 완료,
  실차 검증 전 단계에서 체크포인트. 아래 "39차" 섹션의 "다음 단계"부터
  이어감. 38차는 패치 적용/push까지 완료, 실차 검증만 남음.)

## 39차 (진행 중) — 저속 구간 TTC 게이트 급붕괴로 인한 급정지 느낌, rise-rate 패치 작성 완료, 실차 검증 대기
...
- 패치 `0001-long_mpc-TTC-aLead-weight-lurch-rise-rate.patch` (base
  `c3ea08e`, 38차 패치 위에 쌓임) 생성, `git am` 검증(임시 클론) + ast
  문법 통과. **아직 Master 로컬(C:\dev\ryu)에 미적용.**
- **다음 단계(최우선)**:
  1. Master가 `git am C:\dev\patch\0001-long_mpc-TTC-aLead-weight-lurch-rise-rate.patch`
     적용 후 `git push` (base가 `c3-ms-dev` HEAD `c3ea08e`여야 충돌 없음).
  2. 실차 검증: (a) 저속 추종 감속 시 급정지 느낌 해소 체감, (b) **회귀
     검증 필수** — 저속 실제 위험 cut-in(TTC<=2.5s)에서 danger override
     정상 발동해 반응 지연 없는지, (c) `LEAD_ACCEL_WEIGHT_RISE_RATE=1.0`
     값 승차감 기준 재조정 여부.
  3. 38차(고속)+39차(저속) 패치가 모두 적용된 상태의 통합 실차 검증 —
