...
- 저장 시각: 2026-08-21 (31차 — 추가 업로드 `카메라_인식_추가.zip`
  6개 세그(3개 라우트)로 `GATE_CAUTION=-2.2/GATE_DANGER=-5.0` 후보
  재검증. 여러 세그에서 락온 1초 이상 전부터 DANGER(frac=1.0)
  연속 유지되는 매우 깨끗한 실측 사례 다수 확보(seg12/seg15 ×
  83e6b133f5 등) — 30차 잠정 제안이 훨씬 탄탄해짐. **아직 패치
  안 함.** 남은 유일한 공백은 "곡선 중 CAUTION 레벨 글리치 빈도"
  검증(현재 seg19 1건뿐, 이번 6개는 전부 고속도로라 기여 없음).)

## 다음 세션(또는 다음 메시지)에서 이어갈 것 (31차 기준, 최우선)
0. **선행 확인**: 26차 patch(`5cc0900`)는 로컬 `C:\dev\ryu`에
   `git am` 이미 적용됨/origin 미push (29차 확인). 새 문턱 패치는
   그 위에 적층.
1. **사용자 확인 필요**: 31차까지의 검증(곡선 오탐 1건 + 고속도로
   실측 다수, DANGER까지 1초 이상 여유로 도달하는 깨끗한 사례
   확보)으로 패치 진행해도 되는지, 아니면 곡선 라우트를 더
   확보해서 오탐 빈도를 정량화할지. 근거는 이미 충분히 강해졌다고
   판단됨(FINDINGS.md 31차 결론 참고) — 사용자가 "진행"이라 하면
   바로 2번.
...
3. 원본 CSV(`/home/claude/work/seg7_12.csv`,
   `/home/claude/work/seg19.csv`, `/home/claude/work/extra.csv`)는
   컨테이너 로컬이라 이 세션 끝나면 사라짐 — 재검증 필요 시 zip
   재업로드 필요(`앞차_카메라_인식_미감속.zip`,
   `20260821_115242_000002e5--1723e8b850--19.zip`,
   `카메라_인식_추가.zip`, 전부 이미 한 번 올렸던 파일).
4. 완료 후 26차 WIP 원래 다음 과제(아래 26차 섹션 "다음 세션에서
   이어갈 것")로 복귀.

## 31차 (완료, 위 최우선 과제로 이어짐) — 6개 세그 추가 검증으로
문턱 재설계 근거 대폭 보강, 패치 직전 단계
