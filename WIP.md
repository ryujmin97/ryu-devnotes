## 88차 (완료 — 분석만, 코드 변경 없음) — 곡선_고속도로_램프 실차 로그, c3-ms-curv(81/82차) 이전 baseline으로 route/vturn 결합 실측 확인

**배경**: 사용자가 "이번패치 이전" 실차 로그(`0000032a--dfc68039a9--17/18`,
qcamera+clip 포함)를 업로드, route/vturn 적용 상태 분석 요청. 추출 결과
`meta.json` commit `2d5174e`(79차, `c3-ms-dev` 브랜치)로 확인 —
**81/82차(vturn_safe_time 2.0s, route 500m TBT게이트 제거, 원복측 대칭버퍼)가
반영된 `c3-ms-curv`가 아니라 그 이전 baseline**임을 확정. 향후 동일 구간을
`c3-ms-curv`로 재주행하면 이번 결과와 직접 before/after 비교 가능.

**연속된 커브 2개(고속도로 램프 합류부, seg18 t=7170~7211) 분석**:
1. **커브1(t≈7170~7190, 우회전, steer -10~-11.6°, curvature +0.0024~0.0029)**:
   **route는 이 구간에 전혀 참가하지 않고 vturn이 단독으로 전 구간 담당**
   — qcamera로도 이 구간엔 분기 안내표지판이 안 보임(일반 도로 곡선).
   **81차가 코드 리딩만으로 추정했던 "TBT 안내 미근접 시 route가
   `-500<xDistToTurn<500` 게이트에 막혀 min() 후보에서 아예 빠지는" 구조적
   사각지대를, 이번에 실측 로그로 처음 확인함**(표본 1건, 대조군은 바로
   아래 커브2). 최저 vEgo≈84.2km/h(desiredSpeed≈89km/h), 최대감속
   ≈-0.95m/s². 탈출 후 vturn이 3~4초 만에 완전 해제(89→200km/h) — 82차
   원복버퍼 미적용 baseline이므로 빠른 해제가 정상(82차 이후엔 이보다도
   더 당겨질 것으로 예상).
2. **커브2(t≈7194~7207, 좌회전, curvature -0.0002~-0.0010)**: **route가
   대부분 구간을 담당**(vturn은 값 자체는 계산되나 min()에서 항상 짐) —
   qcamera로 커브1/커브2 사이(t=7190.5)에 분기/램프 안내표지판 확인,
   TBT 이벤트 근접으로 게이트 통과 추정과 일치. 최저 vEgo≈81.9km/h
   (desiredSpeed≈84km/h), 최대감속 -0.94m/s². 탈출 후 회복은 route
   특유의 계단식 급점프(84→129km/h, 약 1.6초) — DP 자체에 저역통과
   필터가 없어 구간 경계에서 값이 크게 뛰는 특성(82차가 발견한 route측
   버그와는 별개, 이건 정상 동작 특성).

**[신규, 저우선] src 하드-스위치 플리커 실측 재확인**: 이 두 커브 구간에서
src 전환 51회 중 39회(76%)가 1초 이내 재전환 — vturn/route 값이 서로
근접(수 km/h 차)할 때 매 프레임 승자가 바뀌는 현상. desiredSpeed 자체
변동폭이 작아 aEgo/승차감에 뚜렷한 악영향은 안 보이나(harsh_brake
0건), 81/82차 WIP에 이미 남아있던 "min() 하드-스위치 소스전환 리뷰"
미착수 항목이 실제로 빈번히 발생함을 정량 확인.

**안전지표**: harsh_brake 0건, ttc_danger 0건(리드 위험 없음).
`turn_speed_violations` 2건(커브1 진입 초입 t≈7161.5~7162.8, overshoot
최대 5.11km/h, 지속 0.5~0.66초) — 경미한 트랜지언트, 곡선 진입 시작
부분의 짧은 과속이라 심각하지 않음.

**결론**: vturn/route 둘 다 코드대로 정상 작동(오탐/버그 없음).
81차가 지목한 TBT 게이트 사각지대(일반 커브에서 route 미참가)가 이번에
실측으로 재현 확인됨 — 81차 패치(mode 2에서 게이트 제거)가 이 케이스를
해결하는지는 `c3-ms-curv`로 동일/유사 구간 재주행 시 커브1에서도
route가 참가하는지 확인하면 검증 가능.

**코드 변경 없음(분석만).**

**다음(사용자 결정 대기)**:
1. `c3-ms-curv`(81/82차 반영)로 동일/유사 램프 구간 재주행 시 커브1에서도
   route가 참가하는지, 회복 속도가 더 빨라지는지 before/after 비교.
2. src 하드-스위치 플리커(51회 중 39회 <1s) 완화 여부 — 여전히
   저우선(체감/aEgo 영향 미확인 상태), 필요시 히스테리시스/블렌딩
   설계 검토.
3. turn_speed_violations 2건(overshoot 5.11kph)은 경미해 추가 조치
   불필요로 판단, 참고용으로만 기록.
