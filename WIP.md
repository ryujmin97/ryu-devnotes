...
- 저장 시각: 2026-08-22 (37차 계속 3 — 패치 작성+로직 합성검증까지 완료,
  실차 검증 전 단계에서 체크포인트. 다음 세션은 아래 "37차 계속 3
  (진행 중)" 섹션의 "다음 단계"부터 이어감)
...
- **다음 단계(최우선)**:
  ...
  2. 실차 검증: 원래 옆차선/측면차량 오탐 재현 시나리오 재현 시
     `dPath` 게이트에 걸려 리드 미채택되거나, 채택되더라도
     `sccFallback=True`로 `LeadBlend`가 작동해 급감속으로 안 이어지는지
     확인.
  3. **회귀 검증 필수**: `SCC_FALLBACK_DPATH_GATE=2.0m`가 정상적인
