...
- 저장 시각: 2026-08-21 (30차 — 세그7/세그12 zip 재업로드받아
  스윕 완료 + 세그19(곡선) zip으로 오탐 검증까지 마쳤으나, **아직
  패치 안 함**. FINDINGS.md에 30차 항목으로 정정 기록: 28차에서
  "대표값"으로 썼던 min_filt_rate=-3.196(세그7)이 사실은 센서
  글리치였고, 진짜 조기 신호는 락온 4.8초 전의 별도 -2.9대 dip임을
  발견. `GATE_CAUTION=-2.2 / GATE_DANGER=-5.0` 잠정 제안했고
  **사용자에게 곡선 라우트 추가 검증 여부 물어본 상태에서 저장**.)

## 다음 세션(또는 다음 메시지)에서 이어갈 것 (30차 기준, 최우선)
0. **선행 확인**: 26차 patch(`5cc0900`)는 로컬 `C:\dev\ryu`에
   `git am` 이미 적용됨/origin 미push (29차 확인). 새 문턱 패치는
   그 위에 적층.
1. **사용자 답변 대기 중인 질문**: 세그19(route `1723e8b850`) 곡선
   오탐 검증 1건으로 충분한지, 곡선 라우트를 더 확보해서 재검증할지
   확인 필요. "이 정도로 진행"이면 바로 2번, "더 검증"이면 곡선
   라우트 zip 재요청 후 동일 스윕(`SIM_GATE_CAUTION`/
   `SIM_GATE_DANGER` env override, `toolkit/sim_frac_rate.py`) 반복.
2. 확정되면 `long_mpc.py`의 `VISION_CLOSING_RATE_GATE_CAUTION=-2.2`,
   `VISION_CLOSING_RATE_GATE_DANGER=-5.0`(또는 재검증 후 조정값)로
   패치 → `git format-patch` → `C:\dev\patch\` 전달 → `git am`
   안내(26차 patch 위에 적층 전제).
3. 원본 CSV(`/home/claude/work/seg7_12.csv`,
   `/home/claude/work/seg19.csv`)는 컨테이너 로컬이라 이 세션
   끝나면 사라짐 — 다음 세션에서 재검증하려면 zip 재업로드 필요
   (세그7: `앞차_카메라_인식_미감속.zip`, 세그19:
   `20260821_115242_000002e5--1723e8b850--19.zip`, 이미 한 번
   올렸던 파일들이니 사용자가 보관 중일 것).
4. 완료 후 26차 WIP 원래 다음 과제(아래 26차 섹션 "다음 세션에서
   이어갈 것")로 복귀.

## 30차 (완료, 위 최우선 과제로 이어짐) — 28차 min_filt_rate가
글리치였음을 발견, 문턱 재설계 근거 정정, 곡선 오탐 1차 검증
