...
- 저장 시각: 2026-08-21 (28차 — 27차에서 미확보였던 세그7/세그12 zip
  재업로드됨, `sim_frac_rate.py` 신규 작성으로 26차 frac_rate 필터
  로직 프레임 단위 재현 → 두 사례 모두 게이트 전 구간 미발동(0.000)
  정량 확정. FINDINGS.md `[VALIDATED]`로 격상 완료. **코드 변경은
  아직 미적용**, 다음이 최우선 과제.)

## 다음 세션(또는 다음 메시지)에서 이어갈 것 (28차 기준, 최우선)
