...
- 저장 시각: 2026-08-21 (29차 — 사용자 확인: **26차 patch(`5cc0900`)는
  로컬 `C:\dev\ryu`에 `git am` 이미 적용됨, origin push는 안 함**
  (28차 WIP 3번 항목의 불확실성 해소, 다음 문턱 패치는 26차 patch가
  이미 적용된 로컬 위에 순차 적용된다고 확정하고 작성하면 됨). 코드
  작업은 28차에서 멈춘 지점 그대로 — 문턱 재설계값 아직 미확정,
  세그7/세그12 zip 재업로드 대기 중이라 시뮬레이션도 미실행. 이번
  세션에서 한 것은 `sim_frac_rate.py`에 `SIM_GATE_CAUTION`/
  `SIM_GATE_DANGER` 환경변수 override만 추가(문턱 후보 스윕을 파일
  수정 없이 반복 실행하기 위함, toolkit 변경이므로 세션 종료 시
  README/CHANGELOG 갱신 필요 — 아직 안 함, 다음 이어받을 때 처리).)

## 다음 세션(또는 다음 메시지)에서 이어갈 것 (29차 기준, 최우선)
0. **선행 확인 완료**: 26차 patch는 로컬 git am 적용됨/origin
   미push. 아래 1~3 진행 후 새 문턱 패치를 만들 때 "26차 patch가
   이미 적용된 위에 얹는 패치"로 작성하면 됨(재확인 불필요).
1. 세그7 zip(`20260821_112042_000002e4--866476e5c3--7.zip`)/세그12
   zip(`20260821_112542_000002e4--866476e5c3--12.zip`) 재업로드 대기
   중 — 업로드되면 `extract_log.py`로 CSV 재추출.
2. `SIM_GATE_CAUTION` 후보(-2.5, -3.0)와 `SIM_GATE_DANGER` 후보
   (-8.0, -10.0) 조합으로 `sim_frac_rate.py` 스윕, 각 후보에서
   frac_rate가 락온 시점 대비 몇 초 전부터 발동하는지 확인.
3. (여유 있으면) 곡선 구간 포함 라우트로 새 문턱의 오탐(false
   positive) 위험도 함께 확인 — 23차에 확인된 raw rate -12~-25 스냅
   사례, 26차 필터(클램프+중앙값) 적용 후에도 낮아진 CAUTION에
   걸릴 수 있는지 미검증.
4. 통과하는 값으로 `long_mpc.py`의 `VISION_CLOSING_RATE_GATE_CAUTION`
   (+ 필요시 `GATE_DANGER`)만 패치 → `git format-patch` →
   `C:\dev\patch\` 전달 → `git am` 안내(26차 patch 위에 적층 전제).
5. 완료 후 26차 WIP 원래 다음 과제(아래 26차 섹션 "다음 세션에서
   이어갈 것")로 복귀.
