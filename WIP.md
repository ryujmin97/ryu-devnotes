...
- 저장 시각: 2026-08-22 (35차 계속 2 — "Clip 선택" 버튼 무반응 버그를
  logs.js 캐시 버스터 미갱신으로 확정, patch 0004 전달, 적용 대기)

## 35차 계속 2 — "Clip 선택" 버튼 무반응 원인: 캐시 버스터 미갱신
...
- **다음 세션(또는 다음 메시지)에서 이어갈 것 — 최우선**:
  1. 사용자가 `c3-ms-dev`/`c3-ms-test` 두 브랜치 모두에 patch 0004
     `git am` + `git push` 적용.
  2. 실기기에서 **강제 새로고침(캐시 무시) 후** carrotweb 접속 —
     "Clip 선택" 버튼이 clip 파일 체크박스만 선택/해제 토글하는지
     확인.
  3. clip 실제 길이(20초대)도 이 기회에 같이 확인.
  4. 검증 통과 후 → 34차(A/B 실차 비교) 복귀.
