...
**Repository**: `ryujmin97/ryu`(base `1b77b799`=323차, 코드 변경 없음, 읽기만) /
`ryu-devnotes`(base `6bb18b819`=325차, 이 항목 추가 전)

**Branch**: `c3-ms-dev` / `main`

**세션 시작 확인(§3/§33)**: `git ls-remote`로 원격 fresh 재확인 -- `ryu`
HEAD `1b77b799`(323차, 변경 없음), `ryu-devnotes` HEAD `6bb18b819`
(325차, 변경 없음). 다른 작업자 개입 흔적 없음.

**배경**: 325차가 ep4(seg--4) 프레임 트레이스로 "10m/5m 해상도가 지워
버리는 두 번째 curve apex를 2.5m만 분리 검출"하는 사례(구간2, 활성
시간 비대칭의 주된 기여분)를 확인. 이를 근거로 326차 논의가 시작됨.

**설계가 흔들린 과정(§28, 추측을 검증완료로 취급하지 않기 위해 전 과정
기록)**:
1. 1차 제안: "distance_interval=10.0 -> 2.5, macro sample=4 -> 16,
   fine sample_fine=1 -> 4"로 **600m 전 구간을 2.5m로 전환**하는 안.
   `carrot_man.py`(1b77b799 기준) 실측으로 대응표까지 확정함(macro
   half-chord 40m/fine half-chord 10m 물리적 크기 보존 확인,
   `ROUTE_CLUSTER_MAX_GAP_M=40.0`은 미터 단위라 무변경 가능하나
   `ROUTE_CLUSTER_MIN_POINTS=2`는 포인트 개수 기준이라 grid가
   촘촘해지면 실질적 클러스터 승격 문턱이 낮아지는 부작용 있음을 확인).
2. 사용자 지적으로 정정: **이 "전 구간 2.5m" 안은 317차가 이미 채택한
   "orphan/곡선후보 주변 국소 윈도우만 재샘플" 설계와 배치됨**(317차
   WIP 항목에 명시된 기존 결정, 318차는 이 국소 윈도우가 아니라
   "전체 600m를 5m로" 최악의 경우만 벤치마크했었음 -- 국소 윈도우
   자체는 아직 벤치마크된 적 없음, 318차 한계 2번).
3. 재정정 시도: "relative_coords 계측 patch(국소 트리거 조건을 실측
   근거로 설계하기 위함, 323차 `routeOrphanRawPath`는 orphan **확정
   후**에만 발행되므로 트리거 판정 이전 시점 데이터로는 부적합)를
   먼저 진행 -> 실제 로그로 트리거조건/윈도우폭/hysteresis/grid
   간격(5m vs 2.5m)을 결정 -> 그 다음 production 패치"안이 한 차례
   더 논의됨(§31, 317/318차부터 이월된 미승인 항목).
4. **최종 결정(사용자, 소요시간 문제로 위 3번 기각)**: 계측 patch 선행
   사이클을 생략한다. 대신 곧바로 "10m 기본 + 곡선후보 주변 국소
   구간만 2.5m 재샘플"하는 patch를 작성 -> 실차 검증 -> 그 로그로
   문제점을 역추적하는 순서로 진행한다.

**326차 확정 설계(다음 세션 패치 작성의 기준)**:
```
600m route
  |
  +- 평상시 ------------> 기존 10m 처리(무변경)
  ...
  |         |
  |         v
  |    그 구간만 2.5m 재샘플(macro sample=16/40m,
  |    fine sample_fine=4/10m -- 물리적 chord 크기는
  |    10m production과 동일하게 보존)
  |         |
  |         v
  |    기존 curvature/apex/cluster/ACTIVE 로직에 그대로 투입
  |
  +- 후보 없으면 --------> 2.5m 계산 자체 없음
```

**무변경 유지(안전장치, 지선생 제안·사용자 동의)**:
- `ROUTE_CLUSTER_MAX_GAP_M`, `ROUTE_CLUSTER_MIN_POINTS`, continuity,
  streak/confidence, ACTIVE 진입/해제 공식, `autoNaviSpeedDecelRate`,
  `AutoNaviSpeedCtrlEnd`, `route_active` 상태기계 -- 전부 무변경.
- 이번 실차에서 검증하는 것은 "국소 2.5m geometry가 놓치던 apex를
  잡아 사전감속에 실제로 도움이 되는가"이지, geometry+gate+감속을
  동시에 바꾸는 실험이 아님(326차 논의 초반부터 일관된 원칙, 322차
  이전 설계 원칙과도 동일선상).
- 이번 2.5m 값은 **production 최종값 선언이 아니라 실차 1차 후보**임을
  명시 -- 문제가 나오면 5m/윈도우폭/threshold를 다음 커밋에서 조정.

**미확정(다음 세션 패치 작성 착수 전 반드시 정해야 할 것)**:
1. "곡선 후보"로 판정할 10m 기준 trigger 조건(곡률 threshold? 10m
   결과에서 감속후보 발생 여부? 곡률 변화율?) -- 아직 코드/로그 기준
   미확정.
2. 국소 윈도우의 물리적 폭(후보 지점 전/후 몇 m를 2.5m로 재계산할지).
3. 진입/이탈 hysteresis 필요 여부(잦은 grid 전환에 의한 flicker 방지).
4. 2.5m 구간을 얼마나 유지할지(후보 통과 후 즉시 10m로 복귀할지,
   일정 거리/시간 유지할지).
5. 코드 레벨: 현재 macro/fine curvature 루프의 `distance = -10.0`
   하드코딩 초기값(213차 도입, `resample_10m_np()`가
   `resampled_points[0]==start_point`가 되게 하는 값과 결합된 상수)을
   국소 2.5m 블록에서 `-distance_interval`(국소 grid 기준)로 일반화
   해야 함 -- 이번 세션에서는 지적만 됐고 실제 코드는 아직 열어보지
   않음.

**이번 세션에 병행하기로 한 것(사용자 지시, 패치 작성과 별개)**:
- 기존 324차(`x18seg_324_episodes.csv`)/325차(`ep4_trace_325cha.csv`)
  로그에서 가능한 범위까지 10m/5m/2.5m A/B 비교(apex 위치/속도,
  route ACTIVE 시작-종료, 기존 10m 대비 선행감속 시간)를 신규
  업로드분(`x18seg` 원본 zip 재포함)까지 반영해 재실행 -- 다음 세션
  착수 항목.

**영향받는 실차 제어 로직**: 없음(이번 항목은 설계 확정 기록뿐, `ryu`
코드 변경 없음).

**검증**: 정적 분석/로그 검증/시뮬레이션/실차 검증 전부 해당 없음
(설계 체크포인트, 코드 변경 없음).

**Devnotes**: 이 WIP 항목 신규 추가만. `FINDINGS.md`/
`PARAMS_REGISTRY.md`/toolkit 변경 없음.

**미확인 사항**:
- 위 "미확정" 5개 항목 전부.
- 국소 윈도우 케이스 자체의 실행시간 벤치마크(318차 한계 2번, 아직도
  미해소).
- `carrot_navi_route()` 실제 호출 주기(Hz) -- 이월(307차부터).

**다음 작업**:
1. 위 "미확정" 1~5번을 `carrot_man.py`(1b77b799 기준) 코드 확인 +
   기존 324/325차 로그로 최대한 구체화.
2. 신규 업로드된 `x18seg` 원본 zip으로 324/325차 corpus 재추출/검증
   (컨테이너 재시작 시 이전 작업 유실 가능성 대비, §33).
3. 위 1/2번 결과를 근거로 실제 patch(diff) 작성 -> `git format-patch`
   -> 독립 클론에서 `git apply --check`/`git am`/`py_compile`/
   회귀검증 -> 사용자 전달.
4. 이월 항목(변경 없음): `PROVISIONAL_PROMOTE_STREAK=3` 조정 여부,
   232건 persistent orphan qcamera 표본 분류, 84건 phase-resolved
   감속손실량 정량화, `carrot_navi_route()` Hz 확인.
...
**Repository**: `ryujmin97/ryu`(HEAD `020ea86`=307차, 변경 없음) /
`ryu-devnotes`(base `732c505`=319차, 이 항목 추가 전)

**Branch**: `c3-ms-dev` / `main`

**세션 시작 확인(§3/§8/§33)**: 사용자가 로컬 push 시 "원격이 앞서
있다"고 보고 -- `git ls-remote`로 재확인한 결과 처음엔 origin이
`0567a13`(318차)로 그대로였고 **사용자 로컬만 10개 커밋 뒤처져
있었음**을 확인, 사용자가 `git am --abort` + `git pull --ff-only`로
동기화 완료. 이 과정에서 314~316차 원문을 재확인해 319차(당시
작성 중이던 patch)의 결론이 이미 있는 작업과 겹치고 일부는 상충함을
발견 -- **당초 만든 patch를 회수하고 로컬에서 재작성**했으나, 재작성
직전 fresh clone으로 원격을 다시 확인한 결과 **원격이 다시
`732c505`로 앞서 있었고, 그 커밋이 바로 회수했어야 할 원본(첫 번째)
319차 patch였음을 확인** -- 사용자가 회수 요청 전에 이미 적용·push한
것으로 보인다. §9/§14 원칙상 이미 push된 커밋은 되돌리거나 고치지
않고, 이 항목으로 그 위에 정정만 추가한다.

**한 일**:
1. `FINDINGS.md`에 320차 정정 항목 신규 작성(위) -- 319차의 "원거리
   =아티팩트" 결론을 314차 실측과 대조해 철회, "근접 신규 아형"이
   310차 "교차로 회전부"와 동일 현상임을 명시, 318차 "다음 작업
   1번" 종료 처리.
2. `toolkit/README.md`의 `group_orphan_episodes_319.py` 섹션에
   안내문 추가 -- 근접/원거리 후보 분류 목적에는
   `sim_route_310_provisional_streak_real_corpus.py --check-cruise
   --check-comfort`가 이미 검증된 정식 도구이며 우선 사용할 것.

**결론**: 318차 "다음 작업 1번"은 310~316차로 이미 완료되어 있었다.
319차의 실측 자체(orphan 3433건 재확인, 53개 에피소드, 8곳 프레임
확인)는 유효하나 "원거리=아티팩트" 해석은 철회. 향후 유사 작업 시
WIP.md 상단 1~2개 회차만 보고 "기록 없음"이라 판단하지 않고 관련
키워드로 전체를 검색할 것(재발방지, 아래 참고).

**재발 방지**: 다음 세션은 특정 주제(예: 근접+이동+활성 orphan 후보)
관련 기록 유무를 판단할 때 `grep -n <키워드>`로 WIP.md/FINDINGS.md
전체를 검색한 뒤 판단할 것. 오늘 두 세션(319차 실제 실행분 + 이
320차) 모두 "WIP.md 상단만 보고 미기록으로 오판"이 근본 원인이었다.

**영향받는 실차 제어 로직**: 없음(`ryu` 코드 변경 없음, ANALYSIS_ONLY).

**검증**:
- 정적 분석: 해당 없음(신규 코드 없음)
- 로그 검증: 해당 없음(310~316차 기존 기록 재확인만)
- 시뮬레이션: 해당 없음
- 실차 검증: 미실시

**Devnotes**: `FINDINGS.md` 320차 항목(정정 경위 전체 기록), 이 WIP
항목, `toolkit/README.md`(안내문 추가). 319차 항목/파일은 수정하지
않음(§14).

**미확인 사항**:
- 319차가 뽑은 근접/원거리 에피소드와 310~316차의 4곳/5개 사건이
  좌표 수준으로 정확히 일치하는지 직접 대조하지 않음(seg+시간대
  근접성만 근거).
- seg1(주차차량 협착)/seg6 t=856.5(진입로 횡단)는 310~316차 목록에
  없는 구간이나 낮은 우선순위 부정 결과라 추가 조사 안 함.

**다음 작업**:
1. (이월, 312차부터) `PROVISIONAL_PROMOTE_STREAK=3` 조정 여부/설계안
   A 채택 여부 최종 결정 -- 310~316차 누적 결과 "실제 불편 증거 0건"
   상태이므로 서두를 근거가 약함.
2. (이월, 317차부터) "국소 재샘플" 원안 실측용 `relative_coords`
   신규 계측 patch -- 사용자 승인 필요.
3. (이월, 311차부터) seg16 cruiseEnabled False->True 전환 시점의
   "커브 통과 후 재개입" 가설 코드 레벨 추적 -- 계속 미착수.
4. (이월, 316차부터) `harsh_brake_events`/`steering_oscillation_detector`
   임계값 민감도 확인.
5. (신규, 낮은 우선순위) `C:\dev\ryu-devnotes`에 미적용 상태로 남아있는
   오래된 patch 3개(288차/291차/293차 관련) 처리 여부 확인 필요.
...
**Repository**: `ryujmin97/ryu`(HEAD `020ea86`=307차, 변경 없음) /
`ryu-devnotes`(base = 위 317차 체크포인트, 이 항목 추가 전)

**Branch**: `c3-ms-dev` / `main`

**세션 시작 확인(§3/§33)**: 사용자가 "이전세션 중단, 계속" 지시.
컨테이너가 리셋되어 사라지기 전 직전 세션이 만든 devnotes 변경분
4개 파일(`FINDINGS.md`/`toolkit/CHANGELOG.md`/`toolkit/README.md`/
신규 스크립트) + corpus zip을 사용자가 재업로드. 원격을 `git
ls-remote`로 fresh 재확인해 `ryu`=`020ea86`(변경 없음)을 확인. 처음
패치를 origin `b90c8b9`(316차) 기준으로 만들어 전달했으나, 사용자
로컬 저장소가 이미 위 "317차 체크포인트"(코드 변경 없는 순수 설계
논의 커밋, `ryu`/`ryu-devnotes` push 전 로컬 전용)만큼 앞서 있음이
`git status`/`git show`로 드러남 -- **§8 원칙대로 즉시 작업을
멈추고 그 커밋의 실제 내용(WIP.md diff)을 전달받아 확인**한 결과,
이 항목이 실제로 실행한 시뮬레이션·실측과 겹치지 않고 정확히 그
체크포인트의 "다음 작업 1번"을 이어서 수행한 것임을 확인 -- 아래
내용은 그 체크포인트 base 위에 "317차 계속"(§14)으로 다시 구성한
것.

재업로드된 4개 파일을 origin(`b90c8b9`) 기준 원본과 diff 대조 --
전부 **순수 추가만(prepend/insert) 있고 기존 내용 삭제·수정 없음**을
확인 후 채택(§27). WIP.md 항목만 이번 세션이 위 체크포인트를 반영해
재작성.

**한 일**:
1. 위 체크포인트가 코드로 이미 확정한 사실("포인트 밀도"와 "곡률
   chord 길이"가 서로 독립된 두 축)을 근거로, "chord 고정 + 밀도만
   국소 상향"이 판별자로 유효할 것이라는 가설을 신규
   `toolkit/sim_route_317_orphan_local_fine_resample.py`로 검증
   (§21 기존 toolkit 확인 후 신규 -- 동일 목적 도구 없음 확인).
2. Part1: 폭(L=0/4/8/12/16/20/30m) x 간격(10/5/2.5m) x 위상 3중 스윕
   합성 self-test. 결과: L=0(순수 노이즈)은 전 간격에서 클러스터
   성립률 0% 유지, L>0(폭 있는 커브)은 간격을 좁힐수록 위상 무관
   성립률 단조 증가(역행 사례 0건) -- **self-test 5/5 PASS**.
3. Part2 사전 확인: 체크포인트 "다음 작업 1번"이 요구한 실측을 위해
   x17seg corpus(20171행, `020ea86`)에서 `navRouteNavd`(원본
   pre-resample 폴리라인) 존재 여부를 `decode_rlog.py`로 직접 확인
   -- **0건**(전체 corpus가 `routeSource=="tcp_navi"`이며
   `carrotMan.naviPaths`는 이미 10m로 리샘플된 결과만 담음). **원안
   ("orphan 주변 원본 폴리라인 국소 재샘플") 자체는 이 corpus로
   실측 검증이 구조적으로 불가능함을 확정**.
4. Part2 대안: 같은 메커니즘의 다른 예측("그리드 위상이 바뀌면 같은
   물리적 지점이 orphan<->cluster를 오갈 것", 219/220차 재앵커링
   사실 이용)을 이미 로그된 `routeOrphanSingletonDist`/
   `routeClusterCount`/`routeApexDist` 컬럼만으로 실측. orphan
   프레임 3433개 → 316개 에피소드로 그룹화(§265/267
   CONTINUITY_MATCH_TOLERANCE_M=10m 재사용) → **phase-resolved
   84건(26.6%)**, persistent 232건(73.4%).
5. `toolkit/README.md`(317차 단락 추가) / `toolkit/CHANGELOG.md`
   (2026-09-08 317차 항목 추가) 갱신.
6. `FINDINGS.md` 317차 항목 신규 작성(Part1/Part2 결과 + 한계 명시).
7. 신규 스크립트를 독립 clean clone에서 `git apply --check` →
   `git am` → `py_compile` → 재실행으로 재현성 검증, 이어서 이번
   세션 자체 환경에서도 x17seg corpus를 zip에서 재추출(20171행
   재현)해 스크립트를 다시 실행 -- **84/316(26.6%) 동일 결과 재현**.

**결론**: 체크포인트가 제시한 "국소 파인그리드 재확인" 아이디어는
메커니즘 자체는 합성 검증으로 유효함이 확인됐다(Part1 PASS). 다만
체크포인트의 "다음 작업 2번"(곡률 loop 실행시간 프로파일링)은 이번
세션 범위에 포함하지 않았고, "다음 작업 1번"(시뮬레이션)만 수행함 --
아래 다음 작업 참고. 이 corpus의 실제 데이터로는 원안 그대로(원본
폴리라인 기반) 검증할 수 없다(원본 폴리라인 로그 부재). 대신 같은
메커니즘의 대리 지표(그리드 위상 민감도)를 실측한 결과 26.6%가
위상에 따라 해소되는 것으로 나타나 방향은 일치하지만, 이것이 "국소
재샘플을 실제로 적용했다면 풀렸을 것"이라는 직접 증거는 아니다.

**한계(§28)**:
1. Part1(합성)은 개념 검증일 뿐 -- x17seg 실제 orphan 316건이 이
   메커니즘 때문이라는 직접 증거는 아니다.
2. Part2의 "phase 해소" 판정은 10m tolerance 근사 매칭이며, 인접한
   서로 다른 커브를 동일 커브로 오인할 가능성을 배제 못한다.
3. persistent 232건(73.4%)이 모든 위상에서 왜 안 풀리는지(진짜
   폭<10m 좁은 커브 vs 235차류 "미약 후보 노이즈") 원인 미분리.
4. 원안("원본 폴리라인 국소 재샘플") 실측 검증에는 `relative_coords`를
   cereal에 발행하는 신규 계측 patch(§31 사용자 승인 필요) + 그
   빌드로 신규 주행이 필요 -- 이번 세션 범위 밖.
5. 체크포인트 "다음 작업 2번"(곡률 loop 프로파일링)은 이번 세션에서
   착수하지 않음.
...
- 정적 분석: `py_compile` 통과(독립 clean clone 및 이번 세션 환경
  양쪽에서 재확인)
- 로그 검증: 완료(x17seg 20171행 재추출 재현 확인, Part2
  phase-resolution 실측 결과(84/316, 26.6%) 독립 재실행으로 재현)
- 시뮬레이션: 완료(Part1 self-test PASS, 24개 조합 전부 단언 통과)
- 실차 검증: 해당 없음(코드/파라미터 변경 없음, ANALYSIS_ONLY)

**Devnotes**: `FINDINGS.md` 317차 항목 신규, 이 WIP 항목("317차
계속"), `toolkit/README.md`/`toolkit/CHANGELOG.md` 갱신,
`toolkit/sim_route_317_orphan_local_fine_resample.py` 신규(438줄).

**미확인 사항**:
- persistent 232건이 "진짜 좁은 커브"인지 "미약 노이즈"인지 미분류.
- phase-resolved 84건의 실제 감속 손실량(apex_speed 차이) 미정량화.
- 원본 폴리라인 계측 patch에 대한 사용자 승인 여부 미결정.
- 체크포인트 "다음 작업 2번"(곡률 loop 실행시간 프로파일링) 미착수.

**다음 작업**:
1. 체크포인트 "다음 작업 2번" -- 곡률 loop 자체의 실제 프레임당
   실행시간을 x17seg corpus 기준으로 프로파일링(10m 대비 5m 부하
   차이를 숫자로 확인).
2. persistent 232건 중 일부를 표본추출해 qcamera 대조 -- 진짜
   폭<10m 좁은 커브(예: 교차로 코너)인지, 235차류 "미약 후보 노이즈"
   인지 분류.
3. "국소 재샘플" 원안을 진짜로 검증하려면: `relative_coords`(10m
   리샘플 이전 원본)를 cereal에 발행하는 계측 patch를 `ryu`에 추가할지
   사용자 승인 필요(§31) -- 승인되면 다음 세션에서 패치 설계 + 신규
   주행 요청.
4. phase-resolved 84건 중 일부를 표본추출해, cluster로 해소된 그
   인접 프레임의 실제 apex_speed가 orphan 프레임 대비 얼마나 다른
   감속을 명령했을지 정량화(316차가 harsh_brake/조향진동으로 이미
   확인한 "실제 불편 증거 0건" 결론과 이어붙일 수 있는지 검토).
5. `PROVISIONAL_PROMOTE_STREAK=3` 조정 여부/설계안 A 채택 여부 최종
   결정(312차부터 이월, 체크포인트가 재확인한 대로 여전히 별개로
   유효).
...
**Repository**: `ryujmin97/ryu`(HEAD `020ea86`=307차, 변경 없음) /
`ryu-devnotes`(base `f60a849`=314차, 이 항목 추가 전)

**Branch**: `c3-ms-dev` / `main`

**세션 시작 확인(§2/§3/§33)**: 컨테이너가 리셋되어 fresh clone으로
재확인 -- `ryu`=`020ea86`(변경 없음), `ryu-devnotes`=`f60a849`(314차
patch가 정상 push되었음을 확인). 다른 AI 개입 흔적 없음. 직전 세션이
FINDINGS.md 삽입 스크립트 오류(상대경로 문제)로 중단되어, 그 세션이
콘솔에 남긴 요약(findings_315.md 초안, 157줄)을 그대로 신뢰하지 않고
**핵심 발견(seg13->seg14 경계)을 처음부터 재현·재검증**한 뒤 정식
기록했다(직전 세션 산출물 파일 자체는 컨테이너 리셋으로 소실됨,
§28 -- 검증 없이 이전 요약을 그대로 옮기지 않음).

**한 일**:
1. 동일 x17seg zip 재추출(20171행, `020ea86` 확인, 재현성 재확인).
2. `sim_route_310_provisional_streak_real_corpus.py`로 "원거리+
   이동중" 19건을 다시 나열, 물리적 위치 기준으로 5개 사건으로
   재분류(seg6/seg14-t1358/seg4/seg3/seg13-seg14경계) -- 314차가
   재검증한 seg6(1건) 외 나머지 18건이 모두 이 5개 물리적 사건 안에
   포함됨을 확인.
3. seg14 t=1358.97(510m 고정) 사례: qcamera 없이 CSV 추이만으로도
   즉시 `matched` 승격 확인(정상). seg4(510m->490m) 사례: t=727.47
   부터 `matched` 안정화 확인(정상, 초반 flicker만 있었음). seg3
   사례는 312/313차가 이미 확인한 근접 다중후보 구간의 원거리 꼬리로
   판단, 추가 조사 불필요.
4. **신규 핵심 발견**: seg13(t>=1304s)에서 시작해 seg14(t<=1339s)로
   이어지는 후보를 `routeOrphanSingletonDist` 원시값으로 직접 추적한
   결과, `sim_route_310...py`의 episode 재구성 로직이 11개 별도
   episode로 쪼개 놓았을 뿐 실제로는 **t=1304.07s(510m)부터
   t=1339.22s(0m, 통과)까지 약 35초간 끊김없이 이어진 하나의 물리적
   후보**임을 확인(거리가 10m 간격으로 단조 감소).
5. 이 구간(t=1304~1339, seg13 뒷부분+seg14 앞부분, 1100+ 프레임) 전체를
   `routeApexMode`/`routeClusterCount` 기준 전수 조사 -- apex_mode
   집합이 `{'none','new'}` 뿐이고 `routeClusterCount`가 0이 아닌
   프레임이 전 구간 중 1건(그마저도 이 후보가 아닌 후속 커브의 것)뿐임을
   확인. **즉 이 커브는 접근 시작부터 통과까지 production이 단 한
   번도 apex를 잡지 못했다.**
6. `carrot_man.py`의 `ROUTE_CLUSTER_MIN_POINTS=2`/`route_find_clusters()`
   코드를 직접 열람해 306/307차가 지목한 게이트임을 재확인(이번
   세션은 코드 무변경, 확인만).
7. qcamera 5개 시점(t=1322.2/1328.2/1333.2/1337.4/1339.2, 약
   240m/160m/80m/30m/0m 환산) 대조 -- 160m부터 도로가 우측으로
   굽어지는 형상 + 우측 커브 경고표지판, 80m/30m에서는 우회전 굴곡이
   뚜렷이 확인됨. 240m 시점은 아직 형상이 불명확(314차 방법론 교훈과
   일치 -- 원거리는 근접 승격 후 대조가 원칙이나 이번엔 애초에 끝까지
   승격이 안 된 사례라 그 시점 자체를 볼 수밖에 없었음).
8. `aEgo` 최솟값(-2.76m/s^2)이 이 구간 종료 직후(t=1358.97, 리드차량
   관련, leadDRel=45m)에 발생함을 확인해 **이 커브와 무관**함을
   명확히 배제 -- 과대해석 방지(§28).

**변경 이유**: 306/307차가 코드+합성 재현으로 확정했던 min_points=2
게이트 결함 가설에 대해, 기존 유일한 실측 근거(309차 ep108, 별도
route1~4 corpus)와 독립된 두 번째 실증 사례를 x17seg corpus에서
확보. 특히 이번 사례는 접근 시작부터 통과까지 전 구간 미승격이라는
점에서 ep108보다 완전한 형태의 재현.

**한계(§28)**:
1. 이 구간 내내 vEgo 12.0~17.0m/s로 완만히 변화, harsh brake/조향
   진동 등 "실제 운전자 불편" 신호 확인되지 않음(`steeringAngleDeg`
   -10.0~4.6도) -- "게이트 결함이 실차에서 발생함"과 "그로 인한
   불편이 있었음"은 여전히 별개 질문이며, 후자는 미확인 상태 유지.
2. 표본 1개 corpus(x17seg)뿐 -- 다른 로그에서 이 지점/유사 지점
   재현되는지 미확인.
3. `ryu` 본체 변경 없음(§27), ANALYSIS_ONLY.

**부수 관찰**: 314차가 "seg17 rlog.zst 잘림으로 추출 실패"라 기록했던
것과 달리 이번 세션에서는 seg17이 정상 추출됨(972행). 원인 불명 --
재현 안 되는 현상이라 §33 원칙대로 사실만 기록, 임의 해석하지 않음.

**검증**:
- 정적 분석: 완료(`py_compile` PASS, 기존 스크립트 그대로 재사용)
- 로그 검증: 완료(x17seg 20171행 재추출, seg13->seg14 경계 1100+
  프레임 전수 조사)
...
**미확인 사항**:
- 이 커브에서 실제 운전자 불편이 있었는지(§28 범위 밖으로 명시적
  배제).
- 다른 corpus에서 동일/유사 지점 재현 여부.
- seg17 추출 성공/실패가 비재현적인 이유.

**Devnotes**:
- `FINDINGS.md`: 315차 항목 추가(신규 실증 사례)
- `WIP.md`: 이 항목
- toolkit 변경 없음(기존 `sim_route_310_provisional_streak_real_corpus.py`
  / `verify_and_extract_frames.py` 재사용만, §21)

**다음 작업**:
1. "원거리+이동중 19건" 표본검증은 이번 세션으로 사실상 완료(5개
   물리적 사건으로 전부 설명됨) -- 추가 조사 불필요.
2. 근접+이동중 57건 중 harsh_brake_events/steering_oscillation_detector
   교차검증(310차부터 이월, 계속 미착수) -- 이번에도 착수 못함.
3. min_points=2 게이트 자체의 설계 수정 여부는 "실제 불편 증거 0건"
   상태이므로 여전히 서두를 근거가 약함(사용자 계획 ②단계 결정 보류
   계속).
4. (낮은 우선순위) seg17 rlog.zst 추출 성공/실패 비재현 원인 규명.
...
**Repository**: `ryujmin97/ryu`(HEAD `020ea86`=307차, 변경 없음) /
`ryu-devnotes`(base `95e1db3`=311차, 이 항목 추가 전)

**Branch**: `c3-ms-dev` / `main`

**세션 시작 확인(§2/§3/§33)**: 컨테이너가 리셋되어 fresh clone으로
재확인 -- `ryu`=`020ea86`(변경 없음), `ryu-devnotes`=`95e1db3`(311차
patch가 정상 push되었음을 확인, 사용자가 실제로 push 완료). 다른
AI(ChatGPT)의 개입 흔적 없음. 사용자가 "계속" 지시 -- 311차 WIP
"다음 작업" 2번(ADAS engaged 23건 중 나머지 20건 qcamera 대조)에
착수.

**한 일**:
1. 컨테이너 리셋으로 사라진 x17seg CSV를 동일 업로드 zip에서
   재추출(pycapnp/zstandard 재설치 포함) -- 20171행, `020ea86` 확인,
   기존 기록과 완전 일치(재현성 재확인, 3회 연속).
2. 311차와 동일한 방식(`sim_route_310_provisional_streak_real_corpus.py`의
   `build_episodes()` 재사용 + 대화식 python 후처리)으로 근접+이동중
   57건 -> `cruiseEnabled=True` 23건 목록을 재구성 -- 311차 결과와
   정확히 일치(재현성 확인).
3. **[신규 관점] 23건을 seg+시간 근접(gap>5s)으로 군집화**한 결과
   실제 물리적 위치가 **단 4곳**뿐임을 발견(seg3 7건/seg4 3건/
   seg14 6건/seg16 7건 -- 하나의 커브 접근 중 `routeProvisionalStreak`
   가 여러 번 끊겨 episode가 쪼개진 것이 원인). 311차가 "20건
   미확인"으로 남긴 작업량이 실제로는 "위치 1곳(seg14) 미확인"
   수준이었음을 확인.
4. **seg14(t=1329.72~1334.82s) qcamera 신규 대조**(`verify_and_
   extract_frames.py` 재사용, 프레임 4개 시점 x 3장=총 9장 추출
   +context1): 가로수 늘어선 다차선 도로가 완만히 좌측으로 굽는
   실제 커브 확인(우측 커브 경고 쉐브론 표지판도 확인). 이번
   세션에서 두 번째로 확인된 "실제 커브 + `cruiseEnabled=True` +
   `routeApexMode` 전 구간 `none`" 사례(첫 번째는 311차 seg16).
5. seg14 구간(t=1327~1340s로 확장) 상세 조회: `leadStatus=True`(리드
   차량 `leadDRel` 32m->34.5m, 정상 추종), `aEgo` 전 구간 0.0~0.4
   (감속 없음, 오히려 약한 가속), `vEgo` 13.3->14.6m/s로 계속 상승,
   `steeringAngleDeg`는 구간 후반부터 서서히 커져 최대 -10.0도(완만한
   커브임을 뒷받침). `harsh_brake_events`/`steering_oscillation_
   detector`(§21 재사용) 적용 결과 **둘 다 0건** -- seg16과 동일하게
   "놓친 candidate는 실측되나 불편 증거는 없음" 패턴 재확인.

**결론**: 310~312차 누적 결과, 이번 x17seg 단일 로그에서 "근접+
이동중+ADAS개입" 진짜 커브 후보로 확인된 물리적 위치는 총 4곳이며,
그중 실제 커브 지형은 2곳(seg14, seg16) -- 둘 다 apex_mode 전 구간
공백이지만 harsh_brake/조향진동 등 실제 불편 신호는 이번 로그
표본에서 전무하다. "306/307차 가설이 실측으로 뒷받침되는가"는
여전히 "production이 후보를 놓쳤다"(계측 사실, 참) 단계에 머물러
있고 "그로 인해 실제 문제가 생겼다"(§28/§29, 아직 미확인) 단계로는
나아가지 못했다.

**한계(§28)**:
1. 단일 로그(17세그먼트) 표본, 4개 위치 모두 완만한 저속 커브 --
   급커브 상황 재현 여부 미확인.
2. seg14/seg16 모두 조향각이 크지 않아(각각 최대 10도, 26.9도 부근)
   "완만해서 감속이 애초에 불필요했다"는 대안 설명을 배제하지 못함
   -- 코드 레벨(곡률-속도 매핑) 확인 없이는 구분 불가.
3. 원거리+이동중 19건(310차부터 이월)은 이번에도 미착수.

**검증**:
- 정적 분석: 해당 없음(신규 파일 없음)
- 로그 검증: 완료(x17seg 20171프레임, 3회 연속 재현성 확인)
- 시뮬레이션: 해당 없음
- 실차 검증: **미완**(seg14도 seg16과 동일하게 "실제 불편" 미확인,
  §29)

**Devnotes**: FINDINGS.md 312차 항목 신규, 이 WIP 항목.
`toolkit/README.md`/`CHANGELOG.md` 변경 없음(311차부터 이월된
`--check-cruise`/위치 군집화 옵션 정식 편입은 여전히 다음 작업 -- 이번
세션에서 군집화 로직 자체는 검증됐으므로 다음 세션에 옵션화하기
용이해짐).

**미확인 사항**:
- 원거리+이동중 19건 lookahead 끝단 아티팩트 가설 코드 레벨 확인
  (310차부터 이월, 계속 미착수).
- seg16 "커브 통과 후 재개입" 가설 코드 레벨 확인(311차 이월).
- seg14/seg16의 "완만해서 감속 불필요"라는 대안 설명 코드 레벨 검증.
- 다른 로그에서도 이번 4개 위치 군집 패턴(episode 다수 = 위치
  소수)이 재현되는지 미확인.

**다음 작업**:
1. `sim_route_310_provisional_streak_real_corpus.py`에
   `--check-cruise`(cruiseEnabled 필터) + 위치 군집화(gap 기준
   episode 병합) 옵션을 정식 추가 -- 311/312차 두 세션에서 검증된
   대화식 로직을 스크립트에 편입. 이렇게 하면 다음 세션은 "episode
   23건"이 아니라 "위치 4곳"을 바로 볼 수 있음.
2. 원거리+이동중 19건의 lookahead 끝단 아티팩트 가설 코드 레벨 확인
   착수(`get_path_after_distance()`/naviPaths 리샘플 경계 확인).
3. seg16 cruiseEnabled False->True 전환 시점의 "커브 통과 후 재개입"
   가설 코드 레벨 추적.
4. 위 결과 누적 후 `PROVISIONAL_PROMOTE_STREAK=3` 조정 여부/설계안
   A 채택 여부 결정(사용자 계획 ②단계) -- 현재까지는 "실제 불편
   증거 0건" 상태이므로 조정을 서두를 근거가 약함을 다음 세션에
   명시적으로 논의 필요.
...
`ryu-devnotes`(HEAD `e6984f9`=303차, 이 항목 추가 전)

**Branch**: `c3-ms-dev` / `main`

**세션 시작 확인(§3/§33)**: fresh clone으로 양쪽 HEAD 직접 조회 --
`ryu` `d2f47d1`(290차, 드리프트 없음), `ryu-devnotes` `e6984f9`(303차,
드리프트 없음). 다른 작업자의 개입 흔적 없음을 확인 후 착수.

**배경**: 303차가 a3b3373495 #4→#5 구간에서 "naviPaths 기반 프레임이
2.05~2.07초 간격으로 3회 점프"하는 현상을 발견했으나, 원인은
"`build_frames()`(300차)가 naviPaths 빈 값 또는 nRoadLimitSpeed<=0인
행을 건너뛰기 때문으로 추정"이라고만 적고 코드 레벨로 확정하지
않았다(FINDINGS.md 303차). 이번 세션은 그 추정을 원본 CSV 실측 +
코드 추적으로 확정하는 것이 목표(§28: 추정 -> 실측 확정).

**한 일**:
1. 신규 `sim_route_304_navipaths_gap_audit.py` 작성(§22) -- 300차
   `build_frames()`의 두 skip 조건(naviPaths 빈 값 / nRoadLimitSpeed
   <=0)을 원본과 100% 동일 로직으로 재현해 각 CSV 행에 KEEP/DROP
   사유를 부여하고, naviPaths가 일정 시간(기본 1.0s) 이상 비어있는
   구간을 전부 나열. 182차 계측 필드(naviPointsActive/navdActive/
   dtRouteInactive/routeSource)를 함께 출력해 182차형 dropout과
   신규 현상을 자동 구분.
2. route1~4(a3b3373495/01742d6c1c/bf794c0073/c8d2619479) 전체
   80,046행에 대해 실행.
3. `carrot_man.py` 재확인 -- `carrot_navi_route()`(897~923행)의
   `navi_points_active` early-return 분기 3곳, `get_path_after_
   distance()`(298행) 정의, `self.navi_points` 대입/초기화 지점
   전수 grep(§30: git log -> 변경 함수 -> 호출자 순으로 확장하는
   대신, 이번엔 이미 아는 함수 내부를 좁혀서 추적).
4. 이번 세션 컨테이너에서는 route_bf794c0073.csv 실물만 재확보
   가능했음 -- 해당 route로 스크립트를 재실행해 직전(동일 대화
   내 이전 컨테이너) 결과와 완전히 동일한 출력(구간 4개, 시각/
   avg_vEgo/naviPointsActive 전부 일치)을 재현함을 자체 검증
   (a3b3373495/01742d6c1c/c8d2619479 수치는 직전 컨테이너에서
   동일 스크립트로 이미 실측된 값을 그대로 인용).

**실측 결과(핵심)**: FINDINGS.md 304차 참고.
- `nRoadLimitSpeed<=0`은 route1~4 전체(80,046행)에서 **단 한 번도
  발생하지 않음** -- 303차가 추정한 두 원인 후보 중 이쪽은 완전히
  기각.
- naviPaths가 비는 현상은 명확히 두 유형으로 갈린다:
  - **182차형**(`naviPointsActive=False`): a3b3373495 로그 시작부
    76.99초(t=135.6~212.6) 1건뿐.
  - **신규 하위유형**(`naviPointsActive=True`/`navdActive=True`/
    `dtRouteInactive=0.0` 유지, 182차 계측 기준 "정상"인데도
    naviPaths만 빔): route1~4 합계 **65건**(a3b3373495 37건/
    01742d6c1c 17건/bf794c0073 4건/c8d2619479 7건). 지속시간이
    **17~56kph 전 속도 범위에서 항상 1.90~1.96초(39~40프레임)로
    고정** -- 이동거리는 9~54m로 제각각이라 **거리 기반이 아니라
    시간 기반(고정 주기) 현상**임을 실측으로 확인. 303차가 발견한
    #4→#5 점프가 바로 이 유형의 3연속 사례.
- 코드 추적: `navi_points_active` early-return(897행) 분기는
  `naviPointsActive=True` 구간에서는 통과되므로 원인이 아니다.
  `carrot_navi_route()`가 실제로 빈 결과를 반환할 수 있는 남은
  지점은 `get_path_after_distance()`(298행)가 만드는 `path`가
  비었을 때(967행 `if path:` 분기) -- 즉 `self.navi_points` 버퍼
  자체가 순간적으로 고갈되는 지점으로 원인 위치가 좁혀짐. 182차가
  계측한 "navi 데이터 파이프라인" 상위 단계와는 구별되는, 한 단계
  더 안쪽(버퍼/윈도우 관리)의 실패모드.

**중요 한계(§28)**:
- `self.navi_points`/`navi_points_start_index`의 실제 런타임 값은
  CSV에 없어 이번 세션에서는 코드 구조 추론까지만 진행 -- 상위
  트리거(TCP 7712 `handle_route()` 배치 수신 주기, 세그먼트 경계
  등)를 확정하려면 그 두 값 자체를 새 cereal 필드로 계측하는 패치가
  필요(182차와 동일 패턴: 계측 추가 -> 다음 실차 로그로 재분석).
- 정적 분석 + 로그 실측(4개 route CSV)만 수행 -- **실차 검증: 미실시**.
- 이 현상이 300~303차가 다뤄온 route 상태머신(ACTIVE/INERT, RELEASE
  판정 등)의 판단 결과 자체를 얼마나 왜곡하는지는 이번 세션에서
  정량화하지 않음 -- 303차 `miss_frames`/`B_survive` 류 지표 해석에
  이번 발견을 반영하는 것은 다음 세션 과제.

**미확인/다음 작업**:
1. `self.navi_points` 버퍼 소진 상위 트리거 확정(신규 cereal 계측
   패치 필요).
2. 65건 신규 하위유형이 300~303차 route 연속성 분석 결론(특히
   303차 "naviPaths apex 거리값 연속성은 판별 feature로 부적합할
   가능성" 관찰)에 미치는 영향 재평가.
3. STEP3(LOST 재정의 설계)/STEP4/STEP5(303차에서 이월된 counterfactual)
   -- 이번 세션도 착수하지 않음, 여전히 다음 세션 과제.

**범위**: `ryu` 코드 변경 없음(분석만) + `toolkit/
sim_route_304_navipaths_gap_audit.py` 신규, README/CHANGELOG 갱신.
...
**Repository**: `ryujmin97/ryu`(HEAD `d2f47d1`=290차, 변경 없음) /
`ryu-devnotes`(HEAD -- 297차 patch가 아직 사용자에 의해 push되기
전이라 이 세션도 296차 HEAD `34dadfb`에서 시작, 297차 변경사항은
로컬에 그대로 유지한 채 이어서 작업)

**Branch**: `c3-ms-dev` / `main`

**세션 시작 확인(§3)**: `ryu` 드리프트 없음. `ryu-devnotes`는 296차
HEAD -- 297차 patch가 아직 push 전 상태임을 확인, 297차 내용(WIP/
FINDINGS/toolkit README·CHANGELOG/`sim_route_297_reacquire_gap_real_corpus.py`)은
로컬에 그대로 유지.

**배경**: 297차가 남긴 "다음 작업 1순위"(15건 qcamera 육안 대조)를
착수.

**한 일**:
1. `verify_and_extract_frames.py`(기존 도구, §21 재사용)로 4개
   route 전체 15개 이벤트 시각을 세그먼트 자동 매칭 + 프레임 추출
   (`--context 1`로 전후 1프레임씩 포함, 총 45장). **15/15 전부
   OK 매칭**(diff_s 0.021s 이하, out-of-range/오매칭 없음).
2. 45장 전부 육안 확인(대상 프레임 15장 + 전후 문맥 프레임 30장).
   판정 기준: 화면상 도로 형상이 실제로 굽어있는지, 표지판/가드레일/
   방음벽 등으로 곡선 여부를 육안 확인 가능한지.
3. 신규 `evidence/route_297_seamless_release_qcamera/`에 45장 전체 +
   분류표(`classification.md`) 보관(§21/§22 -- 재사용 도구만 쓰고
   신규 toolkit 스크립트는 추가하지 않음, 산출물만 evidence/에 커밋 --
   대용량 CSV 아닌 개별 소형 jpg라 §23 제한 대상 아님, 기존
   `evidence/vision_radar_ttc_limit/` 등과 동일 관례).
...
| 실제 커브 확인 | 7건 | 46.7% |
| 불분명/약함 | 3건 | 20.0% |
| 커브 없음(맵/GPS 후보 노이즈 의심) | 5건 | 33.3% |

- **7건(#1,2,6,11,12,13,14)**은 표지판/가드레일/방음벽 곡선이 화면상
  뚜렷하게 확인됨 -- 특히 저속 목표(#13 33.2kph, #14 26.0kph)는 화면상
  급커브와 시각적으로도 일치.
- **5건(#3,7,8,9,10)**은 전후 프레임 포함해도 도로가 명백히 직선 --
  이 중 1건(#3, t=536.76)은 **터널 내부**에서 발생, 전후 프레임 모두
  완전 직선 확인. 293/295차가 다루던 "터널 GPS 열화 가설"과 동일 유형
  위치에서 나온 사례로, `horizontalAccuracy` 상시 0.0(295차)이라 정량
  검증은 여전히 불가능하지만 정성적으로는 그 가설과 정합적임(§28,
  이것만으로 가설을 확정하지 않음 -- 표본 1건).
  나머지 4건(#7,8,9,10)은 특별한 지형학적 특이점(교차로 신호등 1건
  포함) 없이 그냥 직선도로 -- 맵 candidate 소스 자체의 노이즈로 추정
  (294차가 다른 통계에서 확인한 "과반이 커브 없음" 패턴과 유사한 방향).
- **3건(#4,5,15)**은 산악지형/시가지에서 목표 지점이 화면 밖(원거리)일
  가능성이 있어 명확히 판정하지 못함 -- "커브 없음"으로 단정하지 않고
  불분명으로 남김(§28, 근거 부족 시 추측 금지).
- **부수 관찰(신규)**: #1/#2, #4/#5, #9/#10 세 쌍은 각각 같은 물리적
  위치를 6~7초 간격으로 두 번 잡은 것으로 보임(같은 도로 구간, 근접한
  t값) -- 15건이 15개의 독립된 물리적 상황이 아니라 일부는 "같은
  커브에 대해 continuity가 두 번 끊겼다 재탐색"된 것일 가능성. 정확한
  GPS 좌표 대조로 확정하지는 않음(이번 세션 범위 밖, 다음 세션 후보).

**결론**: 297차 실측 24.6%(15/61) 중 "seamless forced-release가 실제
유효한 커브에서 발생"하는 비율은 대략 절반 미만(46.7%, 불분명 포함
시 최대 66.7%)이다. 즉 이 코드 경로 개선(재탐색된 apex가 감속
대상이면 ACTIVE 유지)의 실질적 효용은 297차가 시사한 것(15/15
100% needs_decel)보다는 낮을 수 있다 -- **"감속 대상인가"(물리량)와
"실제 커브인가"(candidate 소스 자체의 타당성)는 서로 다른 질문이며,
후자의 상당수(33.3%, 최대 53.3%)가 실패한다면 개선안이 "잘못된
candidate에 대해서도 계속 ACTIVE를 유지"하는 부작용을 가질 수 있음
-- 다음 세션 코드 변경 설계 시 이 트레이드오프를 반드시 반영해야 함.

**검증**:
- 정적 분석: 해당 없음(기존 도구 재사용, 신규 코드 없음)
- 로그 검증: 완료(15/15 세그먼트 매칭 OK, 45장 육안 확인)
- 시뮬레이션: 해당 없음
- 실차 검증: 해당 없음(ANALYSIS_ONLY, `ryu` 코드 무변경, §29)

**미확인/남은 것**:
- #1/#2, #4/#5, #9/#10 쌍이 실제로 동일 물리적 위치인지 GPS 좌표
  직접 대조로 확정 필요(다음 세션)
- #4,5,15 불분명 판정 3건을 naviPaths 곡률 재계산값과 직접 대조하면
  더 명확히 판정 가능할 수 있음(이번 세션은 육안만 사용)
- 터널 내부 후보(#3) 1건만으로는 터널 가설을 뒷받침하는 근거로 쓰기엔
  표본 부족 -- 추가 사례 필요
- 코드 변경 여전히 미착수(§31 승인 전, 위 트레이드오프 반영한 설계
  필요)

**Devnotes**:
- `evidence/route_297_seamless_release_qcamera/`: 신규(45장 jpg +
  manifest.json 4개 + classification.md)
- `toolkit/README.md`/`CHANGELOG.md`: 미변경(신규 스크립트 없음,
  기존 `verify_and_extract_frames.py` 재사용뿐이라 §22 "신규 도구
  추가" 케이스 아님)
- WIP/FINDINGS: 이 항목

**다음 작업**:
1. #1/#2, #4/#5, #9/#10 GPS 좌표 대조로 "같은 물리적 커브 재탐색
   2회" 가설 확정
2. 33.3%(5/15) "커브 없음" 판정을 naviPaths 재계산 곡률값과 교차
   검증(coordinates에서 실제 curvature=0에 가까운지 수치로 확인)
3. 위 결과 종합해 코드 변경안(§31) 설계 착수 여부 결정 -- "감속
   대상이면 무조건 ACTIVE 유지"가 아니라 "감속 대상 + candidate
   신뢰도"를 함께 보는 방향 검토 필요
4. (293차부터 이월) ep108 클러스터링 코드 레벨 추적
5. (294차부터 이월) ep47/48/101/105 패턴 코드 레벨 추적

**패치**: `0001-298cha-devnotes.patch`(297차 patch 위에 순서대로 적용
필요 -- 아래 사용자 작업 순서 참고)
...
**Repository**: `ryujmin97/ryu`(HEAD `d2f47d1`=290차, 변경 없음, fresh clone
드리프트 없음 확인) / `ryu-devnotes`(HEAD `ad36b2e`=293차, 이 항목 추가 전)

**Branch**: `c3-ms-dev` / `main`

**세션 시작 확인(§3)**: 양쪽 fresh clone. HANDOFF.md/CURRENT_STATUS.md
없음. `ryu-devnotes` HEAD가 293차(`ad36b2e`)와 동일 -- 다른 작업자 push
없음, 드리프트 없음 확인. 이전 세션이 도구 사용 한도로 중단되어 이어서
진행.

**배경**: 293차에서 "사용자 결정 대기"로 남긴 세 항목 중 (a) 나머지
28건 qcamera 육안 대조를 이어서 진행. route1~4 corpus(80145행)를
재추출하고 293차와 동일한 seg/t 매핑으로 `extract_dashcam_frames.py`
호출을 재현.

**한 일**:
1. route1~4 재추출(`extract_log.py --with-navi-paths`, ryu HEAD
   `d2f47d1` 그대로) -- 22801+22799+23999+10546=80145행, 293차와 동일한
   결과 재확인.
2. `sim_route_289_margin_ab_real_log.py` --new-ratio 1.05,
   `sim_route_292_continuity_root_cause.py` 순서로 재실행 -- 289차/292차/
   293차와 정확히 일치하는 51/27/21/11, 연장 30건, flicker 4→1건,
   `lost_no_candidate` 계열 35건(코드 필터 기준 실측치 -- 293차
   WIP/FINDINGS에 적은 "36건"과 1건 차이 있음, 아래 §주의 참고) 확인.
3. `lost_no_candidate` 35건 중 293차가 이미 본 7건(ep9,10,12,13,14,15,46)
   을 제외한 나머지 28건 전부 seg/t 매핑 후 `extract_dashcam_frames.py`
   로 프레임 추출(28장) + 육안 대조 완료.

**§주의 -- 건수 정정**: 293차 WIP/FINDINGS는 `lost_no_candidate`를
"36건"으로 기록했으나, 이번 세션에서 `sim292_continuity_breakdown_qcamera_candidates.csv`를
`sub_cause.str.startswith('lost_no_candidate')`로 직접 필터링한 실측치는
**35건**(7건 기리뷰 + 28건 신규)이었다. 원인은 미확인(반올림/카운트
스크립트 차이 추정) -- 코드/스크립트 재확인 없이 임의로 "36"과 "35" 중
하나를 정답으로 단정하지 않음(§28). 다음 세션에서 원인 확인 필요.

**28건 육안 대조 결과 (293차 7건 포함 전체 35건 집계)**:

| 분류 | 건수 | ep 목록 |
|---|---|---|
| 터널 구간 | 2 | 9, 10 |
| 뚜렷한 실제 커브 확인(소실 타당) | 13 | 22,23,24 / 30,31,32 / 41 / 46 / 67 / 71 / 82,83 / 91 |
| **커브 없음/직선(원인 미상)** | **18** | 12,13,14,15 / 16,17 / 36 / 37 / 40 / 47,48 / 52,53 / 73,74,75 / 101 / 105 |
| 판단 보류(역광 등) | 2 | 66, 79 |

이번 세션 신규 검토 28건 내 세부 소견:
- **뚜렷한 커브 확인**: ep22/23/24(산길 우→좌→우 연속 S커브), ep30/31/32
  (커브 경고 표지판 동반 좌커브 연속), ep41(완만 우커브), ep67(가드레일
  좌회전 형상, 역광 있으나 판독 가능), ep71(279차 이후 tolerance 확장
  대상이었던 바로 그 S커브 진입부, t=2116~2122.2), ep82/83(고가 하부
  우커브, 2.3초 간격 -- 동일 커브 내 연속 소실로 추정), ep91(마주오는
  차량과 함께 뚜렷한 우커브).
- **커브 없음/직선**: ep16/17(고속도로 직선), ep36(직선), ep37(교차로
  인근 완만), ep40(직선 시골길), **ep47/48(도심 다차선 직선, 산업단지
  인근 -- 46차 이후 반복되는 "지도 곡률 노이즈" 패턴과 유사)**,
  ep52/53(교차로/개활지), ep73/74/75(고속도로 직선, 육교/지하차도 인근,
  대전 방향 이정표 구간), **ep101(도심 교차로 직선, 아파트단지)**,
  **ep105(도심 대로 직선, 아파트단지)** -- 특히 ep47/48/101/105 네 건
  모두 도심/아파트단지 직선 대로라는 공통점.
- **판단 보류**: ep66(역광 심함, 재확인 필요), ep79(완만한 좌커브/
  교차로 -- 커브라 하기엔 약하고 교차로 형상과 겹쳐 애매).

**핵심 발견**: 293차는 8건 표본만으로 "화면상 특이점 없음"을 산발적
관찰로 남겼으나, 35건 전수 대조 결과 **과반(18/35, 51%)이 명확히
직선/커브 없음** 구간에서 발생했다. 반면 실제 커브가 뚜렷한 경우는
13/35(37%)에 그친다. 이는 289차가 우려했던 "margin 완화의 부작용으로
continuity 소실이 늘어난다"는 프레임보다, **route candidate 소스(navi
맵 곡률 데이터)가 직선 도로에서도 허위 후보를 발생시키고 이 허위
후보가 몇 프레임 뒤 소실되는 것이 `lost_no_candidate`의 상당 비중을
차지한다**는 가설 쪽으로 무게가 옮겨감을 시사한다. 특히 ep47/48/101/105
가 모두 도심 아파트단지 직선 대로라는 공통점은 우연이라기엔 패턴이
뚜렷함.

**아직 검증 안 된 것**: 이 가설은 여전히 qcamera 육안 대조 수준이며,
실제 route candidate 생성 로직(navi 맵 곡률 추출 -> `route_find_clusters()`)
코드/데이터 레벨로 "왜 직선 구간에서 candidate가 생겼다 사라지는가"를
추적하지 않았다(§28: 증상→재현조건→...→원인 순서 중 아직 "재현조건/
입력" 단계에 머물러 있음). 코드/파라미터 변경 없음(§27/§28).

**다음 작업 후보 (사용자 결정 필요)**:
- (b) `horizontalAccuracy` 컬럼을 `extract_log.py`에 추가해 터널 가설(ep9/10)
  및 이번 "직선인데 candidate 발생" 18건의 GPS 신뢰도 재검증 (§22 신규
  toolkit 변경 -- README/CHANGELOG 동반 갱신 필요, 이번 세션 미착수)
- (c) ep108(주택가, `lost_with_candidates_present`, 클러스터링 혼선 의심)
  소스코드 레벨 분석 착수 (293차부터 대기 중)
- **(신규, d)** 이번 세션 신규 발견: ep47/48/101/105 "도심 직선인데
  candidate 발생" 패턴을 `route_find_clusters()` / navi 맵 곡률 추출
  코드 레벨로 직접 추적 -- 왜 직선 도로에서 곡률 후보가 생성되는지
  원인 규명 필요.
- "36건" vs "35건" 카운트 불일치 원인 확인 (§주의 참고)

**검증**: 정적 분석 -- 미실시. 로그/시뮬레이션 -- 293차 파이프라인
재실행 결과 일치 확인. qcamera 육안 대조 -- 28건 완료(위 표). 실차
검증 -- 미실시(§29).
...
**Repository**: `ryujmin97/ryu`(HEAD `d2f47d1`=290차 패치 반영, fresh clone으로
드리프트 없음 확인) / `ryu-devnotes`(HEAD `eda924a`=290차, 이 항목 추가 전)

**Branch**: `c3-ms-dev` / `main`

**세션 시작 확인(§3)**: `ryu`/`ryu-devnotes` 양쪽 fresh clone. HANDOFF.md/
CURRENT_STATUS.md 없음. 드리프트 없음.

**배경**: 사용자가 route1~4 zip(283차부터 쓰던 것과 동일 파일명
`a3b3373495`/`bf794c0073`/`01742d6c1c`/`c8d2619479`, 재업로드)과
`params_backup-6.json`을 올리며 "라우트가 vturn보다 값이 작아 이긴 경우를
분석하고, 라우트를 vturn 속도와 비슷하게 하려면 파라미터를 어떻게
조정하면 되는지" 질문.

**한 일**:
1. route1~4 재추출(`extract_log.py`, ryu HEAD `d2f47d1` 그대로) --
   22801/10546/22799/23999행, 283차 이후 기록과 행수 일치(동일 corpus
   재확인).
2. 신규 `toolkit/analyze_route_vturn_gap_291.py` 작성 -- `src=='route'`
   프레임마다 vturn 후보(`max(abs(vTurnSpeed), AutoCurveSpeedLowerLimit)`)와
   route 후보(`desiredSpeed`, 곧 `route_speed`) 격차(gap)를 계산. gap이
   유의미한 프레임을 연속구간(에피소드)으로 묶고, `--vturn-active-thresh`로
   "vturn도 같은 구간에서 실제로 무언가 인지 중(raw가 saturation ceiling
   ~250 근처가 아님)"인 부분집합을 별도 집계하도록 설계(단순 전체집계는
   "vturn이 아직 커브를 못 본 상태"까지 섞여 해석을 오도하기 때문,
   아래 note 참고).
3. `params_backup-6.json` 실측값(AutoCurveSpeedLowerLimit=20,
   MapTurnSpeedFactor=110%, AutoNaviSpeedDecelRate=70(=0.70m/s²),
   AutoCurveSpeedFactor=80%, AutoCurveSpeedAggressiveness=100%,
   TurnSpeedControlMode=2) 기준으로 실행 + `carrot_man.py`/`carrot_serv.py`
   코드 추적으로 두 소스의 계산 경로 차이 확인.

**핵심 발견**:
- route1~4 전체 `src=='route'` 프레임 중 vturn 후보값을 gap>=1kph로
  이긴 비율이 거의 100%(route1 100.0%/route2 100.0%/route3 99.8%/route4
  100.0%), 평균 gap 52~116kph, p90 129~202kph.
- **그러나 이 중 다수는 "route가 vturn을 실력으로 이겼다"가 아니라
  "vturn이 아직 그 커브를 전혀 인지하지 못한 상태(raw가 ±250 근처
  saturation)에서 route만 navi map으로 먼저 감속을 시작한" 상황** --
  `--vturn-active-thresh 200`(|vTurnSpeed raw|<200)으로 걸러도 여전히
  route1 934건/route2 167건/route3 835건/route4 318건이 남고 평균
  gap도 21.6~60.2kph로 완전히 사라지지는 않지만, 전체집계 대비 뚜렷이
  줄어듦(route2는 116.0->21.6kph로 5배 이상 축소) -- gap의 상당 부분이
  "같은 커브에 대한 진짜 이견"이 아니라 "vturn의 인지 시점이 route보다
  늦다"는, 234~236차가 이미 설계 의도로 확정한 바로 그 구조(route는
  vturn이 못 보는 원거리를 먼저 감속시키는 역할)의 재확인임.
- 프레임 단위로 직접 대조한 사례(route4 t=3173.06~3174.01, apex_dist
  110->100m, vEgo 40.6->42.7kph)에서: 이 시점 `desiredSpeed`(route)는
  아직 `v_ego_kph`를 그대로 통과 중(=아직 감속 시작 전, apexSpeed 목표는
  이미 22~27kph로 계산돼 있으나 `AutoNaviSpeedDecelRate`(0.70m/s²)
  물리공식상 감속을 시작해야 할 거리(약 59m)보다 apex가 아직 멀어서
  대기 중)인 반면 `vTurnSpeed` raw는 150~226(=vturn이 아직 이 커브를
  거의 못 봄, `vturn_lookahead_horizon_s`=8.0s*11.4m/s≈91m로 100m는
  horizon 경계 밖) -- gap 200 근처가 "route가 이 커브를 vturn보다 훨씬
  가혹하게 계산해서"가 아니라 "이 시점엔 애초에 vturn이 이 커브에 대해
  아무 의견이 없어서"임을 원본 텔레메트리로 직접 확인.
- apex_dist<=30m(=vturn 입장에서도 충분히 가까운 근접구간)로 좁혀도
  817개 프레임에서 여전히 gap>=1kph가 남고, 상위 사례(route2
  t=3962.6~3964.2, apex_dist 10~30m)는 route desired가 vEgo에 근접한
  53~55kph인데 vTurnSpeed raw가 -215~-249(또한 saturation 근처, 부호도
  반대)로 확인 -- 이 구간은 "같은 커브를 두고 route가 더 가혹하게
  깎았다"가 아니라 "이 근접거리에서도 vturn이 이 지점을 커브로 인지하지
  않고 있다"는 별개의(아마도 map 곡률 오탐 또는 실제로 vturn 기준
  안전한 완만한 굽이) 상황 -- **qcamera/dashcam 대조 없이는 route
  오차인지 map 데이터 오탐인지 결론 낼 수 없음, 이번 세션 범위 밖.**

**결론(사용자 질문에 대한 답)**:
1. "route가 vturn보다 작아서 이기는 경우"의 절대다수는 **버그가 아니라
   설계대로 동작 중** -- route가 vturn의 시야(≈8초/거리 horizon) 밖에서
   먼저 감속을 시작하는 것 자체가 route 존재 이유(234~236차 결론)이므로,
   이 넓은 gap을 "줄이는" 방향의 파라미터 조정은 route의 핵심 기능을
   약화시키는 트레이드오프가 됨 -- 권장하지 않음.
2. 그럼에도 "route를 vturn과 비슷하게" 만들고 싶다면(같은 커브를 둘 다
   인지 중인 근접구간에서의 격차를 줄이고 싶다면) 코드 추적 결과 후보는:
   - **`MapTurnSpeedFactor`(현재 110%)**: 279차부터 route의 macro/fine
     곡률->속도 테이블(`V_CURVE_LOOKUP`) 결과에 곱해지는 **route 전용**
     배율(vturn에는 영향 없음, `carrot_man.py` L1004-1066). 이 값을
     올리면(예: 110->130~150) route의 커브 목표속도 자체가 비례해서
     올라가 vturn과의 격차가 좁혀짐 -- side effect가 route 쪽에만
     한정되는 가장 "외과적인" 단일 knob.
   - `AutoNaviSpeedDecelRate`(현재 70=0.70m/s²): route가 목표속도로
     접근하는 감속 스케줄(언제부터/얼마나 가파르게 깎기 시작하는지)만
     결정, apex 목표속도 자체는 안 바꿈 -- 올리면 route가 더 늦게/더
     급하게 개입해 "장시간 낮게 유지되는 gap 구간" 자체는 줄어들지만
     apex 근처 최종 목표차이는 그대로.
   - `AutoCurveSpeedLowerLimit`(현재 20, route/vturn 공용 하한): 두
     소스 모두에 동일하게 적용되는 바닥이라 이 값을 올려도 두 소스
     사이 비율 자체는 거의 안 바뀜(둘 다 같이 올라감) -- gap이 이
     바닥값 부근에 몰려있는 극히 낮은 속도 구간에서만 제한적 효과.
   - (반대 방향) `AutoCurveSpeedFactor`/`AutoCurveSpeedAggressiveness`를
     낮춰 vturn을 route 쪽으로 끌어내리는 것도 가능하나, 이건 route가
     관여하지 않는 모든 일반 커브의 vturn 반응까지 전부 바꿔버려
     side effect 범위가 `MapTurnSpeedFactor`보다 훨씬 큼.
3. **다음 세션 진행하려면**: (a) `--vturn-active-thresh` 부분집합만
   추려서 `MapTurnSpeedFactor` what-if 시뮬레이션(287~290차 방식대로
   ryu 코드는 안 건드리고 오프라인 재계산)으로 후보값(120/130/140 등)별
   gap 축소 폭 실측, (b) apex_dist<=30m인데도 vturn이 인지 못하는
   817건 중 대표 사례 qcamera 대조(235차 방식)로 진짜 지형 이견인지
   navi map 곡률 오탐인지 원인 규명 -- 둘 다 사용자 결정/우선순위
   대기, 코드/파라미터 변경 미착수.

**검증**:
- 정적 분석: 신규 스크립트 실행 확인(4개 CSV 전부 정상 처리)
- 로그 검증: 완료(위 "핵심 발견" 참고, 원본 텔레메트리 프레임 직접 대조)
- 시뮬레이션: 미실시(what-if 파라미터 시뮬레이션은 다음 세션 과제)
- 실차 검증: **해당 없음(파라미터 변경 자체가 없었음)**

**미확인 사항**: apex_dist<=30m인데 vturn이 saturation인 817건의 진짜
원인(navi 곡률 오탐 vs 실제로 vturn 기준 안전한 완만한 굽이 vs 진짜
사각지대) -- 다음 세션 우선 후보.

**다음 작업**:
- (사용자 결정 대기) `MapTurnSpeedFactor` what-if 시뮬레이션 착수 여부
- (사용자 결정 대기) apex_dist<=30m 근접 불일치 사례 qcamera 대조 착수 여부
...
**Repository**: `ryujmin97/ryu`(변경 없음, HEAD `4d20122`=282차, fresh
clone으로 드리프트 없음 확인) / `ryu-devnotes`(HEAD `3398ffd`=285차, 이
항목 추가 전)

**Branch**: `c3-ms-dev` / `main`

**세션 시작 확인(§3)**: `ryu-devnotes` HEAD `3398ffd`(285차), `ryu` HEAD
`4d20122`(282차, 변경 없음). HANDOFF.md/CURRENT_STATUS.md 없음, 동시작업
없음 확인.

**배경**: 284차 "다음 작업" 2번(235차 S커브 위치 재식별)이 원본 corpus
(`0000039a--7b602ffb85` seg12-16) 미보관으로 계속 보류 중이었는데,
사용자가 "그 로그는 없으니, 업로드된 로그(route1~4)에서 유사한 로그를
찾아서 분석"으로 방향 전환 지시. 즉 235차와 물리적으로 동일한 위치를
찾는 대신, 현재 보유 corpus(283/285차와 동일한 route1~4)에서 "성격이
비슷한 S커브 패턴"을 찾아 `ROUTE_RELEASE_HOLD_S=0.0`의 두 번째 커브
반응성을 대체 검증하기로 함.

**한 일**:
1. toolkit README/analysis_helpers.py를 먼저 확인(§21) -- 기존
   `curve_apex_vs_gap_delta()`(46차)가 커브 이벤트 분리는 하지만
   "연속된 두 커브의 방향반전+짧은 간격" 페어링 기능은 없어 신규
   스캐너 필요로 판단.
2. 신규 toolkit `scan_consecutive_curve_pairs.py` 작성 -- 기존 커브
   이벤트 분리 방식(`curve_apex_vs_gap_delta()`와 동일 임계값 5.0/3.0deg)
   을 재사용하되, 연속 이벤트 쌍의 gap+부호반전 여부로 S커브 후보를
   추출하고 커브2 구간의 src 분포/route_frac/aEgo range를 부가 계산하는
   기능을 추가(§27, 최소 확장).
3. route1~4(a3b3373495/01742d6c1c/c8d2619479/bf794c0073, 283/285차와
   동일 corpus, 4개 CSV 총 80145행)를 재추출 후 스캐너 실행 -- 커브
   이벤트 181개, S커브 후보(gap<=2.0s+부호반전) 62건. 대부분(steer
   100~460deg)은 교차로 좌우회전으로 route_frac=0 -- 이번 목적과 무관.
4. `route_frac>0.1`(route 로직이 실제로 개입한) 후보 중 대표 2건을
   선정해 `extract_dashcam_frames.py`로 qcamera 프레임 추출/육안 확인:
   - **route2(01742d6c1c) t=2149.16~2154.16**(steer +12.4->-15.8deg,
     gap 0.597s): 가드레일+커브 경고표지판이 보이는 산길/고가 성격의
     실제 굽이길 확인. steer가 0을 관통하며 연속 반전 -- 진짜 단일
     S자 커브(교차로 아님).
   - **route1(a3b3373495) t=1055.8~1061.9**(steer +24.5->-20.8deg,
     gap 1.249s): 시골 2차선 도로(전원주택/농로 배경)의 실제 굽이길
     확인.
5. 두 위치 모두 CSV 상세 데이터(t 단위 src/desiredSpeed/routeApexIdx/
   aEgo)로 "두 번째 커브 반응성"을 직접 확인: 커브2 접근 중
   route<->vturn이 프레임 단위로 번갈아 승리하는 flicker가 재관측됨 --
   **283차가 이미 확인한 "min() arbitration에 히스테리시스 없음" 기존
   이슈와 동일 성격**(신규 이슈 아님, FINDINGS.md 기존 항목과 부합).
   이 flicker에도 불구하고 desiredSpeed는 매끄럽게 감소하고 aEgo도
   두 위치 모두 정상 주행 변동 범위 -- **`ROUTE_RELEASE_HOLD_S=0.0`으로
   인한 뚜렷한 pump/반응 지연 시그니처는 두 대체 위치 어디에서도
   관측 안 됨**.

**결론**:
- 235차 corpus와 물리적으로 동일한 위치는 아니지만, "성격이 비슷한
  S커브"에서 반응성을 대체 점검한 결과 281/283/285차의 기존 결론
  ("hold=0.0이 pump/진동을 유발하지 않는다")과 **일관됨**을 추가로
  확인.
- 223차 당시 우려("2차 곡선 반응 지연 해소 여부")에 대해서도, 이번
  대체 위치 2건에서는 커브2 접근 중 route가 정상적으로 개입해
  desiredSpeed를 감속 방향으로 갱신하고 있어 **지연의 징후는 없음**
  -- 다만 이는 235차가 확정한 정확한 그 물리적 위치(간격 250~290ms의
  더 급한 2차 커브)에 대한 검증은 아니므로, 완전한 대체는 아니고
  참고 근거로만 취급.

**검증**:
- 정적 분석: 신규 스크립트 `python3 -m py_compile` 통과(pandas/numpy만
  사용, 표준 toolkit 패턴).
- 로그 검증: route1~4 CSV(283/285차와 행수 완전 일치, 동일 corpus
  확인) 재추출 + 스캐너 실행 완료.
- qcamera 육안 확인: **완료**(대표 2건, 각 6프레임).
- 실차 검증: 새로운 실차 주행 없음(283차 실측 로그의 추가 분석).

**미확인 사항**:
- 235차가 확정한 실제 물리적 S커브 위치(`0000039a--7b602ffb85`
  seg12-16, 간격 250~290ms) 자체의 재검증은 여전히 그 corpus 재업로드
  전까지 불가능 -- 이번 286차는 대체/참고 근거일 뿐 완전한 해소가
  아님.
- `autoNaviSpeedCtrlEnd` 실제 device 파라미터값 (281차부터 계속
  미확인).
- route_frac=0인 나머지 60건(대부분 교차로 회전)은 이번 목적과 무관해
  상세 분석하지 않음 -- 필요 시 별도 이슈로 분리 검토 가능.
- device dirty=True 원인 (283차부터 계속 미확인, §33 성격).

**다음 작업**:
1. 사용자가 원하면 235차 원본 corpus(`0000039a--7b602ffb85` seg12-16)
   재업로드 시 정확한 위치 재검증 착수(여전히 최우선 대체 불가 항목).
2. (선택) route_frac=0 대다수 후보(교차로 좌우회전) 중 이상 패턴이
   있는지 별도 스캔 -- 이번 세션 목적과 무관해 보류.
3. `PARAMS_REGISTRY.md`의 `ROUTE_RELEASE_HOLD_S` 항목에 이번 286차
   결과(대체 위치 참고 근거로 기존 결론 보강)를 append.
...
**Repository**: `ryujmin97/ryu`(변경 없음, HEAD `4d20122`=282차) /
`ryu-devnotes`(HEAD `8a2f020`=283차, 이 항목 추가 전)

**Branch**: `c3-ms-dev` / `main`

**배경**: 사용자가 283차가 쓰던 로컬 보관 위치(`C:\dev\devnotes\works\`)를
`C:\dev\logs\`로 변경했고, 기존 `works` 폴더는 삭제했다고 알려옴. 동시에
"로그데이터 추출분이 추가로 나오면 나한테 전달해달라"고 재확인 요청.

**세션 시작 확인(§3)**: `ryu-devnotes` HEAD `8a2f020`(283차, 드리프트
없음), `ryu` HEAD `4d20122`(282차, 변경 없음). HANDOFF.md/
CURRENT_STATUS.md 없음.

**한 일**:
1. `SETUP.md`의 "CSV 보관(Google Drive)" 섹션에 로컬 보관 경로 변경
   내용을 추가 기록(Drive 연결이 없을 때의 대체 관행으로, 283차가 쓴
   `C:\dev\devnotes\works\`를 `C:\dev\logs\`로 대체 -- 기존 Drive 섹션
   본문은 삭제하지 않고 하단에 갱신 노트만 추가, §24/§14 원칙과 동일하게
   기존 기록 보존).
2. 이번 세션에 사용자가 원본 route zip 4건(`a3b3373495`/`01742d6c1c`/
   `c8d2619479`/`bf794c0073`, 각각 qcamera.ts+qlog.zst+rlog.zst 포함,
   283차가 이미 CSV로 추출/검증했던 바로 그 4개 route의 원본)을 재업로드
   -- 283차 "다음 작업" 1번(pump 후보 2건 qcamera 육안 재확인)에 필요한
   영상 자료가 이걸로 확보됨. 이번 세션에서는 문서 갱신만 하고 분석은
   진행하지 않음(사용자가 별도로 지시 시 이어감).
3. ryu 코드/toolkit 스크립트는 변경 없음 -- §27 최소변경 원칙상 문서 갱신
   범위로 한정.

**적용된 새 관행(다음 세션부터 적용)**:
- 로그 CSV 등 대용량 추출 산출물의 로컬 보관 경로: `C:\dev\logs\<이름>\`
  (기존 `C:\dev\devnotes\works\`는 폐기, 사용자가 삭제 완료).
- Claude/ChatGPT가 로그를 추출/가공한 결과물이 새로 나오면 매 세션마다
  파일로 사용자에게 전달한다(§18 원칙 재확인, 이미 표준 절차이나 사용자가
  명시적으로 재확인 요청).

**검증**: 문서 변경만 -- 정적/로그/시뮬레이션/실차 검증 해당 없음.

**미확인 사항**: 없음.

**다음 작업**:
1. 사용자가 원하면 이번에 재업로드된 원본 zip 4건으로 283차 "다음 작업"
   1번(pump 후보 2건, t≈772.2/t≈3179.6 부근 qcamera 육안 확인) 착수.
2. 283차 "다음 작업" 2번(235차 S커브 위치 재식별)은 235차 원본 corpus
   (`0000039a--7b602ffb85` seg12-16)가 아직 없어 여전히 보류 -- 필요 시
   사용자에게 해당 zip 재업로드 요청.
...
**Repository**: `ryujmin97/ryu`(Base commit `027774f`=276차 반영분, §3
fresh clone으로 확인) / `ryu-devnotes`(HEAD `fe13383`=276차 devnotes,
이 항목 추가 전)

**Branch**: `c3-ms-dev` / `main`

**배경**: 276차 패치를 기기에 반영한 사용자가 실차 스크린샷 2장을
첨부. "과속(이동식)" 문구가 뜰 때 우측 "대구부산고속도로/route=off/
vturn=-229" 블록이 위로 이동하면서 그 위에 있던 "도착: 188.9km" 잔여
거리/시간 줄과 글자가 뒤섞여 겹침. 또한 그 사이 경로안내창(TBT HUD)
박스 자체도 위로 커짐(276차가 도입한 `road_block_shift`=80px 때문).
사용자 지시: "과속 문구가 떠도 박스 크기/도로명·route·vturn 위치는
절대 변하지 않게 하고, 과속 문구만 여백을 고려해 아래쪽에 적당히
배치하라."

**원인 재확인**: 276차에서 `szSdiDescr`(과속/구간단속중 등)와
`szPosRoadName`(도로명/route=/vturn=)이 동시에 보일 때 겹치지 않게
한다는 명목으로 도로명 블록 전체를 80px 위로 밀고 박스 높이도 그만큼
늘렸음(`road_block_shift`). 그런데 이 블록의 최상단 줄(도로명, 원래
y=155)이 80px 밀리면 y=75가 되는데, 이는 그 위에 **고정 좌표로 그려
지는 ETA 잔여시간 줄**(`tbt_y+75`, "37.1분(10:33)" 형태)과 정확히
같은 y좌표 -- 서로 다른 블록인데 같은 줄에 겹쳐 그려져 화면에서
글자가 뒤섞임. road_block_shift는 도로명/route=/vturn= 블록 자체는
서로 겹치지 않게 했지만, 그 블록이 위로 밀려나면서 부딪힐 수 있는
"그 위에 이미 고정으로 있던 다른 요소"(ETA 시각)는 고려하지 못한
설계 결함이었음.

**한 일**:
1. `road_block_shift`/`has_sdi` 관련 로직 전부 제거. `tbt_extra_h`도
   다시 순수하게 도로명 줄바꿈(`name_wrapped`)에 의해서만 늘어나도록
   원복(276차 이전 동작과 동일 -- §27 최소변경 원칙에 따라 "필요한
   부분만" 되돌림).
2. 도로명(155)/route=(195)/vturn=(235) 세 줄의 y좌표에서 뺐던
   `road_block_shift`를 제거해 항상 고정 좌표만 사용하도록 복원.
   -> 이제 `szSdiDescr` 유무와 무관하게 이 세 줄과 박스 높이는 절대
   변하지 않음(사용자 요구사항 그대로).
3. `szSdiDescr`(과속/구간단속중) 자체의 y좌표만 220->250으로 조정
   (`TBT_SDI_Y` 상수 신설). 박스 하단(tbt_y+280)까지 30px, 바로 위
   vturn= 줄(235)까지 15px 여백. szSdiDescr는 좌측 정렬(x=tbt_x+20),
   vturn=은 우측 정렬(right_x 근처)이라 통상적인 문구 길이에서는
   폭 방향으로 겹치지 않으나, 두 문구가 모두 매우 길어지는 극단적인
   경우까지는 좌표 계산만으로 완전히 보장되지 않음(아래 미확인 사항).

**검증**:
- 정적 분석: 좌표식을 Python으로 격리 재현. "도로명 최상단 줄 -
  ETA 잔여시간 줄" 간격이 (도로명 1줄일 때 80px / 2줄 줄바꿈 시
  40px)로 276차 이전과 완전히 동일함을 재확인(회귀 해소, 235/155/195
  세 줄도 szSdiDescr 유무와 무관하게 좌표 불변 확인). szSdiDescr(250)
  - vturn=(235) 간격 15px, szSdiDescr - 박스 하단 간격 30px 확인.
  파일 중괄호 개수(`{`/`}`) 수정 전/후 동일(501/500, 기존부터 있던
  1개 불일치이며 이번 변경과 무관, 276차 검증 시에도 동일 확인함).
- Qt/nanovg/capnp 빌드 환경이 세션 컨테이너에 없어 실제 컴파일은
  미실시(276차와 동일하게 §29에 따라 명시).
- 패치 독립 검증: `ryujmin97/ryu` origin/c3-ms-dev(HEAD `027774f`)
  기준 fresh clone에서 `git apply --check` 통과, `git am` 적용 성공
  확인.
- **실차 검증: 미실시.** 기기 화면 반영 여부는 사용자가 패치 적용 후
  직접 확인 필요.

**미확인 사항**:
- szSdiDescr와 vturn= 문구가 각각 최대 길이로 동시에 나타나는 극단적
  케이스에서 폭 방향 겹침 가능성은 좌표 계산만으로는 완전히 배제되지
  않음 -- 실제 화면에서 재확인 필요.
- 도로명이 2줄로 줄바꿈되는 경우 ETA 잔여시간 줄과의 간격이 40px로
  줄어드는 것은 276차 이전부터 있던 기존 동작이며 이번 회귀와는
  무관 -- 이번 세션 범위 밖.

**다음 작업**:
- 사용자 실차/기기 반영 후 화면 캡처로 (1) 과속 문구가 떠도 도로명/
  route=/vturn=이 ETA 시각과 겹치지 않는지, (2) 박스 크기가 과속
  문구 유무와 무관하게 동일한지, (3) 과속 문구 자체의 위치가 적당한지
  재확인.
- 필요 시 TBT_SDI_Y(현재 250) 미세조정.

## 276차 (완료 -- 코드 수정+독립 검증(git am) 완료, 실차/기기 반영 검증 아님) -- TBT HUD: 구간단속중 표시 시 도로명/route=/vturn= 가려짐 + 박스 하단 잘림 수정

**Worker**: Claude

**Repository**: `ryujmin97/ryu`(Base commit `8c512cb`, §3 fresh clone으로
확인) / `ryu-devnotes`(HEAD `7dd0bde`=275차 devnotes, 이 항목 추가 전)

**Branch**: `c3-ms-dev` / `main`

**배경**: 사용자가 실제 기기 화면 스크린샷 2장 첨부. "구간단속중" 문구가
뜨는 구간에서는 평소 같이 보이던 "부산외곽순환고속도로 / route=off /
vturn=249" 하단 블록이 통째로 사라짐. 또한 경로안내창(TBT HUD) 박스
전체를 위로 약간 이동시켜 하단이 잘리는 현상도 없애달라는 요청.

**분석**: `selfdrive/ui/carrot.cc`의 `TurnInfoDrawer::drawTurnInfoHud()`에서
1) `szSdiDescr`(구간단속중 등 SDI 안내문)와 `szPosRoadName`(도로명/
   route=/vturn= 블록)이 `if (szSdiDescr...) {...} else if (szPosRoadName...)
   {...}` 구조로 서로 배타적으로만 그려지고 있었음(둘 중 하나만 표시).
   도로명 줄바꿈 계산(`wrap_name_lines`) 자체도 `szSdiDescr.length()==0`
   조건이 걸려 있어 SDI 문구가 있으면 아예 계산조차 안 됨.
2) 박스 y좌표(`tbt_y = s->fb_h - 250`) 기준 박스 하단이
   `tbt_y - 60 + TBT_BOX_H = tbt_y + 280`인데, 이는 `fb_h + 30` --
   즉 프레임버퍼 하단보다 30px 아래로 내려가 있어 항상 하단이 잘리는
   구조적 버그였음(고정폭 340짜리 박스 자체가 화면 밖으로 넘어가게
   좌표가 잡혀 있었음, 사용자 신고와 정확히 일치).

**한 일**:
1. `has_sdi`/`has_road`/`road_block_shift`(둘 다 있을 때만 `TBT_LINE_STEP*2`
   =80px) 도입. `szSdiDescr` 유무와 무관하게 도로명 줄바꿈을 계산하도록
   조건 수정, `tbt_extra_h` 초기값에 `road_block_shift` 반영(기존 줄바꿈
   확장분은 대입이 아닌 누적(`+=`)으로 수정해 겹쳐쓰기 방지).
2. `else if` -> `if`로 변경해 두 블록이 동시에 그려지도록 함. 겹침 방지는
   도로명/route=/vturn= 세 줄의 y좌표에서 `road_block_shift`만큼 빼는
   것으로 처리(§27 최소변경 -- SDI 박스 쪽 좌표/로직은 무변경).
3. `tbt_y`를 `fb_h-250` -> `fb_h-290`으로 조정(박스 위로 40px 이동, 박스
   하단이 `fb_h+30` -> `fb_h-10`으로 10px 여유를 두고 화면 안에 들어옴).

**검증**:
- 정적 분석: 좌표 계산식을 Python으로 격리 재현해 5가지 케이스
  (SDI만/도로명만-1줄/도로명만-2줄/둘다-1줄/둘다-2줄) 시뮬레이션 --
  "둘 다" 케이스에서 vturn 줄(y=155)과 SDI 줄(y=220) 사이 65px 여유
  확인(겹침 없음), 기존 단독 케이스는 좌표값 완전히 동일(회귀 없음)
  확인. 파일 내 중괄호 개수(`{`/`}`)가 수정 전/후 동일(501/500, 기존
  1개 불일치는 수정 전부터 존재하던 것으로 확인 -- 이번 변경과 무관).
- Qt/nanovg/capnp 빌드 환경이 이 세션 컨테이너에 없어 실제 컴파일은
  미실시(§29 원칙에 따라 명시).
- 패치 독립 검증: `ryujmin97/ryu` origin/c3-ms-dev(HEAD `8c512cb`) 기준
  fresh clone에서 `git apply --check` 통과, `git am` 적용 성공 확인.
- **실차 검증: 미실시.** 기기 화면 반영 여부도 사용자가 패치 적용 후
  직접 확인 필요.

**미확인 사항**:
- road_block_shift=80px가 실제 폰트 렌더링(nanovg)에서 시각적으로
  충분한 여백인지는 좌표 계산상으로만 확인했고 실제 화면 렌더링으로는
  미확인.
- SDI 문구가 두 줄 이상으로 길어지는 경우(현재 `fit_bottom_text_size`가
  폰트 축소만 하고 줄바꿈은 하지 않음)는 이번 변경 범위 밖 -- 기존
  동작 그대로 유지.

**다음 작업**:
- 사용자 실차/기기 반영 후 화면 캡처로 겹침 여부 및 하단 잘림 해소
  여부 재확인.
- 필요 시 road_block_shift 값(현재 80px) 미세조정.
