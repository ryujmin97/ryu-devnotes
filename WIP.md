...
6. (신규, 260819-6 세션) "커브 탈출 후 재가속 지연" 가설 — 사용자가
   요청한 핵심 질문(vturn/model/route 공통 적용, 완전 탈출 전 재가속
   시작 가능 여부)인데 `curve_exit_no_accel_scan` 기본 임계값이
   시내/연속커브 도로에서 오탐투성이라 이번 로그로 확증/반증 둘 다
   못함. 다음 세션에서 이어갈 작업(우선순위 높음, 사용자 핵심 관심사):
   (a) 스캔 도구에 leadStatus 필터 + 직선 지속시간 강화 조건 추가,
   (b) 개선된 도구로 기존 로그들(route1~6) 재스캔해 진짜 후보 있는지
   확인, (c) 그래도 깨끗한 사례가 안 나오면 선행차 없는 개활지
   단일 커브 위주 로그를 사용자에게 요청 검토.

## 참고 — 코드 diff 상태
이번 세션 코드 변경 없음. 위 1번(extract_log.py 패치)은 여전히 코드
작성 전, 방향만 제안된 상태. 6번(curve_exit_no_accel_scan 필터 추가)도
분석 도구 쪽 개선이며 아직 코드 작성 전, 방향만 제안된 상태.
