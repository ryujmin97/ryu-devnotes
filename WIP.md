## 58차 (검증도구 정리, 코드 변경 없음(ryu), toolkit 신규 1건+README 소급보강) — 오늘 커밋 5개 시뮬레이션 검증

**요청**: "오늘 커밋된 내용을 참조해서, 문제가 되었던 상황에 새로 패치된
브랜치를 검증하기 위한 시뮬레이션 tool을 만들어 레포에 올릴 수 있나."

**현황 파악**: 오늘 커밋 5개(`1f0d292`/`e17e078`=58차1번,
`a35a39f`=58차2번, `ff50b03`/`1145aea`=58차3번+후속수정) 중 58차2번은
`sim_low_speed_decel.py`, 58차3번(A+B+외곽게이트 후속수정까지)은
`sim_vision_track_ab.py`가 이미 toolkit에 있어 커버 중이었음. **58차1번
(VisionTrack 게이트 완화 + v_lead 직접보정)만 세션 당시 스크래치
(`work/test_visiontrack_gate.py`)로 검증하고 toolkit 미편입 — 컨테이너
리셋으로 소실돼 재현 불가 상태였음.**

**조치**: `toolkit/sim_vision_gate_v_lead.py` 신규 작성 — (1) VisionTrack
게이트 완화(`VISION_TRACK_PROB_GATE` 0.97→0.70/`CNT_GATE` 20→10)를
구게이트/신게이트 나란히 비교(실제 원거리 vision lead의 흔한 prob대인
0.75~0.85에서 구게이트는 영원히 모델예측에 갇히고 신게이트는 프레임10
(0.5s)에 전환됨을 실증), (2) `process_lead()`의 `v_lead` min() 안전측
보정(58차1번 원 검증수치 24.0→19.0 재현 + "정지차량_미인식" 실사례
근사 27.0→6.0 추가) 재현. 8개 시나리오 전부 PASS.

**전체 검증**: 오늘 커밋 관련 3개 스크립트(`sim_vision_gate_v_lead.py`
신규 8건 + `sim_vision_track_ab.py` 7건 + `sim_low_speed_decel.py` 4건
=19개 시나리오) 전부 재실행해 PASS 확인 — 오늘 5개 커밋 전부 toolkit
시뮬레이션으로 커버 완료.

**README.md/CHANGELOG.md**: 신규 스크립트 섹션 추가 + `sim_low_speed_decel.py`/
`sim_vision_track_ab.py` 누락돼 있던 README 섹션도 이번에 소급 보강.

**주의**: 이 도구들은 전부 **로직 단위(순수 파이썬) 합성검증**이며 실제
acados MPC 파이프라인이나 실차 로그 재현은 아님(capnp 의존으로 radard.py/
long_mpc.py 직접 import가 이 컨테이너에서 불가 — 기존 세션들과 동일한
한계, 코드가 실제로 위 로직과 100% 일치하는지는 diff 대조로 확인했음).
**실차 검증은 여전히 최우선으로 대기 중**(아래 58차3번 후속수정 섹션
"다음(최우선)" 참고, 이번 작업으로 바뀌지 않음).
