...
2. TTC danger override 4건 중 3건 qcamera로 정탐 확인(정지/서행
   ...
   무관.
3. **[신규, NEEDS_VALIDATION] route1 seg4 t=356~368**: 실제 우측
   cutin(흰색 SUV, qcamera 확인)이 발생했는데 비전 dRel이 9.75초간
   40~95m 사이로 극심 진동(방안C/E/G 노이즈 판정 로직상 대부분
   "노이즈"로 분류될 패턴) → 이 동안 `aEgo`가 -0.54~+0.59에 머물며
   실질 반응 없음 → 운전자가 직접 브레이크 개입(t=359.8) → 이후
   레이더 락온까지(t=367.05) 약 7초 추가 소요. **61~67차
   discontinuity suppress 로직의 설계 전제(짧은 discontinuity)를
   벗어나는 장기 진동 사각지대일 가능성** — 표본 1건, 원본 코드
   replay 검증 안 됨.
...
**다음 세션 최우선**:
1. **route1 seg4(t=356~368) 원본 코드로 replay 검증** — 방안C/E/G의
   discontinuity suppress가 이 9초+ 구간 전체를 억제하고 있었는지
   실측 확인. 만약 그렇다면 억제 지속시간 상한(예: N초 이상 진동
   지속 시 노이즈 판정 해제) 설계 검토.
2. 70차에서 이월된 항목(방안F/H 진행 여부, 세그7 후반 저속 근접 gap
   오실레이션 조사 착수 여부, `sim_jerk_boost.py` 실물 확인)도
   여전히 사용자 결정 대기 중.
