...
- 저장 시각: 2026-08-22 (46차, 진행중 — 사용자 요청으로 세그먼트 1개
  ...
  **[갱신] route3(`866476e5c3` seg18, "곡선_vturn_이상함") 완료 + 46차
  종합** — 신규 발견: vturn이 정점 통과 후 곡선이 아직 안 끝났는데도
  1초 만에 감속 거의 해제(103→149km/h), 우연히 겹친 `cam`(카메라단속)
  제약이 가려서 실제 과속으로는 안 이어짐. **"탈출후 가속지연"은 vturn/
  model 원인이 아닐 가능성이 높아짐**(3 route 전부 반대 방향: 오히려
  빠르거나 조기해제) — 다음 조사는 route/cam/vCruiseCluster 캡 쪽으로.
  **정점 감속 부족은 3 route 전부 일관 재현**(3/3) — 이번 세션 최우선
  확정 문제. 46차 종합 결론 FINDINGS.md 최하단 항목 참고.
  **46차 3개 route 분석 전부 완료. 다음 세션 최우선: (a) `abs(vturn_speed)
  <120` 게이트 재설계, (b) decel_rate 자체 과소 여부 분리검증, (c) 탈출
  지연 재조사는 route/cam 쪽으로.**
