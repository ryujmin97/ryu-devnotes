...
- 저장 시각: 2026-08-22 (35차 계속 2 — 캐시 버스터 patch(0004)까지
  두 브랜치 적용·push 완료, 실차 검증(강제 새로고침 후)만 남음)
...
- **다음 세션(또는 다음 메시지)에서 이어갈 것 — 최우선**:
  1. ~~사용자가 두 브랜치 모두에 patch 0004 적용~~ → **완료**.
     `c3-ms-dev`(`f9241db..4fe22cd`), `c3-ms-test`(`331d49a..4d2f6a5`)
     둘 다 `git am` 충돌 없이 적용 + push 확인.
  2. **[남음]** 실기기에서 **강제 새로고침(캐시 무시) 후** carrotweb
     접속 — "Clip 선택" 버튼이 clip 파일 체크박스만 선택/해제
     토글하는지 확인.
