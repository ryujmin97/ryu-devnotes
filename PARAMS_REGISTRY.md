...
| VISION_RATE_REF_MARGIN | **5.0 m/s** (63차 계속9 설계, 미구현) | 방안E: `ref_rate=-(vEgo-leadVLead)` 기준 `plausible_min=ref_rate-MARGIN`을 raw_rate 클램프 하한에 추가 | **REJECTED(설계 재검토 필요, 2026-08-24, 63차 계속10)** — seg3(r1-3, cutin 급감속) 실측 재생검증 결과, 이 마진이 seg14뿐 아니라 **진짜 위험한 cutin(seg3)의 frac_rate 응답도 심하게 억제**함을 확인(vision-only 구간 window max frac_rate: PATCHED_C 1.000 → PATCHED_E 0.209, frac(전체) 1.000→0.530). 근본 원인: 이 구간에서 leadVRel/leadVLead(모델 자체 추정)가 실제 급접근을 완전히 놓침(vRel이 +1~+3.2m/s로 opening처럼 보고, 실측 dRel은 84m→20m로 1.9초간 평균 -32.8m/s 급감) — vision_dRel_rate 크로스체크는 원래 이 "vLead가 위험을 놓치는" 실패모드를 잡으려고 만든 장치인데, 방안E의 ref_rate가 바로 그 실패하는 leadVLead에서 계산되므로 **안전장치가 자신이 보완해야 할 신호에 종속되는 구조적 모순**. 현재 설계 그대로는 구현 보류 권고, margin 산정 기준 재설계 또는 방안E 자체 재고 필요. 근거: FINDINGS.md 63차 계속10 |
