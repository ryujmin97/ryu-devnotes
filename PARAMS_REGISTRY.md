...
| 상수 | 현재값 | 용도 | 검증상태 |
|---|---|---|---|
| vturn_lookahead_horizon_s | **6.5s** (정정, 기존 4.5s에서 확대) | 커브 조기감속 예측구간 | NEEDS_VALIDATION (2026-08-20, "조여드는 커브 중간 급감속" 사용자 보고 대응으로 4.5s→6.5s +2s 확대. 근거 사례(260819-7 seg6)의 조임 지속시간이 8.6s라 6.5s로도 완전 커버는 못할 수 있음 — 기존 "검증됨" 태그는 4.5s 시절 값 기준이었으므로 재검증 필요, FINDINGS.md 참고) |
...
- 2026-08-20 (7차): vturn_lookahead_horizon_s 4.5s→6.5s 확대(commit
  `1827c1e`, 미push 상태 patch 파일로 전달). "곡선 진입 전 사전감속
  부족으로 곡선 내 급감속" 사용자 보고 + 기존 [INVESTIGATING] 260819-7
  seg6 사례(조임 8.6s) 근거. 실차 검증 미완료.
