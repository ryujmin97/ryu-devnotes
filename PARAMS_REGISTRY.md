...
| `AutoNaviSpeedDecelRate` [사용자 UI 설정값, 83차 신규, **218차 계속 70->100 변경**] | **사용자 실측 100**(=1.00 m/s², 218차 계속에서 기존 70(=0.70)에서 변경 -- 아래 변경기록 참고) | `carrot_navi_route()`(route 커브 진입측 감속)의 `accel_limit`으로 직접 사용. 필요거리=`(v_ego²-target²)/(2×accel_limit)` — 값을 낮출수록 더 이르게·완만하게 감속 시작(UI 툴팁: "Lower number, slows down from a greater distance") | [84차 조치완료, NEEDS_VALIDATION] 300m 고정 캡이었던 시절엔 100→60km/h 시 0.70에서 이미 ≈303m로 상한 초과 우려가 있었으나, **84차에서 캡 자체를 동적화(300~500m)해 이 문제를 완화** — 아래 `route_lookahead_dynamic_cap` 행 참고. 단 accel_limit 자체의 "낮출수록 항상 좋다"가 아닌 성질(감속폭에 따라 최적값이 갈림)은 여전히 유효. **[중요] route 전용 파라미터 아님** — 과속카메라 감속(`sdi_speed`)/TBT 회전 감속(`atc_desired`)/도로제한속도 감속과 전부 공유(`carrot_serv.py` L847/L983/L994), 조정 시 넷 다 동시에 영향받음. **[218차 계속, 변경기록]** 기존값: 70(0.70 m/s²) -> 변경값: 100(1.00 m/s², 사용자 디바이스 적용 완료). 변경 이유: 217차 문서 5~7단계(감속률/램프 재검토) 착수를 위해 사용자가 route 기준 감속률을 1.0으로 새로 정하고, 코드상 route 전용 분리가 아직 없어 4개 소비처(route/카메라/TBT회전/도로제한속도) 전역 동시 변경으로 진행하기로 결정(사용자 확인). 영향: 램프 하강률(`accel_limit_kmh=rate*3.6`) 2.52->3.6 km/h/s로 상승, 199차 boost 상한(`ROUTE_VEGO_BOOST_MAX_MSS`=3.0)과의 여유폭 축소(0.7 기준 대비 boost 배수 4.3배->3.0배)이나 여전히 base<cap 유지되어 boost 메커니즘 자체는 유효. 카메라/TBT회전/도로제한속도 감속도 전부 0.7->1.0으로 동시에 완만해짐(사용자 인지 하에 결정). 검증 결과: **미실시** — 이 변경이 실제 주행에 반영된 로그(카메라/TBT/도로제한속도 포함)는 아직 없음, NEEDS_VALIDATION. **[287차 갱신, §33]** 오늘(2026-09-06) 사용자가 업로드한 실차 `params_backup-6.json`(283~286차가 분석한 route1~4, 16:56~18:03 주행 당시 설정값)을 확인한 결과 실측값은 **70(=0.70 m/s²)** — 274차 WIP가 "90으로 변경"이라 기록했던 값도, 이 행의 218차 등록값(100)도 아닌 세 번째 값(274차 변경 자체가 이 표에는 반영된 적이 없었음도 함께 확인됨). 사용자 확인: 주행 당시 설정값이 맞으나 이후 다시 바꿨을 수 있어 정확한 시점별 변경 이력은 불확실, **route 판정 결과에 대세 영향은 없음**(사용자 판단). 283~286차의 route 실측 분석은 이 값을 가정치로 쓰지 않고 실제 arbitration 승자(`src`)를 직접 관측하는 방식이라 이번 불일치로 인한 재검증은 불필요. "현재 등록값"은 사용자가 명시적으로 재확인하기 전까지 100/90/70 중 확정하지 않고 남겨둠(§33, 임의 확정 안 함). |
| `AutoNaviSpeedCtrlEnd` [사용자 UI 설정값, 221차부터 등록 시도했으나 (a)/(b) 결정 미결로 미등록 상태 이월, **287차 최초 등록**] | **사용자 실측 8**(초, 287차 `params_backup-6.json` 확인 — 283~286차가 분석한 route1~4 주행(2026-09-06 16:56~18:03) 당시 설정값) | route(`carrot_man.py` L907/L1037, apex_dist/apex_speed 물리계산 + sharpest_candidate_speed 계산)와 카메라 감속(`carrot_serv.py` L1052/1063, `sdi_speed`) 양쪽에 공유되는 목표속도 도달 여유시간(safe_time) — `AutoNaviSpeedDecelRate`와 동일한 공유 구조(221차 확인). | NEEDS_VALIDATION (287차 신규 등록) — 221차에서 "7→10초, (a)디바이스 전역변경/(b)route 전용 분리" 결정이 미결로 남았던 항목. 실측 8은 7도 10도 아니라 그 (a)/(b) 논의와 무관하게 이후 별도로 조정됐을 가능성 — 정확한 변경 이력은 미확인(사용자 확인 대기, §33). 과거 일부 시뮬레이션(222차 `7.0` 추정, 253차 `2.2s` 가정)이 이 실측치와 다른 가정값을 썼던 점에 유의 — 해당 결론 재검토 필요 여부는 영향이 크지 않다는 사용자 판단에 따라 이번 세션에서는 보류. |
...
| `route_lookahead_dynamic_cap` [84차 신규, 85차 max_m 500->600 상향, **217차 롤백됨(아래 `route_lookahead_m` 행 참고)**] | `min_m=300.0`/`max_m=600.0`(85차, 기존 500.0)/`assumed_target_kph=30.0` | `carrot_man.py compute_route_lookahead_distance()` — `carrot_navi_route()`의 `get_path_after_distance()` 호출 시 fetch할 거리(구 300m 고정값)를 `(v_ego²-target²)/(2×accel_limit)`로 계산 후 [min_m,max_m]로 clip. `assumed_target_kph`는 실제 커브 목표속도가 아니라 캡 크기 산정용 가정값(이 함수 호출 시점엔 실제 곡률 기반 목표속도를 아직 모름). 저속(<=60km/h 부근)은 accel_limit 무관 항상 min_m(기존 300m와 동일, 회귀 없음). | NEEDS_VALIDATION (2026-08-26, 85차 갱신) — 84차가 max_m=500m로 도입했으나 "120→60km/h 풀커버는 accel=0.70 기준 이론상 ≈595m 필요, 500m는 절충값"이라는 84차 자체 지적에 따라 85차에서 600m로 상향(≈595m 이론치를 온전히 커버). `toolkit/sim_route_dynamic_cap.py` 600m 기준 재검증 — 사용자 실측 accel=0.70 기준 110km/h+에서 ceil(600m) 도달, 저속 floor(300m) 유지, 단조성, 예외 안전폴백 4건 전부 PASS. `git am` verify-am-85(base `2a91c3f`) diff 0 + `py_compile` 통과. **217차에서 이 동적 캡 자체를 롤백**(apexIdx flicker 악화 우려 -- "동적 캡에 의한 apex_idx 재선정"이 원인으로 지목됨, `route_lookahead_m=300.0` 고정 원복). |
| `route_lookahead_m` [`carrot_man.py carrot_navi_route()`, 217차 300m 고정 원복, **221차 600m 재확장(고정값)**] | `600.0`(221차, 기존 300.0) | `get_path_after_distance()` 호출 시 fetch할 route 전방 거리(고정값, 84/85차의 "동적" 캡과는 별개 메커니즘). | NEEDS_VALIDATION (2026-09-03, 221차 신규) -- 84/85차의 "동적"(vEgo 기반 300~600m) 캡을 217차가 apexIdx flicker 악화 우려로 롤백한 것과 방향은 같아 보이나, 221차는 "고정 600m"(vEgo와 무관하게 항상 600m 조회)로 메커니즘이 다름 -- apex 선택은 여전히 `candidates[0]`(가장 가까운 감속필요지점, distances 오름차순)이라 300m 안에 있던 기존 apex 선택 자체는 안 바뀜(600m 확장은 후보 리스트 뒤쪽에 원거리 후보를 추가할 뿐). 단, 220차가 확정한 apexIdx flicker 근본원인("리샘플 그리드 재앵커링")은 lookahead 크기와 무관하게 발생하는 현상이라 600m 확장이 이를 악화시키는지는 이번 세션에서 검증되지 않음(§28, 추측만으로 안전 확정 안 함). **실차 검증 전까지 219/220차 apexIdx flicker 작업과 함께 A/B(naviPointsActive 활성구간 프레임당 apexIdx 변경빈도, 215차 지표) 반드시 대조할 것.** 정적검증(`py_compile`+`git am` diff-0)만 완료, 합성/실차 검증 모두 미실시. |
...
| `ROUTE_RELEASE_HOLD_S` [223차 신규, `carrot_man.py`] | **0.0 s** (282차, 기존 2.0s) | apex RELEASE 직후 이 시간 동안 route curve 검색/apex 선정/감속 계산을 완전히 스킵(§11 — curve A apex 직후 curve B가 즉시 감지돼 route가 바로 재부착되는 현상 방지 목적으로 223차 신설). **282차 변경**: 266차 confidence blend 도입 이후 이 방지 역할이 구조적으로 중복됨을 소스 추적+합성 시뮬레이션(281차, `sim_route_281_release_hold_ab.py`)으로 확인 — RELEASE의 3개 트리거(passed/lost/new, speed_reached, dist_reached) 전부가 hold와 무관하게 이미 즉시 재-ACTIVE를 차단(streak=1→confidence=0.0 등). 시나리오 B에서 hold=2.0이 근접 2연속 커브 반응을 confidence 자연축적 대비 최대 약 1.6초 추가 지연시킴을 확인, 사용자가 0.0으로 무력화 확정. 상수/분기 구조는 유지(즉시 2.0 복원 가능). | **NEEDS_VALIDATION (2026-09-06, 283차 갱신)** — 283차 실차 로그(4개 route, 16:56~18:03, 80145행)로 1차 검증: gap<2.0s 재-ACTIVE 11건 전부 저속 교차로 상황, aEgo pump(재가속 후 급재감속) 뚜렷한 시그니처는 발견 안 됨(2건 후보는 저속 회전 상황이라 hold와 무관할 가능성 높음, **qcamera 육안 재확인은 미실시**). 253차 우려(진동)는 이번 corpus에서 재현 안 됨으로 잠정 결론이나 qcamera 확인 전까지 미확정. 223차 당시 우려(2차 곡선 반응 지연 해소 여부)는 GPS/naviPaths 위치 매칭으로 235차 S커브 위치를 이번 corpus에서 재식별하지 못해 여전히 미검증. `autoNaviSpeedCtrlEnd` 실제값도 여전히 미확인. device build 확인 결과 gitCommit=`7571e63`(280차)+dirty=True — git 메타데이터만으로는 282차 코드 반영을 확정할 수 없음(283차 실측 거동상 hold≈0 적용 정황은 있음). 실차 qcamera 육안 재확인: 미실시. 상세: WIP.md 283차, toolkit `verify_route_release_hold_283_real_log.py`. **[285차 갱신] qcamera 육안 재확인 완료**: pump 후보 2건 모두 hold와 무관한 정상 상황으로 확정 -- 후보1(t≈772.2, aEgo range 1.17m/s²)은 고가도로 하부 Y자 분기 구간의 정상 감속, 후보2(t≈3179.6, aEgo range 1.79m/s², 가장 큰 후보)는 직선 시가지 도로에서 선행차 cut-in+전방 적색신호로 인한 감속(커브/route 로직과 무관). 253차 우려("hold 제거로 인한 인위적 ENGAGE/RELEASE 진동")는 이 corpus에서 **재현 안 됨으로 최종 확정**. 235차 S커브 재식별(223차 원 우려였던 "2차 곡선 반응 지연" 항목)은 여전히 corpus 미보관으로 미검증 상태 유지. 상세: WIP.md 285차. **[286차 갱신] 대체 S커브 위치로 참고 검증 추가**: 235차 원본 corpus는 여전히 미보관이나, 신규 toolkit `scan_consecutive_curve_pairs.py`로 route1~4에서 성격이 비슷한 S커브 패턴 2건(route2 t=2149~2154, route1 t=1055~1062, 둘 다 qcamera로 실제 굽이길 확인)을 찾아 두 번째 커브 반응성을 대체 점검 -- 두 곳 모두 route/vturn arbitration이 정상 개입, desiredSpeed 매끄러운 감소, aEgo 정상 범위로 hold=0.0으로 인한 반응 지연/pump 징후 없음(기존 결론과 일관). 단 235차가 확정한 정확한 물리적 위치(간격 250~290ms)에 대한 직접 검증은 아니므로 참고 근거로만 취급 -- **NEEDS_VALIDATION 상태 유지**. 상세: WIP.md 286차, toolkit `scan_consecutive_curve_pairs.py`. |
| `MapTurnSpeedFactor` [사용자 UI 설정값, 201차 최초 실측 확정, **210차 곱셈 제거로 사실상 dead**] | 사용자 실측 130(=1.30) — 값 자체는 그대로지만 210차 이후 어디에도 곱해지지 않음 | (210차 이전) `carrot_serv.py::update_navi()`에서 `route_speed = max(route_speed * mapTurnSpeedFactor, autoCurveSpeedLowerLimit)`. (210차 이후) 이 줄에서 `* mapTurnSpeedFactor`를 제거(`route_speed = max(route_speed, autoCurveSpeedLowerLimit)`만 남음) — repo 내 이 곱셈이 유일한 사용처였으므로 파라미터 자체는 이제 route 감속에 관여하지 않는다. Params 읽기(`self.mapTurnSpeedFactor` 대입, L308)와 UI 슬라이더("경로턴속도반영비율")는 최소변경 원칙상 그대로 남겨둠. | [201차 ROOT_CAUSE_CONFIRMED → 210차 REMOVED] 201차가 "route를 상시 30% 불리하게 만듦"으로 확정한 뒤, 210차에서 사용자 실차 스크린샷(vEgo=53, route=62.7)으로 **반대 방향 부작용**을 추가 확인: 205/207차가 `carrot_navi_route()`에서 만든 vEgo 상한(`out_speed=min(out_speed, max(v_ego_kph, sharpest_candidate_speed), 150)`)이 이 곱셈에 의해 상한 적용 *이후* 다시 30% 부풀려져, "route는 현재속도보다 빠른 속도를 권하지 않는다"(205차 설계 의도)는 불변식이 최종 출력 단계에서 깨지고 있었다(62.7/1.30=48.2로 vEgo 상한 적용 전 raw 값과 일치). 사용자 판단(210차)으로 곱셈 자체를 제거. 12세그 실차로그 재시뮬레이션(t=197.0~198.4, vEgo 50.2~51.2)으로 OLD(62~64, vEgo 초과)→NEW(42~50, 전 프레임 vEgo 이하) 확인. **실차 검증 미실시**(패치만 작성, NEEDS_VALIDATION). **[279차 REVIVED — 새 위치, 사용자 요청("apex 목표속도를 높이고 싶다")]** 210차가 삭제한 지점(`carrot_serv.py` 구 `calculate_current_speed` 경로)은 223차 이후 그 계산 자체가 완전히 폐기되어 되살릴 자리가 없었음. 대신 현재(258/266차) route 아키텍처의 실제 apex 목표속도 산출 지점인 `carrot_man.py`의 `V_CURVE_LOOKUP_BP`/`V_CRUVE_LOOKUP_VALS`(곡률→속도 테이블) 보간 결과(macro/fine 둘 다)에 곱하도록 새로 연결. 210차가 지적한 회귀(곱셈이 vEgo 상한 적용 *이후*에 걸려 불변식이 깨짐)와 달리, 이번엔 목표속도 산출 단계(vEgo 클램프보다 훨씬 앞)에 곱해 224/227차 이후의 vEgo 상한 클램프(§4)가 뒤에서 그대로 걸리므로 210차류 회귀 재발 구조가 아님(정적 검증: factor=1.0은 기존과 완전 동일, factor=1.5/0.9로 후보 포함/제외가 예상대로 바뀜을 isolated 계산으로 확인). **부수 발견**: `common/params_keys.h`의 default가 90(=0.9배)으로 남아있어 그대로 되살렸다면 이 값을 안 만진 사용자에게 10% 추가 감속 회귀가 생겼을 것 — `carrot_settings.json` 명시 default(100)에 맞춰 90→100으로 함께 수정(279차). 기존 130/150 등으로 이미 값을 조정해둔 사용자는 그 persistent 값이 default보다 우선이므로 영향 없음. **실차 검증: 미실시.** |

| `ROUTE_FINE_OVERRIDE_MIN_DIST_M` [367차 계속4 신규] | **150.0 (m)** | `route_curvature_macro_fine()`의 fine(10m) 대체 조건 앞 게이트. `distances[j]>=`이 값이면 fine 대체를 건너뛰고 macro(40m chord) 값 유지 -- 362차가 확정한 근본원인(고속도로 원거리 완만한 굴곡이 앞뒤 정합성 확인 없이 fine으로 잘못 채택돼 route=만 70~80으로 오탐 표시)에 대한 대응. `distances[j]`는 이 함수의 10m(main)/2.5m(local) 양쪽 호출부 모두에서 차량 현재위치 기준 절대거리로 일관됨(362차 계속2 확인) -- 게이트 하나로 두 경로 모두 적용 | **NEEDS_VALIDATION** (2026-09-12, 367차 계속4 신규 도입 — 근거: `d1cd25bdf1` 검증오탐 anchor 2건(apexDist=210.0m x2) vs `2cbdaca9d2` qcamera 확인 진짜커브 2건(apexDist=50m/120m), 표본 4건. 후보구간(150~180m) 중 보수적 값 150m를 사용자가 채택. **곡률 크기/비율/고립도 기반 게이트(RATIO/PERSIST/ISOLATION, 362~367차)는 전부 정탐-오탐 분포가 겹쳐 채택 불가였으나 거리축은 표본상 뚜렷이 분리** — 방향 전환의 첫 코드 반영. offline replay 스윕(150/165/180m 비교)은 사용자 결정으로 생략, 실차 검증으로 직행. 패치 `0001-367-4-route_curvature_macro_fine-150m-fine.patch`(base `bd21c7e4a87f`) §6/§7 전체 PASS, **실차 검증: 미실시**. 363차/366차 TP corpus(R<30m, 1137건)의 apexDist 분포로 사후 보강 필요. FINDINGS.md 367차 계속4 참고) |
...
- **[288차 갱신] 최초 실차 로그 검증 완료(여전히 NEEDS_VALIDATION 유지,
  ...
  **NEEDS_VALIDATION을 유지하는 이유**: 값(1.1) 자체가 부적절하다는
  ...
  구간 실제 도로 형상)도 미실시.
- **[289차 갱신] 1.1→1.05 완화 what-if 시뮬레이션 완료(값 변경 아직
  없음, NEEDS_VALIDATION 유지).** 사용자 요청("route가 너무 짧게
  작동하다 릴리즈됨, 마진을 1.05로 낮추면?")에 답하기 위해 동일 corpus
  (route1~4, 80145행)로 margin 관여 72건을 1.05 기준 재시뮬레이션 --
  flicker train 4건→1건으로 감소하나, 지속시간이 늘어나는 30건 중
  28건은 margin 완화 자체보다 apex candidate continuity 소실로의
  재분류 때문(순수 margin 효과는 2건뿐). 즉 1.05로 낮춰도 사용자가
  호소하는 "짧게 작동" 현상의 근본 원인이 margin이 아니라 continuity
...
- **[343차 갱신] 사용자 지시로 ACTIVE 릴리즈 OR-조건에서 이 상수를 쓰는
  `speed_reached` 판정 자체를 삭제(값 변경이 아니라 조건 삭제).** `v_ego_ms
  <=target_ms`(STEP2/INERT 게이트, 별도 상수 없음)는 그대로 유지 --
  342차/342차 계속 what-if 시뮬레이션이 이미 "이 조건만 단독 제거"가
  이 corpus 기준 accel_commanded 0건임을 확인한 결과를 그대로 반영한
  것(상세 근거는 아래로 옮기지 않고 WIP.md 343차 참고). **상수 정의
  자체(1.05)는 코드에 남아있으나 더 이상 release 판정에 쓰이지 않는다**
  -- toolkit 스크립트 다수(342차 what-if 등)가 여전히 이 값을 import해
  "조건을 뺐을 때"를 시뮬레이션하는 용도로 참조하므로 삭제하지 않음(§27).
  즉 이 항목의 "NEEDS_VALIDATION" 딱지는 이제 "값이 부적절한가"가 아니라
  "이 조건 자체를 뺀 것이 안전한가"로 의미가 바뀐 상태 -- 실차 검증
  전혀 미실시(§29), what-if 시뮬레이션(1개 route)만 근거.
