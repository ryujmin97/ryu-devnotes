...
| `route_lookahead_dynamic_cap` [84차 신규] | `min_m=300.0`/`max_m=500.0`/`assumed_target_kph=30.0` | `carrot_man.py compute_route_lookahead_distance()` — `carrot_navi_route()`의 `get_path_after_distance()` 호출 시 fetch할 거리(구 300m 고정값)를 `(v_ego²-target²)/(2×accel_limit)`로 계산 후 [min_m,max_m]로 clip. `assumed_target_kph`는 실제 커브 목표속도가 아니라 캡 크기 산정용 가정값(이 함수 호출 시점엔 실제 곡률 기반 목표속도를 아직 모름). 저속(<=60km/h 부근)은 accel_limit 무관 항상 min_m(기존 300m와 동일, 회귀 없음). | NEEDS_VALIDATION (2026-08-26, 84차) — `toolkit/sim_route_dynamic_cap.py`로 로직 단위 검증(사용자 실측 accel=0.70 기준 100km/h+에서 500m 도달, 저속 floor 유지, 예외 안전폴백) 완료. 실차 반응 미확인 — 특히 max_m=500m가 충분한지(120→60km/h 풀커버는 accel=0.70 기준 이론상 ≈595m 필요, 500m는 절충값), assumed_target_kph=30.0이 적절한지 튜닝 여지 있음. |
