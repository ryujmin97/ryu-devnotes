...
| `MapTurnSpeedFactor` [사용자 UI 설정값, 201차 최초 실측 확정] | **사용자 실측 130**(=1.30, `params_keys.h` 기본값 90=0.90 아님) | `carrot_serv.py::update_navi()`에서 `route_speed = max(route_speed * mapTurnSpeedFactor, autoCurveSpeedLowerLimit)` — route 램프 통과 후 최종 arbitration 후보값에 곱해지는 배율. **1.30은 route를 상시 30% 불리하게 만듦**(다른 소스는 이 배율 미적용). | [201차, ROOT_CAUSE_CONFIRMED로 이어짐] 스크린샷 HUD `route=390.0`(route 비활성 시 표시값, `carrot_serv.py` L1152)이 route 비활성 시 out_speed 기본값 300의 정확히 1.30배임을 실측으로 역산 확인 — devnotes에서 이 파라미터가 정량적으로 다뤄진 최초 사례. 199차 패치 실차 로그에서 북대전IC 커브(t≈450~510, 239프레임) 내내 route가 arbitration에서 한 번도 못 이긴 원인 중 하나로 확정(감속측 램프 지연과 겹쳐 작용). 이 값이 사용자의 의도적 설정인지 재확인 필요 — WIP.md 201차 "다음 작업" 참고. |
