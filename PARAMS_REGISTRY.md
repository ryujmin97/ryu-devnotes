...
| `AutoNaviSpeedDecelRate` [사용자 UI 설정값, 83차 신규] | **사용자 실측 70**(=0.70 m/s², 기본값 120=1.20보다 낮춤) | `carrot_navi_route()`(route 커브 진입측 감속)의 `accel_limit`으로 직접 사용. 필요거리=`(v_ego²-target²)/(2×accel_limit)` — 값을 낮출수록 더 이르게·완만하게 감속 시작(UI 툴팁: "Lower number, slows down from a greater distance") | NEEDS_VALIDATION (2026-08-26, 83차) — **300m lookahead 상한(`get_path_after_distance(...,300)`)과 상호작용**: 100→60km/h는 0.70에서 이미 ≈303m로 상한 초과, 120→60km/h를 정확히 300m에 맞추려면 이론상 ≈1.39(UI 140, 여유 없는 경계값 — 실사용은 150~160 권장). 반대로 80→30km/h처럼 감속폭이 큰 경우는 0.70(≈303m, 초과)보다 0.80(≈265m, 여유 35m)이 오히려 300m 안에 더 잘 들어맞음 — **"낮출수록 항상 좋다"가 아니라 진입속도·감속폭 조합에 따라 300m 캡 초과 여부가 갈림.** 사용자 실측값(70)은 고속(100km/h대)+큰 감속폭 커브 조합에서 이미 캡 경계에 걸릴 수 있음(실차 재현 미검증). **[중요] route 전용 파라미터 아님** — 과속카메라 감속(`sdi_speed`)/TBT 회전 감속(`atc_desired`)/도로제한속도 감속과 전부 공유(`carrot_serv.py` L847/L983/L994), 조정 시 넷 다 동시에 영향받음. |
