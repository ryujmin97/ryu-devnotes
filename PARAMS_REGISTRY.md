...
| VISION_CLOSING_RATE_MAX_PLAUSIBLE | 30.0 m/s | raw_rate 클램프 상한(접근 방향만) | PARTIALLY_VALIDATED (2026-08-21, 26차 신설. 합성 시나리오 검증 완료(devnotes toolkit), 실차 반영은 33차(`8114a46`, 사용자 로컬)로 처음 커밋됨 — 실측 검증 전) |
| VISION_CLOSING_RATE_MEDIAN_WINDOW | 3 frames | 클램프된 rate의 중앙값 필터 윈도우 | PARTIALLY_VALIDATED (2026-08-21, 26차 신설. 위와 동일 상태 — 스냅-복귀형 1프레임 노이즈를 3프레임 다수결로 억제하도록 설계, 합성검증 완료/실차 미검증) |
| VISION_CLOSING_RATE_GATE_CAUTION | **-2.2 m/s** (26차 최초 -5.5 → 30/31차 재설계로 확정) | filt rate 절대값 게이트, 이 값 이상이면 frac_rate=0 | PARTIALLY_VALIDATED (2026-08-21. 26차 최초값(-5.5, LEAD_ACQ_TTC_*와 동일 재사용)은 28차 세그7/세그12 실측 대조로 과보수 확정(실측 피크 -3.2~-3.5m/s가 CAUTION보다 낮아 전혀 미발동). 30차 -2.2로 잠정 하향, 31차 6개 세그(3개 라우트) 재검증으로 강하게 뒷받침(seg12/15 등에서 락온 1초+ 전부터 DANGER 안정 도달, seg8은 여전히 미발동). 32차 곡선 오탐 검증 결과 이 문턱 자체가 새 오탐 리스크를 만들지 않음 확인(발견된 곡선 글리치는 기존 baseline 문턱에서도 이미 DANGER였던 별개의 사전 존재 결함, FINDINGS.md 32차 참고). **33차: 사용자 로컬(`c:\\dev\\ryu`)에 커밋(`8114a46`, c3-ms-dev) 완료 — origin push 및 실차 실측(acados MPC 파이프라인 integration 후 a_target 반응) 검증은 아직.** 근거: FINDINGS.md 28/30/31/32차) |
| VISION_CLOSING_RATE_GATE_DANGER | **-5.0 m/s** (26차 최초 -10.0 → 30/31차 재설계로 확정) | filt rate 절대값 게이트, 이 값 이하면 frac_rate=1.0 | PARTIALLY_VALIDATED (위 GATE_CAUTION과 동일 이력/상태 — 33차 로컬 커밋 `8114a46` 완료, 실차 검증 대기) |
