...
| 상수 | 현재값 | 용도 | 검증상태 |
|---|---|---|---|
| vturn_lookahead_horizon_s | **8.0s** (정정, 4.5s→6.5s→8.0s 2단계 확대) | 커브 조기감속 예측구간 | NEEDS_VALIDATION (2026-08-20, 1차 4.5s→6.5s push 완료(`4c15987`) 후 같은 세션에서 근거 사례(260819-7 seg6, 조임 지속시간 8.6s)를 더 가깝게 커버하기 위해 2차 6.5s→8.0s(`c4e3093`, patch 전달·미push). 8.0s도 8.6s보다 근소하게 짧음. 두 단계 모두 실차 검증 없음, FINDINGS.md 참고) |
...
- 2026-08-20 (8차, 같은 트랙 이어감): 1차 push(`4c15987`) 확인 직후
  ...
  patch(`c4e3093`) 전달, 아직 미push. `vturn_lookahead_horizon_s`가
