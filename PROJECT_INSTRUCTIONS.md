...
- `/mnt/user-data/outputs/`에 갱신된 `WIP.md` 생성 → present_files로 전달
- 코드 변경분이 있으면 지금까지의 작업을 patch 파일로도 만들어
  `/mnt/user-data/outputs/`에 같이 둔다 (완성 여부와 무관하게 WIP 상태
  그대로 patch화 — `git am` 적용 안내 포함)
- 평소 세션 종료 절차(FINDINGS/LAST_ANALYZED/PARAMS_REGISTRY 갱신 +
  PowerShell push 안내)와 동일하게 진행하되, "세션 종료"가 아니라
  "중단 지점 저장"이라는 걸 사용자에게 분명히 알린다
