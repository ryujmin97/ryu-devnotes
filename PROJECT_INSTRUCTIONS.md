...
아래 2가지를 어기지 않는다 (계정이 달라도 예외 없음 — 과거 여러 계정
세션이 이 지침 없이 매번 새로 스크립트를 작성해 시간을 낭비한 전례가
반복돼 이 문단을 프로젝트 지침 본문에 직접 명시함):**
1. **먼저 찾는다.** 뭔가를 계산/재현/검증해야 하면 코드를 새로 짜기
   ...
   그대로(또는 옵션만 조금 바꿔서) 재사용한다 — "완전히 똑같지는 않으니
   새로 짜야겠다"는 판단이 들면, 그 판단이 맞는지 먼저 README의 함수
   시그니처/옵션을 실제로 열어봤는지 스스로 점검한다.
2. **새로 만들면 반드시 레포에 넣는다.** README를 확인했는데도 없는
   경우에만 새로 작성한다. 새로 작성한 스크립트/함수는 (합성검증만
   끝났든 실측검증까지 됐든 검증 상태와 무관하게) 그 자리에서
   `/home/claude/devnotes/toolkit/`에 저장하고, `toolkit/README.md`에
   섹션을, `toolkit/CHANGELOG.md`에 한 줄 요약을 추가한다. `work/`
   폴더에만 스크래치로 남겨두고 끝내지 않는다 — 컨테이너는 세션마다
   리셋되므로 `work/`에만 있으면 다음 세션(같은 계정이든 다른 계정이든)
   에서 그대로 사라져 똑같은 도구를 또 새로 작성하는 낭비가 반복된다.
   세션/체크포인트 종료 시 이 toolkit 변경분도 다른 devnotes 파일과
   함께 반드시 push한다(아래 "세션 종료 시" 2/3번 대상에 포함).
