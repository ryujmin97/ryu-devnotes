...
def select_new_desired_curvature(
    ...
    """controlsd.py state_control()의 140차 패치 분기를 그대로 재현.
    ...
    if not lat_active:
        ...
        if len(lat_plan_curvatures) == 0:
            return prev_curvature, "mpcBranch_emptyFallback"
        else:
...
def run_matrix():
    cases = [
        # (설명, lat_active, lanefull, offset_active, curvatures_len, 기대_branch)
        ("latActive=False -> 무조건 유지 (offset/lanefull 무관)",
         False, False, True, 5, "latInactive"),
        ("레인모드, curvatures 있음 -> 기존과 동일(MPC)",
         True, True, False, 5, "mpcBranch_used"),
        ("레인모드, curvatures 없음 -> 기존과 동일(폴백)",
         True, True, False, 0, "mpcBranch_emptyFallback"),
        ("레인리스 + offset=0(기본값) -> 기존과 100% 동일(모델액션)",
         True, False, False, 5, "modelActionBranch"),
        ("레인리스 + offset!=0, curvatures 있음 -> 신규: MPC(offset반영) 사용",
         True, False, True, 5, "mpcBranch_used"),
        ("레인리스 + offset!=0, curvatures 없음(초기화 등) -> 안전 폴백",
         True, False, True, 0, "mpcBranch_emptyFallback"),
    ]

    passed = 0
    for desc, lat_active, lanefull, offset_active, n_curv, expected in cases:
