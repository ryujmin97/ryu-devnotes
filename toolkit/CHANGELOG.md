## 2026-09-12 (372차)
- `measure_cpu_periodic.sh`: 신규. 353/354차부터 이월된 "CPU% 정량 실측"
  공백을 메꾸기 위해 Termux에서 SSH로 콤마 디바이스 `top -b -n1`
  load average/CPU us·sy·id/최상위 프로세스/열존 최고온도를 주기적으로
  수집해 CSV로 누적하는 쉘 스크립트. 사용자가 부팅 13분/19분 시점 수동
  측정(load average 6.87~9.71, `ui` 63~72%)한 것을 계기로, 단발 측정은
  워밍업 오염+baseline 부재로 원인 판단 불가(§28)하다고 판단해 작성.
  실제 콤마 샘플 출력으로 파싱 로직 검증 완료(uptime/load/cpu/proc 필드
  전부 정확 추출 확인), 디바이스 SSH 실행 자체는 미실시(이 세션 환경
  제약, 다음 단계는 README.md 참고). 상세: WIP.md 372차 참고.
...
- `sim_route_365_heading_isolation_gate.py`: 신규. 362차 오탐 corpus
  ...
  변화 프로파일을 직접 계산 -- 오탐 지점은 앞뒤 여러 세그먼트가 평평한데
  정확히 그 지점(±1세그먼트)에서만 6.2~6.5도 급변하는 패턴 확인(두
  독립 오탐 지점에서 동일 재현). "곡률 크기" 대신 "heading 꺾임의 국소
  집중도"를 `isolation_score`로 정량화해 게이트 후보 제시 -- th=0.885~0.92
  구간에서 오탐 억제 86.2%(고정)와 정탐 생존 84~90%가 동시에 높은 안정적
  평탄부 확인(RATIO/PERSIST가 보였던 "절벽" 구조 없음). 표본 작음(오탐
  29건)/정탐 필터가 363차 R 계산과 정확히 대응하지 않는 대리 지표라 추가
  검증 필요, 코드 패치 없음. 상세: WIP.md/FINDINGS.md 365차.

## 2026-09-12 (364차 계속)
- `sim_route_364_gate_persistence_design.py`: 코드 변경 없음, 362차 오탐
  corpus(`d1cd25bdf1`, 24,000행)와 363차 급커브 corpus(`0000039a--7b602ffb85`
  seg12-16, 5,999행)로 실측 완료. `low_frac` 0.10~0.13은 정탐 94~98%
  보존하나 오탐 억제 28~33%에 불과, `0.14`는 오탐 억제 거의 미개선(67%→66%)
  인데 정탐이 21%로 급락하는 절벽 확인. 단일 파라미터로는 미해결 판정,
  `ryu` 코드 패치는 보류. 상세는 README.md/FINDINGS.md/WIP.md 364차(계속)
  참고.
...
- `group_orphan_episodes_319.py`: **신규**. 318차 "다음 작업 1번"(근접+
  ...
  컨테이너가 리셋되어 원본 그룹화 로직(232건/73건/16곳)이 전량 소실,
  재현 불가 확정 -- 재발 방지 목적으로 그룹화(같은 seg, 시간
  gap<=1.0s)/계층화(near/far x moving x active, 5개 stratum) 기준을
  코드로 고정. x17seg corpus 적용 결과 orphan 3433건(재확인) → 53개
  에피소드. 그중 8곳 qcamera 육안 대조 -- 원거리(~510m)군은 lookahead
  경계 아티팩트 가설 재확인, 근접군 중 `dist_mean≈0`인 7건이
  진출입로(지하주차장 등) 신규 아형으로 확인. 상세는 FINDINGS.md
  319차 참고.
...
- `sim_route_281_release_hold_ab.py` 신규 -- `ROUTE_RELEASE_HOLD_S`(2초)
  ...
  C: 재가속 재개입[버그, 폐기]). 시나리오 B에서 hold=2.0s가 confidence
  자연축적(0.4~1.15s) 대비 최대 약 1.6초 순수 추가 지연을 유발함을 확인
  (체크포인트, 실 corpus/실차 검증 전). 상세: README.md/WIP.md 281차.
...
- `sim_route_260_confidence_signals.py`: 신규 -- 259차가 확정한 "apex 후보
  ...
  Master가 제시한 4개 신호 중 3개(persistence/curvature consistency/
  speed-drop strength)를 프레임 단위로 계측. `sim_route_234_spatial_apex_
  continuity.py`의 build_speeds_distances/gate_candidates/find_clusters를
  재사용(§21)하되, 단일 locked apex만 추적하던 `ContinuityState`를 "현재
  살아있는 모든 클러스터"를 동시에 추적하는 `MultiTrackContinuity`로
  확장(신규). 사용자가 재업로드한 `0000039a--7b602ffb85` seg12-16(234차/
  244차/251차 known-good 터널 corpus)로 검증:
  1) 터널윈도우(t2190-2225)는 stage2 클러스터링 자체에서 0건 생존 --
     251차 표(stage2 skip-gate=0)와 정확히 일치, 스크립트 정합성 확인.
  2) 전체 65개 track 중 36개(55%)가 streak=1(한 프레임만 존재 후 소멸)에서
     끝나는 반면, 소수(6개)만 streak 7~222까지 지속 -- **persistence
     신호가 노이즈/실제커브를 강하게 분리**하는 것을 실측 확인, Master
     설계 방향을 지지하는 최초 정량 증거.
  3) curvature_consistency(부호 일관성)는 거의 전부 1.000으로 계측 --
     짧은 streak(1~3)에서는 표본 부족으로 자명하게 1.0이 나와 **단독으로는
     판별력 없음**(streak 최소치 조건과 결합 필요, 원 신호 정의 재검토 필요).
  4) speed_drop_strength는 IC gore(mean 0.357)/S커브(mean 0.346) 두 실제
     커브 구간 간 뚜렷한 차이 없음 -- 이 corpus만으로는 노이즈 대조군
     (터널이 stage2에서 이미 전멸)이 없어 판별력 검증 불가.
  5) GPS positional reliability(4번째 신호)는 `extract_log.py` CSV에
     `horizontalAccuracy` 컬럼이 없어 이번 세션에서 계측 불가 -- 컬럼
     추가 필요(다음 작업).
  상세: WIP.md 260차/FINDINGS.md 260차 참고. 코드 변경 없음(§27), 실차
  검증 미실시(§29, 오프라인 로그 재분석 한정).
