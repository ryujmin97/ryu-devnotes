...
**364차 계속 실측 결과** (`--gate persist`, `min_run=2` 고정, `low_frac` 0.10~0.15 스윕):

| `low_frac` | 363차 corpus(R<30m, 1140프레임) 정탐 생존율 | 362차 corpus(R<50m, 170프레임) 오탐 생존율 |
|---|---|---|
| 0.10 | 98% | 72% |
| 0.12 | 96% | 69% |
| 0.13 | 94% | 67% |
...
`low_frac=0.10~0.13`은 정탐 보존은 우수하나 오탐 억제가 28~33%에 불과. `0.14`는 오탐 억제가 거의 개선되지 않는데(67%→66%) 정탐이 21%로 급락 -- 두 corpus의 `low_frac` 분포가 겹쳐 있어 단일 파라미터로는 분리 불가로 판정, `ryu` 코드 패치는 보류. 상세는 WIP.md/FINDINGS.md 364차(계속) 참고.
...
**2026-09-04 추가(234차 계속6, 실측 nRoadLimitSpeed 재검증)**: 동일 구간
(t=095600~100000, seg12-16) rlog를 234차 계속5 반영 후 `extract_log.py`로
재추출 -> `nRoadLimitSpeed`가 10/30/50/70/100(kph) 실측값으로 정상 채록됨
확인. `gate_base_kph()` 함수 신설(실측값 있으면 그것을, 없으면 기존
vEgo_kph로 폴백) 후 severity gate를 실측 기준으로 재실행.

**핵심 발견 1 (가정 a 해소, 큰 개선)**: sanity check(s0 vs 실측 published
apex_dist, ±15m) 정합률이 **24.4%(vEgo 근사) -> 98.2%(실측
nRoadLimitSpeed)**로 대폭 개선 -- vEgo 근사가 저정합의 주원인이었음이
확정적으로 증명됨. 이제 baseline(s0) 재구성이 실제 232차 프로덕션 apex
선택을 신뢰도 높게 대변한다고 볼 수 있음.

**핵심 발견 2 (기존 234차 계속4/5 결론 재검토 필요, 중요)**: 정확한
gate 기준으로 다시 계산하니 절대 수치가 크게 달라짐 -- vEgo 근사 기준
"97->60->16->3"건이었던 것이 실측 기준으로는 **"172->62->55->48"건**으로
바뀜. 즉:
- stage1(+30%gate)의 효과(172->62, 약 64% 감소)는 여전히 크고 견고함.
- 반면 stage2(+spatial cluster, 62->55)/stage3(+continuity, 55->48)의
  추가 개선폭은 이전에 믿었던 것(60->16->3, ~95% 추가 감소)보다 훨씬
  작음(62->48, ~23% 추가 감소) -- **②③의 효과가 vEgo 근사 오차로
  과대평가되어 있었을 가능성**. 터널 구간(t=2190~2225, 0.70 gate로 이미
  전부 해소)에 대한 234차 계속2 dashcam 대조 결론은 이번에도 그대로
  재현되어 변화 없음(active=0 유지).
- **잔여 48건의 stage3 점프는 기존에 알려진 터널 구간이 아니라 새로운
  구간(t≈2108~2116)에 집중적으로 몰려 있음** -- 이전 세션들에서 조사된
  적 없는 새 후보 구간. 다음 세션에서 dashcam 대조로 이 구간의 실제
  도로 형상(예: 근접한 두 커브가 실제로 연속되는 구간인지)을 확인 필요
  -- ambiguous 0%였던 continuity tolerance 재검증에도 좋은 테스트케이스가
  될 수 있음.

**결론**: ①(0.70 gate)의 실효성은 실측 데이터로 재확인됨. 다만 ②③의
추가 효과와 tolerance 값 확정은 이 새 구간(t≈2108~2116) 조사 없이는
최종 확정하지 않는 것이 안전함 -- 실제 patch 착수 전 추가 조사 권장.
...
**2026-09-04 추가(234차 계속6, 실측 nRoadLimitSpeed 재검증)**: `gate_base_kph()`
신설로 severity gate 기준을 vEgo 근사 대신 실측 `nRoadLimitSpeed`로 교체
후 동일 route(재추출본) 재실행. **stage0/1 절대 점프 수가 크게 달라짐
(vEgo 근사 97/60건 -> 실측 172/62건)** -- gate 이후(stage1) 효과는
여전히 견고(172->62, ~64%감소)하나, **stage2/3(+spatial cluster/
+continuity)의 추가 감소폭은 이전 근사 기준(60->16->3, ~95%)보다 훨씬
작음(62->55->48, ~23%)** -- ②③ 효과가 vEgo 근사 오차로 다소 과대평가
되어 있었을 가능성. 10/15/20m 재실행 결과 matched frame 수가 세 값
모두 200으로 동일(ambiguous 0%도 동일) -- **10m 채택 결론 자체는
변화 없음**(오히려 이번 재검증으로 더 명확해짐: tolerance를 늘려도
얻는 게 전혀 없음).
**신규 발견**: stage3 잔여 48건의 점프가 기존에 조사된 터널 구간
...
**2026-09-05 추가(249차, 실차 corpus 첫 검증 -- 247차 §11 검증계획 1번
실측)**: 사용자가 오늘 새로 채록한 실차 로그(`000003ac--fb4b38b538`
x20세그 + `000003ad--be5457810e` x5세그, 13:12~13:36, t가 끊김없이
이어져 단일 연속주행으로 병합, 29126행, 스키마 repo `ryu` HEAD `2cfbcf4`
=245차)로 `--skip-gate` 실행.

**사전 발견 및 수정**: 실행 중 `float("")` 크래시 발견 -- 세그먼트 시작
직후 첫 carState 도착 전 `vEgo` 빈 문자열 구간(116/29126행, 234차
corpus에는 없던 케이스). naviPaths 없음과 동일하게 처리하도록 최소
수정(위 CHANGELOG 249차 참고). `--skip-gate` 없는 기본 모드도 이
수정 이후 정상 실행 확인(회귀 없음).

**주행 특성**: 시내 위주(nRoadLimitSpeed 10/30/50kph) + 짧은 고속구간
(115kph), vEgo 0~61.9kph, 평균 13.9kph. **이 로그에는 터널이 없음**
(naviPaths 공백 구간을 전수 조사한 결과 초반 GPS lock 전 55초 1건 외에는
전부 2초짜리 일상적 갱신갭 47건뿐 -- 234차/244차가 쓰던 터널 포함
corpus, `0000039a--7b602ffb85` seg12-16과는 다른 로그). 따라서 이번
검증은 **터널급 노이즈 억제 여부까지는 증명하지 못함** -- 247차 §11이
지정한 검증 항목 중 "터널 구간 재확인"은 여전히 미해결.

**결과(전체 29126행, >40m 프레임간 점프 건수)**:
| 모드 | stage0 | stage1 | stage2 | stage3 |
|---|---|---|---|---|
| 기본(gate 포함) | 48 | 2 | 3 | 2 |
| `--skip-gate` | 48 | 48(SKIPPED, =stage0) | 21 | **2** |

**핵심 관찰**: gate를 완전히 제거해도(stage1=stage0 그대로 48건 통과)
stage2(클러스터링)+stage3(continuity)만으로 최종 점프 건수가 기존
gate-포함 파이프라인과 동일한 2건까지 떨어짐. 즉 **이 비(非)터널
실차 로그 범위 안에서는, severity gate 없이도 클러스터링+continuity
단독으로 gate-포함 파이프라인과 동등한 flicker 억제 효과**가 실측
확인됨 -- 247차가 제안한 gate 완전삭제 방향을 지지하는 첫 실차 증거.
active frame 수는 gate 제거로 늘어남(stage3 기준 7030->10958, +55.9%)
-- gate가 걸러내던 후보 중 상당수가 노이즈가 아니라 정상 감속 대상
이었을 가능성도 있어, 늘어난 active frame이 전부 "정상적으로 더 많이
개입"인지 "여전히 걸러야 할 노이즈가 남았는데 점프 지표에는 안 걸린
것"인지는 추가로 구분 필요(다음 세션 항목).
**남은 미확정**: (1) 터널 구간 포함 corpus 재검증(위 참고, 필수),
(2) 246차(원거리 apex freeze)/239차(self-elimination) 재현 케이스는
이번 로그에 해당 조건이 있는지 미확인, 아직 미검증.
...
5. **[253차 자체 발견]** 최초 포팅 버전에서 `route_release_time`(design
   ...
   **[253차 후속 갱신]** 사용자가 `dashcam_1788583013065.zip`을 재업로드
   (단, 이번 업로드는 route_ac 세그 0~16 중 15개만 유효 -- 세그16 rlog.zst가
   0바이트로 손상, route_ad는 이번 업로드에 포함되지 않음. 최초 유실분과
   완전히 동일한 데이터가 아니라 부분 재현임에 주의). `extract_log.py
   --with-navi-paths`로 route_ac 재추출(19229행, 세그0-15). 신규 스크립트
   `analyze_route_253_active_run_length.py`(아래 참고, 이 스크립트의
   `replay()`/`Sim252`를 재사용)로 route-active run 개수를 재계산한 결과
   **실측 68건 vs 시뮬(수정본) 51건**(최초 버그판 관측치였던 "실측 61건 vs
   시뮬 163건"과 달리, 수정 후에는 시뮬이 실측보다 적은 방향으로 역전) --
   이는 최초 관측된 3배 가까운 과다분절이 RELEASE-hold 누락 버그의 인공물
   이었음을 뒷받침하는 정황 증거. 다만 이 재추출본은 세그 구성이 달라
   (0-15만, 원래 유실분은 route_ac 24030행+route_ad 5096행 전체) 완전한
   1:1 재현은 아니며, 이 corpus는 vEgo 최대 61.9kph로 239차 원 CRITICAL의
