#!/usr/bin/env python3
"""328차 patch(route_local_curve_merge)를 x18seg 실측 routeOrphanRawPath에
재생해, 실제 orphan 프레임에서 국소 2.5m 병합이 (a) 트리거되는지
(local_used), (b) 병합 후 min_points=2 클러스터로 승격되는지, (c) 병합
window의 실제 point 개수/커버리지가 이론과 일치하는지를 실측한다.

이 CSV(x18seg, 2026-09-09 06:27~06:43)는 328차 패치 *이전*(1b77b799,
323차 빌드) 코드로 기록됐다(328차 WIP "327차와 동일 세트" 확인 --
routeOrphanSingletonCount/routeClusterCount 등은 328차 병합 로직을
전혀 반영하지 않은 10m-only 1차패스 결과). 따라서 328차 패치의 실제
효과를 보려면 raw path(routeOrphanRawPath)를 오프라인에서 328차 함수로
재생하는 수밖에 없다 -- 324차가 grid counterfactual에서 쓴 것과 동일한
근사(§28 한계와 동일): orphan이 아니었던 프레임은 검증 대상이 아니다.

아래 5개 함수는 `ryu`(base b31016fe, 328차 패치 적용 후) carrot_man.py
502~686행에서 **verbatim 복사**(로직 변경 없음, docstring도 원문 유지) --
route_find_clusters/calculate_curvature는 이미 검증된
sim_route_322d_stateful_replay.py에서 그대로 import(§21).

사용:
  python3 sim_route_329_local_merge_replay.py <CSV>
"""
import argparse
import csv
import sys
