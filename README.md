...
**실제 로그(raw rlog) 검증 완료(54차)** — route4(`d45a15f8fc`)
24건 위반 이벤트로 검증, raw 신호 자체도 이벤트 근접까지는 뚜렷한
하강이 없고(이론적 8.0s horizon보다 훨씬 늦게 감지) filtered는
raw 대비 평균 2초+ 추가 지연됨을 확인. 상세는 FINDINGS.md 54차
항목 참고.
**사용**:
```bash
